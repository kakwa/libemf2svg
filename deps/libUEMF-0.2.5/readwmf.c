/**
 Example program demonstrating how to code a program to read an WMF file.  This
 example just examines the contents
 of one WMF file and prints the contents record by record, showing all fields
 (except for bitmaps).
 ...
 Build with:  gcc -Wall -o readwmf readwmf.c uemf.c uwmf.c uemf_print.c
 uemf_endian.c uemf_utf.c uwmf_print.c uwmf_endian.c -lm
*/
...
/**
  \fn myMetaFileProc(char *contents, unsigned int length, PWMF_WORKING_DATA
  lpData)
...
int main(int argc, char *argv[]) {
    ...
    if (wmf_readdata(argv[1], &contents, &length)) {
        printf("readwmf: fatal error: could not open or successfully read "
               "file:%s\n",
               argv[1]);
        exit(EXIT_FAILURE);
    }
