/**
  ...
  EMF+ is much more object based than is EMF or WMF, so the U_PMR_*_set and most
  U_PMF_*_set functions
  return a pointer to a PseudoObject.  PseudoObjects are structs that contain a
  data field to hold the
  object in EMF+ file byte order, size information, and some type information.
  This is sufficient to allow
  complex records to be built up from the various sorts of nested objects which
  they normally contain.
  If something goes wrong a NULL pointer is returned and recsize is set to 0.

  EMF+ does not use a separate set of endian functions, _get and _set routines
  convert from/to
  the EMF+ file byte order on the fly.

        WARNING:  Microsoft's EMF+ documentation is little-endian for everything
  EXCEPT
        bitfields,  which are big-endian.   See EMF+ manual section 1.3.2
        That documentation also uses 0 as the MOST significant bit, N-1 as the
  least.
        This code is little-endian throughout, and 0 is the LEAST significant
  bit
...
/* Prototypes for functions used here and defined in uemf_endian.c, but which
are not supposed
...
/**
    \brief  Utility function for writing one or more EMF+ records in a
   PseudoObject to the EMF output file
...
int U_PMR_write(U_PSEUDO_OBJ *po, U_PSEUDO_OBJ *sum, EMFTRACK *et) {
    ...
    sum = U_PO_append(sum, "EMF+",
                      4); /* indicates that this comment holds an EMF+ record */
    if (!sum)
        goto end;
    ...
    rec = U_EMRCOMMENT_set(sum->Used,
                           sum->Data); /* stuff it into the EMF comment */
    if (!emf_append((PU_ENHMETARECORD)rec, et, 1))
        goto end; /* write it to the EMF file, delete the record, check status
                     */
    status = 1;
end:
    return (status);
...
/**
    ...
    \return 1 on success, 0 on error.
    \param  PenID       Index of U_PMF_PEN object to use in the EMF+ object
   table (0-63, inclusive)
    \param  PathID      Index of U_PMF_PATH object to use in the EMF+ object
   table (0-63, inclusive)
...
*/
int U_PMR_drawline(uint32_t PenID, uint32_t PathID, U_PMF_POINTF Start,
                   U_PMF_POINTF End, int Dashed, U_PSEUDO_OBJ *sum,
                   EMFTRACK *et) {
    U_DPSEUDO_OBJ *dpath;
    U_PSEUDO_OBJ *poPath;
    U_PSEUDO_OBJ *po;
    ...
    if (dpath) {
        if (U_PATH_moveto(dpath, Start, PTP_value) &&
            U_PATH_lineto(dpath, End, PTP_value)) {
...
/**
    ...
    \param  Vpos        StringAlignment Enumeration.  Always drawn on baseline,
   but using one of these three modes.
    \param  FontID      Index of U_PMF_FONT object to use in the EMF+ object
   table (0-63, inclusive)
    \param  BrushID     U_PSEUDO_OBJ containing a U_PMF_ARGB or a U_PMF_4NUM.
   Color or U_PMF_BRUSH object in the EMF+ object table (0-63, inclusive)
    \param  FormatID    index of U_PMF_STRINGFORMAT object to use in the EMF+
   Object Table.
    \param  Sfs         StringFormat structure. Ignored values: StringAlignment,
   LineAlign, Flags
    ...
    \param  x           X position in pixels of left side of EM box of first
   character
    ...
    For most fonts Ascent and Descent are used to adjust the bounding box to
   properly position the
    baseline.  Some fonts, like Verdana, are strange and they position the
   baseline on the bottom of
    the bounding box if that box has the same height as the font.  For those
   fonts specify 0.0 for
    both Ascent and Descent.
*/
int U_PMR_drawstring(const char *string, int Vpos, uint32_t FontID,
                     const U_PSEUDO_OBJ *BrushID, uint32_t FormatID,
                     U_PMF_STRINGFORMAT Sfs, const char *FontName,
                     U_FLOAT Height, U_FontInfoParams *fip, uint32_t FontFlags,
    ...
    if (Vpos < U_SA_Near || Vpos > U_SA_Far)
        return (0);
    ...
    poFont = U_PMF_FONT_set(U_PMF_GRAPHICSVERSIONOBJ_set(2), Height, U_UT_World,
                            FontFlags, slen, UFontName);
    if (poFont) {
        ...
        if (po) {
            ...
            if (poSF) {
                ...
                if (po) {
                    ...
                    rw = 4 * Height *
                         slen; /* This could probably be any value */
                    rh = Height;
                    rx = x;
                    if (fip->LineGap > -fip->Descent) {
                        aval = fip->yMax;
                    } // sylfaen, palatino
                    else {
                        aval = fip->Ascent;
                    } // others
                    if (fip->LineGap && (fip->LineGap < -fip->Descent)) {
                        dval = ((double)(fip->Descent - fip->LineGap)) /
                               ((double)fip->EmSize);
                    } // shruti and some others
                    else {
                        dval = ((double)fip->Descent) / ((double)fip->EmSize);
                    }
                    switch (Vpos) {
                    case U_SA_Near:
                        ...
                        break;
                    case U_SA_Center:
                        rd = 0.5 * (Height * aval / ((double)fip->EmSize) +
                                    Height * (1.0 + dval));
                        break;
                    case U_SA_Far:
                        rd = Height * (1.0 + dval);
                    ...
                    ry = y - rd; /* draw from upper left corner, which is
                                    shifted to put baseline on y */
...
(void) U_PMR_drawline(OBJ_PEN_BLACK_1,OBJ_PATH_1, (U_PMF_POINTF){x- 50, ry}, (U_PMF_POINTF){x- 50, ry + Height * ( 1.0 -  (((double) (fip->LineGap - fip->Descent)) / ((double) fip->EmSize)) )},  0, sum, et);
                    ...
                    slen = strlen(string);
                    po = U_PMR_DRAWSTRING_set(FontID, BrushID, FormatID, slen,
                                              poRect, Text16);
                    if (po) {
                        U_PMR_write(po, sum, et);
                        status = 1; /* Success!!! */
...
/**
    \brief Allocate and construct an array of U_POINT16 objects from a set of
   U_PMF_POINTF objects, endianness in and out is LE
...
U_PMF_POINT *POINTF_To_POINT16_LE(U_PMF_POINTF *points, int count) {
    ...
    for (i = 0; i < count; i++) {
        ...
        if (U_BYTE_SWAP) {
            U_swap4(&ptfl, 2);
        } /* on BE platforms swap going in and coming out */
...
/**
    \brief Look up the name of the EMR+ record by type.  Returns U_EMR_INVALID
   if out of range.
...
char *U_pmr_names(unsigned int idx) {
    if (idx < U_PMR_MIN || idx > U_PMR_MAX) {
        idx = 0;
    }
    static char *U_PMR_NAMES[U_PMR_MAX + 1] = {"U_PMR_INVALID",
                                               "U_PMR_Header",
                                               "U_PMR_EndOfFile",
                                               "U_PMR_Comment",
                                               "U_PMR_GetDC",
...
int U_OID_To_OT(uint32_t OID) {
    int otype;
    if (OID == U_PMF_BRUSH_OID) {
        otype = U_OT_Brush;
    } else if (OID == U_PMF_PEN_OID) {
        otype = U_OT_Pen;
    } else if (OID == U_PMF_PATH_OID) {
        otype = U_OT_Path;
    } else if (OID == U_PMF_REGION_OID) {
        otype = U_OT_Region;
    } else if (OID == U_PMF_IMAGE_OID) {
        otype = U_OT_Image;
    } else if (OID == U_PMF_FONT_OID) {
        otype = U_OT_Font;
    } else if (OID == U_PMF_STRINGFORMAT_OID) {
        otype = U_OT_StringFormat;
    } else if (OID == U_PMF_IMAGEATTRIBUTES_OID) {
        otype = U_OT_ImageAttributes;
    } else if (OID == U_PMF_CUSTOMLINECAP_OID) {
        otype = U_OT_CustomLineCap;
    } else {
        otype = U_OT_Invalid;
    }
    return (otype);
...
int U_OID_To_BT(uint32_t OID) {
    int otype;
    if (OID == U_PMF_HATCHBRUSHDATA_OID) {
        otype = U_BT_HatchFill;
    } else if (OID == U_PMF_LINEARGRADIENTBRUSHDATA_OID) {
        otype = U_BT_LinearGradient;
    } else if (OID == U_PMF_PATHGRADIENTBRUSHDATA_OID) {
        otype = U_BT_PathGradient;
    } else if (OID == U_PMF_SOLIDBRUSHDATA_OID) {
        otype = U_BT_SolidColor;
    } else if (OID == U_PMF_TEXTUREBRUSHDATA_OID) {
        otype = U_BT_TextureFill;
    } else {
        otype = -1;
    }
    return (otype);
...
int U_OID_To_CLCDT(uint32_t OID) {
    int otype;
    if (OID == U_PMF_CUSTOMLINECAPDATA_OID) {
        otype = U_CLCDT_Default;
    } else if (OID == U_PMF_CUSTOMLINECAPARROWDATA_OID) {
        otype = U_CLCDT_AdjustableArrow;
    } else {
        otype = -1;
    }
    return (otype);
...
int U_OID_To_IDT(uint32_t OID) {
    int otype;
    if (OID == U_PMF_BITMAP_OID) {
        otype = U_IDT_Bitmap;
    } else if (OID == U_PMF_METAFILE_OID) {
        otype = U_IDT_Metafile;
    } else {
        otype = -1;
    }
    return (otype);
...
int U_OID_To_RNDT(uint32_t OID) {
    int otype;
    if (OID == U_PMF_REGIONNODECHILDNODES_OID) {
        otype = U_RNDT_Kids;
    } /* there are 5 types, which must be specified separately */
    else if (OID == U_PMF_RECTF_OID) {
        otype = U_RNDT_Rect;
    } else if (OID == U_PMF_REGIONNODEPATH_OID) {
        otype = U_RNDT_Path;
    } else {
        otype = -1;
    }
...
/**
    ...
    \param Data  Data to copy into the PseudoObject's data. If NULL, space is
   allocated, but is cleared instead of filled.
    \param Size  Number of bytes to allocate for Data (may be >Use if padding is
   present)
    \param Use   Number of data bytes in Data (whether or not Data is actually
   copied)
    \param Type  Type numbers are from manual section: 1.2.3.47 -> 0x01020347

    If Data is NULL and Size is 0 an empty PseudoObject is created.  One byte of
   storage
...
U_PSEUDO_OBJ *U_PO_create(char *Data, size_t Size, size_t Use, uint32_t Type) {
    ...
    if (po) {
        ...
        if (po->Data) {
            ...
            po->Type = Type;
            if (Data) {
                memcpy(po->Data, Data, Use);
            } /* if Use < Size uninitialized bytes will be present! */
            else {
                memset(po->Data, 0, tSize);
            }
        } else {
            free(po);
            po = NULL;
...
/**
    ...
    \param Data  Data to copy into the PseudoObject's data. If NULL, space is
   allocated (if necessary) and cleared instead of filled.
    \param Size  Number of data bytes in Data
*/
U_PSEUDO_OBJ *U_PO_append(U_PSEUDO_OBJ *po, const char *Data, size_t Size) {
    /* po cannot be NULL,as in U_PO_po_append(), because there would be no way
     * to determine the TYPE of the resulting PO */
    if (po) {
        if (!po->Data || po->Used + Size > po->Size) {
            ...
            if (!newData) {
                ...
                po = NULL; /* skip the rest of the actions, does not affect po
                              in caller */
            } else {
                po->Data = newData;
            }
        ...
        if (po) { /* po->Data ready to append new data */
            if (Data) {
                memcpy(po->Data + po->Used, Data, Size);
            } else {
                memset(po->Data + po->Used, 0, Size);
            }
            po->Used += Size;
        }
    }
...
/**
    ...
    \param StripE Set: leading Elements in Src->Data is not copied, Clear: it is
   copied.
*/
U_PSEUDO_OBJ *U_PO_po_append(U_PSEUDO_OBJ *po, U_PSEUDO_OBJ *Src, int StripE) {
    ...
    if ((StripE && (Src->Used == 4)) || !Src->Used) {
        return (po);
    } /* appending nothing is not an error */
    char *Data = Src->Data;
    ...
    if (!ipo) {
        ipo = U_PO_create(NULL, 0, 0,
                          Src->Type); /* create an empty pseudoobject */
    }
    if (ipo) {
        ...
        if (ipo) {
            if (Data) {
                if (StripE) {
                    memcpy(ipo->Data + ipo->Used, Data + 4, Size);
                } /* Size is already 4 less, skip the leading Elements value */
                else {
                    memcpy(ipo->Data + ipo->Used, Data, Size);
                } /* copy everything */
            } else {
                memset(ipo->Data + ipo->Used, 0, Size);
            } /* set everything  */
            ipo->Used += Size;
        }
    }
...
/**
    ...
    \param po   Address of a pointer to the U_PSEUDO_OBJ structure, Pointer is
   set to NULL.
...
/** \brief create a PseudoObject with data in the correct byte order for an EMF+
   file.
    \returns The PseudoObject on success, NULL on error.

    \param Type  the type of the PseudoObject that is created.  Allowed values
   are in U_PID_Values.
    \param List  an array of U_SERIAL_DESC structures containing the data to
   store.

      The U_PMF_SERIAL_set() function should not ever be called directly by end
   user code.

   Each U_SERIAL_DESC element in List consists of Data fields and a description
   of that data.  List is terminated
     by the first U_SERIAL_DESC element having a TE value of U_XX.

   Data fields: an array of a basic type of Units bytes repeated Reps times with
   ...
   Reps:  Number of repeats of the unit in data fields.
           If a Ptr is NULL, and Units*Reps is not zero, then Units*Reps 0x00
   bytes are stored.
           If a Ptr is NULL, and Units*Reps is zero, this U_SERIAL_DESC is
   ignored.
           if a Ptr is NOT NULL, and Units * Reps is not zero, then the data is
   stored in the indicated byte order.
           If a Ptr is NOT NULL, and Units or Reps is zero an error is signaled.

   TE:   (Target Endian) the byte order in which to store each unit of a data
   field as defined in U_Endian.
         Byte swapping is only enabled when Units is 2 or 4.  In addition to the
   byte order values U_XE, U_LE,
         and U_BE, and the array terminator U_XX, the value may also be U_RP.
   U_RP means there is only a
         single unit in the data fields, but it is to be copied to the target
   Reps times.  That is, the data
         was passed in with a form of run length encoding.

   Creates an empty PseudoObject if all pointers are NULL and all sizes are
   zero.
 */
U_PSEUDO_OBJ *U_PMF_SERIAL_set(uint32_t Type, const U_SERIAL_DESC *List) {
    ...
    for (lptr = List; lptr->TE != U_XX; lptr++) {
        if (!(lptr->Units && lptr->Reps) && lptr->Ptr)
            return (po);
    ...
    if (po) {
        ...
        for (lptr = List; lptr->TE != U_XX; lptr++) {
            ...
            if (FSize) { /* Ptr is not NULL, that would have been detected
                            already */
                hptr = cptr;
                if (lptr->TE & U_RP) {
                    U_PMF_REPCPY_DSTSHIFT(&cptr, lptr->Ptr, lptr->Units,
                                          lptr->Reps);
                } else {
                    U_PMF_MEMCPY_DSTSHIFT(&cptr, lptr->Ptr, FSize);
                }
                if (((lptr->TE & U_LE) && U_IS_BE) ||
                    ((lptr->TE & U_BE) && U_IS_LE)) {
                    if (lptr->Units == 2) {
                        U_swap2(hptr, lptr->Reps);
                    } else if (lptr->Units == 4) {
                        U_swap4(hptr, lptr->Reps);
                    }
                }
            }
        }
...
/**
    ...
    \param Elements Number of elements in Points.  May be zero, which creates an
   empty path.
    \param Points   Array of U_PMF_POINTF values.
    \param First    Apply to first point,      unsigned byte, lower 4 bits hold
   the PathPointType flag upper 4 bits hold the PathPointType enumeration. Must
   have U_PPT_Start set.
    \param Others   Apply to all other points, unsigned byte, lower 4 bits hold
   the PathPointType flag upper 4 bits hold the PathPointType enumeration. Must
   have U_PPT_Line or U_PPT_Bezier set.
    \returns pointer to the DU_PSEUDO_OBJ object, NULL on error
*/
U_DPSEUDO_OBJ *U_PATH_create(int Elements, const U_PMF_POINTF *Points,
                             uint8_t First, uint8_t Others) {
    if (Elements) {
        if (!Points) {
            return (NULL);
        }
        if ((First & U_PPT_MASK) != U_PPT_Start) {
            return (NULL);
        }
        if (!(Others & U_PPT_Bezier)) {
            return (NULL);
        } /* will pass if either line or bezier is set */
    }

    U_DPSEUDO_OBJ *Path = (U_DPSEUDO_OBJ *)calloc(
        sizeof(U_DPSEUDO_OBJ), 1); /* make poTypes and poPoints NULL */
    const U_SERIAL_DESC List[] = {{NULL, 0, 0, U_XX}};
    if (Path) {
        Path->Elements = Elements;
        Path->poPoints = U_PMF_SERIAL_set(
            U_PMF_RAW_OID, List); /* Empty PO to hold points as raw data */
        if (!Elements) {
            Path->poTypes = U_PMF_SERIAL_set(
                U_PMF_RAW_OID, List); /* Empty PO to hold types as raw data */
        } else {
            Path->poPoints = U_PO_append(Path->poPoints, (char *)Points,
                                         Elements * sizeof(U_PMF_POINTF));
            if (Path->poPoints) {
                U_PSEUDO_OBJ *tpo = U_PMF_PATHPOINTTYPE_set2(
                    Elements, First | U_PPT_Start,
                    Others); /* PO holding types, has leading Elements value */
                Path->poTypes = U_PO_po_append(
                    NULL, tpo,
                    U_PMF_DROP_ELEMENTS); /* remove the leading Elements value*/
                U_PO_free(&tpo);
                if (!Path->poTypes) {
                    U_PO_free(&Path->poPoints);
                }
            }
            if (!Path->poPoints) {
                U_DPO_free(&Path);
...
int U_DPO_free(U_DPSEUDO_OBJ **dpo) {
    ...
    if (kpo->poPoints) {
        U_PO_free(&kpo->poPoints);
    }
    if (kpo->poTypes) {
        U_PO_free(&kpo->poTypes);
    }
    free(*dpo);
    *dpo = NULL;
    return (1);
...
/**
    \brief Clear U_DPSEUDO_OBJ's.  Memory is retained, Elements and Used values
   are set to 0.
    \returns 1 on success, 0 on error.

    It is much more efficient to clear a DPO and reuse it than to free that DPO
   and create another.
*/
...
/**
    ...
    \param Flags  Flags may be (U_PTP_None, U_PTP_DashMode, U_PTP_PathMarker,
   U_PTP_NoBit, U_PTP_CloseSubpath)
    \returns 1 on success, 0 on error.
*/
...
/**
    ...
    \param Path    Address of a DoublePseudoObject holding the path to append
   to.
...
int U_PATH_lineto(U_DPSEUDO_OBJ *Path, U_PMF_POINTF Point, uint8_t Flags) {
    if (!Path || !Path->Elements) {
        return (0);
    } /* must be at least one point to extend from */
    if (Path->poTypes->Data[Path->Elements - 1] & U_PTP_CloseSubpath) {
        return (0);
    } /* cannot extend a closed subpath */
    U_PSEUDO_OBJ *tpo;
    U_PSEUDO_OBJ *tpo2;
...
int U_PATH_closepath(U_DPSEUDO_OBJ *Path) {
    ...
    if (*Type & U_PPT_Start) {
        return (0);
    } /* single point closed path makes no sense */
...
/**
   ...
   to.
    ...
    \param Flags     Flags (U_PTP_None, U_PTP_DashMode, U_PTP_PathMarker,
   U_PTP_NoBit, but NOT U_PTP_CloseSubpath)
    \param StartSeg  If set, use U_PPT_Start PathPointType enumeration for first
   point, otherwise use U_PPT_Line.
    \returns 1 on success, 0 on error.
*/
int U_PATH_polylineto(U_DPSEUDO_OBJ *Path, uint32_t Elements,
                      const U_PMF_POINTF *Points, uint8_t Flags,
                      uint8_t StartSeg) {
    if (!Path || !Points) {
        return (0);
    }
    ...
    if (StartSeg) {
        First = (Flags & U_PTP_NotClose) | U_PPT_Start;
    } else {
        First = (Flags & U_PTP_NotClose) | U_PPT_Line;
    }
...
/**
   ...
   to.
    ...
    \param Points    Bezier points.  Optional starting point, then N sets of 3,
   example: [P1] (Q12A Q12B P2) (Q23A Q23B P3).
    \param Flags     Flags (U_PTP_None, U_PTP_DashMode, U_PTP_PathMarker,
   U_PTP_NoBit, but NOT U_PTP_CloseSubpath)
    \param StartSeg  If set, use U_PPT_Start PathPointType enumeration for first
   point, otherwise use U_PPT_Bezier.
    \returns 1 on success, 0 on error.

    If Start is set   Elements must be 1 + multiple of 3. Ie, P1 Q12A Q12B P2
   Q23A Q23B P3

    If Start is clear Elements must be a multiple of 3.   Ie, (P1, already in
   path) Q12A Q12B P2 Q23A Q23B P3
*/
int U_PATH_polybezierto(U_DPSEUDO_OBJ *Path, uint32_t Elements,
                        const U_PMF_POINTF *Points, uint8_t Flags,
                        uint8_t StartSeg) {
    if (!Path || !Points) {
        return (0);
    ...
    if (!Elements) {
        if (StartSeg) {
            return (0);
        } /* cannot have both a NEW segment and ZERO points */
        else {
            return (1);
        } /* harmless - do nothing */
    }
    if (StartSeg && ((Elements - 1) % 3)) {
        return (0);
    } /* new segment    must be 1 + N*3 points */
    if (!StartSeg && (Elements % 3)) {
        return (0);
    ...
    if (StartSeg) {
        ...
        First = (Flags & U_PTP_NotClose) | U_PPT_Bezier;
    }
...
/**
   ...
   U_PTP_NoBit, but NOT U_PTP_CloseSubpath)
    \returns 1 on success, 0 on error.
*/
int U_PATH_polygon(U_DPSEUDO_OBJ *Path, uint32_t Elements,
                   const U_PMF_POINTF *Points, uint8_t Flags) {
...
U_PMF_POINTF U_eparam(U_FLOAT a, U_FLOAT b, U_PMF_POINTF *center, double Ang,
                      double Theta) {
    U_PMF_POINTF point;
...
U_PMF_POINTF U_qparam(double Alpha, double a, double b, U_PMF_POINTF *Point,
                      double Ang, double Theta, int mode) {
...
/**
    \brief Append an "arcto" set of points to a path (Bezier points are
   calculated, and these are appended
    ...
    \param Sweep     Sweep angle, -360<= angle <=360, degrees clockwise from
   Start
    \param Rot       Rotation angle to apply to coordinate system (Start and
   Rect), positive is degrees clockwise
    \param Rect      U_PMF_RECTF that defines the bounding rectangle.
    \param Flags     Flags (U_PTP_None, U_PTP_DashMode, U_PTP_PathMarker,
   U_PTP_NoBit, but NOT U_PTP_CloseSubpath)
    \param StartSeg  If set, the arc starts a new segment, if clear, continue
   the existing segment.  Starting a new segment does not automatically apply
   U_PATH_closepath to the existing path.
    \returns 1 on success, 0 on error.

    Based on Luc Maisonobe's work, http://www.spaceroots.org/documents/ellipse/
*/
int U_PATH_arcto(U_DPSEUDO_OBJ *Path, U_FLOAT Start, U_FLOAT Sweep, U_FLOAT Rot,
                 U_PMF_RECTF *Rect, uint8_t Flags, int StartSeg) {
    U_PMF_POINTF Bz[3];
    U_PMF_POINTF Center;
    U_PMF_POINTF P1, P2;
    double a, b;
    ...
    /* Start should be between 0 and 360 degrees, but it does not really matter
     * because sin, and cos will accept anything */
    /* the sign on Sweep and Start is correct bacause LM's derivation has y
     * positive up, but GDI+ has y positive down. */
    a = Rect->Width / 2.0;
    b = Rect->Height / 2.0;
    if (!a || !b) {
        return (0);
    ...
    while (!done) {
        ...
        if (!fpoint) {
            if (StartSeg) {
                U_PATH_moveto(Path, P1, Flags);
            } else {
                U_PATH_lineto(Path, P1, Flags);
            }
            fpoint = 1;
        }
...
/**
    \brief Allocate and construct an array of U_PMF_POINTF objects which have
   been subjected to a U_XFORM
...
*/
U_PMF_POINTF *pointfs_transform(U_PMF_POINTF *points, int count,
                                U_XFORM xform) {
...
/**
    \brief Allocate and construct an array of U_PMF_RECTF objects which have
   been subjected to a U_XFORM
    ...
    \param Xform   U_XFORM to apply.  Rotation is ignored, only translation is
   applied.
...
/**
    \brief  Utility function calculate the transformation matrix needed to make
   a gradient run precisely corner to corner of a rectangle
    \param  Angle   Rotation in degrees clockwise of the gradient. 0 is
   horizontal gradient.
    \param  w       Width of the rectangle
    \param  h       Height of the rectangle
    \param  x       X coordinate of upper left corner of rectangle
    \param  y       Y coordinate of upper left corner of rectangle
    \param  Periods Periods of gradient corner to corner.  1.0 is one, corner to
   corner.
    \return Transformation matrix.  All values are zero if Periods, w, or h are
   less than or equal to zero.
*/
U_PMF_TRANSFORMMATRIX tm_for_gradrect(U_FLOAT Angle, U_FLOAT w, U_FLOAT h,
                                      U_FLOAT x, U_FLOAT y, U_FLOAT Periods) {
//! \cond
#define CLOSE_TO_IS_REALLY_ZERO(A)                                             \
    ((A) > 1.0e-10 || (A) < -1.0e-10 ? (A) : 0.0) //! \hideinitializer
                                                  //! \endcond
    ...
    if ((Periods <= 0.0) || (w <= 0.0) || (h <= 0.0)) {
        ...
        /*
          ...
          The scale value sets the gradient period to match exactly with the
          inscribed (w,h) rectangle
            in the direction specified by the angle.
          The numberator of scale is the max of the four dot product values of
          the rotated X basis unit vector with (w,h),
            with each of the vectors {w,h}, {-w,h}, {-w,-h}, {w,h}.  Those
          vectors run from each corner in turn
            to the opposite corner.  The one most parallel to the rotated unit
          vector will have both terms positive.

          Trig results like cos(pi/2) are not stable between platforms due to
          minor differences in the
            implementation. Detect these and make them zero, which then allows
          binary comparison of output files.
            Otherwise the binary comparisons between platforms would fail
          because of a bunch of insignificant digits.
        */
        ...
        tm.m21 = scale * sd;
        tm.m22 = scale * cd;
        /* offset is to one corner of the square, depending on which quadrant
...
/**
    \brief Create a U_PSEUDO_OBJ containing a U_PMR_FILLPATH and U_PMR_DRAWPATH
   records.
    \returns pointer to U_PSEUDO_OBJ or NULL on error.
    \param  PathID      U_PMF_PATH object in the EMF+ object table (0-63,
   inclusive)
    \param  PenID       U_PMF_PEN object in the EMF+ object table (0-63,
   inclusive)
    \param  BrushID     U_PSEUDO_OBJ containing a U_PMF_ARGB or a U_PMF_4NUM.
   Color or U_PMF_BRUSH object in the EMF+ object table (0-63, inclusive)
*/
U_PSEUDO_OBJ *U_PMR_drawfill(uint32_t PathID, uint32_t PenID,
...
/**
   ...
   \returns 1 on success, 0 on error.
   \param Src   where the data is coming from.  It is incremented by the number
   of bytes retrieved.
   \param Dst   where the data will be stored.  This must either be NULL (in
   which case the Src
           is advanced and nothing is stored, or it must be allocated to Reps *
   Units bytes!!!!
   ...
   \param Reps  number of repeats of units in the data field.
           ...
           If a Ptr is NOT NULL, and Units or Reps, is zero an error is
   signaled.
   ...
   \param SE    logical (Source Endian). Only relevant for Sizes of 2 or 4
         ...
         U_XE no change   (this is used when the data has already been set to
   the proper orientation or it is not known)
         U_LE source is Little Endian
         U_BE source is Big Endian.
         U_XX error

*/
int U_PMF_SERIAL_get(const char **Src, void *Dst, size_t Units, size_t Reps,
                     int SE) {
    if (!Src || !*Src || SE == U_XX) {
        return (0);
    }
    ...
    if (!Dst) {
        return (1);
    } /* "fake" get, no data was retrieved, so we are done */
    if (SE == U_XE) {
        return (1);
    }
    if (SE == U_LE && U_IS_LE) {
...
/**
      \brief Conditionally extract an array of data from a source, allocating
   space to hold it.
      \returns 1 on success, 0 on error.
      \param Src   where the data is coming from.  It is incremented by the
   number of bytes retrieved.
      \param Dst   Caller must free.  Where the pointer to the data will be
   stored.  Reps * Units bytes will be allocated,
      \param Units number of bytes in each unit of the data field
      \param Reps  number of repeats of units in the data field.
           If a Ptr is NULL, then Units*Reps 0 bytes are stored.
           If a Ptr is NOT NULL, and Units or Reps, is zero an error is
...
*/
int U_PMF_SERIAL_array_copy_get(const char **Src, void **Dst, size_t Units,
                                size_t Reps, int SE, int Cond) {
    if (!Src || !*Src || !Dst || SE == U_XX) {
        return (0);
    }
    if (!Cond) {
        *Src += Units * Reps;
...
/**
    \brief Calculate the length in bytes of a relative path object composed of
   U_PMF_INTEGER7 and U_PMF_INTER15 values
    \return >=0 length == success, <0  error
    \param  contents   Start of a relative path consisting of int7 and int15 X,Y
   pairs.
...
/**
    \brief Calculate the length in bytes of objects which are a 4 byte Count
   followed by Count * float bytes
    \return >=0 length == success, <0  error
...
/**
    \brief Create a string containing the curly bracket form of the 16 byte GUID
   value
    ...
    Text form is Data1-Data2-Data3-Data4, the first 3 are stored as little
   endian integers, the last as a string (big endian).
*/
char *U_PMF_CURLYGUID_set(uint8_t *GUID) {
    char *string = malloc(64);
    if (string) {
        sprintf(string, "{%2.2X%2.2X%2.2X%2.2X-%2.2X%2.2X-%2.2X%2.2X-%2.2X%2."
                        "2X-%2.2X%2.2X%2.2X%2.2X%2.2X%2.2X}",
                GUID[3], GUID[2], GUID[1], GUID[0], GUID[5], GUID[4], GUID[7],
                GUID[6], GUID[8], GUID[9], GUID[10], GUID[11], GUID[12],
...
int U_PMF_KNOWNCURLYGUID_set(const char *string) {
    int status;
    if (!strcmp(string, "{633C80A4-1843-482B-9EF2-BE2834C5FDD4}")) {
        status = U_IEE_BlurEffectGuid;
    } else if (!strcmp(string, "{D3A1DBE1-8EC4-4C17-9F4C-EA97AD1C343D}")) {
        status = U_IEE_BrightnessContrastEffectGuid;
    } else if (!strcmp(string, "{537E597D-251E-48DA-9664-29CA496B70F8}")) {
        status = U_IEE_ColorBalanceEffectGuid;
    } else if (!strcmp(string, "{DD6A0022-58E4-4A67-9D9B-D48EB881A53D}")) {
        status = U_IEE_ColorCurveEffectGuid;
    } else if (!strcmp(string, "{A7CE72A9-0F7F-40D7-B3CC-D0C02D5C3212}")) {
        status = U_IEE_ColorLookupTableEffectGuid;
    } else if (!strcmp(string, "{718F2615-7933-40E3-A511-5F68FE14DD74}")) {
        status = U_IEE_ColorMatrixEffectGuid;
    } else if (!strcmp(string, "{8B2DD6C3-EB07-4D87-A5F0-7108E26A9C5F}")) {
        status = U_IEE_HueSaturationLightnessEffectGuid;
    } else if (!strcmp(string, "{99C354EC-2A31-4F3A-8C34-17A803B33A25}")) {
        status = U_IEE_LevelsEffectGuid;
    } else if (!strcmp(string, "{74D29D05-69A4-4266-9549-3CC52836B632}")) {
        status = U_IEE_RedEyeCorrectionEffectGuid;
    } else if (!strcmp(string, "{63CBF3EE-C526-402C-8F71-62C540BF5142}")) {
        status = U_IEE_SharpenEffectGuid;
    } else if (!strcmp(string, "{1077AF00-2848-4441-9489-44AD4C2D7A2C}")) {
        status = U_IEE_TintEffectGuid;
    } else {
        status = U_IEE_Unknown;
    }
    return (status);
}
...
This accepts a string that is 16 bytes long = 32 characters hex (no dash spaces
or brackets) as text.
Text form is; Data1|Data2|Data3|Data4, first three are stored as little endian
integers of 4,2,2 bytes, respectively,
last is stored like a string (big endian), after conversion from text hex to
binary.
...
/**
    ...
    \return pointer to 16 byte buffer holding the long GUID binary form, or NULL
   on error.
...
uint8_t *U_OID_To_GUID(uint32_t OID) {
    uint8_t *lf = NULL;
    if (OID == U_PMF_IE_BLUR_OID) {
        lf = U_LOAD_GUID("633C80A41843482B9EF2BE2834C5FDD4");
    } else if (OID == U_PMF_IE_BRIGHTNESSCONTRAST_OID) {
        lf = U_LOAD_GUID("D3A1DBE18EC44C179F4CEA97AD1C343D");
    } else if (OID == U_PMF_IE_COLORBALANCE_OID) {
        lf = U_LOAD_GUID("537E597D251E48DA966429CA496B70F8");
    } else if (OID == U_PMF_IE_COLORCURVE_OID) {
        lf = U_LOAD_GUID("DD6A002258E44A679D9BD48EB881A53D");
    } else if (OID == U_PMF_IE_COLORLOOKUPTABLE_OID) {
        lf = U_LOAD_GUID("A7CE72A90F7F40D7B3CCD0C02D5C3212");
    } else if (OID == U_PMF_IE_COLORMATRIX_OID) {
        lf = U_LOAD_GUID("718F2615793340E3A5115F68FE14DD74");
    } else if (OID == U_PMF_IE_HUESATURATIONLIGHTNESS_OID) {
        lf = U_LOAD_GUID("8B2DD6C3EB074D87A5F07108E26A9C5F");
    } else if (OID == U_PMF_IE_LEVELS_OID) {
        lf = U_LOAD_GUID("99C354EC2A314F3A8C3417A803B33A25");
    } else if (OID == U_PMF_IE_REDEYECORRECTION_OID) {
        lf = U_LOAD_GUID("74D29D0569A4426695493CC52836B632");
    } else if (OID == U_PMF_IE_SHARPEN_OID) {
        lf = U_LOAD_GUID("63CBF3EEC526402C8F7162C540BF5142");
    } else if (OID == U_PMF_IE_TINT_OID) {
        lf = U_LOAD_GUID("1077AF0028484441948944AD4C2D7A2C");
    }
    return (lf);
}
...
/**
    ...
    \param Src Source in memory (if this is NULL, fill with that many zero bytes
   instead)
    \param Size Number of bytes to move
*/
...
/**
    ...
    \param Dst   Destination in memory
    \param Src   Source in memory (if this is NULL, fill with that many zero
   bytes instead)
    \param Size  number of bytes in single instance that is template.
    \param Reps  Number of instances of the template to opy
*/
void U_PMF_REPCPY_DSTSHIFT(char **Dst, const void *Src, size_t Size,
                           size_t Reps) {
    for (; Reps; Reps--) {
        if (Src) {
            memcpy(*Dst, Src, Size);
...
/**
    \brief save pointer to data and shift source pointer by the amount of data
   moved
    \param Dst Destination in memory
...
/**
    ...
    \param  contents   Record from which to extract data, will be incremented by
   header size.
    \return Flags field

    In many records the only value needed from the header is Flags.  Rather than
   mapping
    the entire Header and returning it, in these instances this function may be
   called to
...
uint16_t U_PMF_HEADERFLAGS_get(const char *contents) {
    ...
    U_PMF_SERIAL_get(&cptr, &Flags, 2, 1,
                     U_LE); /* EMF+ manual documents it as BE, but this code
                               implements it as LE*/
    return (Flags);
}

/**
    \brief Retrieve whichever header fields are requested.  NULL pointers do not
   retrieve.
    \param  contents   Record from which to extract data, will be incremented by
   header size.
...
*/
int U_PMF_HEADERFIELDS_get(const char *contents, uint16_t *Type,
    ...
    U_PMF_SERIAL_get(&contents, Flags, 2, 1,
                     U_LE); /* EMF+ manual documents it as BE, but this code
                               implements it as LE*/
    U_PMF_SERIAL_get(&contents, Size, 4, 1, U_LE);
...
/**
    ...
    \param  contents   Record from which to extract data, will be offset by
   header size.
...
int U_PMF_CMN_HDR_get(const char **contents, U_PMF_CMN_HDR *Header) {
    ...
    if (Header) {
        ...
        U_PMF_SERIAL_get(contents, &(Header->Flags), 2, 1,
                         U_LE); /* EMF+ manual documents it as BE, but this
                                   code implements it as LE*/
...
int U_PMF_LEN_OPTPENDATA(const char *PenData, uint32_t Flags) {
    ...
    if (Flags & U_PD_Transform) {
        length += sizeof(U_PMF_TRANSFORMMATRIX);
    }
    ...
    if (Flags & U_PD_DLData) {
        length += U_PMF_LEN_FLOATDATA(PenData + length);
    }
    ...
    if (Flags & U_PD_CustomStartCap) {
        length += U_PMF_LEN_BYTEDATA(PenData + length);
    }
...
U_PSEUDO_OBJ *U_PMF_BRUSH_set(uint32_t Version, const U_PSEUDO_OBJ *Bd) {
    ...
    const U_SERIAL_DESC List[] = {{&Version, 4, 1, U_LE},
                                  {&Type, 4, 1, U_LE},
...
/**
    ...
    \return Pointer to PseudoObject, NULL on error
    \param  Version    EmfPlusGraphicsVersion object
    \param  Ld         U_PSEUDO_OBJ containing one of the 2 types of Linecap
   data
...
*/
U_PSEUDO_OBJ *U_PMF_FONT_set(uint32_t Version, U_FLOAT EmSize,
                             uint32_t SizeUnit, int32_t FSFlags,
                             uint32_t Length, const uint16_t *Font) {
    uint32_t cbFont =
        2 * wchar16len(Font); /* this need not be 2*Length parameter */
    ...
    const U_SERIAL_DESC List[] = {
        ...
        {Font, cbFont, 1, U_LE},
        {NULL, pad, (pad ? 1 : 0),
         (pad ? U_XE : U_XX)}, /* Entire record must be a multiple of 4 */
        {NULL, 0, 0, U_XX}};
...
*/
U_PSEUDO_OBJ *U_PMF_IMAGEATTRIBUTES_set(uint32_t Version, uint32_t WrapMode,
                                        uint32_t ClampColor,
                                        uint32_t ObjectClamp) {
    uint32_t Reserved = 0;
    const U_SERIAL_DESC List[] = {
        {&Version, 4, 1, U_LE},     {&Reserved, 4, 1, U_LE},
        {&WrapMode, 4, 1, U_LE},    {&ClampColor, 4, 1, U_LE},
        {&ObjectClamp, 4, 1, U_LE}, {&Reserved, 4, 1, U_LE},
        {NULL, 0, 0, U_XX}};
...
/**
    ...
    \return Pointer to PseudoObject, NULL on error
    \param  Version    EmfPlusGraphicsVersion object
    \param  Points     U_PSEUDO_OBJ containing array of points (of type
   PMFPointR, PMFPoint, or PMFPointF, determined by U_PPF_P and U_PPF_C bits in
   Flags)
    \param  Types      U_PSEUDO_OBJ containing array of types
   (U_PMF_PATHPOINTTYPE or U_PMF_PATHPOINTTYPERLE, determined by  U_PPF_R big in
   Flags)

    EMF+ manual 2.2.1.6, Microsoft name: EmfPlusPath Object
*/
U_PSEUDO_OBJ *U_PMF_PATH_set(uint32_t Version, const U_PSEUDO_OBJ *Points,
                             const U_PSEUDO_OBJ *Types) {
    int ctype, RelAbs, rtype;
    int pad;
    if (Points) {
        if (Points->Type == U_PMF_POINTR_OID) {
            RelAbs = 1;
            ctype = 0;
        } else if (Points->Type == (U_PMF_POINT_OID | U_PMF_ARRAY_OID)) {
            RelAbs = 0;
            ctype = 1;
        } else if (Points->Type == (U_PMF_POINTF_OID | U_PMF_ARRAY_OID)) {
            RelAbs = 0;
            ctype = 0;
        } else {
            return (NULL);
    ...
    if (Types) {
        if (Types->Type == (U_PMF_PATHPOINTTYPERLE_OID | U_PMF_ARRAY_OID)) {
            rtype = 1;
        } else if (Types->Type == (U_PMF_PATHPOINTTYPE_OID | U_PMF_ARRAY_OID)) {
            rtype = 0;
        } else {
            return (NULL);
        }
    ...
    uint16_t Flags =
        (rtype ? U_PPF_R : 0) | (ctype ? U_PPF_C : 0) | (RelAbs ? U_PPF_P : 0);
    ...
    const U_SERIAL_DESC List[] = {
        ...
        {Points->Data + 4, Points->Used - 4, 1,
         U_XE},                                      /* omit Points Elements */
        {Types->Data + 4, Types->Used - 4, 1, U_XE}, /* omit Types  Elements */
        {NULL, pad, (pad ? 1 : 0),
         (pad ? U_XE : U_XX)}, /* if no padding is needed the Units will be
...
/**
    \brief  Create and set a U_PMF_PATH PseudoObject that uses U_PMF_POINTF
   coordinates
    \return Pointer to PseudoObject, NULL on error
    \param  Version    EmfPlusGraphicsVersion object
...
U_PSEUDO_OBJ *U_PMF_PATH_set2(uint32_t Version, const U_DPSEUDO_OBJ *Path) {
    ...
    const U_SERIAL_DESC List[] = {
        ...
        {&Flags, 2, 1, U_LE},
        {NULL, 2, 1, U_LE}, /*  Reserved field */
        {Path->poPoints->Data, 4, 2 * Path->Elements,
         U_XE}, /*  raw OID, so no leading Elements to omit */
        {Path->poTypes->Data, 1, Path->Elements,
         U_XE}, /*  raw OID, so no leading Elements to omit */
        {NULL, pad, (pad ? 1 : 0),
         (pad ? U_XE : U_XX)}, /* if no padding is needed the Units will be
                                  zero and nothing will happen */
...
/**
    \brief  Create and set a U_PMF_PATH PseudoObject that uses U_PMF_POINT (int
   16) coordinates
    \return Pointer to PseudoObject, NULL on error
    \param  Version    EmfPlusGraphicsVersion object
    \param  Path       U_DPSEUDO_OBJ containing a path.
...
U_PSEUDO_OBJ *U_PMF_PATH_set3(uint32_t Version, const U_DPSEUDO_OBJ *Path) {
    ...
    U_PMF_POINT *Points16 = POINTF_To_POINT16_LE(
        (U_PMF_POINTF *)Path->poPoints->Data, Path->Elements);
    if (!Points16) {
        return (NULL);
    }
    const U_SERIAL_DESC List[] = {
        ...
        {&Flags, 2, 1, U_LE},
        {NULL, 2, 1, U_LE}, /*  Reserved field */
        {Points16, 2, 2 * Path->Elements,
         U_XE}, /*  raw data, so no leading Elements to omit */
        {Path->poTypes->Data, 1, Path->Elements,
         U_XE}, /*  raw data, so no leading Elements to omit */
        {NULL, pad, (pad ? 1 : 0),
         (pad ? U_XE : U_XX)}, /* if no padding is needed the Units will be
...
*/
U_PSEUDO_OBJ *U_PMF_PEN_set(uint32_t Version, const U_PSEUDO_OBJ *PenData,
                            const U_PSEUDO_OBJ *Brush) {
    if (!PenData || (PenData->Type != U_PMF_PENDATA_OID)) {
        return (NULL);
    }
    if (!Brush || (Brush->Type != U_PMF_BRUSH_OID)) {
        return (NULL);
    }
    const U_SERIAL_DESC List[] = {{&Version, 4, 1, U_LE},
                                  {NULL, 4, 1, U_LE},
...
/**
    ...
    \return Pointer to PseudoObject, NULL on error
    \param  Version    EmfPlusGraphicsVersion object
    \param  Count      Number of CHILD nodes.  This is one less than the total
   number of U_PMF_REGIONNODE objects in Nodes.
    \param  Nodes      U_PSEUDO_OBJ containing U_PMF_REGIONNODE object (Nodes
   defining region, may be a single element or a binary tree)

    EMF+ manual 2.2.1.8, Microsoft name: EmfPlusRegion Object
*/
U_PSEUDO_OBJ *U_PMF_REGION_set(uint32_t Version, uint32_t Count,
                               const U_PSEUDO_OBJ *Nodes) {
    if (!Nodes || Nodes->Type != U_PMF_REGIONNODE_OID)
        return (NULL);
    const U_SERIAL_DESC List[] = {{&Version, 4, 1, U_LE},
                                  {&Count, 4, 1, U_LE},
...
/**
    ...
    \return Pointer to PseudoObject, NULL on error
    \param  Sfs       pointer to U_PMF_STRINGFORMAT structure, with no variable
   part
    \param  Sfd       (optional) U_PSEUDO_OBJ containing U_PMF_STRINGFORMATDATA
   object
...
U_PSEUDO_OBJ *U_PMF_STRINGFORMAT_set(U_PMF_STRINGFORMAT *Sfs,
    ...
    if (Sfd) {
        if ((!Sfs->TabStopCount && !Sfs->RangeCount) ||
            (Sfd->Type != U_PMF_STRINGFORMATDATA_OID))
            return (NULL);
    } else {
        if (Sfs->TabStopCount || Sfs->RangeCount)
            return (NULL);
...
/**
    ...
    \return Pointer to PseudoObject, NULL on error
    \param  BrushID     U_PMF_BRUSH object in the EMF+ object table (0-63,
   inclusive)
...
U_PSEUDO_OBJ *U_PMF_4NUM_set(uint32_t BrushID) {
    ...
    const U_SERIAL_DESC List[] = {{&BrushID, 4, 1, U_LE}, {NULL, 0, 0, U_XX}};
...
*/
U_PSEUDO_OBJ *U_PMF_ARGB_set(uint8_t Alpha, uint8_t Red, uint8_t Green,
                             uint8_t Blue) {
    const U_SERIAL_DESC List[] = {{&Blue, 1, 1, U_XE},
                                  {&Green, 1, 1, U_XE},
                                  {&Red, 1, 1, U_XE},
                                  {&Alpha, 1, 1, U_XE},
                                  {NULL, 0, 0, U_XX}};
...
/**
    ...
    \return Pointer to PseudoObject containing the count, followed by the array
   of colors, NULL on error
...
U_PSEUDO_OBJ *U_PMF_ARGBN_set(uint32_t Count, U_PMF_ARGB *Colors) {
    const U_SERIAL_DESC List[] = {
        {&Count, 4, 1, U_LE}, {Colors, 4, Count, U_XE}, {NULL, 0, 0, U_XX}};
...
/**
    ...
    EMF+ manual 2.2.2.1, Microsoft name: EmfPlusARGB Object
*/
U_PMF_ARGB U_PMF_ARGBOBJ_set(uint8_t Alpha, uint8_t Red, uint8_t Green,
                             uint8_t Blue) {
    U_PMF_ARGB argb;
    char *ptr = (char *)&argb;
...
/**
    ...
    \param  Bm       U_PSEUDO_OBJ containing an U_PMF_BITMAPDATA or
   U_PMF_COMPRESSEDIMAGE object
...
U_PSEUDO_OBJ *U_PMF_BITMAP_set(const U_PMF_BITMAP *Bs, const U_PSEUDO_OBJ *Bm) {
    ...
    if (Bm->Type != U_PMF_BITMAPDATA_OID &&
        Bm->Type != U_PMF_COMPRESSEDIMAGE_OID)
        return (NULL);
    uint32_t Pad =
        UP4(Bm->Used) -
        Bm->Used; /* undocumented padding, must be present for at least PNG */
    const U_SERIAL_DESC List[] = {{Bs, 4, 5, U_LE},
...
/**
    ...
    \param  Bm     An array of bytes, meaning depends on fields in U_PMF_BITMAP
   object and the PixelFormat enumeration.

    EMF+ manual 2.2.2.3, Microsoft name: EmfPlusBitmapData Object
*/
U_PSEUDO_OBJ *U_PMF_BITMAPDATA_set(const U_PSEUDO_OBJ *Ps, int cbBm,
                                   const char *Bm) {
    if (Ps && (Ps->Type != U_PMF_PALETTE_OID))
        return (NULL);
    if (!Bm && cbBm)
        return (NULL);
    const U_SERIAL_DESC List[] = {
...
/**
    ...
    \param  Elements   number of elements in Positions, must agree with the
   number of Colors.
    \param  Positions  positions along gradient line. The first position MUST be
   0.0 and the last MUST be 1.0.
    \param  Colors     U_PSEUDO_OBJ containing an array of U_PMF_ARGB objects:
   object colors at positions on gradient line

    EMF+ manual 2.2.2.4, Microsoft name: EmfPlusBlendColors Object
*/
U_PSEUDO_OBJ *U_PMF_BLENDCOLORS_set(uint32_t Elements, const U_FLOAT *Positions,
                                    const U_PSEUDO_OBJ *Colors) {
    if (!Colors || !Positions ||
        Colors->Type != (U_PMF_ARGB_OID | U_PMF_ARRAY_OID)) {
        return (NULL);
    }
    uint32_t CElements = (Colors->Used - 4) / 4;
    if (CElements != Elements) {
        return (NULL);
    }
    const U_SERIAL_DESC List[] = {
        {&CElements, 4, 1, U_LE},
        {Positions, 4, CElements, U_LE},
        {Colors->Data + 4, Colors->Used - 4, 1,
         U_XE}, /* omit Elements part of this PseudoObject */
        {NULL, 0, 0, U_XX}};
...
/**
    ...
    Positions always start at 0.0 and always end at 1.0.  It is not well
   documented but other
...
U_PSEUDO_OBJ *U_PMF_BLENDCOLORS_linear_set(uint32_t Elements,
    ...
    dB = ((double)EndColor.Blue - (double)StartColor.Blue) /
         (double)(Elements - 1);
    dG = ((double)EndColor.Green - (double)StartColor.Green) /
         (double)(Elements - 1);
    dR = ((double)EndColor.Red - (double)StartColor.Red) /
         (double)(Elements - 1);
    dA = ((double)EndColor.Alpha - (double)StartColor.Alpha) /
         (double)(Elements - 1);
    P = StartPos;
    B = StartColor.Blue;
    G = StartColor.Green;
    R = StartColor.Red;
    A = StartColor.Alpha;
    for (i = 0; i < Elements; i++, pC++,
        pP++) { /* hopefully the rounding errors are not a problem, used
                   doubles to minimize that */
        *pP = P;
        P += dP;
...
/**
    ...
    \param  Positions  positions along gradient line.  The first position MUST
   be 0.0 and the last MUST be 1.0.
...
U_PSEUDO_OBJ *U_PMF_BLENDFACTORS_set(uint32_t Elements,
    ...
    if (!Positions || !Factors)
        return (NULL);
    const U_SERIAL_DESC List[] = {{&Elements, 4, 1, U_LE},
...
U_PSEUDO_OBJ *U_PMF_BLENDFACTORS_linear_set(uint32_t Elements,
    ...
    F = StartFactor;
    for (i = 0; i < Elements;
         i++) { /* hopefully the rounding errors are not a
                   problem, used doubles to minimize that */
        *pP++ = P;
        P += dP;
        *pF++ = F;
        F += dF;
...
U_PSEUDO_OBJ *U_PMF_CHARACTERRANGE_set(int32_t First, int32_t Length) {
    const U_SERIAL_DESC List[] = {
        {&First, 4, 1, U_LE}, {&Length, 4, 1, U_LE}, {NULL, 0, 0, U_XX}};
...
/**
    ...
    \param  Widths     Array of U_FLOAT Line or gap widths (0.0 <-> 1.0,
   fraction of total line width )
...
/**
    ...
    \param  Image       Stored image in one of the supported formats (GIF, PNG,
   etc.).
...
U_PSEUDO_OBJ *U_PMF_COMPRESSEDIMAGE_set(int32_t cbImage, const char *Image) {
    if (!cbImage || !Image)
        return (NULL);
    const U_SERIAL_DESC List[] = {{Image, cbImage, 1, U_XE},
                                  {NULL, 0, 0, U_XX}};
...
U_PSEUDO_OBJ *U_PMF_CUSTOMENDCAPDATA_set(const U_PSEUDO_OBJ *Clc) {
    if (!Clc || Clc->Type != U_PMF_CUSTOMLINECAP_OID)
        return (NULL);
    /* PO Used is size_t, might be 8 bytes, value in record must be 4 bytes */
...
/**
    ...
    \return Pointer to PseudoObject, NULL on error
    \param  Width       Arrow cap width (is multiplied by line width before
   draw)
    \param  Height      Arrow cap length (is multiplied by line width before
   draw)
...
*/
U_PSEUDO_OBJ *U_PMF_CUSTOMLINECAPARROWDATA_set(
    U_FLOAT Width, U_FLOAT Height, U_FLOAT MiddleInset, uint32_t FillState,
    uint32_t StartCap, uint32_t EndCap, uint32_t Join, U_FLOAT MiterLimit,
    U_FLOAT WidthScale) {
    const U_SERIAL_DESC List[] = {
        {&Width, 4, 1, U_LE},
        {&Height, 4, 1, U_LE},
...
/**
    ...
    \param  MiterLimit Maximum (miter length / line width)
    \param  WidthScale Scale for U_PMF_CUSTOMLINECAP object
    \param  Clcod      U_PSEUDO_OBJ containing a U_PMF_CUSTOMLINECAPOPTIONALDATA
   object
...
U_PSEUDO_OBJ *U_PMF_CUSTOMLINECAPDATA_set(uint32_t Flags, uint32_t Cap,
                                          U_FLOAT Inset, uint32_t StartCap,
                                          uint32_t EndCap, uint32_t Join,
                                          U_FLOAT MiterLimit,
                                          U_FLOAT WidthScale,
                                          const U_PSEUDO_OBJ *Clcod) {
    if (!Clcod || Clcod->Type != U_PMF_CUSTOMLINECAPOPTIONALDATA_OID)
        return (NULL);
    const U_SERIAL_DESC List[] = {
        {&Flags, 4, 1, U_LE},
        {&Cap, 4, 1, U_LE},
...
/**
    ...
    EMF+ manual 2.2.2.14, Microsoft name: EmfPlusCustomLineCapOptionalData
   Object
*/
U_PSEUDO_OBJ *U_PMF_CUSTOMLINECAPOPTIONALDATA_set(const U_PSEUDO_OBJ *Fill,
                                                  const U_PSEUDO_OBJ *Line) {
    if (Fill && (Fill->Type != U_PMF_FILLPATHOBJ_OID))
        return (NULL);
    if (Line && (Line->Type != U_PMF_LINEPATH_OID))
        return (NULL);
    const U_SERIAL_DESC List[] = {
        ...
        {NULL, 0, 0, U_XX}};
    U_PSEUDO_OBJ *po =
        U_PMF_SERIAL_set(U_PMF_CUSTOMLINECAPOPTIONALDATA_OID, List);
    return (po);
}
...
U_PSEUDO_OBJ *U_PMF_CUSTOMSTARTCAPDATA_set(const U_PSEUDO_OBJ *Clc) {
    if (!Clc || Clc->Type != U_PMF_CUSTOMLINECAP_OID)
        return (NULL);
    const U_SERIAL_DESC List[] = {{Clc->Data, Clc->Used, 1, U_XE},
                                  {NULL, 0, 0, U_XX}};
...
/**
    \brief  Utility function to create and set a U_PMF_DASHEDLINEDATA
   PseudoObject from one of a predefined set of patterns
    \return Pointer to PseudoObject, NULL on error
...
U_PSEUDO_OBJ *U_PMF_DASHEDLINEDATA_set2(U_FLOAT Unit, int StdPat) {
    ...
    if ((StdPat <= 0) || (StdPat > U_DD_LongDotLongDot)) {
        return (NULL);
    }
    p = &(SB[StdPat][0]);
...
/**
    \brief  Utility function to create and set a U_PMF_DASHEDLINEDATA
   PseudoObject from the bits that are set in a uint32_t
    \return Pointer to PseudoObject, NULL on error
    \param  Unit       Length of the repeat unit
    \param  BitPat     uint32_t holding the bit pattern, the lowest order bit
   MUST be set and the highest order MUST be clear.

    Make a line with a dot/dash pattern defined by the bits in the BitPat value.
   If a bit is set it is drawn,
    if clear it is not.  Every bit drawn has length Unit/32, and consecutive
   drawn bits are merged together.
    The lowest order bit is the first bit that may be drawn, the highest the
   last.
...
U_PSEUDO_OBJ *U_PMF_DASHEDLINEDATA_set3(U_FLOAT Unit, uint32_t BitPat) {
    ...
    if (!(0x00000001 & BitPat))
        return (
            NULL); /* Pattern must start with a drawn segment, this bit must be
                      set */
    if (0x80000000 & BitPat)
        return (NULL); /* Pattern must end with an undrawn segment, this bit
                          must be clear */
    int i = 0;
    int k;
    int lastType = 1;
...
U_PSEUDO_OBJ *U_PMF_FILLPATHOBJ_set(const U_PSEUDO_OBJ *Path) {
    if (!Path || (Path->Type != U_PMF_PATH_OID))
        return (NULL);
    const U_SERIAL_DESC List[] = {{Path->Data, Path->Used, 1, U_XE},
                                  {NULL, 0, 0, U_XX}};
...
U_PSEUDO_OBJ *U_PMF_FOCUSSCALEDATA_set(U_FLOAT ScaleX, U_FLOAT ScaleY) {
    uint32_t tmp = 2;
    const U_SERIAL_DESC List[] = {{&tmp, 4, 1, U_LE},
                                  {&ScaleX, 4, 1, U_LE},
                                  {&ScaleY, 4, 1, U_LE},
                                  {NULL, 0, 0, U_XX}};
...
/**
    \brief  Create and set a U_PMF_GRAPHICSVERSION object (Signature always set
   to 0xDBC01)
    \return Pointer to PseudoObject, NULL on error
...
/**
    \brief  Create and set a U_PMF_GRAPHICSVERSION object Structure (Signature
   always set to 0xDBC01)
...
/**
    ...
    \param  Fg         U_PSEUDO_OBJ containing a U_ARGB object, Foreground hatch
   pattern line color
    \param  Bg         U_PSEUDO_OBJ containing a U_ARGB object, Background hatch
   pattern line color

    EMF+ manual 2.2.2.20, Microsoft name: EmfPlusHatchBrushData Object
*/
U_PSEUDO_OBJ *U_PMF_HATCHBRUSHDATA_set(uint32_t Style, const U_PSEUDO_OBJ *Fg,
                                       const U_PSEUDO_OBJ *Bg) {
    if (!Fg || (Fg->Type != U_PMF_ARGB_OID))
        return (NULL);
    if (!Bg || (Bg->Type != U_PMF_ARGB_OID))
        return (NULL);
    const U_SERIAL_DESC List[] = {{&Style, 4, 1, U_LE},
...
/**
    ...
    \return Pointer to PseudoObject, NULL on error
    \param  Value 7 bit signed integer (stored in an integer, range 63 <-> -64,
   inclusive)
...
U_PSEUDO_OBJ *U_PMF_INTEGER7_set(int Value) {
    ...
    U_PSEUDO_OBJ *po =
        U_PO_create((char *)&utmp, 1, 1,
                    U_PMF_INTEGER7_OID); /* simple method is OK, no
                                            possibility of Endian issues */
    return (po);
}
...
/**
    ...
    \return Pointer to PseudoObject, NULL on error
    \param  Value 15 bit signed integer (stored in an integer, range 32677 <->
   -32678, inclusive)
...
U_PSEUDO_OBJ *U_PMF_INTEGER15_set(int Value) {
    uint16_t utmp;
    if (Value < -32678 || Value > 32677)
        return (NULL);
    utmp = U_TEST_INT15 | (U_MASK_INT15 & *(unsigned int *)&Value);
    const U_SERIAL_DESC List[] = {{&utmp, 2, 1, U_BE}, {NULL, 0, 0, U_XX}};
    U_PSEUDO_OBJ *po = U_PMF_SERIAL_set(U_PMF_INTEGER15_OID, List);
    return (po);
}

/**
    \brief  Create and set a U_PMF_LANGUAGEIDENTIFIER value in 4 byte unsigned
   int, in NATIVE byte order
    \return LID value in least significant two bytes and 0 in most significant
   two bytes.
...
U_PMF_LANGUAGEIDENTIFIER U_PMF_LANGUAGEIDENTIFIEROBJ_set(int SubLId,
    ...
    utmp32 = ((SubLId & U_FF_MASK_SUBLID) << U_FF_SHFT_SUBLID) |
             ((PriLId & U_FF_MASK_PRILID) << U_FF_SHFT_PRILID);
    return (utmp32);
}
...
/**
    ...
    \return Pointer to PseudoObject, NULL on error
    \param  LId       Language Identifier as produced by
   U_PMF_LANGUAGEIDENTIFIEROBJ_set().
...
U_PSEUDO_OBJ *U_PMF_LANGUAGEIDENTIFIER_set(U_PMF_LANGUAGEIDENTIFIER LId) {
    ...
    const U_SERIAL_DESC List[] = {{&utmp16, 2, 1, U_LE}, {NULL, 0, 0, U_XX}};
...
/**
    ...
    \param  Lgbod      U_PSEUDO_OBJ containing a
   U_PMF_LINEARGRADIENTBRUSHOPTIONALDATA object (variable part of a
   U_PMF_LINEARGRADIENTBRUSHDATA object)


    EMF+ manual 2.2.2.24, Microsoft name: EmfPlusLinearGradientBrushData Object
*/
U_PSEUDO_OBJ *
U_PMF_LINEARGRADIENTBRUSHDATA_set(const U_PMF_LINEARGRADIENTBRUSHDATA *Lgbd,
                                  const U_PSEUDO_OBJ *Lgbod) {
    if (!Lgbd || !Lgbod ||
        (Lgbod->Type != U_PMF_LINEARGRADIENTBRUSHOPTIONALDATA_OID))
        return (NULL);
    const U_SERIAL_DESC List[] = {
        {Lgbd, 4, 6, U_LE},
        {&(Lgbd->StartColor), 4, 2, U_XE},
        {&(Lgbd->StartColor), 4, 2,
         U_XE}, /* repeat the start/end colors.  Supposedly reserved. */
        //    {NULL,                                4,           2, U_LE}, /*
        //    zero fill the two Reserved fields, no matter what is passed in */
        {(Lgbod->Used ? Lgbod->Data : NULL), Lgbod->Used, 1,
         U_XE}, /* optional Data can exist and Used can be zero, SERIAL_set
                   would throw an error on that */
        {NULL, 0, 0, U_XX}};
    U_PSEUDO_OBJ *po =
        U_PMF_SERIAL_set(U_PMF_LINEARGRADIENTBRUSHDATA_OID, List);
    return (po);
}
...
/**
    ...
    \return Pointer to PseudoObject, NULL on error
    \param  Flags      Bits are set that indicate which of the following were
   included. The caller must clear before passing it in.
    \param  Tm         (optional) U_PSEUDO_OBJ containing a
   U_PMF_TRANSFORMMATRIX object
    \param  Bc         (optional) U_PSEUDO_OBJ containing a U_PMF_BLENDCOLORS
   object or NULL
    \param  BfH        (optional) U_PSEUDO_OBJ containing a U_PMF_BLENDFACTORS
   (H) object or NULL
    \param  BfV        (optional) U_PSEUDO_OBJ containing a U_PMF_BLENDFACTORS
   (V) object or NULL (WARNING, GDI+ defines this field but does not render it.
   DO NOT USE.)
   ...
   Object
    ...
    The rectangular gradients repeat in a tiled pattern.  Tm can rotate and
   offset the gradient within each tile.
    The gradient wraps when it is offset.

*/
U_PSEUDO_OBJ *U_PMF_LINEARGRADIENTBRUSHOPTIONALDATA_set(
    uint32_t *Flags, const U_PSEUDO_OBJ *Tm, const U_PSEUDO_OBJ *Bc,
    const U_PSEUDO_OBJ *BfH, const U_PSEUDO_OBJ *BfV) {
    if (!Flags)
        return (NULL);
    if (Tm && (Tm->Type != U_PMF_TRANSFORMMATRIX_OID))
        return (NULL);
    if (Bc && (Bc->Type != U_PMF_BLENDCOLORS_OID))
        return (NULL);
    if (BfH && (BfH->Type != U_PMF_BLENDFACTORS_OID))
        return (NULL);
    if (BfV && (BfV->Type != U_PMF_BLENDFACTORS_OID))
        return (NULL);
    if (Bc && (BfH || BfV))
        return (NULL);
    const U_SERIAL_DESC List[] = {
        ...
        {NULL, 0, 0, U_XX}};
    U_PSEUDO_OBJ *po =
        U_PMF_SERIAL_set(U_PMF_LINEARGRADIENTBRUSHOPTIONALDATA_OID, List);
    if (Tm) {
        *Flags |= U_BD_Transform;
    }
    if (Bc) {
...
/**
    ...
    \param  Elements   Members in Lengths
    \param  Pd         Array of U_PMF_ARGB holding colors of palettes. (Palette
   Data)

    EMF+ manual 2.2.2.28, Microsoft name: EmfPlusPalette Object
*/
U_PSEUDO_OBJ *U_PMF_PALETTE_set(uint32_t Flags, uint32_t Elements,
                                const U_PMF_ARGB *Pd) {
    if (!Pd)
        return (NULL);
    const U_SERIAL_DESC List[] = {{&Flags, 4, 1, U_LE},
                                  {&Elements, 4, 1, U_LE},
                                  {Pd, 4, Elements, U_XE},
                                  {NULL, 0, 0, U_XX}};
...
/**
    ...
    \param  Gradient     U_PSEUDO_OBJ containing an Array of U_PMF_ARGB holding
   colors of Gradient
    \param  Boundary     U_PSEUDO_OBJ containing a U_PMF_BOUNDARYPATHDATA or
   U_PMF_BOUNDARYPOINTDATA object.  (Boundary Data)
    \param  Data         variable part of U_PMF_LINEARGRADIENTBRUSHDATA, exact
   composition depends on Flags

    EMF+ manual 2.2.2.29, Microsoft name: EmfPlusPathGradientBrushData Object
*/
U_PSEUDO_OBJ *U_PMF_PATHGRADIENTBRUSHDATA_set(uint32_t Flags, int32_t WrapMode,
    ...
    if ((Flags & U_BD_Path) &&
        (!Boundary || (Boundary->Type != U_PMF_BOUNDARYPATHDATA_OID)))
        return (NULL);
    if (!(Flags & U_BD_Path) &&
        (!Boundary || (Boundary->Type != U_PMF_BOUNDARYPOINTDATA_OID)))
        return (NULL);
    if (!Gradient || (Gradient->Type != (U_PMF_ARGB_OID | U_PMF_ARRAY_OID)))
        return (NULL);
    if (!(Flags & U_BD_Transform) && !(Flags & U_BD_PresetColors) &&
        !(Flags & U_BD_BlendFactorsH) && !(Flags & U_BD_FocusScales) &&
        (!Data || (Data->Type != U_PMF_PATHGRADIENTBRUSHOPTIONALDATA_OID)))
        return (NULL);
...
/**
    ...
    \return Pointer to PseudoObject, NULL on error
    \param  Flags        PaletteStyle flags
    \param  Tm           U_PSEUDO_OBJ containing a U_PMF_TRANSFORMMATRIX.
   (Transformation matrix)
    \param  Pd           U_PSEUDO_OBJ containing a U_PMF_BLENDCOLORS or
   U_PMF_BLENDFACTORS object.  (Pattern Data)
    \param  Fsd          U_PSEUDO_OBJ containing a U_PMF_FOCUSSSCALEDATA object.
   (Focus Scale Data)
...
U_PSEUDO_OBJ *U_PMF_PATHGRADIENTBRUSHOPTIONALDATA_set(uint32_t Flags,
    ...
    if (Tm && (Tm->Type != U_PMF_TRANSFORMMATRIX_OID))
        return (NULL);
    if (Pd && !(Flags & (U_BD_PresetColors | U_BD_BlendFactorsH)))
        return (NULL);
    if ((Flags & U_BD_PresetColors) && ((Flags & U_BD_BlendFactorsH) || !Pd ||
                                        (Pd->Type != U_PMF_BLENDCOLORS_OID)))
        return (NULL);
    if ((Flags & U_BD_BlendFactorsH) && ((Flags & U_BD_PresetColors) || !Pd ||
                                         (Pd->Type != U_PMF_BLENDFACTORS_OID)))
        return (NULL);
    if (Fsd && !(Flags & U_BD_FocusScales))
        return (NULL);
    if ((Flags & U_BD_FocusScales) &&
        (!Fsd || (Fsd->Type != U_PMF_BLENDCOLORS_OID)))
        return (NULL);
    const U_SERIAL_DESC List[] = {
        {(Tm ? Tm->Data : NULL), (Tm ? Tm->Used : 0), 1, U_XE},
        ...
        {NULL, 0, 0, U_XX}};
    U_PSEUDO_OBJ *po =
        U_PMF_SERIAL_set(U_PMF_PATHGRADIENTBRUSHOPTIONALDATA_OID, List);
    return (po);
}
...
/**
    ...
    \param  Ppt          Array of unsigned bytes, lower 4 bits hold the
   PathPointType flag upper 4 bits hold the PathPointType enumeration.
...
U_PSEUDO_OBJ *U_PMF_PATHPOINTTYPE_set(uint32_t Elements, const uint8_t *Ppt) {
    if (!Elements || !Ppt)
        return (NULL);
    const U_SERIAL_DESC List[] = {
        {&Elements, 4, 1, U_LE}, {Ppt, 1, Elements, U_XE}, {NULL, 0, 0, U_XX}};
    U_PSEUDO_OBJ *po =
        U_PMF_SERIAL_set(U_PMF_PATHPOINTTYPE_OID | U_PMF_ARRAY_OID, List);
    return (po);
}

/**
    \brief  Create and set an ARRAY of U_PMF_PATHPOINTTYPE objects, with a
   preceding Elements count
    \return Pointer to PseudoObject, NULL on error
    \param  Elements Number of elements to add. First is added once and Others
   Elements-1 times.
    \param  First    Apply to first point,      unsigned byte, lower 4 bits hold
   the PathPointType flag upper 4 bits hold the PathPointType enumeration.
    \param  Others   Apply to all other points, unsigned byte, lower 4 bits hold
   the PathPointType flag upper 4 bits hold the PathPointType enumeration.

    EMF+ manual 2.2.2.31, Microsoft name: EmfPlusPathPointType Object
*/
U_PSEUDO_OBJ *U_PMF_PATHPOINTTYPE_set2(uint32_t Elements, uint8_t First,
                                       uint8_t Others) {
    if (!Elements)
        return (NULL);
    const U_SERIAL_DESC List[] = {
        ...
        {&First, 1, 1, U_XE},
        {&Others, 1, Elements - 1,
         U_RP}, /* replicate the one value N-1 times */
        {NULL, 0, 0, U_XX}};
    U_PSEUDO_OBJ *po =
        U_PMF_SERIAL_set(U_PMF_PATHPOINTTYPE_OID | U_PMF_ARRAY_OID, List);
    return (po);
...
/**
    ...
    \return Pointer to PseudoObject containing first the number of elements,
   then an array of U_PMF_PATHPOINTTYPERLE, NULL on error
    \param  Elements     Number of entries in the arrays
    \param  Bz           Array of unsigned bytes, if not zero, element has
   Bezier bit set
...
*/
U_PSEUDO_OBJ *U_PMF_PATHPOINTTYPERLE_set(uint32_t Elements, const uint8_t *Bz,
                                         const uint8_t *RL,
                                         const uint8_t *Ppte) {
    uint8_t utmp;
    if (!Bz || !RL || !Ppte)
    ...
    U_PSEUDO_OBJ *po =
        U_PO_create(NULL, 4 + 2 * Elements, 0,
                    U_PMF_PATHPOINTTYPERLE_OID | U_PMF_ARRAY_OID);
    ...
    if (po) {
        ...
        for (; Elements; Elements--, Bz++, RL++, Ppte++) {
            ...
            if (*RL > 0x3F)
                goto end; /* run length too big for field */

            utmp = (*Bz ? 1 : 0) |
                   ((*RL & 0x3F) << 2); /* bit 1 is not used and is set to 0 */
...
/**
    ...
    \param  Pod       U_PSEUDO_OBJ containing first the PenData flags then a
   U_PMF_PENOPTIONALDATA object (the second part
                        may be an empty if Flags is 0)


    EMF+ manual 2.2.2.33, Microsoft name: EmfPlusPenData Object
*/
U_PSEUDO_OBJ *U_PMF_PENDATA_set(uint32_t Unit, U_FLOAT Width,
                                const U_PSEUDO_OBJ *Pod) {
    if (Pod && ((Pod->Type != U_PMF_PENOPTIONALDATA_OID) || Pod->Used < 4))
        return (NULL);
    const U_SERIAL_DESC List[] = {
        {(Pod ? Pod->Data : NULL), 4, 1,
         U_XE}, /* the Flags field, clear if no optional data */
        {&Unit, 4, 1, U_LE},
        {&Width, 4, 1, U_LE},
        /* next is the (optional) U_PMF_PENOPTIONALDATA part or a terminator */
        {(Pod ? Pod->Data + 4 : NULL), (Pod ? Pod->Used - 4 : 0), (Pod ? 1 : 0),
         (Pod ? U_XE : U_XX)},
        {NULL, 0, 0, U_XX}};
...
/**
    ...
    \return Pointer to PseudoObject, NULL on error.  Returned PO contains first
   the Flags, then the PO proper.
   ...
   object (Transformation matrix)
    ...
    \param  DLData          U_PSEUDO_OBJ containing a U_PMF_DASHEDLINEDATA
   object     Dash and space widths
    \param  PenAlignment    PenAlignment enumeration
    \param  CmpndLineData   U_PSEUDO_OBJ containing a U_PMF_COMPOUNDLINEDATA
   object    Compount Line (parallel lines drawn instead of one)
    \param  CSCapData       U_PSEUDO_OBJ containing a U_PMF_CUSTOMSTARTCAPDATA
   object  Custom start cap
    \param  CECapData       U_PSEUDO_OBJ containing a U_PMF_CUSTOMENDCAPDATA
   object    Custom end cap


    EMF+ manual 2.2.2.34, Microsoft name: EmfPlusPenOptionalData Object
*/
U_PSEUDO_OBJ *
U_PMF_PENOPTIONALDATA_set(uint32_t Flags, U_PSEUDO_OBJ *Tm, int32_t StartCap,
                          int32_t EndCap, uint32_t Join, U_FLOAT MiterLimit,
                          int32_t Style, int32_t DLCap, U_FLOAT DLOffset,
                          U_PSEUDO_OBJ *DLData, int32_t PenAlignment,
                          U_PSEUDO_OBJ *CmpndLineData, U_PSEUDO_OBJ *CSCapData,
                          U_PSEUDO_OBJ *CECapData) {
    if ((Flags & U_PD_Transform) &&
        (!Tm || (Tm->Type != U_PMF_TRANSFORMMATRIX_OID)))
        return (NULL);
    if ((Flags & U_PD_DLData) &&
        (!DLData || (DLData->Type != U_PMF_DASHEDLINEDATA_OID)))
        return (NULL);
    if ((Flags & U_PD_CLData) &&
        (!CmpndLineData || (CmpndLineData->Type != U_PMF_COMPOUNDLINEDATA_OID)))
        return (NULL);
    if ((Flags & U_PD_CustomStartCap) &&
        (!CSCapData || (CSCapData->Type != U_PMF_CUSTOMSTARTCAPDATA_OID)))
        return (NULL);
    if ((Flags & U_PD_CustomEndCap) &&
        (!CECapData || (CECapData->Type != U_PMF_CUSTOMENDCAPDATA_OID)))
        return (NULL);

    /* prepend the Flags field to the PseudoObject proper */
    const U_SERIAL_DESC List[] = {
        {&Flags, 4, 1, U_LE},
        {((Flags & U_PD_Transform) ? Tm->Data : NULL),
         ((Flags & U_PD_Transform) ? Tm->Used : 0), 1, U_XE},
        {((Flags & U_PD_StartCap) ? (char *)&StartCap : NULL),
         ((Flags & U_PD_StartCap) ? 4 : 0), 1, U_LE},
        {((Flags & U_PD_EndCap) ? (char *)&EndCap : NULL),
         ((Flags & U_PD_EndCap) ? 4 : 0), 1, U_LE},
        {((Flags & U_PD_Join) ? (char *)&Join : NULL),
         ((Flags & U_PD_Join) ? 4 : 0), 1, U_LE},
        {((Flags & U_PD_MiterLimit) ? (char *)&MiterLimit : NULL),
         ((Flags & U_PD_MiterLimit) ? 4 : 0), 1, U_LE},
        {((Flags & U_PD_LineStyle) ? (char *)&Style : NULL),
         ((Flags & U_PD_LineStyle) ? 4 : 0), 1, U_LE},
        {((Flags & U_PD_DLCap) ? (char *)&DLCap : NULL),
         ((Flags & U_PD_DLCap) ? 4 : 0), 1, U_LE},
        {((Flags & U_PD_DLOffset) ? (char *)&DLOffset : NULL),
         ((Flags & U_PD_DLOffset) ? 4 : 0), 1, U_LE},
        {((Flags & U_PD_DLData) ? DLData->Data : NULL),
         ((Flags & U_PD_DLData) ? DLData->Used : 0), 1, U_XE},
        {((Flags & U_PD_NonCenter) ? (char *)&PenAlignment : NULL),
         ((Flags & U_PD_NonCenter) ? 4 : 0), 1, U_LE},
        {((Flags & U_PD_CLData) ? CmpndLineData->Data : NULL),
         ((Flags & U_PD_CLData) ? CmpndLineData->Used : 0), 1, U_XE},
        {((Flags & U_PD_CustomStartCap) ? CSCapData->Data : NULL),
         ((Flags & U_PD_CustomStartCap) ? CSCapData->Used : 0), 1, U_XE},
        {((Flags & U_PD_CustomEndCap) ? CECapData->Data : NULL),
         ((Flags & U_PD_CustomEndCap) ? CECapData->Used : 0), 1, U_XE},
        {NULL, 0, 0, U_XX}};
...
U_PSEUDO_OBJ *U_PMF_POINT_set(uint32_t Elements, const U_PMF_POINT *Coords) {
    ...
    const U_SERIAL_DESC List[] = {{&Elements, 4, 1, U_LE},
                                  ...
                                  {NULL, 0, 0, U_XX}};
    U_PSEUDO_OBJ *po =
        U_PMF_SERIAL_set(U_PMF_POINT_OID | U_PMF_ARRAY_OID, List);
    return (po);
}

/**
    \brief  Create and set an ARRAY of U_PMF_POINTF objects, with a leading
   Elements value
    \return Pointer to PseudoObject, NULL on error
    \param  Elements     Number of pairs of points in Coords
    \param  Coords       Array of X,Y pairs.
...
U_PSEUDO_OBJ *U_PMF_POINTF_set(uint32_t Elements, const U_PMF_POINTF *Coords) {
    ...
    const U_SERIAL_DESC List[] = {{&Elements, 4, 1, U_LE},
                                  ...
                                  {NULL, 0, 0, U_XX}};
    U_PSEUDO_OBJ *po =
        U_PMF_SERIAL_set(U_PMF_POINTF_OID | U_PMF_ARRAY_OID, List);
    return (po);
}
...
/**
    ...
    \return Pointer to PseudoObject, NULL on error
    \param  Elements     Number of pairs of points in Coords
    \param  Coords       Array of X,Y pairs.  These are absolute coordinates,
   they are converted to Relative here.
...
U_PSEUDO_OBJ *U_PMF_POINTR_set(uint32_t Elements, const U_PMF_POINTF *Coords) {
    ...
    U_PSEUDO_OBJ *po = U_PO_create(
        NULL, 4 + 4 * Elements, 0,
        U_PMF_POINTR_OID); /* not exactly an array, so no U_PMF_ARRAY_OID */
    U_PSEUDO_OBJ *holdpo = po;
    if (!po)
        goto end;
    ...
    for (Xf = Yf = 0.0; Elements; Elements--, Coords++) {
        ...
        X = (Xf >= UINT16_MAX ? UINT16_MAX
                              : (Xf <= INT16_MIN ? INT16_MIN : Xf));
        Y = (Yf >= UINT16_MAX ? UINT16_MAX
                              : (Yf <= INT16_MIN ? INT16_MIN : Yf));
        Xf = U_ROUND(Coords->X);
        Yf = U_ROUND(Coords->Y);

        /* this is not a very efficient method, too much mucking around with
         * memory */

        poi = U_PMF_INTEGER7_set(X);
        if (!poi)
            poi = U_PMF_INTEGER15_set(X); /* This one must work because of the
                                             range checking, above */
        po = U_PO_append(po, poi->Data, poi->Used);
        U_PO_free(&poi);
        if (!po)
            goto end;
        ...
        if (!poi)
            poi = U_PMF_INTEGER15_set(Y); /* This one must work because of the
                                             range checking, above */
        po = U_PO_append(po, poi->Data, poi->Used);
        U_PO_free(&poi);
        if (!po)
    ...
    /* Because the values stored were some unpredictable combination of 1 and 2
    bytes, the last byte may not end
...
*/
U_PSEUDO_OBJ *U_PMF_RECT4_set(int16_t X, int16_t Y, int16_t Width,
                              int16_t Height) {
    const U_SERIAL_DESC List[] = {{&X, 2, 1, U_LE},
                                  {&Y, 2, 1, U_LE},
                                  {&Width, 2, 1, U_LE},
                                  {&Height, 2, 1, U_LE},
                                  {NULL, 0, 0, U_XX}};
...
U_PSEUDO_OBJ *U_PMF_RECT_set(U_PMF_RECT *Rect) {
    const U_SERIAL_DESC List[] = {{Rect, 2, 4, U_LE}, {NULL, 0, 0, U_XX}};
    U_PSEUDO_OBJ *po = U_PMF_SERIAL_set(U_PMF_RECT_OID, List);
    return (po);
}
...
/**
    ...
    \return Pointer to PseudoObject, NULL on error.  PseudoObject contains
   Elements followed by the array of U_PMF_RECT objects.
...
U_PSEUDO_OBJ *U_PMF_RECTN_set(uint32_t Elements, U_PMF_RECT *Rects) {
    ...
    U_SERIAL_DESC *List =
        (U_SERIAL_DESC *)malloc((Elements + 2) * sizeof(U_SERIAL_DESC));
...
*/
U_PSEUDO_OBJ *U_PMF_RECTF4_set(U_FLOAT X, U_FLOAT Y, U_FLOAT Width,
                               U_FLOAT Height) {
    const U_SERIAL_DESC List[] = {{&X, 4, 1, U_LE},
                                  {&Y, 4, 1, U_LE},
                                  {&Width, 4, 1, U_LE},
                                  {&Height, 4, 1, U_LE},
                                  {NULL, 0, 0, U_XX}};
...
U_PSEUDO_OBJ *U_PMF_RECTF_set(U_PMF_RECTF *Rect) {
    const U_SERIAL_DESC List[] = {{Rect, 4, 4, U_LE}, {NULL, 0, 0, U_XX}};
    U_PSEUDO_OBJ *po = U_PMF_SERIAL_set(U_PMF_RECTF_OID, List);
    return (po);
}
...
/**
    ...
    \return Pointer to PseudoObject, NULL on error.  PseudoObject contains
   Elements followed by the array of U_PMF_RECTF objects.
    \param  Elements   Number of elements in Rects
...
U_PSEUDO_OBJ *U_PMF_RECTFN_set(uint32_t Elements, U_PMF_RECTF *Rects) {
    ...
    *Lptr = (U_SERIAL_DESC){NULL, 0, 0, U_XX};
    U_PSEUDO_OBJ *po =
        U_PMF_SERIAL_set(U_PMF_RECTF_OID | U_PMF_ARRAY_OID, List);
    free(List);
    return (po);
}
...
/**
    ...
    \param  Rnd       (optional) U_PSEUDO_OBJ containing a U_PMF_REGIONNODEPATH,
   U_PMF_RECTF, or U_PMF_REGIONNODECHILDNODES object (Region Node Data)
...
U_PSEUDO_OBJ *U_PMF_REGIONNODE_set(int32_t Type, const U_PSEUDO_OBJ *Rnd) {
    ...
    if (Rnd) {
        ...
        if ((pType > 0) && (pType != Type)) {
            return (NULL);
        }
        if ((pType == 0) &&
            ((Type < U_RNDT_And) || (Type > U_RNDT_Complement))) {
            return (NULL);
        }
        if ((Type == U_RNDT_Rect) && (Rnd->Type != U_PMF_RECTF_OID)) {
            return (NULL);
        }
        if ((Type == U_RNDT_Path) && (Rnd->Type != U_PMF_REGIONNODEPATH_OID)) {
            return (NULL);
        }

    } else { /* only U_RNDT_Empty and  U_RNDT_Infinite do not have data */
        if ((Type != U_RNDT_Empty) || (Type != U_RNDT_Infinite)) {
            return (NULL);
        }
    }
...
/**
    ...
    \return Pointer to PseudoObject containing a U_PMF_REGIONNODECHILDNODES_OID
...
U_PSEUDO_OBJ *U_PMF_REGIONNODECHILDNODES_set(const U_PSEUDO_OBJ *Left,
    ...
    if (!Left || (Left->Type != U_PMF_REGIONNODE_OID)) {
        return (NULL);
    }
    if (!Right || (Right->Type != U_PMF_REGIONNODE_OID)) {
        return (NULL);
    }

    const U_SERIAL_DESC List[] = {{Left->Data, Left->Used, 1, U_XE},
...
U_PSEUDO_OBJ *U_PMF_SOLIDBRUSHDATA_set(const U_PSEUDO_OBJ *Color) {
    if (!Color || (Color->Type != U_PMF_ARGB_OID)) {
        return (NULL);
    }

    const U_SERIAL_DESC List[] = {{Color->Data, Color->Used, 1, U_XE},
                                  {NULL, 0, 0, U_XX}};
...
/**
    ...
    \param  Ranges        (optional) U_PSEUDO_OBJ containing an array of
   U_PMF_CHARACTERRANGE objects
...
U_PSEUDO_OBJ *U_PMF_STRINGFORMATDATA_set(uint32_t TabStopCount,
    ...
    if (Ranges &&
        (Ranges->Type != (U_PMF_CHARACTERRANGE_OID | U_PMF_ARRAY_OID))) {
        return (NULL);
    }
...
/**
    ...
    \param  WrapMode      WrapMode enumeration
    \param  Tbod          U_PSEUDO_OBJ containing an
   U_PMF_TEXTUREBRUSHOPTIONALDATA object


    EMF+ manual 2.2.2.45, Microsoft name: EmfPlusTextureBrushData Object
*/
U_PSEUDO_OBJ *U_PMF_TEXTUREBRUSHDATA_set(uint32_t Flags, uint32_t WrapMode,
                                         const U_PSEUDO_OBJ *Tbod) {
    if (Flags & ~U_BD_MASKB) {
        return (NULL);
    } /* a bit was set that is not supported for this record */
    if (WrapMode > U_WM_Clamp) {
        return (NULL);
    }
    if (!Tbod || (Tbod->Type != (U_PMF_TEXTUREBRUSHOPTIONALDATA_OID))) {
        return (NULL);
    }
...
/**
    ...
    \return Pointer to PseudoObject, NULL on error
    \param  Tm            (optional) U_PSEUDO_OBJ containing an
   U_PMF_TRANSFORMMATRIX object
    \param  Image         (optional) U_PSEUDO_OBJ containing an U_PMF_IMAGE
   object
...
U_PSEUDO_OBJ *U_PMF_TEXTUREBRUSHOPTIONALDATA_set(const U_PSEUDO_OBJ *Tm,
    ...
    if (Image && (Image->Type != (U_PMF_IMAGE_OID))) {
        return (NULL);
    }
    ...
    const U_SERIAL_DESC List[] = {
        ...
        {NULL, 0, 0, U_XX}};
    U_PSEUDO_OBJ *po =
        U_PMF_SERIAL_set(U_PMF_TEXTUREBRUSHOPTIONALDATA_OID, List);
    return (po);
}
...
U_PSEUDO_OBJ *U_PMF_TRANSFORMMATRIX_set(U_PMF_TRANSFORMMATRIX *Tm) {
    ...
    const U_SERIAL_DESC List[] = {{Tm, 4, 6, U_LE}, {NULL, 0, 0, U_XX}};
...
U_PSEUDO_OBJ *U_PMF_IE_BLUR_set(U_FLOAT Radius, uint32_t ExpandEdge) {
    const U_SERIAL_DESC List[] = {
        {&Radius, 4, 1, U_LE}, {&ExpandEdge, 4, 1, U_LE}, {NULL, 0, 0, U_XX}};
...
/**
    ...
    \return Pointer to PseudoObject, NULL on error
    \param  Brightness  -255 to 255, 0 is unchanged, positive increases,
   negative decreases
    \param  Contrast    -100 to 100, 0 is unchanged, positive increases,
   negative decreases
...
U_PSEUDO_OBJ *U_PMF_IE_BRIGHTNESSCONTRAST_set(int32_t Brightness,
                                              int32_t Contrast) {
    const U_SERIAL_DESC List[] = {
        {&Brightness, 4, 1, U_LE}, {&Contrast, 4, 1, U_LE}, {NULL, 0, 0, U_XX}};
...
/**
    ...
    \return Pointer to PseudoObject, NULL on error
    \param  CyanRed      -100 to 100, 0 is unchanged, positive increases Red   &
   decreases Cyan,    negative is opposite
    \param  MagentaGreen -100 to 100, 0 is unchanged, positive increases Green &
   decreases Magenta, negative is opposite
    \param  YellowBlue   -100 to 100, 0 is unchanged, positive increases Blue  &
   decreases Yellow,  negative is opposite


    EMF+ manual 2.2.3.3, Microsoft name: ColorBalanceEffect Object
*/
U_PSEUDO_OBJ *U_PMF_IE_COLORBALANCE_set(int32_t CyanRed, int32_t MagentaGreen,
                                        int32_t YellowBlue) {
    const U_SERIAL_DESC List[] = {{&CyanRed, 4, 1, U_LE},
...
/**
    ...
    \param  Intensity    adjustment to apply.  "Adjust" determines what field
   this is and range values.


    EMF+ manual 2.2.3.4, Microsoft name: ColorCurveEffect Object
*/
U_PSEUDO_OBJ *U_PMF_IE_COLORCURVE_set(uint32_t Adjust, uint32_t Channel,
                                      int32_t Intensity) {
    const U_SERIAL_DESC List[] = {{&Adjust, 4, 1, U_LE},
                                  {&Channel, 4, 1, U_LE},
                                  {&Intensity, 4, 1, U_LE},
                                  {NULL, 0, 0, U_XX}};
...
U_PSEUDO_OBJ *U_PMF_IE_COLORLOOKUPTABLE_set(const uint8_t *BLUT,
                                            ...
                                            const uint8_t *RLUT,
                                            const uint8_t *ALUT) {
    if (!BLUT || !GLUT || !RLUT || !ALUT)
        return (NULL);

    const U_SERIAL_DESC List[] = {{BLUT, 1, 256, U_XE},
                                  {GLUT, 1, 256, U_XE},
                                  {RLUT, 1, 256, U_XE},
                                  {ALUT, 1, 256, U_XE},
                                  {NULL, 0, 0, U_XX}};
...
/**
    ...
    \return Pointer to PseudoObject, NULL on error
    \param  Matrix     5 x 5 color transformation matrix, First 4 rows are [{4
   multiplier values},0.0] for R,G,B,A, last Row is [{4 color translation
   valuess}, 1.0]
...
U_PSEUDO_OBJ *U_PMF_IE_COLORMATRIX_set(const U_FLOAT *Matrix) {
    ...
    const U_SERIAL_DESC List[] = {{Matrix, 4, 25, U_LE}, {NULL, 0, 0, U_XX}};
...
U_PSEUDO_OBJ *U_PMF_IE_HUESATURATIONLIGHTNESS_set(int32_t Hue,
                                                  int32_t Saturation,
                                                  int32_t Lightness) {
    const U_SERIAL_DESC List[] = {{&Hue, 4, 1, U_LE},
                                  {&Saturation, 4, 1, U_LE},
                                  {&Lightness, 4, 1, U_LE},
                                  {NULL, 0, 0, U_XX}};
    U_PSEUDO_OBJ *po =
        U_PMF_SERIAL_set(U_PMF_IE_HUESATURATIONLIGHTNESS_OID, List);
    return (po);
}
...
*/
U_PSEUDO_OBJ *U_PMF_IE_LEVELS_set(int32_t Highlight, int32_t Midtone,
                                  int32_t Shadow) {
    const U_SERIAL_DESC List[] = {{&Highlight, 4, 1, U_LE},
                                  {&Midtone, 4, 1, U_LE},
                                  {&Shadow, 4, 1, U_LE},
                                  {NULL, 0, 0, U_XX}};
...
/**
    ...
    \param  Rects      Array of U_RECTL rectangular area(s) to apply red eye
   correction
...
U_PSEUDO_OBJ *U_PMF_IE_SHARPEN_set(U_FLOAT Radius, int32_t Sharpen) {
    const U_SERIAL_DESC List[] = {
        {&Radius, 4, 1, U_LE}, {&Sharpen, 4, 1, U_LE}, {NULL, 0, 0, U_XX}};
...
/**
    ...
    \return Pointer to PseudoObject, NULL on error
    \param  Hue         -180 to 180, [positive==clockwise] rotation in degrees
   starting from blue
    \param  Amount      -100 [add black] to 100[add white], 0 is unchanged.
   Change in hue on specified axis
...
U_PSEUDO_OBJ *U_PMF_IE_TINT_set(const int32_t Hue, const int32_t Amount) {
    const U_SERIAL_DESC List[] = {
        {&Hue, 4, 1, U_LE}, {&Amount, 4, 1, U_LE}, {NULL, 0, 0, U_XX}};
...
/* internal routine, not part of the API.
   ...
   The type is something like U_PMR_OFFSETCLIP (the record type, NOT the
   U_PMR_OFFSETCLIP_OID PseudoObject Type!).
   The U_PMR_RECFLAG bit is added both in the data and in the Type of the
   PseudoObject.
   If that bit is already set no harm, no foul.
*/

U_PSEUDO_OBJ *U_PMR_CMN_HDR_set(uint32_t Type, uint16_t Flags,
                                uint32_t DataSize) {
    uint32_t Size = 12 + UP4(DataSize); /* The header itself is always 12, PMR
                                           records must be a multiple of 4 */
    Type |= U_PMR_RECFLAG;
    uint16_t utmp16 = Type;
    const U_SERIAL_DESC List[] = {
        {&utmp16, 2, 1, U_LE},
        {&Flags, 2, 1, U_LE}, /* Microsoft EMF+ manual is BE, but
                                 this field in this implementation
                                 is  LE */
        {&Size, 4, 1, U_LE},
        {&DataSize, 4, 1, U_LE},
        {NULL, 0, 0, U_XX}};
...
/**
    ...
    EMF+ manual mentioned in 2.1.1.1, not otherwise documented, Microsoft name:
   EmfPlusStrokeFillPath Record, Index 0x37
...
U_PSEUDO_OBJ *U_PMR_STROKEFILLPATH_set(void) {
    ...
    const U_SERIAL_DESC List[] = {{ph->Data, ph->Used, 1, U_XE},
                                  {NULL, 0, 0, U_XX}};
...
/**
    ...
    \return Pointer to PseudoObject, NULL on error
    \param  dX          horizontal translation offset to apply to clipping
   region
    \param  dY          vertical   translation offset to apply to clipping
   region
...
U_PSEUDO_OBJ *U_PMR_SETCLIPPATH_set(uint32_t PathID, uint32_t CMenum) {
    ...
    uint16_t utmp16 = ((PathID & U_FF_MASK_OID8) << U_FF_SHFT_OID8) |
                      ((CMenum & U_FF_MASK_CM4) << U_FF_SHFT_CM4);
...
/**
    ...
    \return Pointer to PseudoObject, NULL on error
    \param  CMenum      CombineMode enumeration..
    \param  Rect        U_PSEUDO_OBJ containing an U_PMF_RECTF object or an
   array of U_PMF_RECTF objects (the first is used)
...
U_PSEUDO_OBJ *U_PMR_SETCLIPRECT_set(uint32_t CMenum, const U_PSEUDO_OBJ *Rect) {
    ...
    const U_SERIAL_DESC List[] = {
        {ph->Data, ph->Used, 1, U_XE}, {start, 4, 4, U_XE}, {NULL, 0, 0, U_XX}};
...
/**
    ...
    \param  Data        Private data, may be anything.  Stored in PseudoObject
   without adjusting byte order.
...
/**
    ...
    \param  IsVideo      set = video device, clear= printer.  Ignore all other
   bits.
...
*/
U_PSEUDO_OBJ *U_PMR_HEADER_set(int IsDual, int IsVideo,
                               const U_PSEUDO_OBJ *Version,
                               uint32_t LogicalDpiX, uint32_t LogicalDpiY) {
    if (!Version || (Version->Type != U_PMF_GRAPHICSVERSION_OID)) {
        return (NULL);
    }
    ...
    const U_SERIAL_DESC List[] = {
        {ph->Data, ph->Used, 1, U_XE}, {Version->Data, Version->Used, 1, U_XE},
        {&Flags, 4, 1, U_LE},          {&LogicalDpiX, 4, 1, U_LE},
        {&LogicalDpiY, 4, 1, U_LE},    {NULL, 0, 0, U_XX}};
...
/**
   ...
   inclusive)
    ...
    \param  Sweep       Sweep angle, -360<= angle <=360, degrees clockwise from
   Start
    \param  Rect        U_PSEUDO_OBJ containing a U_PMF_RECT or U_PMF_RECTF
   object


    EMF+ manual 2.3.4.2, Microsoft name: EmfPlusDrawArc Record, Index 0x12
*/
U_PSEUDO_OBJ *U_PMR_DRAWARC_set(uint32_t PenID, U_FLOAT Start, U_FLOAT Sweep,
    ...
    if (!Rect) {
        ...
        if (Rect->Type == U_PMF_RECT_OID) {
            ctype = 1;
        } else if (Rect->Type == U_PMF_RECTF_OID) {
            ctype = 0;
        } else {
            return (NULL);
        }
    ...
    uint16_t utmp16 =
        (ctype ? U_PPF_C : 0) | (PenID & U_FF_MASK_OID8) << U_FF_SHFT_OID8;
...
/**
    ...
    \param  PenID       U_PMF_PEN object in the EMF+ object table (0-63,
   inclusive)
    \param  Points      U_PSEUDO_OBJ containing first Elements, then a
   U_PMF_POINT, U_PMF_POINTR or U_PMF_POINTF object
...
U_PSEUDO_OBJ *U_PMR_DRAWBEZIERS_set(uint32_t PenID,
    ...
    uint16_t utmp16 = (ctype ? U_PPF_C : 0) | (RelAbs ? U_PPF_P : 0) |
                      (PenID & U_FF_MASK_OID8) << U_FF_SHFT_OID8;
...
/**
   ...
   inclusive)
    ...
    \param  Points      U_PSEUDO_OBJ containing a U_PMF_POINT, U_PMF_POINTR or
   U_PMF_POINTF object


    EMF+ manual 2.3.4.4, Microsoft name: EmfPlusDrawClosedCurve Record, Index
   0x17
...
*/
U_PSEUDO_OBJ *U_PMR_DRAWCLOSEDCURVE_set(uint32_t PenID, U_FLOAT Tension,
                                        const U_PSEUDO_OBJ *Points) {
    int ctype, RelAbs;
    if (PenID > 63)
        return (NULL);
...
/**
   ...
   inclusive)
    ...
    \param  NSegs       Number of segments to draw. Starting at Offset go NSegs
   straight lines, must not run out of points..
    \param  Points      U_PSEUDO_OBJ containing an element count then a series
   of U_PMF_POINT or U_PMF_POINTF object


    EMF+ manual 2.3.4.5, Microsoft name: EmfPlusDrawCurve Record, Index 0x18

    Curve is a cardinal spline, using doubled terminator points to generate
   curves for the terminal segments.
...
*/
U_PSEUDO_OBJ *U_PMR_DRAWCURVE_set(uint32_t PenID, U_FLOAT Tension,
                                  uint32_t Offset, uint32_t NSegs,
                                  const U_PSEUDO_OBJ *Points) {
    int ctype;
    if (PenID > 63)
        return (NULL);
    ...
    if (!NSegs) {
        ...
        if (Points->Type == (U_PMF_POINT_OID | U_PMF_ARRAY_OID)) {
            ctype = 1;
        } else if (Points->Type == (U_PMF_POINTF_OID | U_PMF_ARRAY_OID)) {
            ctype = 0;
        } else {
    ...
    uint32_t Elements =
        (Points->Used - 4) /
        (ctype ? 4 : 8); /* This way do not need to worry about byte order */
    if (Offset + NSegs + 1 > Elements) {
        return (NULL);
    }
...
/**
    ...
    \return Pointer to PseudoObject, NULL on error
    \param  FontID      U_PMF_FONT object in the EMF+ object table (0-63,
   inclusive)
    \param  BrushID     U_PSEUDO_OBJ containing a U_PMF_ARGB or a U_PMF_4NUM.
   Color or U_PMF_BRUSH object in the EMF+ object table (0-63, inclusive)
    \param  DSOFlags    DriverStringOptions flags
    \param  HasMatrix   If 1 record contains a TransformMatrix field, if 0 it
   does not.
    \param  GlyphCount  The number of Elements in Glyphs, must agree with the
   number of elements in Points.
    \param  Glyphs      If U_DSO_CmapLookup is set in DSOFlags this is an array
   of UTF16LE characters, otherwise, it is an array of indices into the
   U_PMF_FONT object indexed by Object_ID in flags.
   ...
   Apply to Glyphs & Positions. Present if HasMatrix is 1
...
*/
U_PSEUDO_OBJ *
U_PMR_DRAWDRIVERSTRING_set(uint32_t FontID, const U_PSEUDO_OBJ *BrushID,
                           uint32_t DSOFlags, uint32_t HasMatrix,
                           uint32_t GlyphCount, const uint16_t *Glyphs,
                           const U_PSEUDO_OBJ *Points, const U_PSEUDO_OBJ *Tm) {
    int btype;
    if (FontID > 63) {
        return (NULL);
    }
    ...
    if (!Points || (Points->Type != (U_PMF_POINTF_OID | U_PMF_ARRAY_OID))) {
        return (NULL);
    }
    uint32_t Elements = (Points->Used - 4) / 4;
    if (GlyphCount != Elements) {
        return (NULL);
    }
    if (BrushID) {
        if (BrushID->Used != 4) {
            return (NULL);
        } else if (BrushID->Type == U_PMF_ARGB_OID) {
            btype = 1;
        } else if (BrushID->Type == U_PMF_4NUM_OID) {
            btype = 0;
        } else {
            return (NULL);
        }
    ...
    if (HasMatrix) {
        if (!Tm) {
            return (NULL);
        } else if (Tm->Type != (U_PMF_TRANSFORMMATRIX_OID)) {
            return (NULL);
        }
        Size += Tm->Used;
    }
    uint16_t utmp16 =
        (btype ? U_PPF_B : 0) | (FontID & U_FF_MASK_OID8) << U_FF_SHFT_OID8;
...
/**
    ...
    \return Pointer to PseudoObject, NULL on error
    \param  ImgID       U_PMF_IMAGE object in the EMF+ object table (0-63,
   inclusive)
    \param  ImgAttrID   index of a U_PMF_IMAGEATTRIBUTES object in the object
   table
    \param  SrcUnit     UnitType enumeration
    \param  SrcRect     U_PSEUDO_OBJ containing a U_PMF_RECTF object, Source
   region of image
    \param  DstRect     U_PSEUDO_OBJ containing a U_PMF_RECT or U_PMF_RECTF
   object


    EMF+ manual 2.3.4.8, Microsoft name: EmfPlusDrawImage Record, Index 0x1A
*/
U_PSEUDO_OBJ *U_PMR_DRAWIMAGE_set(uint32_t ImgID, int32_t ImgAttrID,
                                  int32_t SrcUnit, const U_PSEUDO_OBJ *SrcRect,
                                  const U_PSEUDO_OBJ *DstRect) {
    int ctype;
    if (ImgID > 63)
        return (NULL);
    if (!SrcRect || (SrcRect->Type != U_PMF_RECTF_OID)) {
        return (NULL);
    }
    if (!DstRect) {
        return (NULL);
    } else {
        if (DstRect->Type == U_PMF_RECT_OID) {
            ctype = 1;
        } else if (DstRect->Type == U_PMF_RECTF_OID) {
            ctype = 0;
        } else {
            return (NULL);
        }
    ...
    uint16_t utmp16 =
        (ctype ? U_PPF_C : 0) | (ImgID & U_FF_MASK_OID8) << U_FF_SHFT_OID8;
...
/**
    ...
    \param  ImgID       U_PMF_IMAGE object in the EMF+ object table (0-63,
   inclusive)
    \param  etype       Set: effect from previous U_PMR_SERIALIZABLEOBJECT
   record will be applied; Clear:  no effect applied
    \param  ImgAttrID   index of a U_PMF_IMAGEATTRIBUTES object in the object
   table
    \param  SrcUnit     UnitType enumeration
    \param  SrcRect     U_PSEUDO_OBJ containing a U_PMF_RECTF object, Source
   region of image
    \param  Points      U_PSEUDO_OBJ containing an array of 3 (U_PMF_POINT,
   U_PMF_POINTF, or U_PMF_POINTF) objects. These points are the UL, UR, and LL
   vertices of a parallelogram.


    EMF+ manual 2.3.4.9, Microsoft name: EmfPlusDrawImagePoints Record, Index
   0x1B


   WARNING!  Windows XP Preview does not show filter effects, whether or not
   U_PPF_E is set.   They are visible if the EMF+
   file is inserted as an image into PowerPoint.
*/
U_PSEUDO_OBJ *U_PMR_DRAWIMAGEPOINTS_set(uint32_t ImgID, int etype,
                                        int32_t ImgAttrID, int32_t SrcUnit,
    ...
    uint16_t utmp16 = (ctype ? U_PPF_C : 0) | (etype ? U_PPF_E : 0) |
                      (RelAbs ? U_PPF_P : 0) |
                      (ImgID & U_FF_MASK_OID8) << U_FF_SHFT_OID8;
...
/**
   ...
   inclusive)
    ...
    \param  Points      U_PSEUDO_OBJ containing an array of 3 U_PMF_POINT,
   U_PMF_POINTR, or U_PMF_POINTF objects


    EMF+ manual 2.3.4.10, Microsoft name: EmfPlusDrawLines Record, Index 0x0D
*/
U_PSEUDO_OBJ *U_PMR_DRAWLINES_set(uint32_t PenID, int dtype,
                                  const U_PSEUDO_OBJ *Points) {
    int ctype, RelAbs;
    if (PenID > 63) {
        return (NULL);
    ...
    int Size = Points->Used;
    uint16_t utmp16 = (ctype ? U_PPF_C : 0) | (dtype ? U_PPF_D : 0) |
                      (RelAbs ? U_PPF_P : 0) |
                      (PenID & U_FF_MASK_OID8) << U_FF_SHFT_OID8;
...
*/
U_PSEUDO_OBJ *U_PMR_DRAWPIE_set(uint32_t PenID, U_FLOAT Start, U_FLOAT Sweep,
                                const U_PSEUDO_OBJ *Rect) {
    int ctype;
    if (PenID > 63)
        return (NULL);
...
/**
    ...
    \param  PenID       U_PMF_PEN object in the EMF+ object table (0-63,
   inclusive)
    \param  Rects       U_PSEUDO_OBJ containing 1 rect OR  a count N follwed by
   N rects.  Rects may be either U_PMF_RECT or U_PMF_RECTF
...
U_PSEUDO_OBJ *U_PMR_DRAWRECTS_set(uint32_t PenID, const U_PSEUDO_OBJ *Rects) {
    int ctype;
    int just1;
    uint32_t Elements = 1; /* only used when a single rect is passed in, not an
                              array, not even an array with one member*/
    if (PenID > 63) {
        return (NULL);
    }
    if (Rects) {
        if ((Rects->Type & U_PMF_MASK_OID) == U_PMF_RECT_OID) {
            ctype = 1;
        } else if ((Rects->Type & U_PMF_MASK_OID) == U_PMF_RECTF_OID) {
            ctype = 0;
        } else {
            return (NULL);
        }
    ...
    int Size =
        Rects->Used +
        (just1 ? 4 : 0); /* Elements in Rects for array, not for single */
    uint16_t utmp16 =
        (ctype ? U_PPF_C : 0) | (PenID & U_FF_MASK_OID8) << U_FF_SHFT_OID8;
    U_PSEUDO_OBJ *ph = U_PMR_CMN_HDR_set(U_PMR_DRAWRECTS, utmp16, Size);
    const U_SERIAL_DESC List[] = {
        {ph->Data, ph->Used, 1, U_XE},
        {(just1 ? (char *)&Elements : NULL), (just1 ? 4 : 0), 1,
         U_LE}, /* element count if a single Rect was passed in, empty
                   otherwise */
        {Rects->Data, Rects->Used, 1,
         U_XE}, /* Elements + Array, already stored in Rects, if an array was
                   passed in, just rect if a single */
        {NULL, 0, 0, U_XX}};
...
/**
   ...
   Color or U_PMF_BRUSH object in the EMF+ object table (0-63, inclusive)
    ...
    \param  Rect        U_PSEUDO_OBJ containing a U_PMF_RECTF object, string's
   bounding box
...
U_PSEUDO_OBJ *U_PMR_DRAWSTRING_set(uint32_t FontID, const U_PSEUDO_OBJ *BrushID,
                                   uint32_t FormatID, uint32_t Length,
                                   const U_PSEUDO_OBJ *Rect,
                                   const uint16_t *Text) {
    int btype;
    if (FontID > 63) {
    ...
    if (!Rect || Rect->Type != U_PMF_RECTF_OID) {
        return (NULL);
    }
    if (BrushID) {
        if (BrushID->Used != 4) {
...
/**
   ...
   0x16
*/
U_PSEUDO_OBJ *U_PMR_FILLCLOSEDCURVE_set(int ftype, U_FLOAT Tension,
                                        const U_PSEUDO_OBJ *BrushID,
                                        const U_PSEUDO_OBJ *Points) {
...
U_PSEUDO_OBJ *U_PMR_FILLPATH_set(uint32_t PathID, const U_PSEUDO_OBJ *BrushID) {
    ...
    Size = BrushID->Used;
    uint16_t utmp16 =
        (btype ? U_PPF_B : 0) | (PathID & U_FF_MASK_OID8) << U_FF_SHFT_OID8;
...
*/
U_PSEUDO_OBJ *U_PMR_FILLPIE_set(U_FLOAT Start, U_FLOAT Sweep,
                                const U_PSEUDO_OBJ *BrushID,
                                const U_PSEUDO_OBJ *Rect) {
    int btype, ctype;
    if (BrushID) {
...
U_PSEUDO_OBJ *U_PMR_FILLPOLYGON_set(const U_PSEUDO_OBJ *BrushID,
    ...
    uint16_t utmp16 =
        (btype ? U_PPF_B : 0) | (ctype ? U_PPF_C : 0) | (RelAbs ? U_PPF_P : 0);
...
/**
    ...
    \param  BrushID     U_PSEUDO_OBJ containing a U_PMF_ARGB or a U_PMF_4NUM.
   Color or U_PMF_BRUSH object in the EMF+ object table (0-63, inclusive)
    \param  Rects       U_PSEUDO_OBJ containing 1 rect OR  a count N followed by
   N rects.  Rects may be either U_PMF_RECT or U_PMF_RECTF
...
/**
    ...
    \return Pointer to PseudoObject, NULL on error
    \param  RgnID       U_PMF_REGION object in the EMF+ object table (0-63,
   inclusive)
    \param  BrushID     U_PSEUDO_OBJ containing a U_PMF_ARGB or a U_PMF_4NUM.
   Color or U_PMF_BRUSH object in the EMF+ object table (0-63, inclusive)
...
U_PSEUDO_OBJ *U_PMR_FILLREGION_set(uint32_t RgnID,
    ...
    uint16_t utmp16 =
        (btype ? U_PPF_B : 0) | (RgnID & U_FF_MASK_OID8) << U_FF_SHFT_OID8;
...
/**
    ...
    \return Pointer to PseudoObject, NULL on error
    \param  ObjID       Index for this object in the EMF+ object table (0-63,
   inclusive)
    \param  Po          U_PSEUDO_OBJ containing an object type that may be
   stored in the EMF+ object table
*/
U_PSEUDO_OBJ *U_PMR_OBJECT_PO_set(uint32_t ObjID, U_PSEUDO_OBJ *Po) {
    ...
    int otype =
        U_OID_To_OT(Po->Type); /* This will return 0 if the type is not valid
                                  for an object */
    if (!otype) {
        return (NULL);
    }
    U_PSEUDO_OBJ *po = U_PMR_OBJECT_set(
        ObjID, otype, 0, 0, Po->Used,
        Po->Data); /* 0,0 = rec. not continued, TSize value (ignored)  */
    return (po);
}
...
/**
   ...
   inclusive)
    ...
    \param  ntype       Set: object definition continues in next record; Clear:
   this is the sole object definition record
    \param  TSize       If ntype is set the total number of data bytes split
   across multiple records.  If ntype is clear, it is ignored.
    ...
    U_PMR_OBJECT records can only hold a maximum of 65020 bytes of data.  If the
   object is larger than that
      then multiple U_PMR_OBJECT records are created, one after the other.  If
   this
      happens each record has cbData following ph, and the ntype flag is set.
   If all of the data is less than 65020
      then cbData is NOT entered following ph, and the ntype flag is clear.

    Call initially in all cases with ntype clear and TSize = 0.  If the record
   needs to be fragmented
      the function will call itself recursively to do so.

*/
U_PSEUDO_OBJ *U_PMR_OBJECT_set(uint32_t ObjID, int otype, int ntype,
                               uint32_t TSize, size_t cbData,
                               const char *Data) {
    uint32_t CSize;
    int Pad = UP4(TSize) - TSize;
    if ((otype < U_OT_Brush) || (otype > U_OT_CustomLineCap)) {
        return (NULL);
    }
    if (ntype && (cbData > U_OBJRECLIM)) {
        return (NULL);
    }
    if (!Data || !cbData) {
        return (NULL);
    ...
    if (!ntype && !TSize && (cbData > U_OBJRECLIM)) {
        ntype = 1;
        TSize = cbData;
        po = U_PO_create(NULL, TSize + 16 * (1 + (TSize / cbData)), 0,
                         U_PMR_OBJECT_OID);
        if (po) {
            while (cbData) {
                CSize = (cbData > U_OBJRECLIM ? U_OBJRECLIM : cbData);
                U_PSEUDO_OBJ *pot =
                    U_PMR_OBJECT_set(ObjID, otype, ntype, TSize, CSize, Data);
                if (!pot)
                    break;
                U_PSEUDO_OBJ *newpo =
                    U_PO_po_append(po, pot, U_PMF_KEEP_ELEMENTS);
                U_PO_free(&pot);
                if (!newpo)
                    break;
                po = newpo;
        ...
        /* Send in DataSize, U_PMR_CMN_HDR_set will adjust Header Size with 1-3
         * pad bytes if needed */
        uint16_t utmp16 = otype << 8 | (ntype ? U_PPF_N : 0) |
                          (ObjID & U_FF_MASK_OID8) << U_FF_SHFT_OID8;
        U_PSEUDO_OBJ *ph =
            U_PMR_CMN_HDR_set(U_PMR_OBJECT, utmp16, cbData + (ntype ? 4 : 0));
        const U_SERIAL_DESC List[] = {
            {ph->Data, ph->Used, 1, U_XE},
            {(ntype ? &TSize : NULL), (ntype ? 4 : 0), (ntype ? 1 : 0), U_LE},
            {Data, cbData, 1, U_XE},
            {NULL, (Pad ? Pad : 0), (Pad ? 1 : 0),
             (Pad ? U_XE : U_XX)}, /* Either 1-3 pad bytes or a terminator */
            {NULL, 0, 0, U_XX}     /* terminator, possibly a second in the list,
                                      which is harmless */
        };
...
/**
    ...
    \return Pointer to PseudoObject, NULL on error
    \param  Siepb       U_PSEUDO_OBJ containing a "Serialized image effects
   parameter block".  One of the ImageEffects objects.

    EMF+ manual 2.3.5.2, Microsoft name: EmfPlusSerializableObject Record, Index
   0x38


    This sets an ImageEffect in the renderer, which will be applied to the next
   EmfPlusDrawImagePoints
    record that is encountered.  The image effect is "consumed" by that
   EmfPlusDrawImagePoints record, resetting
...
U_PSEUDO_OBJ *U_PMR_SETANTIALIASMODE_set(int SMenum, int aatype) {
    int Size = 0;
    uint16_t utmp16 =
        (aatype ? U_PPF_AA : 0) | (SMenum & U_FF_MASK_AA) << U_FF_SHFT_AA;
...
U_PSEUDO_OBJ *U_PMR_SETCOMPOSITINGMODE_set(int CMenum) {
    ...
    U_PSEUDO_OBJ *ph =
        U_PMR_CMN_HDR_set(U_PMR_SETCOMPOSITINGMODE, utmp16, Size);
    const U_SERIAL_DESC List[] = {{ph->Data, ph->Used, 1, U_XE},
                                  {NULL, 0, 0, U_XX}};
...
U_PSEUDO_OBJ *U_PMR_SETCOMPOSITINGQUALITY_set(int CQenum) {
    ...
    U_PSEUDO_OBJ *ph =
        U_PMR_CMN_HDR_set(U_PMR_SETCOMPOSITINGQUALITY, utmp16, Size);
    const U_SERIAL_DESC List[] = {{ph->Data, ph->Used, 1, U_XE},
                                  {NULL, 0, 0, U_XX}};
...
/**
    ...
    EMF+ manual 2.3.6.4, Microsoft name: EmfPlusSetInterpolationMode Record,
   Index 0x21
*/
U_PSEUDO_OBJ *U_PMR_SETINTERPOLATIONMODE_set(int IMenum) {
    ...
    U_PSEUDO_OBJ *ph =
        U_PMR_CMN_HDR_set(U_PMR_SETINTERPOLATIONMODE, utmp16, Size);
    const U_SERIAL_DESC List[] = {{ph->Data, ph->Used, 1, U_XE},
                                  {NULL, 0, 0, U_XX}};
...
U_PSEUDO_OBJ *U_PMR_SETPIXELOFFSETMODE_set(int POMenum) {
    ...
    U_PSEUDO_OBJ *ph =
        U_PMR_CMN_HDR_set(U_PMR_SETPIXELOFFSETMODE, utmp16, Size);
    const U_SERIAL_DESC List[] = {{ph->Data, ph->Used, 1, U_XE},
                                  {NULL, 0, 0, U_XX}};
...
U_PSEUDO_OBJ *U_PMR_SETRENDERINGORIGIN_set(int32_t X, int32_t Y) {
    ...
    uint16_t utmp16 = 0;
    U_PSEUDO_OBJ *ph =
        U_PMR_CMN_HDR_set(U_PMR_SETRENDERINGORIGIN, utmp16, Size);
    const U_SERIAL_DESC List[] = {{ph->Data, ph->Used, 1, U_XE},
                                  {&X, 4, 1, U_LE},
                                  {&Y, 4, 1, U_LE},
                                  {NULL, 0, 0, U_XX}};
...
/**
    ...
    EMF+ manual 2.3.6.7, Microsoft name: EmfPlusSetTextContrast Record, Index
   0x20
*/
...
/**
    ...
    EMF+ manual 2.3.6.8, Microsoft name: EmfPlusSetTextRenderingHint Record,
   Index 0x1F
*/
U_PSEUDO_OBJ *U_PMR_SETTEXTRENDERINGHINT_set(int TRHenum) {
    ...
    U_PSEUDO_OBJ *ph =
        U_PMR_CMN_HDR_set(U_PMR_SETTEXTRENDERINGHINT, utmp16, Size);
    const U_SERIAL_DESC List[] = {{ph->Data, ph->Used, 1, U_XE},
                                  {NULL, 0, 0, U_XX}};
...
/**
    ...
    \param  DstRect     a U_PSEUDO_OBJ  containing a U_PMF_RECTF object.  with
   SrcRect specifies a transformation
    \param  SrcRect     a U_PSEUDO_OBJ  containing a U_PMF_RECTF object.  with
   DstRect specifies a transformation
    \param  StackID     EMF+ Object Stack Index to use for this graphics
   container


    EMF+ manual 2.3.7.1, Microsoft name: EmfPlusBeginContainer Record, Index
   0x27
*/
U_PSEUDO_OBJ *U_PMR_BEGINCONTAINER_set(int UTenum, U_PSEUDO_OBJ *DstRect,
                                       U_PSEUDO_OBJ *SrcRect,
                                       uint32_t StackID) {
    if (UTenum < U_UT_World || UTenum > U_UT_Millimeter) {
        return (NULL);
    }
    if (!DstRect || (DstRect->Type != U_PMF_RECTF_OID)) {
        return (NULL);
    }
    if (!SrcRect || (SrcRect->Type != U_PMF_RECTF_OID)) {
        return (NULL);
...
U_PSEUDO_OBJ *U_PMR_BEGINCONTAINERNOPARAMS_set(int StackID) {
    ...
    uint16_t utmp16 = 0;
    U_PSEUDO_OBJ *ph =
        U_PMR_CMN_HDR_set(U_PMR_BEGINCONTAINERNOPARAMS, utmp16, Size);
    const U_SERIAL_DESC List[] = {{ph->Data, ph->Used, 1, U_XE},
                                  {&StackID, 4, 1, U_LE},
                                  {NULL, 0, 0, U_XX}};
...
/**
    ...
    \return Pointer to PseudoObject, NULL on error
    \param  StackID     EMF+ Graphics State Stack to restore from. Must have
   been put on the GSS with a U_PMR_SAVE.
...
/**
    ...
    \return Pointer to PseudoObject, NULL on error
    \param  Rects       a U_PSEUDO_OBJ  containing an array of U_PMF_RECT or
   U_PMF_RECTF objects.
...
U_PSEUDO_OBJ *U_PMR_SETTSCLIP_set(U_PSEUDO_OBJ *Rects) {
    ...
    uint32_t Elements;
    if (Rects) {
        if (Rects->Type == (U_PMF_RECT_OID | U_PMF_ARRAY_OID)) {
            ctype = 1;
            Elements = (Rects->Used - 4) / 8;
        } else if (Rects->Type == (U_PMF_RECTF_OID | U_PMF_ARRAY_OID)) {
            ctype = 0;
            Elements = (Rects->Used - 4) / 16;
        } else {
            return (NULL);
        }
    } else {
    ...
    uint16_t utmp16 =
        (ctype ? U_PPF_K : 0) | (Elements & U_FF_MASK_TSC) << U_FF_SHFT_TSC;
...
/**
    ...
    \param  Palette     (optional) a U_PSEUDO_OBJ containing a U_PMF_PALETTE
   object.


    EMF+ manual 2.3.8.2, Microsoft name: EmfPlusSetTSGraphics Record, Index 0x39
*/
U_PSEUDO_OBJ *U_PMR_SETTSGRAPHICS_set(int vgatype, U_PMF_SETTSGRAPHICS *Tsg,
...
/**
    ...
    \param  Tm          a U_PSEUDO_OBJ containing a U_PMF_TRANSFORMMATRIX.
   (Transformation matrix)
...
U_PSEUDO_OBJ *U_PMR_MULTIPLYWORLDTRANSFORM_set(int xmtype, U_PSEUDO_OBJ *Tm) {
    ...
    U_PSEUDO_OBJ *ph =
        U_PMR_CMN_HDR_set(U_PMR_MULTIPLYWORLDTRANSFORM, utmp16, Size);
    const U_SERIAL_DESC List[] = {{ph->Data, ph->Used, 1, U_XE},
...
/**
    ...
    EMF+ manual 2.3.9.2, Microsoft name: EmfPlusResetWorldTransform Record,
   Index 0x2B
*/
U_PSEUDO_OBJ *U_PMR_RESETWORLDTRANSFORM_set(void) {
    ...
    uint16_t utmp16 = 0;
    U_PSEUDO_OBJ *ph =
        U_PMR_CMN_HDR_set(U_PMR_RESETWORLDTRANSFORM, utmp16, Size);
    const U_SERIAL_DESC List[] = {{ph->Data, ph->Used, 1, U_XE},
                                  {NULL, 0, 0, U_XX}};
...
/**
    ...
    EMF+ manual 2.3.9.3, Microsoft name: EmfPlusRotateWorldTransform Record,
   Index 0x2F
*/
U_PSEUDO_OBJ *U_PMR_ROTATEWORLDTRANSFORM_set(int xmtype, U_FLOAT Angle) {
    ...
    uint16_t utmp16 = (xmtype ? U_PPF_XM : 0);
    U_PSEUDO_OBJ *ph =
        U_PMR_CMN_HDR_set(U_PMR_ROTATEWORLDTRANSFORM, utmp16, Size);
    const U_SERIAL_DESC List[] = {{ph->Data, ph->Used, 1, U_XE},
                                  {&Angle, 4, 1, U_LE},
                                  {NULL, 0, 0, U_XX}};
...
/**
    ...
    EMF+ manual 2.3.9.4, Microsoft name: EmfPlusScaleWorldTransform Record,
   Index 0x2E
*/
U_PSEUDO_OBJ *U_PMR_SCALEWORLDTRANSFORM_set(int xmtype, U_FLOAT X, U_FLOAT Y) {
    ...
    uint16_t utmp16 = (xmtype ? U_PPF_XM : 0);
    U_PSEUDO_OBJ *ph =
        U_PMR_CMN_HDR_set(U_PMR_SCALEWORLDTRANSFORM, utmp16, Size);
    const U_SERIAL_DESC List[] = {{ph->Data, ph->Used, 1, U_XE},
                                  {&X, 4, 1, U_LE},
                                  {&Y, 4, 1, U_LE},
                                  {NULL, 0, 0, U_XX}};
...
/**
   ...
   Index 0x2D
*/
U_PSEUDO_OBJ *U_PMR_TRANSLATEWORLDTRANSFORM_set(int xmtype, U_FLOAT Dx,
                                                U_FLOAT Dy) {
    int Size = 2 * 4;
    uint16_t utmp16 = (xmtype ? U_PPF_XM : 0);
    U_PSEUDO_OBJ *ph =
        U_PMR_CMN_HDR_set(U_PMR_TRANSLATEWORLDTRANSFORM, utmp16, Size);
    const U_SERIAL_DESC List[] = {{ph->Data, ph->Used, 1, U_XE},
                                  {&Dx, 4, 1, U_LE},
                                  {&Dy, 4, 1, U_LE},
                                  {NULL, 0, 0, U_XX}};
    U_PSEUDO_OBJ *po =
        U_PMF_SERIAL_set(U_PMR_TRANSLATEWORLDTRANSFORM_OID, List);
    U_PO_free(&ph);
    return (po);
}
...
/*
   ...
   start of U_PMF_*_get() functions
      ...
      These functions all take a blimit value so that they can check if the data
 ...
 */
int U_PMF_CORE1_get(const char *contents, void *v1, void *v2, void *v3,
                    void *v4, void *v5, void *v6, const char **vR) {
    if (v1) {
        U_PMF_MEMCPY_SRCSHIFT(v1, &contents, 4);
        if (v2) {
            U_PMF_MEMCPY_SRCSHIFT(v2, &contents, 4);
            if (v3) {
                U_PMF_MEMCPY_SRCSHIFT(v3, &contents, 4);
                if (v4) {
                    U_PMF_MEMCPY_SRCSHIFT(v4, &contents, 4);
                    if (v5) {
                        U_PMF_MEMCPY_SRCSHIFT(v5, &contents, 4);
                        if (v6) {
                            U_PMF_MEMCPY_SRCSHIFT(v6, &contents, 4);
                        }
                    }
                }
            }
...
*/
int U_PMF_BRUSH_get(const char *contents, uint32_t *Version, uint32_t *Type,
                    const char **Data, const char *blimit) {
    if (!contents || !Version || !Type || !Data || !blimit) {
        return (0);
    }
    if (IS_MEM_UNSAFE(contents, sizeof(U_PMF_BRUSH), blimit))
        return (0);
...
/**
    ...
    Caller must check Data for possible memory access violations.
*/
int U_PMF_CUSTOMLINECAP_get(const char *contents, uint32_t *Version,
                            uint32_t *Type, const char **Data,
                            const char *blimit) {
    if (!contents || !Version || !Type || !Data || !blimit) {
        return (0);
    }
    if (IS_MEM_UNSAFE(contents, sizeof(U_PMF_CUSTOMLINECAP), blimit))
        return (0);
    U_PMF_SERIAL_get(&contents, Version, 4, 1, U_LE);
    U_PMF_SERIAL_get(&contents, Type, 4, 1, U_LE);
    U_PMF_PTRSAV_SHIFT(Data, &contents, 0);
...
/**
    ...
    Caller must check Data for possible memory access violations.
*/
int U_PMF_FONT_get(const char *contents, uint32_t *Version, U_FLOAT *EmSize,
                   uint32_t *SizeUnit, int32_t *FSFlags, uint32_t *Length,
                   const char **Data, const char *blimit) {
    if (!contents || !Version || !EmSize || !SizeUnit || !FSFlags || !Length ||
        !Data || !blimit) {
        return (0);
    }
    if (IS_MEM_UNSAFE(contents, sizeof(U_PMF_FONT), blimit))
        return (0);
    U_PMF_SERIAL_get(&contents, Version, 4, 1, U_LE);
...
/**
    ...
    Caller must check Data for possible memory access violations.
*/
int U_PMF_IMAGE_get(const char *contents, uint32_t *Version, uint32_t *Type,
                    const char **Data, const char *blimit) {
    if (!contents || !Version || !Type || !blimit) {
        return (0);
    }
    if (IS_MEM_UNSAFE(contents, sizeof(U_PMF_IMAGE), blimit))
        return (0);
    U_PMF_SERIAL_get(&contents, Version, 4, 1, U_LE);
    U_PMF_SERIAL_get(&contents, Type, 4, 1, U_LE);
    U_PMF_PTRSAV_SHIFT(Data, &contents, 0);
...
/**
    ...
    EMF+ manual 2.2.1.5, Microsoft name: EmfPlusImageAttributes Object
*/
int U_PMF_IMAGEATTRIBUTES_get(const char *contents, uint32_t *Version,
                              uint32_t *WrapMode, uint32_t *ClampColor,
                              uint32_t *ObjectClamp, const char *blimit) {
    if (!contents || !Version || !WrapMode || !ClampColor || !ObjectClamp ||
        !blimit) {
        return (0);
    }
    if (IS_MEM_UNSAFE(contents, sizeof(U_PMF_IMAGEATTRIBUTES), blimit))
        return (0);
    U_PMF_SERIAL_get(&contents, Version, 4, 1, U_LE);
...
/**
    ...
    \param  Types      array of U_PMF_PATHPOINTTYPERLE and/or
   U_PMF_PATHPOINTTYPE

    EMF+ manual 2.2.1.6, Microsoft name: EmfPlusPath Object

    Caller must check Types for possible memory access violations if type can be
   U_PMF_PATHPOINTTYPERLE.
*/
int U_PMF_PATH_get(const char *contents, uint32_t *Version, uint32_t *Count,
                   uint16_t *Flags, const char **Points, const char **Types,
                   const char *blimit) {
    if (!contents || !Version || !Count || !Flags || !Points || !Types ||
        !blimit) {
        return (0);
    }
    if (IS_MEM_UNSAFE(contents, sizeof(U_PMF_PATH), blimit))
        return (0);
    U_PMF_SERIAL_get(&contents, Version, 4, 1, U_LE);
    ...
    if (*Flags & U_PPF_P) {
        ...
        sizeP = *Count * sizeof(U_PMF_POINT);
    } else {
        sizeP = *Count * sizeof(U_PMF_POINTF);
    }
    if (IS_MEM_UNSAFE(contents, sizeP, blimit))
        return (0);
    U_PMF_PTRSAV_SHIFT(Points, &contents, 0);
    contents += sizeP;
    /* this limit is correct if there are only U_PMF_PATHPOINTTYPE PointTypes,
       it is a lower bound if
       there can also be U_PMF_PATHPOINTTYPERLE */
    sizeT = *Count * sizeof(U_PMF_PATHPOINTTYPE);
    if (IS_MEM_UNSAFE(contents, sizeT, blimit))
        return (0);
...
*/
int U_PMF_PEN_get(const char *contents, uint32_t *Version, uint32_t *Type,
                  const char **PenData, const char **Brush,
                  const char *blimit) {
    if (!contents || !Type || !PenData || !Brush || !blimit) {
        return (0);
    }
    if (IS_MEM_UNSAFE(contents, sizeof(U_PMF_PEN), blimit))
        return (0);
    U_PMF_SERIAL_get(&contents, Version, 4, 1, U_LE);
    U_PMF_SERIAL_get(&contents, Type, 4, 1, U_LE);
...
/**
   ...
   number of U_PMF_REGIONNODE objects in Nodes.
    ...
    EMF+ manual 2.2.1.8, Microsoft name: EmfPlusRegion Object
*/
int U_PMF_REGION_get(const char *contents, uint32_t *Version, uint32_t *Count,
                     const char **Nodes, const char *blimit) {
    if (!contents || !Version || !Count || !Nodes || !blimit) {
        return (0);
    }
    if (IS_MEM_UNSAFE(contents, sizeof(U_PMF_REGION), blimit))
        return (0);
    U_PMF_SERIAL_get(&contents, Version, 4, 1, U_LE);
    U_PMF_SERIAL_get(&contents, Count, 4, 1, U_LE);
...
/**
   ...
   part
    ...
    EMF+ manual 2.2.1.9, Microsoft name: EmfPlusStringFormat Object
*/
int U_PMF_STRINGFORMAT_get(const char *contents, U_PMF_STRINGFORMAT *Sfs,
                           const char **Data, const char *blimit) {
    if (!contents || !Sfs || !Data || !blimit) {
        return (0);
    }
    if (IS_MEM_UNSAFE(contents, sizeof(U_PMF_STRINGFORMAT), blimit))
        return (0);
...
/**
    ...
    EMF+ manual 2.2.2.1, Microsoft name: EmfPlusARGB Object
*/
int U_PMF_ARGB_get(const char *contents, uint8_t *Blue, uint8_t *Green,
                   uint8_t *Red, uint8_t *Alpha, const char *blimit) {
    if (!contents || !Blue || !Green || !Red || !Alpha || !blimit) {
        return (0);
    }
    if (IS_MEM_UNSAFE(contents, sizeof(U_PMF_ARGB), blimit))
        return (0);
...
int U_PMF_BITMAP_get(const char *contents, U_PMF_BITMAP *Bs, const char **Data,
                     const char *blimit) {
    if (!contents || !Bs || !Data || !blimit) {
        return (0);
    }
    if (IS_MEM_UNSAFE(contents, sizeof(U_PMF_BITMAP), blimit))
        return (0);
    U_PMF_SERIAL_get(&contents, Bs, 4, 5,
                     U_LE); // width, height, stride, pixelformat, type
...
/**
    ...
    \param  Data       An array of bytes, meaning depends on fields in
   U_PMF_BITMAP object and the PixelFormat enumeration.

    EMF+ manual 2.2.2.3, Microsoft name: EmfPlusBitmapData Object

    Caller must check Data for possible memory access violations.
*/
int U_PMF_BITMAPDATA_get(const char *contents, U_PMF_PALETTE *Ps,
                         const char **Colors, const char **Data,
                         const char *blimit) {
    if (!contents || !Ps || !Colors || !Data || !blimit) {
        return (0);
    }
    /* this structure is entirely optional */
    if (IS_MEM_UNSAFE(contents, 4 * 2, blimit))
        return (0);
    U_PMF_SERIAL_get(&contents, Ps, 4, 2, U_LE);
    U_PMF_PTRSAV_SHIFT(Colors, &contents, Ps->Elements * sizeof(U_PMF_ARGB));
    /* difficult to know how big the actual bitmap will be, just return the
     * pointer to it untested */
    U_PMF_PTRSAV_SHIFT(Data, &contents, 0);
    return (1);
}
...
/**
    ...
    \param  Positions  Caller must free.  Pointer to memory holding positions
   along gradient line.
    \param  Colors     Caller must NOT free memory, Pointer to memory holding
   colors at positions on gradient line.

    EMF+ manual 2.2.2.4, Microsoft name: EmfPlusBlendColors Object
*/
int U_PMF_BLENDCOLORS_get(const char *contents, uint32_t *Elements,
                          U_FLOAT **Positions, const char **Colors,
                          const char *blimit) {
    if (!contents || !Positions || !Colors || !blimit) {
        return (0);
    }
    if (IS_MEM_UNSAFE(contents, sizeof(U_PMF_BLENDCOLORS), blimit))
        return (0);
    U_PMF_SERIAL_get(&contents, Elements, 4, 1, U_LE);
    if (IS_MEM_UNSAFE(contents, *Elements * 4, blimit))
        return (0);
    if (!U_PMF_SERIAL_array_copy_get(&contents, (void **)Positions, 4,
                                     *Elements, U_LE, 1)) {
        return (0);
    }
...
/**
    ...
    \param  Positions  Caller must free.  Pointer to memory holding positions
   along gradient line.
    \param  Factors    Caller must free.  Pointer to memory holding blending
   factors, 0.0->1.0 values, inclusive along gradient line.

    EMF+ manual 2.2.2.5, Microsoft name: EmfPlusBlendFactors Object
*/
int U_PMF_BLENDFACTORS_get(const char *contents, uint32_t *Elements,
                           U_FLOAT **Positions, U_FLOAT **Factors,
                           const char *blimit) {
    if (!contents || !Elements || !Positions || !Factors || !blimit) {
        return (0);
    }
    if (IS_MEM_UNSAFE(contents, sizeof(U_PMF_BLENDFACTORS), blimit))
        return (0);
    U_PMF_SERIAL_get(&contents, Elements, 4, 1, U_LE);
    if (IS_MEM_UNSAFE(contents, *Elements * 4 * 2, blimit))
        return (0);
    if (!U_PMF_SERIAL_array_copy_get(&contents, (void **)Positions, 4,
                                     *Elements, U_LE, 1)) {
        return (0);
    }
    if (!U_PMF_SERIAL_array_copy_get(&contents, (void **)Factors, 4, *Elements,
                                     U_LE, 1)) {
        free(*Positions);
        return (0);
    }
...
/**
    ...
    Caller must check Data for possible memory access violations.
*/
int U_PMF_BOUNDARYPATHDATA_get(const char *contents, int32_t *Size,
                               const char **Data, const char *blimit) {
    if (!contents || !Size || !Data || !blimit) {
        return (0);
    }
    if (IS_MEM_UNSAFE(contents, sizeof(U_PMF_BOUNDARYPATHDATA), blimit))
        return (0);
    U_PMF_SERIAL_get(&contents, Size, 4, 1, U_LE);
    U_PMF_PTRSAV_SHIFT(Data, &contents, 0);
    return (1);
...
/**
    ...
    \param  Points     Caller must free.  Pointer to memory holding points along
   gradient line. Boundary of the brush.

    EMF+ manual 2.2.2.7, Microsoft name: EmfPlusBoundaryPointData Object
*/
int U_PMF_BOUNDARYPOINTDATA_get(const char *contents, int32_t *Elements,
                                U_PMF_POINTF **Points, const char *blimit) {
    if (!contents || !Elements || !Points || !blimit) {
        return (0);
    }
    if (IS_MEM_UNSAFE(contents, sizeof(U_PMF_BOUNDARYPOINTDATA), blimit))
        return (0);
    U_PMF_SERIAL_get(&contents, Elements, 4, 1, U_LE);
    if (IS_MEM_UNSAFE(contents, *Elements * 2, blimit))
        return (0);
    if (!U_PMF_SERIAL_array_copy_get(&contents, (void **)Points, 4,
                                     *Elements * 2, U_LE, 1)) {
        return (0);
    }
    return (1);
...
/**
    ...
    EMF+ manual 2.2.2.8, Microsoft name: EmfPlusCharacterRange Object
*/
int U_PMF_CHARACTERRANGE_get(const char *contents, int32_t *First,
                             int32_t *Length, const char *blimit) {
    if (!contents || !First || !Length || !blimit) {
        return (0);
    }
    if (IS_MEM_UNSAFE(contents, sizeof(U_PMF_CHARACTERRANGE), blimit))
        return (0);
...
/**
    ...
    \param  Widths     Caller must free.  Pointer to memory holding Line or gap
   widths (0.0 <-> 1.0, fraction of total line width ).

    EMF+ manual 2.2.2.9, Microsoft name: EmfPlusCompoundLineData Object
*/
int U_PMF_COMPOUNDLINEDATA_get(const char *contents, int32_t *Elements,
                               U_FLOAT **Widths, const char *blimit) {
    if (!contents || !Elements || !Widths || !blimit) {
        return (0);
    }
    if (IS_MEM_UNSAFE(contents, sizeof(U_PMF_COMPOUNDLINEDATA), blimit))
        return (0);
    U_PMF_SERIAL_get(&contents, Elements, 4, 1, U_LE);
    if (IS_MEM_UNSAFE(contents, *Elements * sizeof(U_FLOAT), blimit))
        return (0);
...
/**
    ...
    Caller must check Data for possible memory access violations.
*/
int U_PMF_CUSTOMENDCAPDATA_get(const char *contents, int32_t *Size,
                               const char **Data, const char *blimit) {
    if (!contents || !Size || !Data || !blimit) {
        return (0);
    }
    if (IS_MEM_UNSAFE(contents, sizeof(U_PMF_CUSTOMENDCAPDATA), blimit))
        return (0);
    U_PMF_SERIAL_get(&contents, Size, 4, 1, U_LE);
    U_PMF_PTRSAV_SHIFT(Data, &contents, 0);
    return (1);
...
int U_PMF_CUSTOMLINECAPARROWDATA_get(const char *contents,
                                     ...
                                     const char *blimit) {
    if (!contents || !Ccad || !blimit) {
        return (0);
    }
    if (IS_MEM_UNSAFE(contents, sizeof(U_PMF_CUSTOMLINECAPARROWDATA), blimit))
        return (0);
...
/**
    ...
    \return 1 on success, 0 on error
    \param  contents   Record from which to extract data
    \param  Clcd       pointer to U_PMF_CUSTOMLINECAPDATA structure, with no
   variable part
...
int U_PMF_CUSTOMLINECAPDATA_get(const char *contents,
                                ...
                                const char **Data, const char *blimit) {
    if (!contents || !Clcd || !blimit) {
        return (0);
    }
    if (IS_MEM_UNSAFE(contents, sizeof(U_PMF_CUSTOMLINECAPDATA), blimit))
        return (0);
...
/**
    ...
    \return on success 3,5, or 7 (for varying combinations of data present) or 1
   (no data is present), 0 on error
    \param  contents   Record from which to extract data
    \param  Flags      bits set to indicate the presence of FillData and/or
   LineData
...
*/
int U_PMF_CUSTOMLINECAPOPTIONALDATA_get(const char *contents, uint32_t Flags,
                                        const char **FillData,
                                        const char **LineData,
                                        const char *blimit) {
    uint32_t length;
    int status = 1;
    if (!contents || !*FillData || !*LineData || !blimit) {
        return (0);
    }
    /* this structure is entirely optional */
    if (Flags & U_CLCD_FillPath) {
        ...
        if (IS_MEM_UNSAFE(contents, 4, blimit))
            return (0);
        U_PMF_SERIAL_get(&contents, &length, 4, 1, U_LE);
        if (IS_MEM_UNSAFE(contents, length, blimit))
            return (0);
...
/**
    ...
    Caller must check Data for possible memory access violations.
*/
int U_PMF_CUSTOMSTARTCAPDATA_get(const char *contents, int32_t *Size,
                                 const char **Data, const char *blimit) {
    if (!contents || !Size || !Data || !blimit) {
        return (0);
    }
    if (IS_MEM_UNSAFE(contents, sizeof(U_PMF_CUSTOMSTARTCAPDATA), blimit))
        return (0);
    U_PMF_SERIAL_get(&contents, Size, 4, 1, U_LE);
    U_PMF_PTRSAV_SHIFT(Data, &contents, 0);
    return (1);
...
/**
    ...
    \param  contents   Record from which to extract data
    \param  Elements   Members in the array
    \param  Lengths    Caller must free.  Pointer to memory holding lengths of
   dashes and spaces.

    EMF+ manual 2.2.2.16, Microsoft name: EmfPlusDashedLineData Object
*/
int U_PMF_DASHEDLINEDATA_get(const char *contents, int32_t *Elements,
                             U_FLOAT **Lengths, const char *blimit) {
    if (!contents || !Elements || !Lengths || !blimit) {
        return (0);
    }
    if (IS_MEM_UNSAFE(contents, sizeof(U_PMF_DASHEDLINEDATA), blimit))
        return (0);
    U_PMF_SERIAL_get(&contents, Elements, 4, 1, U_LE);
    if (IS_MEM_UNSAFE(contents, *Elements * sizeof(U_FLOAT), blimit))
        return (0);
...
/**
    ...
    Caller must check Data for possible memory access violations.
*/
int U_PMF_FILLPATHOBJ_get(const char *contents, int32_t *Size,
                          const char **Data, const char *blimit) {
    if (!contents || !Size || !Data || !blimit) {
        return (0);
    }
    if (IS_MEM_UNSAFE(contents, sizeof(U_PMF_FILLPATHO), blimit))
        return (0);
    U_PMF_SERIAL_get(&contents, Size, 4, 1, U_LE);
    U_PMF_PTRSAV_SHIFT(Data, &contents, 0);
    return (1);
...
/**
    ...
    EMF+ manual 2.2.2.18, Microsoft name: EmfPlusFocusScaleData Object
*/
int U_PMF_FOCUSSCALEDATA_get(const char *contents, uint32_t *Count,
                             U_FLOAT *ScaleX, U_FLOAT *ScaleY,
                             const char *blimit) {
    if (!contents || !Count || !ScaleX || !ScaleY || !blimit) {
        return (0);
    }
    if (IS_MEM_UNSAFE(contents, sizeof(U_PMF_FOCUSSCALEDATA), blimit))
        return (0);
    U_PMF_SERIAL_get(&contents, Count, 4, 1, U_LE);
    if (*Count != 2) {
        return (0);
...
/**
    ...
    EMF+ manual 2.2.2.19, Microsoft name: EmfPlusGraphicsVersion Object
*/
int U_PMF_GRAPHICSVERSION_get(const char *contents, int *Signature,
                              int *GrfVersion, const char *blimit) {
    uint32_t tmp;
    if (!contents || !Signature || !GrfVersion || !blimit) {
        return (0);
    }
    if (IS_MEM_UNSAFE(contents, sizeof(U_PMF_GRAPHICSVERSION), blimit))
        return (0);
...
/**
    ...
    EMF+ manual 2.2.2.20, Microsoft name: EmfPlusHatchBrushData Object
*/
int U_PMF_HATCHBRUSHDATA_get(const char *contents, uint32_t *Style,
                             U_PMF_ARGB *Foreground, U_PMF_ARGB *Background,
                             const char *blimit) {
    if (!contents || !Style || !Foreground || !Background || !blimit) {
        return (0);
    }
...
int U_PMF_INTEGER7_get(const char **contents, U_FLOAT *Value,
                       const char *blimit) {
    uint8_t tmp;
    if (!contents || !*contents || !Value || !blimit) {
        return (0);
    }
    if (IS_MEM_UNSAFE(*contents, 1, blimit))
        return (0); /* past end of buffer */
    if (**contents & U_TEST_INT7)
        return (0); /* this bit must be 0 in this object type */
...
int U_PMF_INTEGER15_get(const char **contents, U_FLOAT *Value,
    ...
    uint16_t tmp;
    if (IS_MEM_UNSAFE(*contents, 2, blimit))
        return (0); /* past end of buffer */
    if (!(**contents & U_TEST_INT7))
        return (0); /* this bit must be 1 in this object type */
    U_PMF_SERIAL_get(contents, &tmp, 2, 1, U_BE);
    tmp &= U_MASK_INT15; /* drop the 7/15 flag from the most significant bit */
    if (tmp & U_SIGN_INT15) {
        tmp |=
            U_TEST_INT15; /* now it has the bit pattern of a signed int16_t */
        *Value = *(int16_t *)&tmp;
    } else {
        *Value = tmp;
    }
...
/**
    ...
    This type is defined as 16 bits in the manual section, but it is only ever
   used as part of a 32 bit field!
*/
int U_PMF_LANGUAGEIDENTIFIER_get(U_PMF_LANGUAGEIDENTIFIER LId, int *SubLId,
                                 int *PriLId) {
    if (!SubLId || !PriLId) {
        return (0);
    }
...
/**
    ...
    \return 1 on success, 0 on error
    \param  contents   Record from which to extract data
    \param  Lgbd       U_PMF_LINEARGRADIENTBRUSHDATA structure, with no variable
   part
...
int U_PMF_LINEARGRADIENTBRUSHDATA_get(const char *contents,
                                      ...
                                      const char **Data, const char *blimit) {
    if (!contents || !Lgbd || !Data || !blimit) {
        return (0);
    }
    if (IS_MEM_UNSAFE(contents, sizeof(U_PMF_LINEARGRADIENTBRUSHDATA), blimit))
        return (0);
    U_PMF_SERIAL_get(&contents, Lgbd, 4, 6, U_LE); /* Flags, WrapMode, RectF*/
    U_PMF_SERIAL_get(&contents, &(Lgbd->StartColor), 4, 4,
                     U_XE); /* StartColor, EndColor, Reserved1 & 2 */
    U_PMF_PTRSAV_SHIFT(Data, &contents, 0);
    return (1);
}
...
/**
    ...
    \return 1 on success, 0 on error
    \param  contents   Record from which to extract data
    \param  Flags      BrushData flags - indicates which of the following date
   fields are present.
    \param  Tm         Transformation matrix
    \param  Bc         U_PMF_BLENDCOLORS object or NULL
    \param  BfH        U_PMF_BLENDFACTORS (H) object or NULL
    \param  BfV        U_PMF_BLENDFACTORS (V) object or NULL (WARNING, GDI+
   defines this field but does not render it.  DO NOT USE.)
    \param  blimit     one byte past the end of data
...
*/
int U_PMF_LINEARGRADIENTBRUSHOPTIONALDATA_get(
    const char *contents, uint32_t Flags, U_PMF_TRANSFORMMATRIX *Tm,
    const char **Bc, const char **BfH, const char **BfV, const char *blimit) {
    uint32_t Elements;
    if (!contents || !Tm || !Bc || !BfH || !BfV || !blimit) {
        return (0);
    }
    /* all of the fields are optional! */
    *Bc = *BfH = *BfV = NULL;
    if (Flags & U_BD_Transform) {
        if (IS_MEM_UNSAFE(contents, sizeof(U_PMF_ROTMATRIX), blimit))
            return (0);
    ...
    if (Flags & U_BD_PresetColors) {
        if (IS_MEM_UNSAFE(contents, 4, blimit))
            return (0);
        U_PMF_SERIAL_get(&contents, &Elements, 4, 1,
                         U_LE); /* starts with a 4 byte count*/
        if (IS_MEM_UNSAFE(contents,
                          Elements * (sizeof(U_FLOAT) + sizeof(U_PMF_ARGB)),
                          blimit))
            return (0);
        contents -= 4; /* back up to the front of the count, as it is part of
                          the data field */
        U_PMF_PTRSAV_SHIFT(Bc, &contents, 0);
    } else if (Flags & U_BD_BlendFactorsH) {
        U_PMF_SERIAL_get(&contents, &Elements, 4, 1,
                         U_LE); /* starts with a 4 byte count*/
        if (IS_MEM_UNSAFE(contents, Elements * 2 * sizeof(U_FLOAT), blimit))
            return (0);
        contents -= 4; /* back up to the front of the count, as it is part of
                          the data field */
        U_PMF_PTRSAV_SHIFT(
            BfH, &contents,
            4 + (Elements * 2 *
                 sizeof(U_FLOAT))); /* 4 byte count + 2 * 4bytes * Elements */
...
int U_PMF_LINEPATH_get(const char *contents, int32_t *Size, const char **Data,
    ...
    if (IS_MEM_UNSAFE(contents, sizeof(U_PMF_LINEPATH), blimit))
        return (0);
    U_PMF_SERIAL_get(&contents, Size, 4, 1, U_LE);
    U_PMF_PTRSAV_SHIFT(Data, &contents, 0);
    return (1);
...
/**
    ...
    \param  Type
    \param  Size       Bytes in Data
    \param  Data       Various types of data, like an EMF metafile, WMF
   metafile, another EMF+ metafile
    \param  blimit     one byte past the end of data
    ...
    Caller must check Data for possible memory access violations.
*/
int U_PMF_METAFILE_get(const char *contents, uint32_t *Type, uint32_t *Size,
                       const char **Data, const char *blimit) {
    if (!contents || !Type || !Size || !Data || !blimit) {
        return (0);
    }
    if (IS_MEM_UNSAFE(contents, sizeof(U_PMF_METAFILE), blimit))
        return (0);
    U_PMF_SERIAL_get(&contents, &Type, 4, 1, U_LE);
    U_PMF_SERIAL_get(&contents, &Size, 4, 1, U_LE);
    U_PMF_PTRSAV_SHIFT(Data, &contents, 0);
...
/**
    ...
    EMF+ manual 2.2.2.28, Microsoft name: EmfPlusPalette Object
*/
int U_PMF_PALETTE_get(const char *contents, uint32_t *Flags, uint32_t *Elements,
                      const char **Colors, const char *blimit) {
    if (!contents || !Flags || !Elements || !Colors || !blimit) {
        return (0);
    }
    if (IS_MEM_UNSAFE(contents, sizeof(U_PMF_PALETTE), blimit))
        return (0);
    U_PMF_SERIAL_get(&contents, &Flags, 4, 1, U_LE);
    U_PMF_SERIAL_get(&contents, &Elements, 4, 1, U_LE);
    if (IS_MEM_UNSAFE(contents, *Elements * sizeof(U_RGBQUAD), blimit))
        return (0);
    U_PMF_PTRSAV_SHIFT(Colors, &contents, 0);
    return (1);
}
...
/**
    ...
    \param  Gradient     variable part of U_PMF_LINEARGRADIENTBRUSHDATA, Color
   Gradient with Elements members
    \param  Boundary     variable part of U_PMF_LINEARGRADIENTBRUSHDATA,
   U_PMF_BOUNDARYPATHDATA object if BrushDataPath bit set in Flag, else
   U_PMF_BOUNDARYPOINTDATA object
    \param  Data         variable part of U_PMF_LINEARGRADIENTBRUSHDATA, exact
   composition depends on Flags
    \param  blimit       one byte past the end of data
...
int U_PMF_PATHGRADIENTBRUSHDATA_get(const char *contents,
                                    ...
                                    const char **Gradient,
                                    const char **Boundary, const char **Data,
                                    const char *blimit) {
    if (!contents || !Pgbd || !Gradient || !Boundary || !Data || !blimit) {
        return (0);
    }
    if (IS_MEM_UNSAFE(contents, sizeof(U_PMF_PATHGRADIENTBRUSHDATA), blimit))
        return (0);
    uint32_t Size;
    U_PMF_SERIAL_get(&contents, Pgbd, 4, 2, U_LE); /* Flags and WrapMode*/
    U_PMF_SERIAL_get(&contents, &(Pgbd->CenterColor), 4, 1, U_XE);
    U_PMF_SERIAL_get(&contents, &(Pgbd->Center), 4, 3,
                     U_LE); /* Center and Elements */
    if (IS_MEM_UNSAFE(contents, Pgbd->Elements * sizeof(U_PMF_ARGB), blimit))
        return (0);
    U_PMF_PTRSAV_SHIFT(Gradient, &contents,
                       Pgbd->Elements * sizeof(U_PMF_ARGB));
    U_PMF_PTRSAV_SHIFT(Boundary, &contents, 0);
    U_PMF_SERIAL_get(
        &contents, &Size, 4, 1,
        U_LE); /* The first 4 bytes of the Boundary are always a size */
    if (Pgbd->Flags & U_BD_Path) {
        contents += Size;
    } // U_PMF_BOUNDARYPATHDATA
    else {
        contents += Size * 2 * sizeof(U_FLOAT);
    } // U_PMF_BOUNDARYPOINTDATA
    if (Pgbd->Flags & (U_BD_Transform | U_BD_PresetColors | U_BD_BlendFactorsH |
                       U_BD_FocusScales)) { // optional data present
        if (contents >= blimit)
            return (0); // the size is variable but this must still hold
        U_PMF_PTRSAV_SHIFT(Data, &contents, 0);
    } else {
        *Data = NULL;
    } // no optional data present
    return (1);
}
...
/**
   ...
   Object
*/
int U_PMF_PATHGRADIENTBRUSHOPTIONALDATA_get(
    const char *contents, uint32_t Flags, U_PMF_TRANSFORMMATRIX *Matrix,
    const char **Pattern, const char **Data, const char *blimit) {
    int varsize;
    if (!contents || !Flags || !Matrix || !Pattern || !Data || !blimit) {
        return (0);
    }
    /* this structure is entirely optional */
    if (Flags & U_BD_Transform) {
        if (IS_MEM_UNSAFE(contents, sizeof(U_PMF_TRANSFORMMATRIX), blimit))
            return (0);
    ...
    if (Flags & (U_BD_PresetColors | U_BD_BlendFactorsH)) {
        ...
        if (IS_MEM_UNSAFE(contents, varsize, blimit))
            return (0);
...
/**
    ...
    EMF+ manual 2.2.2.32, Microsoft name: EmfPlusPathPointTypeRLE Object
*/
int U_PMF_PATHPOINTTYPERLE_get(const char *contents, int *Bezier, int *RL,
                               int *Ppt, const char *blimit) {
    if (!contents || !Bezier || !RL || !Ppt || !blimit) {
        return (0);
    }
    uint16_t tmp;
    if (IS_MEM_UNSAFE(contents, 2, blimit))
...
/**
    ...
    EMF+ manual 2.2.2.33, Microsoft name: EmfPlusPenData Object
*/
int U_PMF_PENDATA_get(const char *contents, uint32_t *Flags, uint32_t *Unit,
                      U_FLOAT *Width, const char **Data, const char *blimit) {
    if (!contents || !Flags || !Unit || !Width || !Data || !blimit) {
        return (0);
    }
    if (IS_MEM_UNSAFE(contents, 3 * 4, blimit))
        return (0);
    U_PMF_SERIAL_get(&contents, Flags, 4, 1, U_LE);
    U_PMF_SERIAL_get(&contents, Unit, 4, 1, U_LE);
    U_PMF_SERIAL_get(&contents, Width, 4, 1, U_LE);
    if (contents >= blimit)
        return (0); // variable data will extend farther, but this much at least
                    // must be true
    U_PMF_PTRSAV_SHIFT(Data, &contents, 0);
    return (1);
}
...
/**
    ...
    \return 1 on success, 0 on error
    \param  contents       Record from which to extract data
    \param  Flags;         PenData Flags - indicated which of the many fields
   are present.
    \param  Matrix;        Transformation matrix
    \param  StartCap       LineCapType enumeration
    \param  EndCap         LineCapType enumeration
    ...
    This object consists of a large number of optional and or variable values,
   which
...
int U_PMF_PENOPTIONALDATA_get(
    ...
    U_PMF_TRANSFORMMATRIX *Matrix, int32_t *StartCap, int32_t *EndCap,
    uint32_t *Join, U_FLOAT *MiterLimit, int32_t *Style, int32_t *DLCap,
    U_FLOAT *DLOffset, const char **DLData, int32_t *Alignment,
    const char **CmpndLineData, const char **CSCapData, const char **CECapData,
    const char *blimit) {
    if (!contents || !Flags || !Matrix || !StartCap || !EndCap || !Join ||
        !MiterLimit || !Style || !DLCap || !DLOffset || !DLData || !Alignment ||
        !CmpndLineData || !CSCapData || !CECapData || !blimit) {
        return (0);
    }

    if (Flags & U_PD_Transform) {
        if (IS_MEM_UNSAFE(contents, 4 * 6, blimit))
            return (0);
        U_PMF_SERIAL_get(&contents, Matrix, 4, 6, U_LE);
    }
    if (Flags & U_PD_StartCap) {
    ...
    if (Flags & U_PD_CLData) {
        ...
        if (IS_MEM_UNSAFE(contents, U_PMF_LEN_FLOATDATA(contents), blimit))
            return (0);
        U_PMF_PTRSAV_SHIFT(CmpndLineData, &contents,
                           U_PMF_LEN_FLOATDATA(contents));
    }
    if (Flags & U_PD_CustomStartCap) {
        if (IS_MEM_UNSAFE(contents, 4, blimit))
            return (0);
...
/**
    ...
    \return 1 on success, 0 on error
    \param  contents   Record from which to extract data.  On return position is
   offset by sizeof(U_PMF_POINT).
...
int U_PMF_POINT_get(const char **contents, U_FLOAT *X, U_FLOAT *Y,
                    const char *blimit) {
    if (!contents || !X || !Y || !blimit) {
        return (0);
    }
    int16_t tmp;
    if (IS_MEM_UNSAFE(*contents, 2 * 2, blimit))
        return (0);
    U_PMF_SERIAL_get(contents, &tmp, 2, 1, U_LE);
    *X = tmp;
    U_PMF_SERIAL_get(contents, &tmp, 2, 1, U_LE);
...
int U_PMF_POINTR_get(const char **contents, U_FLOAT *X, U_FLOAT *Y,
                     const char *blimit) {
    if (!contents || !*contents | !X || !Y || !blimit) {
        return (0);
    }
    int size = 0;

    if (U_PMF_INTEGER7_get(contents, X, blimit)) {
        size += 1;
    } else if (U_PMF_INTEGER15_get(contents, X, blimit)) {
        size += 2;
    } else {
        return (0);
    }

    if (U_PMF_INTEGER7_get(contents, Y, blimit)) {
        size += 1;
    } else if (U_PMF_INTEGER15_get(contents, Y, blimit)) {
        size += 2;
    } else {
        return (0);
    }
...
/**
    \brief Get data from a variable POINTS object, which may be U_PMF_POINTS,
   U_PMF_POINTF, or U_PMF_POINTR.
    \return 1 on success, 0 on error
    \param  contents   Record from which to extract data
...
int U_PMF_VARPOINTS_get(const char *contents, uint16_t Flags, int Elements,
                        U_PMF_POINTF **Points, const char *blimit) {
    int status = 0;
    if (!contents || !Points || !Elements || !blimit) {
        return (status);
    }
    ...
    *Points = pts;
    ...
    if (Flags & U_PPF_P) {
        for (XFS = YFS = 0.0; Elements; Elements--, pts++) {
            if (!U_PMF_POINTR_get(&contents, &XF, &YF, blimit))
                return (0); /* this should never happen */
            XFS += XF; /* position relative to previous point, first point is
                          always 0,0 */
            YFS += YF;
            pts->X = XFS;
            pts->Y = YFS;
        ...
        for (XF = YF = 0.0; Elements; Elements--, pts++) {
            if (!U_PMF_POINT_get(&contents, &XF, &XF, blimit))
...
/**
    ...
    EMF+ manual 2.2.2.38, Microsoft name: EmfPlusRect Object
*/
int U_PMF_RECT_get(const char **contents, int16_t *X, int16_t *Y,
                   int16_t *Width, int16_t *Height, const char *blimit) {
    if (!contents || !X || !Y || !Width || !Height) {
        return (0);
    }
    if (IS_MEM_UNSAFE(*contents, 2 * 4, blimit))
        return (0);
...
/**
    ...
    EMF+ manual 2.2.2.39, Microsoft name: EmfPlusRectF Object
*/
int U_PMF_RECTF_get(const char **contents, U_FLOAT *X, U_FLOAT *Y,
                    U_FLOAT *Width, U_FLOAT *Height, const char *blimit) {
    if (!contents || !X || !Y || !Width || !Height) {
        return (0);
    }
    if (IS_MEM_UNSAFE(*contents, 4 * 4, blimit))
        return (0);
    U_PMF_SERIAL_get(contents, X, 4, 1, U_LE);
    U_PMF_SERIAL_get(contents, Y, 4, 1, U_LE);
    U_PMF_SERIAL_get(contents, Width, 4, 1, U_LE);
...
/**
    \brief Get data from a variable RECTS object, which may be U_PMF_RECT or
   U_PMF_RECTF
    \return 1 on success, 0 on error
    \param  contents   Record from which to extract data
    ...
    Rects in record may be either U_PMF_RECT or U_PMF_RECTF, but this function
   always
    returns U_PMF_RECTF
*/
int U_PMF_VARRECTS_get(const char **contents, uint16_t Flags, int Elements,
                       U_PMF_RECTF **Rects, const char *blimit) {
    ...
    *Rects = rts;
    ...
    for (; Elements; Elements--, rts++) {
        if (Flags & U_PPF_C) {
            ...
            (void)U_PMF_RECTF_get(contents, &(rts->X), &(rts->Y), &(rts->Width),
                                  &(rts->Height), blimit);
        }
    }
    return (1);
...
/**
    ...
    \param  Data       Depending on Type: U_PMF_REGIONNODEPATH, U_PMF_RECTF, or
   U_PMF_REGIONNODECHILDNODES
    \param  blimit     one byte past the end of data
    ...
    Caller must check Data for possible memory access violations.
*/
int U_PMF_REGIONNODE_get(const char *contents, uint32_t *Type,
                         const char **Data, const char *blimit) {
    if (!contents || !Type || !Data || !blimit) {
        return (0);
    }
    /* Important!  This only checks the constant part, the caller must check
     * that returned data doesn't exceed blimit */
    if (IS_MEM_UNSAFE(contents, sizeof(U_PMF_REGIONNODE), blimit))
        return (0);
    U_PMF_SERIAL_get(&contents, Type, 4, 1, U_LE);
    if (contents >= blimit)
        return (0); // returned Data is variable size, this much at least must
                    // be true
    U_PMF_PTRSAV_COND(Data, contents,
                      !(*Type == U_RNDT_Empty || *Type == U_RNDT_Infinite));
    return (1);
}
...
/**
    ...
    So the data stored in each node must be handled as the tree is followed
   recursively.

    See U_PMF_REGIONNODECHILDNODES_print() and U_PMF_REGIONNODE_print() for an
   example.
...
/**
    ...
    EMF+ manual 2.2.2.42, Microsoft name: EmfPlusRegionNodePath Object
*/
int U_PMF_REGIONNODEPATH_get(const char *contents, int32_t *Size,
                             const char **Data, const char *blimit) {
    if (!contents || !Size || !Data || !blimit) {
        return (0);
    }
    /* Important!  This only checks the constant part, the caller must check
     * that returned data doesn't exceed blimit */
    if (IS_MEM_UNSAFE(contents, sizeof(U_PMF_REGIONNODEPATH), blimit))
        return (0);
    U_PMF_SERIAL_get(&contents, Size, 4, 1, U_LE);
    if (contents >= blimit)
        return (0); // returned Data is variable size, this much at least must
...
int U_PMF_SOLIDBRUSHDATA_get(const char *contents, U_PMF_ARGB *Color,
                             const char *blimit) {
    if (!contents || !Color || !blimit) {
        return (0);
    }
    if (IS_MEM_UNSAFE(contents, sizeof(U_PMF_SOLIDBRUSHDATA), blimit))
        return (0);
...
/**
    ...
    EMF+ manual 2.2.2.44, Microsoft name: EmfPlusStringFormatData Object
*/
int U_PMF_STRINGFORMATDATA_get(const char *contents, uint32_t TabStopCount,
                               uint32_t RangeCount, const U_FLOAT **TabStops,
                               const U_PMF_CHARACTERRANGE **CharRange,
                               const char *blimit) {
    if (!contents || !TabStops || !CharRange || !blimit) {
        return (0);
    }
    if (IS_MEM_UNSAFE(contents, (TabStopCount + 2 * RangeCount) * 4, blimit))
        return (0);
    *TabStops = NULL;
    if (TabStopCount > 0) {
        U_PMF_SERIAL_get(&contents, TabStops, 4, TabStopCount, U_LE);
    }
    *CharRange = NULL;
    if (RangeCount > 0) {
        U_PMF_SERIAL_get(&contents, CharRange, 4, 2 * RangeCount, U_LE);
    }
    return (1);
}
...
/**
    ...
    Caller must check Data for possible memory access violations.
*/
int U_PMF_TEXTUREBRUSHDATA_get(const char *contents, uint32_t *Flags,
                               int32_t *WrapMode, const char **Data,
                               const char *blimit) {
    if (!contents || !Flags || !WrapMode || !Data || !blimit) {
        return (0);
    }
    /* Important!  This only checks the constant part, the caller must check
     * that returned data doesn't exceed blimit */
    if (IS_MEM_UNSAFE(contents, sizeof(U_PMF_TEXTUREBRUSHDATA), blimit))
        return (0);
    U_PMF_SERIAL_get(&contents, Flags, 4, 1, U_LE);
    U_PMF_SERIAL_get(&contents, WrapMode, 4, 1, U_LE);
    if (contents >= blimit)
...
/**
    ...
    \param  Matrix     Transformation matrix, NULL if Flag BrushDataTransform is
   not set.
...
*/
int U_PMF_TEXTUREBRUSHOPTIONALDATA_get(const char *contents, int HasImage,
                                       U_PMF_TRANSFORMMATRIX *Matrix,
                                       const char **Image, const char *blimit) {
    if (!contents || !Image || !blimit) {
        return (0);
    }
    if (Matrix) {
        if (IS_MEM_UNSAFE(contents, sizeof(U_PMF_TRANSFORMMATRIX), blimit))
    ...
    if (HasImage) {
        if (contents >= blimit)
            return (0); // returned Data is variable size, this much at least
                        // must be true
...
/**
    ...
    \return 1 on success, 0 on error
    \param  contents   Record from which to extract data
    \param  Matrix     Transformation matrix, present if Flag BrushDataTransform
   is set.
    \param  blimit     one byte past the end of data
...
/**
    ...
    EMF+ manual 2.2.3.1, Microsoft name: BlurEffect Object
*/
int U_PMF_IE_BLUR_get(const char *contents, U_FLOAT *Radius,
                      uint32_t *ExpandEdge, const char *blimit) {
    if (!contents || !Radius || !ExpandEdge || !blimit) {
        return (0);
    }
    if (IS_MEM_UNSAFE(contents, sizeof(U_PMF_IE_BLUR), blimit))
        return (0);
...
/**
   ...
   negative decreases
    ...
    EMF+ manual 2.2.3.2, Microsoft name: BrightnessContrastEffect Object
*/
int U_PMF_IE_BRIGHTNESSCONTRAST_get(const char *contents, int32_t *Brightness,
                                    int32_t *Contrast, const char *blimit) {
    if (!contents || !Brightness || !Contrast || !blimit) {
        return (0);
    }
    if (IS_MEM_UNSAFE(contents, sizeof(U_PMF_IE_BRIGHTNESSCONTRAST), blimit))
        return (0);
...
/**
   ...
   decreases Yellow,  negative is opposite
    ...
    EMF+ manual 2.2.3.3, Microsoft name: ColorBalanceEffect Object
*/
int U_PMF_IE_COLORBALANCE_get(const char *contents, int32_t *CyanRed,
                              int32_t *MagentaGreen, int32_t *YellowBlue,
                              const char *blimit) {
    if (!contents || !CyanRed || !MagentaGreen || !YellowBlue || !blimit) {
        return (0);
    }
    if (IS_MEM_UNSAFE(contents, sizeof(U_PMF_IE_COLORBALANCE), blimit))
        return (0);
...
/**
   ...
   this is and range values.
    ...
    EMF+ manual 2.2.3.4, Microsoft name: ColorCurveEffect Object
*/
int U_PMF_IE_COLORCURVE_get(const char *contents, uint32_t *Adjust,
                            uint32_t *Channel, int32_t *Intensity,
                            const char *blimit) {
    if (!contents || !Adjust || !Channel || !Intensity || !blimit) {
        return (0);
    }
    if (IS_MEM_UNSAFE(contents, sizeof(U_PMF_IE_COLORCURVE), blimit))
        return (0);
...
/**
    ...
    EMF+ manual 2.2.3.5, Microsoft name: ColorLookupTableEffect Object
*/
int U_PMF_IE_COLORLOOKUPTABLE_get(const char *contents, const uint8_t **BLUT,
                                  const uint8_t **GLUT, const uint8_t **RLUT,
                                  const uint8_t **ALUT, const char *blimit) {
    if (!contents || !BLUT || !GLUT || !RLUT || !ALUT || !blimit) {
        return (0);
    }
    if (IS_MEM_UNSAFE(contents, sizeof(U_PMF_IE_COLORLOOKUPTABLE) + 4 * 256,
                      blimit))
        return (0);
...
int U_PMF_IE_COLORMATRIX_get(const char *contents, U_PMF_IE_COLORMATRIX *Matrix,
    ...
    /* Important!  This only checks the constant part, the caller must check
     * that returned data doesn't exceed blimit */
    if (IS_MEM_UNSAFE(contents, sizeof(U_PMF_IE_COLORMATRIX), blimit))
        return (0);
...
/**
    ...
    EMF+ manual 2.2.3.7, Microsoft name: HueSaturationLightnessEffect Object
*/
int U_PMF_IE_HUESATURATIONLIGHTNESS_get(const char *contents, int32_t *Hue,
                                        int32_t *Saturation, int32_t *Lightness,
                                        const char *blimit) {
    if (!contents || !Hue || !Saturation || !Lightness || !blimit) {
        return (0);
    }
    if (IS_MEM_UNSAFE(contents, sizeof(U_PMF_IE_HUESATURATIONLIGHTNESS),
                      blimit))
        return (0);
...
int U_PMF_IE_LEVELS_get(const char *contents, int32_t *Highlight,
                        int32_t *Midtone, int32_t *Shadow, const char *blimit) {
    if (!contents || !Highlight || !Midtone || !Shadow || !blimit) {
        return (0);
    }
    if (IS_MEM_UNSAFE(contents, sizeof(U_PMF_IE_LEVELS), blimit))
        return (0);
...
/**
    ...
    \param  contents   Record from which to extract data
    \param  Elements   Number of members in Rects
    \param  Rects      Caller must free.  Pointer to memory holding an array of
   U_RECTL.
    \param  blimit     one byte past the end of data

    EMF+ manual 2.2.3.9, Microsoft name: RedEyeCorrectionEffect Object
*/
int U_PMF_IE_REDEYECORRECTION_get(const char *contents, int32_t *Elements,
                                  U_RECTL **Rects, const char *blimit) {
    if (!contents || !Elements || !Rects || !blimit) {
        return (0);
    }
...
/**
    ...
    EMF+ manual 2.2.3.10, Microsoft name: SharpenEffect Object
*/
int U_PMF_IE_SHARPEN_get(const char *contents, U_FLOAT *Radius,
                         int32_t *Sharpen, const char *blimit) {
    if (!contents || !Radius || !Sharpen || !blimit) {
        return (0);
    }
    if (IS_MEM_UNSAFE(contents, sizeof(U_PMF_IE_SHARPEN), blimit))
        return (0);
    U_PMF_SERIAL_get(&contents, Radius, 4, 1, U_LE);
...
int U_PMF_IE_TINT_get(const char *contents, int32_t *Hue, int32_t *Amount,
                      const char *blimit) {
    if (!contents || !Hue || !Amount || !blimit) {
        return (0);
    }
    if (IS_MEM_UNSAFE(contents, sizeof(U_PMF_IE_TINT), blimit))
        return (0);
    U_PMF_SERIAL_get(&contents, Hue, 4, 1, U_LE);
...
/*
   ...
   start of U_PMR_*_get() functions
      ...
      These functions all assume that the size field in the common EMF+ header
   has already
      been checked, so that the extent the record claims exists in the data read
   in for the file.
      Consequently none of them takes a blimit parameter.  They generate a new
   one from the
      header size field and contents if needed.

*/

int U_PMR_common_stack_get(const char *contents, U_PMF_CMN_HDR *Header,
                           uint32_t *StackID) {
    if (!contents || !StackID) {
        return (0);
    }
    ...
    if (lclHeader.Size < sizeof(U_PMF_RESTORE))
        return (0);
    if (Header) {
        memcpy(Header, &lclHeader, sizeof(U_PMF_CMN_HDR));
    }
...
int U_PMR_OFFSETCLIP_get(const char *contents, U_PMF_CMN_HDR *Header,
    ...
    U_PMF_CMN_HDR lclHeader;
    U_PMF_CMN_HDR_get(&contents, &lclHeader);
    if (lclHeader.Size < sizeof(U_PMF_OFFSETCLIP))
        return (0);
    if (Header) {
        memcpy(Header, &lclHeader, sizeof(U_PMF_CMN_HDR));
    }
...
int U_PMR_SETCLIPPATH_get(const char *contents, U_PMF_CMN_HDR *Header,
    ...
    if (!contents || !PathID || !CMenum) {
        return (0);
    }

    U_PMF_CMN_HDR lclHeader;
    U_PMF_CMN_HDR_get(&contents, &lclHeader);
    if (lclHeader.Size < sizeof(U_PMF_SETCLIPPATH))
        return (0);
    if (Header) {
        memcpy(Header, &lclHeader, sizeof(U_PMF_CMN_HDR));
    }
...
/**
    ...
    \param  Rect        Rectangle used with CombineMode enumeration from
   Header.Flags

    EMF+ manual 2.3.1.4, Microsoft name: EmfPlusSetClipRect Record, Index 0x32
*/
int U_PMR_SETCLIPRECT_get(const char *contents, U_PMF_CMN_HDR *Header,
                          int *CMenum, U_PMF_RECTF *Rect) {
    if (!contents || !CMenum || !Rect) {
        return (0);
    }

    U_PMF_CMN_HDR lclHeader;
    U_PMF_CMN_HDR_get(&contents, &lclHeader);
    if (lclHeader.Size < sizeof(U_PMF_SETCLIPRECT))
        return (0);
    if (Header) {
        memcpy(Header, &lclHeader, sizeof(U_PMF_CMN_HDR));
    }
...
int U_PMR_SETCLIPREGION_get(const char *contents, U_PMF_CMN_HDR *Header,
    ...
    U_PMF_CMN_HDR lclHeader;
    U_PMF_CMN_HDR_get(&contents, &lclHeader);
    if (lclHeader.Size < sizeof(U_PMF_SETCLIPREGION))
        return (0);
    if (Header) {
        memcpy(Header, &lclHeader, sizeof(U_PMF_CMN_HDR));
    }
...
int U_PMR_COMMENT_get(const char *contents, U_PMF_CMN_HDR *Header,
    ...
    U_PMF_CMN_HDR lclHeader;
    U_PMF_CMN_HDR_get(&contents, &lclHeader);
    if (lclHeader.Size < sizeof(U_PMF_COMMENT))
        return (0);
    if (Header) {
        memcpy(Header, &lclHeader, sizeof(U_PMF_CMN_HDR));
    }
...
int U_PMR_HEADER_get(const char *contents, U_PMF_CMN_HDR *Header,
                     U_PMF_GRAPHICSVERSION *Version, int *IsDual, int *IsVideo,
                     uint32_t *LogicalDpiX, uint32_t *LogicalDpiY) {
    if (!contents || !Version || !IsDual || !IsVideo || !LogicalDpiX ||
        !LogicalDpiY) {
        return (0);
    }
    uint32_t tmp;
    ...
    U_PMF_CMN_HDR lclHeader;
    U_PMF_CMN_HDR_get(&contents, &lclHeader);
    if (lclHeader.Size < sizeof(U_PMF_HEADER))
        return (0);
    if (Header) {
        memcpy(Header, &lclHeader, sizeof(U_PMF_CMN_HDR));
    }
...
int U_PMR_CLEAR_get(const char *contents, U_PMF_CMN_HDR *Header,
    ...
    U_PMF_CMN_HDR lclHeader;
    U_PMF_CMN_HDR_get(&contents, &lclHeader);
    if (lclHeader.Size < sizeof(U_PMF_CLEAR))
        return (0);
    if (Header) {
        memcpy(Header, &lclHeader, sizeof(U_PMF_CMN_HDR));
    }
...
/**
   ...
   inclusive)
    ...
    \param  Sweep       Sweep angle, -360<= angle <=360, degrees clockwise from
   Start
    \param  Rect        Caller must free.  Bounding rectangle.  Coordinate type
   set by ctype.

    EMF+ manual 2.3.4.2, Microsoft name: EmfPlusDrawArc Record, Index 0x12
*/
int U_PMR_DRAWARC_get(const char *contents, U_PMF_CMN_HDR *Header,
                      uint32_t *PenID, int *ctype, U_FLOAT *Start,
                      U_FLOAT *Sweep, U_PMF_RECTF *Rect) {
    if (!contents || !PenID || !ctype || !Start || !Sweep || !Rect) {
        return (0);
    }
    ...
    U_PMF_CMN_HDR lclHeader;
    U_PMF_CMN_HDR_get(&contents, &lclHeader);
    if (lclHeader.Size < sizeof(U_PMF_DRAWARC))
        return (0);
    if (Header) {
        memcpy(Header, &lclHeader, sizeof(U_PMF_CMN_HDR));
    }
    ...
    *PenID = (lclHeader.Flags >> U_FF_SHFT_OID8) & U_FF_MASK_OID8;
    ...
    if (!U_PMF_VARRECTS_get(&contents, lclHeader.Flags, 1, &Rects, blimit))
        return (0);
...
/**
   ...
   inclusive)
    \param  ctype       Set: int16_t coordinates; Clear: U_FLOAT coordinates
    \param  RelAbs      Set: Coordinates are relative; Clear: Coordinates are
   absolute and their type is set by ctype
    \param  Elements    Number of members in the Data array
    \param  Points      Caller must free.  Array of U_POINT_F = Sequence of
   points to connect.  Coordinate type set by ctype and RelAbs.

    EMF+ manual 2.3.4.3, Microsoft name: EmfPlusDrawBeziers Record, Index 0x19
*/
int U_PMR_DRAWBEZIERS_get(const char *contents, U_PMF_CMN_HDR *Header,
                          uint32_t *PenID, int *ctype, int *RelAbs,
                          uint32_t *Elements, U_PMF_POINTF **Points) {
    if (!contents || !PenID || !ctype || !RelAbs || !Elements || !Points) {
        return (0);
    }
    ...
    U_PMF_CMN_HDR lclHeader;
    U_PMF_CMN_HDR_get(&contents, &lclHeader);
    if (lclHeader.Size < sizeof(U_PMF_DRAWBEZIERS))
        return (0);
    if (Header) {
        memcpy(Header, &lclHeader, sizeof(U_PMF_CMN_HDR));
    }
    ...
    *PenID = (lclHeader.Flags >> U_FF_SHFT_OID8) & U_FF_MASK_OID8;
    U_PMF_SERIAL_get(&contents, Elements, 4, 1, U_LE);
    int status = U_PMF_VARPOINTS_get(contents, lclHeader.Flags, *Elements,
                                     Points, blimit);
    return (status);
}
...
int U_PMR_DRAWCLOSEDCURVE_get(const char *contents, U_PMF_CMN_HDR *Header,
                              ...
                              U_PMF_POINTF **Points) {
    if (!contents || !PenID || !ctype || !RelAbs || !Tension || !Elements ||
        !Points) {
        return (0);
    }
    ...
    U_PMF_CMN_HDR lclHeader;
    U_PMF_CMN_HDR_get(&contents, &lclHeader);
    if (lclHeader.Size < sizeof(U_PMF_DRAWCLOSEDCURVE))
        return (0);
    if (Header) {
        memcpy(Header, &lclHeader, sizeof(U_PMF_CMN_HDR));
    }
...
int U_PMR_DRAWCURVE_get(const char *contents, U_PMF_CMN_HDR *Header,
                        uint32_t *PenID, int *ctype, U_FLOAT *Tension,
                        uint32_t *Offset, uint32_t *NSegs, uint32_t *Elements,
                        U_PMF_POINTF **Points) {
    if (!contents || !PenID || !ctype || !Tension || !Offset || !NSegs ||
        !Elements || !Points) {
        return (0);
    }
    ...
    U_PMF_CMN_HDR lclHeader;
    U_PMF_CMN_HDR_get(&contents, &lclHeader);
    if (lclHeader.Size < sizeof(U_PMF_DRAWCURVE))
        return (0);
    if (Header) {
        memcpy(Header, &lclHeader, sizeof(U_PMF_CMN_HDR));
    }
...
/**
    ...
    \param  FontID      U_PMF_FONT object in the EMF+ object table (0-63,
   inclusive)
    \param  btype       Set: BrushID is an U_PFM_ARGB; Clear: index of
   U_PMF_BRUSH object in EMF+ object table.
    \param  BrushID     Color or index of  U_PMF_BRUSH object in the EMF+ object
   table, depends on Flags bit0
    \param  DSOFlags    DriverStringOptions flags
    \param  HasMatrix   If 1 record contains a TransformMatrix field, if 0 it
   does not.
    \param  Elements    Number of members in Glyphs and Positions array
    \param  Glyphs      Caller must free.  If U_DSO_CmapLookup is set in
   DSOFlags this is an array of UTF16LE characters, otherwise, it is an array of
   indices into the U_PMF_FONT object indexed by Object_ID in flags.
    \param  Points      Caller must free.  Coordinates of each member of Glyphs.
   U_DSO_RealizedAdvance set in DSOFlags Relative then positions are calculated
   relative to the first glyph which is stored in Positions, otherwise, all
   glyph positions are stored in Positions.
    \param  Matrix      Caller must free.  Transformation to apply to Glyphs &
   Positions. Present if HasMatrix is 1

    EMF+ manual 2.3.4.6, Microsoft name: EmfPlusDrawDriverString Record, Index
   0x36
*/
int U_PMR_DRAWDRIVERSTRING_get(const char *contents, U_PMF_CMN_HDR *Header,
                               uint32_t *FontID, int *btype, uint32_t *BrushID,
                               uint32_t *DSOFlags, uint32_t *HasMatrix,
                               uint32_t *Elements, uint16_t **Glyphs,
                               U_PMF_POINTF **Points,
                               U_PMF_TRANSFORMMATRIX **Matrix) {
    if (!contents || !FontID || !btype || !BrushID || !DSOFlags || !HasMatrix ||
    ...
    U_PMF_CMN_HDR lclHeader;
    U_PMF_CMN_HDR_get(&contents, &lclHeader);
    if (lclHeader.Size < sizeof(U_PMF_DRAWDRIVERSTRING))
        return (0);
    if (Header) {
        memcpy(Header, &lclHeader, sizeof(U_PMF_CMN_HDR));
    }
    ...
    *FontID = (lclHeader.Flags >> U_FF_SHFT_OID8) & U_FF_MASK_OID8;
    U_PMF_SERIAL_get(
        &contents, BrushID, 4, 1,
        (*btype ? U_XE : U_LE)); /* color is not byte swapped, ID integer is */
    U_PMF_SERIAL_get(&contents, DSOFlags, 4, 1, U_LE);
    U_PMF_SERIAL_get(&contents, HasMatrix, 4, 1, U_LE);
    U_PMF_SERIAL_get(&contents, Elements, 4, 1, U_LE);
    if (IS_MEM_UNSAFE(contents, *Elements * 2 + *Elements * 2 * 4 + 24, blimit))
        return (0);
    if (!U_PMF_SERIAL_array_copy_get(&contents, (void **)Glyphs, 2, *Elements,
                                     U_LE, (*DSOFlags & U_DSO_CmapLookup))) {
        return (0);
    }
    if (!U_PMF_SERIAL_array_copy_get(&contents, (void **)Points, 4,
                                     *Elements * 2, U_LE,
                                     (*DSOFlags & U_DSO_RealizedAdvance))) {
        return (0);
    }
    if (!U_PMF_SERIAL_array_copy_get(&contents, (void **)Matrix, 4, 6, U_LE,
                                     (*HasMatrix))) {
        return (0);
    }
    return (1);
...
int U_PMR_DRAWELLIPSE_get(const char *contents, U_PMF_CMN_HDR *Header,
                          uint32_t *PenID, int *ctype, U_PMF_RECTF *Rect) {
    if (!contents || !PenID || !ctype || !Rect) {
        return (0);
    }

    U_PMF_CMN_HDR lclHeader;
    U_PMF_CMN_HDR_get(&contents, &lclHeader);
    if (lclHeader.Size < sizeof(U_PMF_DRAWELLIPSE))
        return (0);
    if (Header) {
        memcpy(Header, &lclHeader, sizeof(U_PMF_CMN_HDR));
    }
...
/**
   ...
   table
    ...
    \param  DstRect     Destination rectangle for image.  Coordinate type set by
   ctype.

    EMF+ manual 2.3.4.8, Microsoft name: EmfPlusDrawImage Record, Index 0x1A
*/
int U_PMR_DRAWIMAGE_get(const char *contents, U_PMF_CMN_HDR *Header,
                        uint32_t *ImgID, int *ctype, uint32_t *ImgAttrID,
                        int32_t *SrcUnit, U_PMF_RECTF *SrcRect,
                        U_PMF_RECTF *DstRect) {
    if (!contents || !ImgID || !ctype || !ImgAttrID || !SrcUnit || !SrcRect ||
        !DstRect) {
        return (0);
    }
    ...
    U_PMF_CMN_HDR lclHeader;
    U_PMF_CMN_HDR_get(&contents, &lclHeader);
    if (lclHeader.Size < sizeof(U_PMF_DRAWIMAGE))
        return (0);
    if (Header) {
        memcpy(Header, &lclHeader, sizeof(U_PMF_CMN_HDR));
    }
...
/**
   ...
   record will be applied; Clear:  no effect applied
    ...
    \param  Points      Caller must free.  3 points of a parallelogram..
   Coordinate type set by ctype and RelAbs.

    EMF+ manual 2.3.4.9, Microsoft name: EmfPlusDrawImagePoints Record, Index
   0x1B
*/
int U_PMR_DRAWIMAGEPOINTS_get(const char *contents, U_PMF_CMN_HDR *Header,
                              uint32_t *ImgID, int *ctype, int *etype,
                              int *RelAbs, uint32_t *ImgAttrID,
                              int32_t *SrcUnit, U_PMF_RECTF *SrcRect,
                              uint32_t *Elements, U_PMF_POINTF **Points) {
    if (!contents || !ImgID || !ctype || !etype || !RelAbs || !ImgAttrID ||
        !SrcUnit || !Elements || !Points) {
        return (0);
    }
    ...
    U_PMF_CMN_HDR lclHeader;
    U_PMF_CMN_HDR_get(&contents, &lclHeader);
    if (lclHeader.Size < sizeof(U_PMF_DRAWIMAGEPOINTS))
        return (0);
    if (Header) {
        memcpy(Header, &lclHeader, sizeof(U_PMF_CMN_HDR));
    }
...
int U_PMR_DRAWLINES_get(const char *contents, U_PMF_CMN_HDR *Header,
                        ...
                        uint32_t *Elements, U_PMF_POINTF **Points) {
    if (!contents || !PenID || !ctype || !dtype || !RelAbs || !Elements ||
        !Points) {
        return (0);
    }
    ...
    U_PMF_CMN_HDR lclHeader;
    U_PMF_CMN_HDR_get(&contents, &lclHeader);
    if (lclHeader.Size < sizeof(U_PMF_DRAWLINES))
        return (0);
    if (Header) {
        memcpy(Header, &lclHeader, sizeof(U_PMF_CMN_HDR));
    }
...
int U_PMR_DRAWPATH_get(const char *contents, U_PMF_CMN_HDR *Header,
    ...
    if (!contents || !PathID || !PenID) {
        return (0);
    }

    U_PMF_CMN_HDR lclHeader;
    U_PMF_CMN_HDR_get(&contents, &lclHeader);
    if (lclHeader.Size < sizeof(U_PMF_DRAWPATH))
        return (0);
    if (Header) {
        memcpy(Header, &lclHeader, sizeof(U_PMF_CMN_HDR));
    }
...
int U_PMR_DRAWPIE_get(const char *contents, U_PMF_CMN_HDR *Header,
    ...
    U_PMF_CMN_HDR lclHeader;
    U_PMF_CMN_HDR_get(&contents, &lclHeader);
    if (lclHeader.Size < sizeof(U_PMF_DRAWPIE))
        return (0);
    if (Header) {
        memcpy(Header, &lclHeader, sizeof(U_PMF_CMN_HDR));
    }
...
/**
   ...
   inclusive)
    \param  ctype       Set: int16_t coordinates; Clear: U_FLOAT coordinates
    \param  Elements    Number of members in Rects
    \param  Rects       Caller must free.  Array of U_PMF_RECTF rectangles to
   draw.
...
int U_PMR_DRAWRECTS_get(const char *contents, U_PMF_CMN_HDR *Header,
                        uint32_t *PenID, int *ctype, uint32_t *Elements,
                        U_PMF_RECTF **Rects) {
    if (!contents || !PenID || !Elements || !Rects) {
        return (0);
    }
...
int U_PMR_DRAWSTRING_get(const char *contents, U_PMF_CMN_HDR *Header,
                         uint32_t *FontID, int *btype, uint32_t *BrushID,
                         uint32_t *FormatID, uint32_t *Elements,
                         U_PMF_RECTF *Rect, uint16_t **String) {
    if (!contents || !FontID || !btype || !BrushID || !FormatID || !Elements ||
        !String) {
        return (0);
    }
    ...
    *FontID = (lclHeader.Flags >> U_FF_SHFT_OID8) & U_FF_MASK_OID8;
    ...
    if (IS_MEM_UNSAFE(contents, *Elements * 2, blimit))
        return (0);
    if (!U_PMF_SERIAL_array_copy_get(&contents, (void **)String, 2, *Elements,
                                     U_XE, 1)) {
        return (0);
    }
    return (1);
...
/**
    ...
    \param  contents    Record from which to extract data
    \param  Header      Common header
    \param  btype       Set: BrushID is an U_PFM_ARGB; Clear: is index of
   U_PMF_BRUSH object in EMF+ object table.
    \param  ctype       Set: int16_t coordinates; Clear: U_FLOAT coordinates
...
int U_PMR_FILLCLOSEDCURVE_get(const char *contents, U_PMF_CMN_HDR *Header,
                              ...
                              uint32_t *BrushID, U_FLOAT *Tension,
                              uint32_t *Elements, U_PMF_POINTF **Points) {
    if (!contents || !btype || !ctype || !ftype || !RelAbs || !BrushID ||
        !Tension || !Elements || !Points) {
        return (0);
    }
...
/**
   ...
   U_PMF_BRUSH object in EMF+ object table.
    ...
    \param  BrushID     Color or index of  U_PMF_BRUSH object in the EMF+ object
   table, depending on btype.
    \param  Rect        Caller must free.  Bounding box for elliptical pie
   segment being drawn.  Coordinate type set by ctype.

    EMF+ manual 2.3.4.16, Microsoft name: EmfPlusFillEllipse Record, Index 0x0E
*/
int U_PMR_FILLELLIPSE_get(const char *contents, U_PMF_CMN_HDR *Header,
                          int *btype, int *ctype, uint32_t *BrushID,
                          U_PMF_RECTF *Rect) {
    if (!contents || !btype || !ctype || !BrushID || !Rect) {
        return (0);
    }

    U_PMF_CMN_HDR lclHeader;
    U_PMF_CMN_HDR_get(&contents, &lclHeader);
    if (lclHeader.Size < sizeof(U_PMF_FILLELLIPSE))
        return (0);
    if (Header) {
        memcpy(Header, &lclHeader, sizeof(U_PMF_CMN_HDR));
    }
...
int U_PMR_FILLPATH_get(const char *contents, U_PMF_CMN_HDR *Header,
                       uint32_t *PathID, int *btype, uint32_t *BrushID) {
    if (!contents || !PathID || !btype || !BrushID) {
        return (0);
    }

    U_PMF_CMN_HDR lclHeader;
    U_PMF_CMN_HDR_get(&contents, &lclHeader);
    if (lclHeader.Size < sizeof(U_PMF_FILLPATH))
        return (0);
    if (Header) {
        memcpy(Header, &lclHeader, sizeof(U_PMF_CMN_HDR));
    }
...
/**
   ...
   table, depending on btype.
    ...
    \param  Sweep       Sweep angle, -360<= angle <=360, degrees clockwise from
   Start
    \param  Rect        Bounding box for elliptical pie segment being filled.
   Coordinate type set by ctype.

    EMF+ manual 2.3.4.18, Microsoft name: EmfPlusFillPie Record, Index 0x10
*/
int U_PMR_FILLPIE_get(const char *contents, U_PMF_CMN_HDR *Header, int *btype,
                      int *ctype, uint32_t *BrushID, U_FLOAT *Start,
                      U_FLOAT *Sweep, U_PMF_RECTF *Rect) {
    if (!contents || !btype || !ctype || !BrushID || !Start || !Sweep ||
        !Rect) {
        return (0);
    }
    ...
    U_PMF_CMN_HDR lclHeader;
    U_PMF_CMN_HDR_get(&contents, &lclHeader);
    if (lclHeader.Size < sizeof(U_PMF_FILLPIE))
        return (0);
    if (Header) {
        memcpy(Header, &lclHeader, sizeof(U_PMF_CMN_HDR));
    }
...
/**
   ...
   U_PMF_BRUSH object in EMF+ object table.
    \param  ctype       Set: int16_t coordinates; Clear: U_FLOAT coordinates
    \param  RelAbs      Set: U_PMF_PathPointTypeRLE and/or U_PMF_PathPointType
   objects; Clear: only U_PMF_PathPointType
    \param  BrushID     Color or index of  U_PMF_BRUSH object in the EMF+ object
   table, depending on btype.
    \param  Elements    Number of members in Data.
    \param  Points      Sequence of points to connect with line segments.
   Coordinate type set by ctype and RelAbs.
...
int U_PMR_FILLPOLYGON_get(const char *contents, U_PMF_CMN_HDR *Header,
                          ...
                          U_PMF_POINTF **Points) {
    if (!contents || !btype || !ctype || !RelAbs || !BrushID || !Elements ||
        !Points) {
        return (0);
    }
...
/**
   ...
   draw.
    ...
    EMF+ files have been encountered where BrushID must be a color, because it
   has a value like FFFF0000 but
    the flags are set wrong, so that U_PPF_B is not set.  Detect these by
   BrushID >63 for btype=0 and correct.
    If the opposite problem occurs it cannot be reliably detected, so it cannot
   be corrected.
   ...
   always
    returns U_PMF_RECTF
*/
int U_PMR_FILLRECTS_get(const char *contents, U_PMF_CMN_HDR *Header, int *btype,
                        int *ctype, uint32_t *BrushID, uint32_t *Elements,
                        U_PMF_RECTF **Rects) {
    if (!contents || !btype || !ctype || !BrushID || !Elements || !Rects) {
        return (0);
    }
    ...
    U_PMF_CMN_HDR lclHeader;
    U_PMF_CMN_HDR_get(&contents, &lclHeader);
    if (lclHeader.Size < sizeof(U_PMF_FILLRECTS))
        return (0);
    if (Header) {
        memcpy(Header, &lclHeader, sizeof(U_PMF_CMN_HDR));
    }
...
int U_PMR_FILLREGION_get(const char *contents, U_PMF_CMN_HDR *Header,
                         ...
                         uint32_t *BrushID) {
    if (!contents || !RgnID || !btype || !ctype || !BrushID) {
        return (0);
    }

    U_PMF_CMN_HDR lclHeader;
    U_PMF_CMN_HDR_get(&contents, &lclHeader);
    if (lclHeader.Size < sizeof(U_PMF_FILLREGION))
        return (0);
    if (Header) {
        memcpy(Header, &lclHeader, sizeof(U_PMF_CMN_HDR));
    }
...
/**
   ...
   inclusive)
    ...
    \param  TSize       If ntype is set, holds the total number of data bytes
   split across multiple records.  If ntype is clear, has no meaning.
    \param  Data        Object's data.  Type from otype.
    ...
    Documentation indicates that this table contains only 64 slots, although the
   index
       field which references it can code for values 0-127.
    If a new object has the same index as an existing object the old one is
   deleted and
       the new one goes into its storage slot.
    The continuation bit (U_PPF_N) is documented as indicating that the object
   is continued into
       the next record.  Examination of emf+ records in emf files produced by
   PowerPoint 2003
       show that changing the ObjID also serves as a continued record
   terminator, and that it apparently
       overrides the value for the continue bit.  That is, even though the
   preceding records said
       that it was continued, the change of ObjID terminates that preceding
   record without adding
...
int U_PMR_OBJECT_get(const char *contents, U_PMF_CMN_HDR *Header,
                     ...
                     const char **Data) {
    if (!contents || !ObjID || !otype || !ntype || !Data) {
        return (0);
    }

    U_PMF_CMN_HDR lclHeader;
    U_PMF_CMN_HDR_get(&contents, &lclHeader);
    if (lclHeader.Size < sizeof(U_PMF_OBJECT))
        return (0);
    if (Header) {
        memcpy(Header, &lclHeader, sizeof(U_PMF_CMN_HDR));
    }
    ...
    *otype = (lclHeader.Flags >> U_FF_SHFT_OT) & U_FF_MASK_OT;
    if (*ntype) {
        U_PMF_SERIAL_get(&contents, TSize, 4, 1, U_LE);
    } else {
        *TSize = 0;
    }
    U_PMF_PTRSAV_SHIFT(Data, &contents, 0);
...
/**
    ...
    \param  Size        Bytes in Data.
    \param  Data        "Serialized image effects parameter block".  One of the
   ImageEffects objects.
...
int U_PMR_SERIALIZABLEOBJECT_get(const char *contents, U_PMF_CMN_HDR *Header,
                                 ...
                                 const char **Data) {
    if (!contents || !GUID || !Size || !Data) {
        return (0);
    }

    U_PMF_CMN_HDR lclHeader;
    U_PMF_CMN_HDR_get(&contents, &lclHeader);
    if (lclHeader.Size < sizeof(U_PMF_SERIALIZABLEOBJECT))
        return (0);
    if (Header) {
        memcpy(Header, &lclHeader, sizeof(U_PMF_CMN_HDR));
    }
...
int U_PMR_SETANTIALIASMODE_get(const char *contents, U_PMF_CMN_HDR *Header,
    ...
    if (!contents || !SMenum || !aatype) {
        return (0);
    }

    U_PMF_CMN_HDR lclHeader;
    U_PMF_CMN_HDR_get(&contents, &lclHeader);
    if (lclHeader.Size < sizeof(U_PMF_SETANTIALIASMODE))
        return (0);
    if (Header) {
        memcpy(Header, &lclHeader, sizeof(U_PMF_CMN_HDR));
    }
...
int U_PMR_SETRENDERINGORIGIN_get(const char *contents, U_PMF_CMN_HDR *Header,
    ...
    U_PMF_CMN_HDR lclHeader;
    if (!U_PMF_CMN_HDR_get(&contents, &lclHeader))
        return (0);
    if (lclHeader.Size < sizeof(U_PMF_SETRENDERINGORIGIN))
        return (0);
    if (Header) {
        memcpy(Header, &lclHeader, sizeof(U_PMF_CMN_HDR));
    }
...
int U_PMR_BEGINCONTAINER_get(const char *contents, U_PMF_CMN_HDR *Header,
                             int *UTenum, U_PMF_RECTF *DstRect,
                             U_PMF_RECTF *SrcRect, uint32_t *StackID) {
    if (!contents || !UTenum || !DstRect || !SrcRect || !StackID) {
        return (0);
    }
...
int U_PMR_BEGINCONTAINERNOPARAMS_get(const char *contents,
                                     U_PMF_CMN_HDR *Header, uint32_t *StackID) {
...
/**
    ...
    \param  contents    Record from which to extract data
    \param  Header      Common header
    \param  StackID     State (level) to restore from the EMF+ Graphics Stack.
   Must have been put on the GS with a U_PMR_SAVE.

    EMF+ manual 2.3.7.4, Microsoft name: EmfPlusRestore Record, Index 0x26
*/
int U_PMR_RESTORE_get(const char *contents, U_PMF_CMN_HDR *Header,
                      uint32_t *StackID) {
    return (U_PMR_common_stack_get(contents, Header, StackID));
}
...
/**
    ...
    EMF+ manual 2.3.7.5, Microsoft name: EmfPlusSave Record, Index 0x25
*/
int U_PMR_SAVE_get(const char *contents, U_PMF_CMN_HDR *Header,
                   uint32_t *StackID) {
    return (U_PMR_common_stack_get(contents, Header, StackID));
}
...
/**
    ...
    \param  ctype       Set: int16_t coordinates; Clear: U_FLOAT coordinates
    \param  Elements    Number of members in Data.
    \param  Rects       Caller must free.  Array of rectangles to draw.
   Coordinate type set by ctype.

    EMF+ manual 2.3.8.1, Microsoft name: EmfPlusSetTSClip Record, Index 0x3A
*/
int U_PMR_SETTSCLIP_get(const char *contents, U_PMF_CMN_HDR *Header, int *ctype,
                        uint32_t *Elements, U_PMF_RECTF **Rects) {
    if (!contents || !ctype || !Elements || !Rects) {
        return (0);
    }
    ...
    U_PMF_CMN_HDR lclHeader;
    U_PMF_CMN_HDR_get(&contents, &lclHeader);
    if (lclHeader.Size < sizeof(U_PMF_SETTSCLIP))
        return (0);
    if (Header) {
        memcpy(Header, &lclHeader, sizeof(U_PMF_CMN_HDR));
    }
...
/**
    ...
    \param  contents             Record from which to extract data
    \param  Header               Common header
    \param  vgatype              Set: Palette is VGA basic colors; Clear:
   Palette is ???
    \param  pptype               Set: Palette is present; Clear: Palette is
   absent.
...
int U_PMR_SETTSGRAPHICS_get(const char *contents, U_PMF_CMN_HDR *Header,
                            int *vgatype, int *pptype, uint8_t *AntiAliasMode,
                            uint8_t *TextRenderHint, uint8_t *CompositingMode,
                            uint8_t *CompositingQuality, int16_t *RenderOriginX,
                            int16_t *RenderOriginY, uint16_t *TextContrast,
                            uint8_t *FilterType, uint8_t *PixelOffset,
                            U_PMF_TRANSFORMMATRIX *WorldToDevice,
                            const char **Data) {
    if (!contents || !vgatype || !pptype || !AntiAliasMode || !TextRenderHint ||
        !CompositingMode || !CompositingQuality || !RenderOriginX ||
        !RenderOriginY || !TextContrast || !FilterType || !PixelOffset ||
        !WorldToDevice || !Data) {
        return (0);
    }
    ...
    U_PMF_CMN_HDR lclHeader;
    U_PMF_CMN_HDR_get(&contents, &lclHeader);
    if (lclHeader.Size < sizeof(U_PMF_SETTSGRAPHICS))
        return (0);
    if (Header) {
        memcpy(Header, &lclHeader, sizeof(U_PMF_CMN_HDR));
    }
...
int U_PMR_MULTIPLYWORLDTRANSFORM_get(const char *contents,
                                     U_PMF_CMN_HDR *Header, int *xmtype,
                                     U_PMF_TRANSFORMMATRIX *Matrix) {
    if (!contents || !xmtype || !Matrix) {
        return (0);
    }

    U_PMF_CMN_HDR lclHeader;
    U_PMF_CMN_HDR_get(&contents, &lclHeader);
    if (lclHeader.Size < sizeof(U_PMF_MULTIPLYWORLDTRANSFORM))
        return (0);
    if (Header) {
        memcpy(Header, &lclHeader, sizeof(U_PMF_CMN_HDR));
    }
...
int U_PMR_ROTATEWORLDTRANSFORM_get(const char *contents, U_PMF_CMN_HDR *Header,
                                   int *xmtype, U_FLOAT *Angle) {
    if (!contents || !xmtype || !Angle) {
        return (0);
    }

    U_PMF_CMN_HDR lclHeader;
    U_PMF_CMN_HDR_get(&contents, &lclHeader);
    if (lclHeader.Size < sizeof(U_PMF_ROTATEWORLDTRANSFORM))
        return (0);
    if (Header) {
        memcpy(Header, &lclHeader, sizeof(U_PMF_CMN_HDR));
    }
...
int U_PMR_SCALEWORLDTRANSFORM_get(const char *contents, U_PMF_CMN_HDR *Header,
                                  int *xmtype, U_FLOAT *Sx, U_FLOAT *Sy) {
    if (!contents || !xmtype || !Sx || !Sy) {
        return (0);
    }

    U_PMF_CMN_HDR lclHeader;
    U_PMF_CMN_HDR_get(&contents, &lclHeader);
    if (lclHeader.Size < sizeof(U_PMF_SCALEWORLDTRANSFORM))
        return (0);
    if (Header) {
        memcpy(Header, &lclHeader, sizeof(U_PMF_CMN_HDR));
    }
...
int U_PMR_SETPAGETRANSFORM_get(const char *contents, U_PMF_CMN_HDR *Header,
                               int *PUenum, U_FLOAT *Scale) {
    if (!contents || !PUenum || !Scale) {
        return (0);
    }

    U_PMF_CMN_HDR lclHeader;
    U_PMF_CMN_HDR_get(&contents, &lclHeader);
    if (lclHeader.Size < sizeof(U_PMF_SETPAGETRANSFORM))
        return (0);
    if (Header) {
        memcpy(Header, &lclHeader, sizeof(U_PMF_CMN_HDR));
    }
...
int U_PMR_SETWORLDTRANSFORM_get(const char *contents, U_PMF_CMN_HDR *Header,
    ...
    U_PMF_CMN_HDR lclHeader;
    U_PMF_CMN_HDR_get(&contents, &lclHeader);
    if (lclHeader.Size < sizeof(U_PMF_SETWORLDTRANSFORM))
        return (0);
    if (Header) {
        memcpy(Header, &lclHeader, sizeof(U_PMF_CMN_HDR));
    }
...
int U_PMR_TRANSLATEWORLDTRANSFORM_get(const char *contents,
                                      U_PMF_CMN_HDR *Header, int *xmtype,
                                      U_FLOAT *Dx, U_FLOAT *Dy) {
    if (!contents || !xmtype || !Dx || !Dy) {
        return (0);
    }

    U_PMF_CMN_HDR lclHeader;
    U_PMF_CMN_HDR_get(&contents, &lclHeader);
    if (lclHeader.Size < sizeof(U_PMF_TRANSLATEWORLDTRANSFORM))
        return (0);
    if (Header) {
        memcpy(Header, &lclHeader, sizeof(U_PMF_CMN_HDR));
    }
