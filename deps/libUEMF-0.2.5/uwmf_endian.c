/**
  ...
  @brief Functions for converting WMF records between Big Endian and Little
  Endian byte orders.
*/
...
*/
void brush_swap(char *b, int torev) {
    int Style;
    if (torev) {
        Style = *(uint16_t *)(b + offsetof(U_BRUSH, Style));
    }
...
*/
void font_swap(char *f) {
    U_swap2(f + offsetof(U_FONT, Height),
            5); /*Height, Width, Escapement, Orientation, Weight */
...
*/
void region_swap(char *reg, int torev) {
    int Size;
    if (torev) {
        Size = *(int16_t *)(reg + offsetof(U_REGION, Size));
    }
...
*/
void bitmapcoreheader_swap(char *ch) {
    U_swap4(ch + offsetof(U_BITMAPCOREHEADER, Size_4),
            1); /* Size_4, may not be aligned  */
    U_swap2(ch + offsetof(U_BITMAPCOREHEADER, Width),
            4); /* Width, Height, Planes, BitCount, */
}
...
*/
void polypolygon_swap(char *pp, int torev) {
    int i, totPoints;
    uint16_t nPolys;
    uint16_t *aPolyCounts;
    if (torev) {
        nPolys = *(uint16_t *)(pp + offsetof(U_POLYPOLYGON, nPolys));
    }
    ...
    if (torev) {
        for (totPoints = 0, i = 0; i < nPolys; i++) {
            totPoints += aPolyCounts[i];
        }
    }
...
*/
void scan_swap(char *sc, int torev) {
    int count;
    if (torev) {
        count = *(uint16_t *)sc;
    }
...
*/
void dibheader_swap(char *dh, int torev) {
    int Size;
...
*/
int wmfheader_swap(char *contents, int torev) {
    ...
    if (torev) {
        Size16w = *(uint16_t *)(contents + offsetof(U_WMRHEADER, Size16w));
    }
...
/* **********************************************************************************************
These functions contain shared code used by various U_WMR*_Swap functions.
...
void U_WMRCORE_U16_N16_swap(char *record, int torev) {
    ...
    U_WMRCORE_SIZE16_swap(record, torev);
    if (torev) {
        nPoints = *(uint16_t *)(record + offsetof(U_WMRPOLYGON, nPoints));
    }
...
void U_WMRCORE_U16_CR_2U16_swap(char *record, int torev) {
    int off = U_SIZE_METARECORD;
    U_WMRCORE_SIZE16_swap(record, torev);
    U_swap2(record + off, 1);
    off += 2 + sizeof(U_COLORREF);
...
/**
    \brief Swap a pointer to a U_WMR_whatever record which has not been
   implemented.
...
void U_WMRTEXTOUT_swap(char *record, int torev) {
    int L2;
    if (torev) {
        L2 = *(int16_t *)(record + U_SIZE_METARECORD);
    }                                     /* Length field */
...
void U_WMRESCAPE_swap(char *record, int torev) {
    ...
    if (torev) {
        eFunc = *(uint16_t *)(record + offsetof(U_WMRESCAPE, eFunc));
    }
    U_WMRCORE_N16_swap(record, 2, torev);
    if (!torev) {
        eFunc = *(uint16_t *)(record + offsetof(U_WMRESCAPE, eFunc));
    }
    /* Handle data swapping for three types only, anything else end user code
     * must handle */
    if ((eFunc == U_MFE_SETLINECAP) || (eFunc == U_MFE_SETLINEJOIN) ||
        (eFunc == U_MFE_SETMITERLIMIT)) {
...
void U_WMRDIBCREATEPATTERNBRUSH_swap(char *record, int torev) {
    ...
    if (torev) {
        Style =
            *(uint16_t *)(record + offsetof(U_WMRDIBCREATEPATTERNBRUSH, Style));
    }
    ...
    if (Style == U_BS_PATTERN) {
        ...
        dibheader_swap(record + offsetof(U_WMRDIBCREATEPATTERNBRUSH, Src),
                       torev);
    }
}
...
void U_WMRCREATEBITMAPINDIRECT_swap(char *record,
                                    int torev) { /* in Wine, not in WMF PDF */
    U_WMRNOTIMPLEMENTED_swap(record, torev);
}

void U_WMRCREATEBITMAP_swap(char *record,
                            int torev) { /* in Wine, not in WMF PDF */
    U_WMRNOTIMPLEMENTED_swap(record, torev);
}
...
/**
    \brief Convert an entire WMF in memory from Big Endian to Little Endian (or
   vice versa).
    ...
    \param onerec     1 if this is operating on a single record instead of an
   entire file
...
int U_wmf_endian(char *contents, size_t length, int torev, int onerec) {
    ...
    record = contents;
    if (!onerec) {
        off = wmfheader_swap(record, torev);
        fflush(stdout); /* WMF header is not a normal record, handle it
                           separately */
        record += off;
        offset = off;
    }
    ...
    while (OK) {
        if (record > contents + length) {
            return (0);
        } // this is most likely a corrupt WMF

        memcpy(&Size16, record + offsetof(U_METARECORD, Size16_4),
               4); /* This may not be aligned */
        if (!torev) {
            U_swap4(&Size16, 1);
        }
        iType = *(uint8_t *)(record + offsetof(U_METARECORD, iType));

        // printf("DEBUG U_wmf_endian before switch record:%d offset:%d type:%d
        // name:%s
        // Size16:%d\n",recnum,offset,iType,U_wmr_names(iType),Size16);fflush(stdout);
        switch (iType) {
        case U_WMR_EOF:
            U_WMREOF_swap(record, torev);
            OK = 0;
            break;
        case U_WMR_SETBKCOLOR:
            U_WMRSETBKCOLOR_swap(record, torev);
            break;
        case U_WMR_SETBKMODE:
            U_WMRSETBKMODE_swap(record, torev);
            break;
        case U_WMR_SETMAPMODE:
            U_WMRSETMAPMODE_swap(record, torev);
            break;
        case U_WMR_SETROP2:
            U_WMRSETROP2_swap(record, torev);
            break;
        case U_WMR_SETRELABS:
            U_WMRSETRELABS_swap(record, torev);
            break;
        case U_WMR_SETPOLYFILLMODE:
            U_WMRSETPOLYFILLMODE_swap(record, torev);
            break;
        case U_WMR_SETSTRETCHBLTMODE:
            U_WMRSETSTRETCHBLTMODE_swap(record, torev);
            break;
        case U_WMR_SETTEXTCHAREXTRA:
            U_WMRSETTEXTCHAREXTRA_swap(record, torev);
            break;
        case U_WMR_SETTEXTCOLOR:
            U_WMRSETTEXTCOLOR_swap(record, torev);
            break;
        case U_WMR_SETTEXTJUSTIFICATION:
            U_WMRSETTEXTJUSTIFICATION_swap(record, torev);
            break;
        case U_WMR_SETWINDOWORG:
            U_WMRSETWINDOWORG_swap(record, torev);
            break;
        case U_WMR_SETWINDOWEXT:
            U_WMRSETWINDOWEXT_swap(record, torev);
            break;
        case U_WMR_SETVIEWPORTORG:
            U_WMRSETVIEWPORTORG_swap(record, torev);
            break;
        case U_WMR_SETVIEWPORTEXT:
            U_WMRSETVIEWPORTEXT_swap(record, torev);
            break;
        case U_WMR_OFFSETWINDOWORG:
            U_WMROFFSETWINDOWORG_swap(record, torev);
            break;
        case U_WMR_SCALEWINDOWEXT:
            U_WMRSCALEWINDOWEXT_swap(record, torev);
            break;
        case U_WMR_OFFSETVIEWPORTORG:
            U_WMROFFSETVIEWPORTORG_swap(record, torev);
            break;
        case U_WMR_SCALEVIEWPORTEXT:
            U_WMRSCALEVIEWPORTEXT_swap(record, torev);
            break;
        case U_WMR_LINETO:
            U_WMRLINETO_swap(record, torev);
            break;
        case U_WMR_MOVETO:
            U_WMRMOVETO_swap(record, torev);
            break;
        case U_WMR_EXCLUDECLIPRECT:
            U_WMREXCLUDECLIPRECT_swap(record, torev);
            break;
        case U_WMR_INTERSECTCLIPRECT:
            U_WMRINTERSECTCLIPRECT_swap(record, torev);
            break;
        case U_WMR_ARC:
            U_WMRARC_swap(record, torev);
            break;
        case U_WMR_ELLIPSE:
            U_WMRELLIPSE_swap(record, torev);
            break;
        case U_WMR_FLOODFILL:
            U_WMRFLOODFILL_swap(record, torev);
            break;
        case U_WMR_PIE:
            U_WMRPIE_swap(record, torev);
            break;
        case U_WMR_RECTANGLE:
            U_WMRRECTANGLE_swap(record, torev);
            break;
        case U_WMR_ROUNDRECT:
            U_WMRROUNDRECT_swap(record, torev);
            break;
        case U_WMR_PATBLT:
            U_WMRPATBLT_swap(record, torev);
            break;
        case U_WMR_SAVEDC:
            U_WMRSAVEDC_swap(record, torev);
            break;
        case U_WMR_SETPIXEL:
            U_WMRSETPIXEL_swap(record, torev);
            break;
        case U_WMR_OFFSETCLIPRGN:
            U_WMROFFSETCLIPRGN_swap(record, torev);
            break;
        case U_WMR_TEXTOUT:
            U_WMRTEXTOUT_swap(record, torev);
            break;
        case U_WMR_BITBLT:
            U_WMRBITBLT_swap(record, torev);
            break;
        case U_WMR_STRETCHBLT:
            U_WMRSTRETCHBLT_swap(record, torev);
            break;
        case U_WMR_POLYGON:
            U_WMRPOLYGON_swap(record, torev);
            break;
        case U_WMR_POLYLINE:
            U_WMRPOLYLINE_swap(record, torev);
            break;
        case U_WMR_ESCAPE:
            U_WMRESCAPE_swap(record, torev);
            break;
        case U_WMR_RESTOREDC:
            U_WMRRESTOREDC_swap(record, torev);
            break;
        case U_WMR_FILLREGION:
            U_WMRFILLREGION_swap(record, torev);
            break;
        case U_WMR_FRAMEREGION:
            U_WMRFRAMEREGION_swap(record, torev);
            break;
        case U_WMR_INVERTREGION:
            U_WMRINVERTREGION_swap(record, torev);
            break;
        case U_WMR_PAINTREGION:
            U_WMRPAINTREGION_swap(record, torev);
            break;
        case U_WMR_SELECTCLIPREGION:
            U_WMRSELECTCLIPREGION_swap(record, torev);
            break;
        case U_WMR_SELECTOBJECT:
            U_WMRSELECTOBJECT_swap(record, torev);
            break;
        case U_WMR_SETTEXTALIGN:
            U_WMRSETTEXTALIGN_swap(record, torev);
            break;
        case U_WMR_DRAWTEXT:
            U_WMRDRAWTEXT_swap(record, torev);
            break;
        case U_WMR_CHORD:
            U_WMRCHORD_swap(record, torev);
            break;
        case U_WMR_SETMAPPERFLAGS:
            U_WMRSETMAPPERFLAGS_swap(record, torev);
            break;
        case U_WMR_EXTTEXTOUT:
            U_WMREXTTEXTOUT_swap(record, torev);
            break;
        case U_WMR_SETDIBTODEV:
            U_WMRSETDIBTODEV_swap(record, torev);
            break;
        case U_WMR_SELECTPALETTE:
            U_WMRSELECTPALETTE_swap(record, torev);
            break;
        case U_WMR_REALIZEPALETTE:
            U_WMRREALIZEPALETTE_swap(record, torev);
            break;
        case U_WMR_ANIMATEPALETTE:
            U_WMRANIMATEPALETTE_swap(record, torev);
            break;
        case U_WMR_SETPALENTRIES:
            U_WMRSETPALENTRIES_swap(record, torev);
            break;
        case U_WMR_POLYPOLYGON:
            U_WMRPOLYPOLYGON_swap(record, torev);
            break;
        case U_WMR_RESIZEPALETTE:
            U_WMRRESIZEPALETTE_swap(record, torev);
            break;
        case U_WMR_3A:
            U_WMR3A_swap(record, torev);
            break;
        case U_WMR_3B:
            U_WMR3B_swap(record, torev);
            break;
        case U_WMR_3C:
            U_WMR3C_swap(record, torev);
            break;
        case U_WMR_3D:
            U_WMR3D_swap(record, torev);
            break;
        case U_WMR_3E:
            U_WMR3E_swap(record, torev);
            break;
        case U_WMR_3F:
            U_WMR3F_swap(record, torev);
            break;
        case U_WMR_DIBBITBLT:
            U_WMRDIBBITBLT_swap(record, torev);
            break;
        case U_WMR_DIBSTRETCHBLT:
            U_WMRDIBSTRETCHBLT_swap(record, torev);
            break;
        case U_WMR_DIBCREATEPATTERNBRUSH:
            U_WMRDIBCREATEPATTERNBRUSH_swap(record, torev);
            break;
        case U_WMR_STRETCHDIB:
            U_WMRSTRETCHDIB_swap(record, torev);
            break;
        case U_WMR_44:
            U_WMR44_swap(record, torev);
            break;
        case U_WMR_45:
            U_WMR45_swap(record, torev);
            break;
        case U_WMR_46:
            U_WMR46_swap(record, torev);
            break;
        case U_WMR_47:
            U_WMR47_swap(record, torev);
            break;
        case U_WMR_EXTFLOODFILL:
            U_WMREXTFLOODFILL_swap(record, torev);
            break;
        case U_WMR_49:
            U_WMR49_swap(record, torev);
            break;
        case U_WMR_4A:
            U_WMR4A_swap(record, torev);
            break;
        case U_WMR_4B:
            U_WMR4B_swap(record, torev);
            break;
        case U_WMR_4C:
            U_WMR4C_swap(record, torev);
            break;
        case U_WMR_4D:
            U_WMR4D_swap(record, torev);
            break;
        case U_WMR_4E:
            U_WMR4E_swap(record, torev);
            break;
        case U_WMR_4F:
            U_WMR4F_swap(record, torev);
            break;
        case U_WMR_50:
            U_WMR50_swap(record, torev);
            break;
        case U_WMR_51:
            U_WMR51_swap(record, torev);
            break;
        case U_WMR_52:
            U_WMR52_swap(record, torev);
            break;
        case U_WMR_53:
            U_WMR53_swap(record, torev);
            break;
        case U_WMR_54:
            U_WMR54_swap(record, torev);
            break;
        case U_WMR_55:
            U_WMR55_swap(record, torev);
            break;
        case U_WMR_56:
            U_WMR56_swap(record, torev);
            break;
        case U_WMR_57:
            U_WMR57_swap(record, torev);
            break;
        case U_WMR_58:
            U_WMR58_swap(record, torev);
            break;
        case U_WMR_59:
            U_WMR59_swap(record, torev);
            break;
        case U_WMR_5A:
            U_WMR5A_swap(record, torev);
            break;
        case U_WMR_5B:
            U_WMR5B_swap(record, torev);
            break;
        case U_WMR_5C:
            U_WMR5C_swap(record, torev);
            break;
        case U_WMR_5D:
            U_WMR5D_swap(record, torev);
            break;
        case U_WMR_5E:
            U_WMR5E_swap(record, torev);
            break;
        case U_WMR_5F:
            U_WMR5F_swap(record, torev);
            break;
        case U_WMR_60:
            U_WMR60_swap(record, torev);
            break;
        case U_WMR_61:
            U_WMR61_swap(record, torev);
            break;
        case U_WMR_62:
            U_WMR62_swap(record, torev);
            break;
        case U_WMR_63:
            U_WMR63_swap(record, torev);
            break;
        case U_WMR_64:
            U_WMR64_swap(record, torev);
            break;
        case U_WMR_65:
            U_WMR65_swap(record, torev);
            break;
        case U_WMR_66:
            U_WMR66_swap(record, torev);
            break;
        case U_WMR_67:
            U_WMR67_swap(record, torev);
            break;
        case U_WMR_68:
            U_WMR68_swap(record, torev);
            break;
        case U_WMR_69:
            U_WMR69_swap(record, torev);
            break;
        case U_WMR_6A:
            U_WMR6A_swap(record, torev);
            break;
        case U_WMR_6B:
            U_WMR6B_swap(record, torev);
            break;
        case U_WMR_6C:
            U_WMR6C_swap(record, torev);
            break;
        case U_WMR_6D:
            U_WMR6D_swap(record, torev);
            break;
        case U_WMR_6E:
            U_WMR6E_swap(record, torev);
            break;
        case U_WMR_6F:
            U_WMR6F_swap(record, torev);
            break;
        case U_WMR_70:
            U_WMR70_swap(record, torev);
            break;
        case U_WMR_71:
            U_WMR71_swap(record, torev);
            break;
        case U_WMR_72:
            U_WMR72_swap(record, torev);
            break;
        case U_WMR_73:
            U_WMR73_swap(record, torev);
            break;
        case U_WMR_74:
            U_WMR74_swap(record, torev);
            break;
        case U_WMR_75:
            U_WMR75_swap(record, torev);
            break;
        case U_WMR_76:
            U_WMR76_swap(record, torev);
            break;
        case U_WMR_77:
            U_WMR77_swap(record, torev);
            break;
        case U_WMR_78:
            U_WMR78_swap(record, torev);
            break;
        case U_WMR_79:
            U_WMR79_swap(record, torev);
            break;
        case U_WMR_7A:
            U_WMR7A_swap(record, torev);
            break;
        case U_WMR_7B:
            U_WMR7B_swap(record, torev);
            break;
        case U_WMR_7C:
            U_WMR7C_swap(record, torev);
            break;
        case U_WMR_7D:
            U_WMR7D_swap(record, torev);
            break;
        case U_WMR_7E:
            U_WMR7E_swap(record, torev);
            break;
        case U_WMR_7F:
            U_WMR7F_swap(record, torev);
            break;
        case U_WMR_80:
            U_WMR80_swap(record, torev);
            break;
        case U_WMR_81:
            U_WMR81_swap(record, torev);
            break;
        case U_WMR_82:
            U_WMR82_swap(record, torev);
            break;
        case U_WMR_83:
            U_WMR83_swap(record, torev);
            break;
        case U_WMR_84:
            U_WMR84_swap(record, torev);
            break;
        case U_WMR_85:
            U_WMR85_swap(record, torev);
            break;
        case U_WMR_86:
            U_WMR86_swap(record, torev);
            break;
        case U_WMR_87:
            U_WMR87_swap(record, torev);
            break;
        case U_WMR_88:
            U_WMR88_swap(record, torev);
            break;
        case U_WMR_89:
            U_WMR89_swap(record, torev);
            break;
        case U_WMR_8A:
            U_WMR8A_swap(record, torev);
            break;
        case U_WMR_8B:
            U_WMR8B_swap(record, torev);
            break;
        case U_WMR_8C:
            U_WMR8C_swap(record, torev);
            break;
        case U_WMR_8D:
            U_WMR8D_swap(record, torev);
            break;
        case U_WMR_8E:
            U_WMR8E_swap(record, torev);
            break;
        case U_WMR_8F:
            U_WMR8F_swap(record, torev);
            break;
        case U_WMR_90:
            U_WMR90_swap(record, torev);
            break;
        case U_WMR_91:
            U_WMR91_swap(record, torev);
            break;
        case U_WMR_92:
            U_WMR92_swap(record, torev);
            break;
        case U_WMR_93:
            U_WMR93_swap(record, torev);
            break;
        case U_WMR_94:
            U_WMR94_swap(record, torev);
            break;
        case U_WMR_95:
            U_WMR95_swap(record, torev);
            break;
        case U_WMR_96:
            U_WMR96_swap(record, torev);
            break;
        case U_WMR_97:
            U_WMR97_swap(record, torev);
            break;
        case U_WMR_98:
            U_WMR98_swap(record, torev);
            break;
        case U_WMR_99:
            U_WMR99_swap(record, torev);
            break;
        case U_WMR_9A:
            U_WMR9A_swap(record, torev);
            break;
        case U_WMR_9B:
            U_WMR9B_swap(record, torev);
            break;
        case U_WMR_9C:
            U_WMR9C_swap(record, torev);
            break;
        case U_WMR_9D:
            U_WMR9D_swap(record, torev);
            break;
        case U_WMR_9E:
            U_WMR9E_swap(record, torev);
            break;
        case U_WMR_9F:
            U_WMR9F_swap(record, torev);
            break;
        case U_WMR_A0:
            U_WMRA0_swap(record, torev);
            break;
        case U_WMR_A1:
            U_WMRA1_swap(record, torev);
            break;
        case U_WMR_A2:
            U_WMRA2_swap(record, torev);
            break;
        case U_WMR_A3:
            U_WMRA3_swap(record, torev);
            break;
        case U_WMR_A4:
            U_WMRA4_swap(record, torev);
            break;
        case U_WMR_A5:
            U_WMRA5_swap(record, torev);
            break;
        case U_WMR_A6:
            U_WMRA6_swap(record, torev);
            break;
        case U_WMR_A7:
            U_WMRA7_swap(record, torev);
            break;
        case U_WMR_A8:
            U_WMRA8_swap(record, torev);
            break;
        case U_WMR_A9:
            U_WMRA9_swap(record, torev);
            break;
        case U_WMR_AA:
            U_WMRAA_swap(record, torev);
            break;
        case U_WMR_AB:
            U_WMRAB_swap(record, torev);
            break;
        case U_WMR_AC:
            U_WMRAC_swap(record, torev);
            break;
        case U_WMR_AD:
            U_WMRAD_swap(record, torev);
            break;
        case U_WMR_AE:
            U_WMRAE_swap(record, torev);
            break;
        case U_WMR_AF:
            U_WMRAF_swap(record, torev);
            break;
        case U_WMR_B0:
            U_WMRB0_swap(record, torev);
            break;
        case U_WMR_B1:
            U_WMRB1_swap(record, torev);
            break;
        case U_WMR_B2:
            U_WMRB2_swap(record, torev);
            break;
        case U_WMR_B3:
            U_WMRB3_swap(record, torev);
            break;
        case U_WMR_B4:
            U_WMRB4_swap(record, torev);
            break;
        case U_WMR_B5:
            U_WMRB5_swap(record, torev);
            break;
        case U_WMR_B6:
            U_WMRB6_swap(record, torev);
            break;
        case U_WMR_B7:
            U_WMRB7_swap(record, torev);
            break;
        case U_WMR_B8:
            U_WMRB8_swap(record, torev);
            break;
        case U_WMR_B9:
            U_WMRB9_swap(record, torev);
            break;
        case U_WMR_BA:
            U_WMRBA_swap(record, torev);
            break;
        case U_WMR_BB:
            U_WMRBB_swap(record, torev);
            break;
        case U_WMR_BC:
            U_WMRBC_swap(record, torev);
            break;
        case U_WMR_BD:
            U_WMRBD_swap(record, torev);
            break;
        case U_WMR_BE:
            U_WMRBE_swap(record, torev);
            break;
        case U_WMR_BF:
            U_WMRBF_swap(record, torev);
            break;
        case U_WMR_C0:
            U_WMRC0_swap(record, torev);
            break;
        case U_WMR_C1:
            U_WMRC1_swap(record, torev);
            break;
        case U_WMR_C2:
            U_WMRC2_swap(record, torev);
            break;
        case U_WMR_C3:
            U_WMRC3_swap(record, torev);
            break;
        case U_WMR_C4:
            U_WMRC4_swap(record, torev);
            break;
        case U_WMR_C5:
            U_WMRC5_swap(record, torev);
            break;
        case U_WMR_C6:
            U_WMRC6_swap(record, torev);
            break;
        case U_WMR_C7:
            U_WMRC7_swap(record, torev);
            break;
        case U_WMR_C8:
            U_WMRC8_swap(record, torev);
            break;
        case U_WMR_C9:
            U_WMRC9_swap(record, torev);
            break;
        case U_WMR_CA:
            U_WMRCA_swap(record, torev);
            break;
        case U_WMR_CB:
            U_WMRCB_swap(record, torev);
            break;
        case U_WMR_CC:
            U_WMRCC_swap(record, torev);
            break;
        case U_WMR_CD:
            U_WMRCD_swap(record, torev);
            break;
        case U_WMR_CE:
            U_WMRCE_swap(record, torev);
            break;
        case U_WMR_CF:
            U_WMRCF_swap(record, torev);
            break;
        case U_WMR_D0:
            U_WMRD0_swap(record, torev);
            break;
        case U_WMR_D1:
            U_WMRD1_swap(record, torev);
            break;
        case U_WMR_D2:
            U_WMRD2_swap(record, torev);
            break;
        case U_WMR_D3:
            U_WMRD3_swap(record, torev);
            break;
        case U_WMR_D4:
            U_WMRD4_swap(record, torev);
            break;
        case U_WMR_D5:
            U_WMRD5_swap(record, torev);
            break;
        case U_WMR_D6:
            U_WMRD6_swap(record, torev);
            break;
        case U_WMR_D7:
            U_WMRD7_swap(record, torev);
            break;
        case U_WMR_D8:
            U_WMRD8_swap(record, torev);
            break;
        case U_WMR_D9:
            U_WMRD9_swap(record, torev);
            break;
        case U_WMR_DA:
            U_WMRDA_swap(record, torev);
            break;
        case U_WMR_DB:
            U_WMRDB_swap(record, torev);
            break;
        case U_WMR_DC:
            U_WMRDC_swap(record, torev);
            break;
        case U_WMR_DD:
            U_WMRDD_swap(record, torev);
            break;
        case U_WMR_DE:
            U_WMRDE_swap(record, torev);
            break;
        case U_WMR_DF:
            U_WMRDF_swap(record, torev);
            break;
        case U_WMR_E0:
            U_WMRE0_swap(record, torev);
            break;
        case U_WMR_E1:
            U_WMRE1_swap(record, torev);
            break;
        case U_WMR_E2:
            U_WMRE2_swap(record, torev);
            break;
        case U_WMR_E3:
            U_WMRE3_swap(record, torev);
            break;
        case U_WMR_E4:
            U_WMRE4_swap(record, torev);
            break;
        case U_WMR_E5:
            U_WMRE5_swap(record, torev);
            break;
        case U_WMR_E6:
            U_WMRE6_swap(record, torev);
            break;
        case U_WMR_E7:
            U_WMRE7_swap(record, torev);
            break;
        case U_WMR_E8:
            U_WMRE8_swap(record, torev);
            break;
        case U_WMR_E9:
            U_WMRE9_swap(record, torev);
            break;
        case U_WMR_EA:
            U_WMREA_swap(record, torev);
            break;
        case U_WMR_EB:
            U_WMREB_swap(record, torev);
            break;
        case U_WMR_EC:
            U_WMREC_swap(record, torev);
            break;
        case U_WMR_ED:
            U_WMRED_swap(record, torev);
            break;
        case U_WMR_EE:
            U_WMREE_swap(record, torev);
            break;
        case U_WMR_EF:
            U_WMREF_swap(record, torev);
            break;
        case U_WMR_DELETEOBJECT:
            U_WMRDELETEOBJECT_swap(record, torev);
            break;
        case U_WMR_F1:
            U_WMRF1_swap(record, torev);
            break;
        case U_WMR_F2:
            U_WMRF2_swap(record, torev);
            break;
        case U_WMR_F3:
            U_WMRF3_swap(record, torev);
            break;
        case U_WMR_F4:
            U_WMRF4_swap(record, torev);
            break;
        case U_WMR_F5:
            U_WMRF5_swap(record, torev);
            break;
        case U_WMR_F6:
            U_WMRF6_swap(record, torev);
            break;
        case U_WMR_CREATEPALETTE:
            U_WMRCREATEPALETTE_swap(record, torev);
            break;
        case U_WMR_F8:
            U_WMRF8_swap(record, torev);
            break;
        case U_WMR_CREATEPATTERNBRUSH:
            U_WMRCREATEPATTERNBRUSH_swap(record, torev);
            break;
        case U_WMR_CREATEPENINDIRECT:
            U_WMRCREATEPENINDIRECT_swap(record, torev);
            break;
        case U_WMR_CREATEFONTINDIRECT:
            U_WMRCREATEFONTINDIRECT_swap(record, torev);
            break;
        case U_WMR_CREATEBRUSHINDIRECT:
            U_WMRCREATEBRUSHINDIRECT_swap(record, torev);
            break;
        case U_WMR_CREATEBITMAPINDIRECT:
            U_WMRCREATEBITMAPINDIRECT_swap(record, torev);
            break;
        case U_WMR_CREATEBITMAP:
            U_WMRCREATEBITMAP_swap(record, torev);
            break;
        case U_WMR_CREATEREGION:
            U_WMRCREATEREGION_swap(record, torev);
            break;
        default:
            U_WMRNOTIMPLEMENTED_swap(record, torev);
            break;
