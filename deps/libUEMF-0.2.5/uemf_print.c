...
void rgbquad_print(U_RGBQUAD color) {
    printf("{%u,%u,%u,%u} ", color.Blue, color.Green, color.Red,
           color.Reserved);
}
...
void lcs_gamma_print(U_LCS_GAMMA lg) {
    uint8_t tmp;
    tmp = lg.ignoreHi;
    printf("ignoreHi:%u ", tmp);
    tmp = lg.intPart;
    printf("intPart :%u ", tmp);
    tmp = lg.fracPart;
    printf("fracPart:%u ", tmp);
    tmp = lg.ignoreLo;
    printf("ignoreLo:%u ", tmp);
}
...
void lcs_gammargb_print(U_LCS_GAMMARGB lgr) {
    printf("lcsGammaRed:");
    lcs_gamma_print(lgr.lcsGammaRed);
    printf("lcsGammaGreen:");
    lcs_gamma_print(lgr.lcsGammaGreen);
    printf("lcsGammaBlue:");
    lcs_gamma_print(lgr.lcsGammaBlue);
}
...
void trivertex_print(U_TRIVERTEX tv) {
    printf("{{%d,%d},{%u,%u,%u,%u}} ", tv.x, tv.y, tv.Red, tv.Green, tv.Blue,
           tv.Alpha);
}
...
void xform_print(U_XFORM xform) {
    printf("{%f,%f.%f,%f,%f,%f} ", xform.eM11, xform.eM12, xform.eM21,
...
void ciexyztriple_print(U_CIEXYZTRIPLE cie3) {
    printf("{Red:");
    ciexyz_print(cie3.ciexyzRed);
    printf(", Green:");
    ciexyz_print(cie3.ciexyzGreen);
    printf(", Blue:");
    ciexyz_print(cie3.ciexyzBlue);
    printf("} ");
}
/**
...
void logcolorspacea_print(U_LOGCOLORSPACEA lcsa) {
    ...
    printf("lcsEndpoints:");
    ciexyztriple_print(lcsa.lcsEndpoints);
    printf("lcsGammaRGB: ");
    lcs_gammargb_print(lcsa.lcsGammaRGB);
...
/**
   ...
   occur
    if the data was passed as the struct or a pointer to the struct, as the
   struct may not
...
void logpen_print(U_LOGPEN lp) {
    ...
    printf("lopnWidth:");
    pointl_print(lp.lopnWidth);
    printf("lopnColor:");
    colorref_print(lp.lopnColor);
}
...
void logpalette_print(PU_LOGPALETTE lp) {
    ...
    if (lp->palNumEntries) {
        ...
        for (i = 0; i < lp->palNumEntries; i++) {
            printf("%d:", i);
            logpltntry_print(palPalEntry[i]);
        }
    }
}
...
*/
void rgndata_print(PU_RGNDATA rd, const char *blimit) {
    unsigned int i;
    PU_RECTL rects;
    IF_MEM_UNSAFE_PRINT_AND_RETURN(rd, sizeof(U_RGNDATAHEADER), blimit);
    printf("rdh: ");
    rgndataheader_print(rd->rdh);
    printf(" rects: ");
    if (rd->rdh.nCount) {
...
void emrtext_print(const char *emt, const char *record, const char *blimit,
    ...
    // constant part
    printf("ptlReference:");
    pointl_print(pemt->ptlReference);
    ...
    if (!(pemt->fOptions & U_ETO_NO_RECT)) {
        printf("rcl");
        rectl_print(*((U_RECTL *)(emt + off)));
        off += sizeof(U_RECTL);
    }
    printf("offDx:%u ", *((U_OFFDX *)(emt + off)));
    off = *(U_OFFDX *)(emt + off);
    printf("Dx:");
...
void core3_print(const char *name, const char *label, const char *contents) {
    ...
    if (!strcmp(label, "crColor:")) {
        printf("   %-15s ", label);
        colorref_print(*(U_COLORREF *)&(pEmr->iMode));
        printf("\n");
...
void core8_print(const char *name, const char *contents, int type) {
    ...
    printf("   eyScale:        %f\n", pEmr->eyScale);
    printf("   emrtext:        ");
    emrtext_print(contents + sizeof(U_EMREXTTEXTOUTA) - sizeof(U_EMRTEXT),
...
void core9_print(const char *name, const char *contents) {
    ...
    printf("\n");
    printf("   ptlStart:       ");
    pointl_print(pEmr->ptlStart);
    printf("\n");
    printf("   ptlEnd:         ");
    pointl_print(pEmr->ptlEnd);
    printf("\n");
}
...
void core10_print(const char *name, const char *contents) {
    ...
    PU_POINT16 papts = (PU_POINT16)((char *)pEmr->aPolyCounts +
                                    pEmr->nPolys * sizeof(U_POLYCOUNTS));
    IF_MEM_UNSAFE_PRINT_AND_RETURN(papts, pEmr->cpts * sizeof(U_POINT16),
                                   blimit);
    for (i = 0; i < pEmr->cpts; i++) {
        printf(" [%d]:", i);
...
void core13_print(const char *name, const char *contents) {
    ...
    printf("\n");
    printf("   Dest:           ");
    pointl_print(pEmr->Dest);
    printf("\n");
    printf("   cDest:          ");
    pointl_print(pEmr->cDest);
    printf("\n");
    printf("   Blend:          ");
    blend_print(pEmr->Blend);
    printf("\n");
    printf("   Src:            ");
    pointl_print(pEmr->Src);
    printf("\n");
    printf("   xformSrc:       ");
    xform_print(pEmr->xformSrc);
    printf("\n");
    printf("   crBkColorSrc:   ");
    colorref_print(pEmr->crBkColorSrc);
    printf("\n");
    printf("   iUsageSrc:      %u\n", pEmr->iUsageSrc);
    printf("   offBmiSrc:      %u\n", pEmr->offBmiSrc);
    printf("   cbBmiSrc:       %u\n", pEmr->cbBmiSrc);
...
void U_EMRSETPIXELV_print(const char *contents) {
    ...
    printf("   ptlPixel:       ");
    pointl_print(pEmr->ptlPixel);
    printf("\n");
    printf("   crColor:        ");
    colorref_print(pEmr->crColor);
    printf("\n");
}
...
void U_EMRANGLEARC_print(const char *contents) {
    ...
    printf("   ptlCenter:      "), pointl_print(pEmr->ptlCenter);
    printf("\n");
    printf("   nRadius:        %u\n", pEmr->nRadius);
...
void U_EMRROUNDRECT_print(const char *contents) {
    ...
    printf("   rclBox:         "), rectl_print(pEmr->rclBox);
    printf("\n");
    printf("   szlCorner:      "), sizel_print(pEmr->szlCorner);
    printf("\n");
}
...
void U_EMRCREATEPALETTE_print(const char *contents) {
    ...
    printf("   ihPal:          %u\n", pEmr->ihPal);
    printf("   lgpl:           "),
        logpalette_print((PU_LOGPALETTE) & (pEmr->lgpl));
    printf("\n");
}
...
void U_EMRSETPALETTEENTRIES_print(const char *contents) {
    ...
    if (pEmr->cEntries) {
        ...
        for (i = 0; i < pEmr->cEntries; i++) {
            printf("%d:", i);
            logpltntry_print(aPalEntries[i]);
        }
        printf("\n");
    }
}
...
/**
    ...
    \param off        offset in bytes to the first byte in this record (needed
   for EMF+ record printing)
    ...
    if (cbData >= 4) {
        ...
        if (cIdent == U_EMR_COMMENT_PUBLIC) {
            ...
            loff = 16; /* Header size of the header part of an EMF+ comment
                          record */
            while (loff < cbData + 12) { // EMF+ records may not fill the entire
                                         // comment, cbData value includes
                                         // cIdent, but not U_EMR or cbData
...
void U_EMRFRAMERGN_print(const char *contents) {
    ...
    printf("   cbRgnData:      %u\n", pEmr->cbRgnData);
    printf("   ihBrush:        %u\n", pEmr->ihBrush);
    printf("   szlStroke:      "), sizel_print(pEmr->szlStroke);
    printf("\n");
    const char *minptr =
        MAKE_MIN_PTR(((const char *)&pEmr->RgnData + pEmr->cbRgnData), blimit);
    printf("   RegionData: ");
...
void U_EMRPLGBLT_print(const char *contents) {
    ...
    printf("\n");
    printf("   aptlDst(UL):    ");
    pointl_print(pEmr->aptlDst[0]);
    printf("\n");
    printf("   aptlDst(UR):    ");
    pointl_print(pEmr->aptlDst[1]);
    printf("\n");
    printf("   aptlDst(LL):    ");
    pointl_print(pEmr->aptlDst[2]);
    printf("\n");
    printf("   Src:            ");
    pointl_print(pEmr->Src);
    printf("\n");
...
void U_EMREXTCREATEPEN_print(const char *contents) {
    ...
    printf("   cbBits:         %u\n", pEmr->cbBits);
    printf("   elp:            ");
    extlogpen_print((PU_EXTLOGPEN) & (pEmr->elp));
    printf("\n");
}
...
void U_EMRPIXELFORMAT_print(const char *contents) {
    ...
    printf("   Pfd:            ");
    pixelformatdescriptor_print(pEmr->pfd);
    printf("\n");
}
...
void U_EMRSMALLTEXTOUT_print(const char *contents) {
    ...
    if (!(pEmr->fuOptions & U_ETO_NO_RECT)) {
        ...
        printf("   rclBounds:      ");
        rectl_print(*(PU_RECTL)(contents + roff));
        printf("\n");
        roff += sizeof(U_RECTL);
    }
...
/**
    ...
    \returns record length for a normal record, 0 for EMREOF, -1 for a bad
   record
    \param contents   pointer to a buffer holding all EMR records
    \param blimit     pointer to the byte after the last byte in the buffer
   holding all EMR records
    \param recnum     number of this record in contents
    \param off        offset to this record in contents
*/
int U_emf_onerec_print(const char *contents, const char *blimit, int recnum,
                       size_t off) {
    ...
    */
    if (!U_emf_record_sizeok(record, blimit, &nSize, &iType, 1))
        return (-1);

    uint32_t crc;
#if U_BYTE_SWAP
    // This is a Big Endian machine, EMF crc values must be calculated on Little
    // Endian form
    ...
    /* print the record header before checking further.
       ...
       The _print routines will stop at the actual problem and print another
       corrupt message.
    */
    if (!U_emf_record_safe(record)) {
        printf(
    ...
    switch (lpEMFR->iType) {
    case U_EMR_HEADER:
        U_EMRHEADER_print(record);
        break;
    case U_EMR_POLYBEZIER:
        U_EMRPOLYBEZIER_print(record);
        break;
    case U_EMR_POLYGON:
        U_EMRPOLYGON_print(record);
        break;
    case U_EMR_POLYLINE:
        U_EMRPOLYLINE_print(record);
        break;
    case U_EMR_POLYBEZIERTO:
        U_EMRPOLYBEZIERTO_print(record);
        break;
    case U_EMR_POLYLINETO:
        U_EMRPOLYLINETO_print(record);
        break;
    case U_EMR_POLYPOLYLINE:
        U_EMRPOLYPOLYLINE_print(record);
        break;
    case U_EMR_POLYPOLYGON:
        U_EMRPOLYPOLYGON_print(record);
        break;
    case U_EMR_SETWINDOWEXTEX:
        U_EMRSETWINDOWEXTEX_print(record);
        break;
    case U_EMR_SETWINDOWORGEX:
        U_EMRSETWINDOWORGEX_print(record);
        break;
    case U_EMR_SETVIEWPORTEXTEX:
        U_EMRSETVIEWPORTEXTEX_print(record);
        break;
    case U_EMR_SETVIEWPORTORGEX:
        U_EMRSETVIEWPORTORGEX_print(record);
        break;
    case U_EMR_SETBRUSHORGEX:
        U_EMRSETBRUSHORGEX_print(record);
        break;
    case U_EMR_EOF:
        U_EMREOF_print(record);
        nSize = 0;
        break;
    case U_EMR_SETPIXELV:
        U_EMRSETPIXELV_print(record);
        break;
    case U_EMR_SETMAPPERFLAGS:
        U_EMRSETMAPPERFLAGS_print(record);
        break;
    case U_EMR_SETMAPMODE:
        U_EMRSETMAPMODE_print(record);
        break;
    case U_EMR_SETBKMODE:
        U_EMRSETBKMODE_print(record);
        break;
    case U_EMR_SETPOLYFILLMODE:
        U_EMRSETPOLYFILLMODE_print(record);
        break;
    case U_EMR_SETROP2:
        U_EMRSETROP2_print(record);
        break;
    case U_EMR_SETSTRETCHBLTMODE:
        U_EMRSETSTRETCHBLTMODE_print(record);
        break;
    case U_EMR_SETTEXTALIGN:
        U_EMRSETTEXTALIGN_print(record);
        break;
    case U_EMR_SETCOLORADJUSTMENT:
        U_EMRSETCOLORADJUSTMENT_print(record);
        break;
    case U_EMR_SETTEXTCOLOR:
        U_EMRSETTEXTCOLOR_print(record);
        break;
    case U_EMR_SETBKCOLOR:
        U_EMRSETBKCOLOR_print(record);
        break;
    case U_EMR_OFFSETCLIPRGN:
        U_EMROFFSETCLIPRGN_print(record);
        break;
    case U_EMR_MOVETOEX:
        U_EMRMOVETOEX_print(record);
        break;
    case U_EMR_SETMETARGN:
        U_EMRSETMETARGN_print(record);
        break;
    case U_EMR_EXCLUDECLIPRECT:
        U_EMREXCLUDECLIPRECT_print(record);
        break;
    case U_EMR_INTERSECTCLIPRECT:
        U_EMRINTERSECTCLIPRECT_print(record);
        break;
    case U_EMR_SCALEVIEWPORTEXTEX:
        U_EMRSCALEVIEWPORTEXTEX_print(record);
        break;
    case U_EMR_SCALEWINDOWEXTEX:
        U_EMRSCALEWINDOWEXTEX_print(record);
        break;
    case U_EMR_SAVEDC:
        U_EMRSAVEDC_print(record);
        break;
    case U_EMR_RESTOREDC:
        U_EMRRESTOREDC_print(record);
        break;
    case U_EMR_SETWORLDTRANSFORM:
        U_EMRSETWORLDTRANSFORM_print(record);
        break;
    case U_EMR_MODIFYWORLDTRANSFORM:
        U_EMRMODIFYWORLDTRANSFORM_print(record);
        break;
    case U_EMR_SELECTOBJECT:
        U_EMRSELECTOBJECT_print(record);
        break;
    case U_EMR_CREATEPEN:
        U_EMRCREATEPEN_print(record);
        break;
    case U_EMR_CREATEBRUSHINDIRECT:
        U_EMRCREATEBRUSHINDIRECT_print(record);
        break;
    case U_EMR_DELETEOBJECT:
        U_EMRDELETEOBJECT_print(record);
        break;
    case U_EMR_ANGLEARC:
        U_EMRANGLEARC_print(record);
        break;
    case U_EMR_ELLIPSE:
        U_EMRELLIPSE_print(record);
        break;
    case U_EMR_RECTANGLE:
        U_EMRRECTANGLE_print(record);
        break;
    case U_EMR_ROUNDRECT:
        U_EMRROUNDRECT_print(record);
        break;
    case U_EMR_ARC:
        U_EMRARC_print(record);
        break;
    case U_EMR_CHORD:
        U_EMRCHORD_print(record);
        break;
    case U_EMR_PIE:
        U_EMRPIE_print(record);
        break;
    case U_EMR_SELECTPALETTE:
        U_EMRSELECTPALETTE_print(record);
        break;
    case U_EMR_CREATEPALETTE:
        U_EMRCREATEPALETTE_print(record);
        break;
    case U_EMR_SETPALETTEENTRIES:
        U_EMRSETPALETTEENTRIES_print(record);
        break;
    case U_EMR_RESIZEPALETTE:
        U_EMRRESIZEPALETTE_print(record);
        break;
    case U_EMR_REALIZEPALETTE:
        U_EMRREALIZEPALETTE_print(record);
        break;
    case U_EMR_EXTFLOODFILL:
        U_EMREXTFLOODFILL_print(record);
        break;
    case U_EMR_LINETO:
        U_EMRLINETO_print(record);
        break;
    case U_EMR_ARCTO:
        U_EMRARCTO_print(record);
        break;
    case U_EMR_POLYDRAW:
        U_EMRPOLYDRAW_print(record);
        break;
    case U_EMR_SETARCDIRECTION:
        U_EMRSETARCDIRECTION_print(record);
        break;
    case U_EMR_SETMITERLIMIT:
        U_EMRSETMITERLIMIT_print(record);
        break;
    case U_EMR_BEGINPATH:
        U_EMRBEGINPATH_print(record);
        break;
    case U_EMR_ENDPATH:
        U_EMRENDPATH_print(record);
        break;
    case U_EMR_CLOSEFIGURE:
        U_EMRCLOSEFIGURE_print(record);
        break;
    case U_EMR_FILLPATH:
        U_EMRFILLPATH_print(record);
        break;
    case U_EMR_STROKEANDFILLPATH:
        U_EMRSTROKEANDFILLPATH_print(record);
        break;
    case U_EMR_STROKEPATH:
        U_EMRSTROKEPATH_print(record);
        break;
    case U_EMR_FLATTENPATH:
        U_EMRFLATTENPATH_print(record);
        break;
    case U_EMR_WIDENPATH:
        U_EMRWIDENPATH_print(record);
        break;
    case U_EMR_SELECTCLIPPATH:
        U_EMRSELECTCLIPPATH_print(record);
        break;
    case U_EMR_ABORTPATH:
        U_EMRABORTPATH_print(record);
        break;
    case U_EMR_UNDEF69:
        U_EMRUNDEF69_print(record);
        break;
    case U_EMR_COMMENT:
        U_EMRCOMMENT_print(record, off);
        break;
    case U_EMR_FILLRGN:
        U_EMRFILLRGN_print(record);
        break;
    case U_EMR_FRAMERGN:
        U_EMRFRAMERGN_print(record);
        break;
    case U_EMR_INVERTRGN:
        U_EMRINVERTRGN_print(record);
        break;
    case U_EMR_PAINTRGN:
        U_EMRPAINTRGN_print(record);
        break;
    case U_EMR_EXTSELECTCLIPRGN:
        U_EMREXTSELECTCLIPRGN_print(record);
        break;
    case U_EMR_BITBLT:
        U_EMRBITBLT_print(record);
        break;
    case U_EMR_STRETCHBLT:
        U_EMRSTRETCHBLT_print(record);
        break;
    case U_EMR_MASKBLT:
        U_EMRMASKBLT_print(record);
        break;
    case U_EMR_PLGBLT:
        U_EMRPLGBLT_print(record);
        break;
    case U_EMR_SETDIBITSTODEVICE:
        U_EMRSETDIBITSTODEVICE_print(record);
        break;
    case U_EMR_STRETCHDIBITS:
        U_EMRSTRETCHDIBITS_print(record);
        break;
    case U_EMR_EXTCREATEFONTINDIRECTW:
        U_EMREXTCREATEFONTINDIRECTW_print(record);
        break;
    case U_EMR_EXTTEXTOUTA:
        U_EMREXTTEXTOUTA_print(record);
        break;
    case U_EMR_EXTTEXTOUTW:
        U_EMREXTTEXTOUTW_print(record);
        break;
    case U_EMR_POLYBEZIER16:
        U_EMRPOLYBEZIER16_print(record);
        break;
    case U_EMR_POLYGON16:
        U_EMRPOLYGON16_print(record);
        break;
    case U_EMR_POLYLINE16:
        U_EMRPOLYLINE16_print(record);
        break;
    case U_EMR_POLYBEZIERTO16:
        U_EMRPOLYBEZIERTO16_print(record);
        break;
    case U_EMR_POLYLINETO16:
        U_EMRPOLYLINETO16_print(record);
        break;
    case U_EMR_POLYPOLYLINE16:
        U_EMRPOLYPOLYLINE16_print(record);
        break;
    case U_EMR_POLYPOLYGON16:
        U_EMRPOLYPOLYGON16_print(record);
        break;
    case U_EMR_POLYDRAW16:
        U_EMRPOLYDRAW16_print(record);
        break;
    case U_EMR_CREATEMONOBRUSH:
        U_EMRCREATEMONOBRUSH_print(record);
        break;
    case U_EMR_CREATEDIBPATTERNBRUSHPT:
        U_EMRCREATEDIBPATTERNBRUSHPT_print(record);
        break;
    case U_EMR_EXTCREATEPEN:
        U_EMREXTCREATEPEN_print(record);
        break;
    case U_EMR_POLYTEXTOUTA:
        U_EMRPOLYTEXTOUTA_print(record);
        break;
    case U_EMR_POLYTEXTOUTW:
        U_EMRPOLYTEXTOUTW_print(record);
        break;
    case U_EMR_SETICMMODE:
        U_EMRSETICMMODE_print(record);
        break;
    case U_EMR_CREATECOLORSPACE:
        U_EMRCREATECOLORSPACE_print(record);
        break;
    case U_EMR_SETCOLORSPACE:
        U_EMRSETCOLORSPACE_print(record);
        break;
    case U_EMR_DELETECOLORSPACE:
        U_EMRDELETECOLORSPACE_print(record);
        break;
    case U_EMR_GLSRECORD:
        U_EMRGLSRECORD_print(record);
        break;
    case U_EMR_GLSBOUNDEDRECORD:
        U_EMRGLSBOUNDEDRECORD_print(record);
        break;
    case U_EMR_PIXELFORMAT:
        U_EMRPIXELFORMAT_print(record);
        break;
    case U_EMR_DRAWESCAPE:
        U_EMRDRAWESCAPE_print(record);
        break;
    case U_EMR_EXTESCAPE:
        U_EMREXTESCAPE_print(record);
        break;
    case U_EMR_UNDEF107:
        U_EMRUNDEF107_print(record);
        break;
    case U_EMR_SMALLTEXTOUT:
        U_EMRSMALLTEXTOUT_print(record);
        break;
    case U_EMR_FORCEUFIMAPPING:
        U_EMRFORCEUFIMAPPING_print(record);
        break;
    case U_EMR_NAMEDESCAPE:
        U_EMRNAMEDESCAPE_print(record);
        break;
    case U_EMR_COLORCORRECTPALETTE:
        U_EMRCOLORCORRECTPALETTE_print(record);
        break;
    case U_EMR_SETICMPROFILEA:
        U_EMRSETICMPROFILEA_print(record);
        break;
    case U_EMR_SETICMPROFILEW:
        U_EMRSETICMPROFILEW_print(record);
        break;
    case U_EMR_ALPHABLEND:
        U_EMRALPHABLEND_print(record);
        break;
    case U_EMR_SETLAYOUT:
        U_EMRSETLAYOUT_print(record);
        break;
    case U_EMR_TRANSPARENTBLT:
        U_EMRTRANSPARENTBLT_print(record);
        break;
    case U_EMR_UNDEF117:
        U_EMRUNDEF117_print(record);
        break;
    case U_EMR_GRADIENTFILL:
        U_EMRGRADIENTFILL_print(record);
        break;
    case U_EMR_SETLINKEDUFIS:
        U_EMRSETLINKEDUFIS_print(record);
        break;
    case U_EMR_SETTEXTJUSTIFICATION:
        U_EMRSETTEXTJUSTIFICATION_print(record);
        break;
    case U_EMR_COLORMATCHTOTARGETW:
        U_EMRCOLORMATCHTOTARGETW_print(record);
        break;
    case U_EMR_CREATECOLORSPACEW:
        U_EMRCREATECOLORSPACEW_print(record);
        break;
    default:
        U_EMRNOTIMPLEMENTED_print("?", record);
        break;
    } // end of switch
    return (nSize);
}
