/**
  ...
  @brief Functions for converting EMF records between Big Endian and Little
  Endian byte orders.

  EMF files use Little Endian order.
  On a Big Endian machine the data must be converted to/from Little Endian when
  it is writen to/read from a file.
  On a Little Endian machine no conversion is required, but it is good to be
  able to test the routines on either platform.
  When "torev" is true these routines convert from the native byte order to the
  reverse.
  When "torev" is false these routines convert from the reverse byte order to
  the native.
  Routines that do not use that variable swap byte order, and the way in which
  they do so does not depend
  on the native byte order.

  The only function here which should be called directly is U_emf_endian(), and
  then,except for testing purposes, only on a BE machine.

  Many variables are initialized to zero even though they will always be set
  because
  some versions of gcc give spurious "may be used uninitialized" warnings
  otherwise.
*/
...
/* Note: U_swap4 is also used by uwmf_endian.c, in cases where the 32 bit data
...
/**
    U_COLORREF and U_RGBQUAD do NOT need to be swapped, they are always stored
   in memory in the proper order.
*/

/**
    \brief Convert rect and rectl objects from Upper Left and Lower Right corner
   points.
    \param rect U_RECTL object
    \param count number to convert
*/
void rectl_swap(PU_RECTL rect, unsigned int count) {
...
/**
    ...
    \param count number to convert
*/
void sizel_swap(PU_SIZEL sz, unsigned int count) {
    U_swap4(sz, 2 * count);
}
...
/**
    ...
    \param count number to convert
*/
void pointl_swap(PU_POINTL pt, unsigned int count) {
    U_swap4(pt, 2 * count);
}
...
/**
    ...
    \param count number to convert
*/
void point16_swap(PU_POINT16 pt, unsigned int count) {
    U_swap2(pt, 2 * count);
}
...
/**
    ...
    \param count number to convert
*/
void trivertex_swap(PU_TRIVERTEX tv, unsigned int count) {
    for (; count; count--, tv++) {
        U_swap4(tv, 2);         /* x,y */
...
/**
    ...
    \param count number to convert
*/
void gradient3_swap(PU_GRADIENT3 g3, unsigned int count) {
    U_swap4(g3, 3 * count);
}
...
/**
    ...
    \param count number to convert
*/
void gradient4_swap(PU_GRADIENT4 g4, unsigned int count) {
...
*/
void logbrush_swap(PU_LOGBRUSH lb) {
...
*/
void xform_swap(PU_XFORM xform) {
    U_swap4(xform, 6);
}
...
*/
void ciexyztriple_swap(PU_CIEXYZTRIPLE cie3) {
    U_swap4(cie3, 9);
}
/**
...
*/
void logcolorspacea_swap(PU_LOGCOLORSPACEA lcsa) {
...
*/
void logcolorspacew_swap(PU_LOGCOLORSPACEW lcsa) {
    U_swap4(lcsa, 5); // lcsSignature lcsVersion lcsSize lcsCSType lcsIntent
    ciexyztriple_swap(&(lcsa->lcsEndpoints));
    // ordered bytes:                           lcsGammaRGB
...
*/
void logfont_swap(PU_LOGFONT lf) {
    U_swap4(lf, 5); // lfHeight lfWidth lfEscapement lfOrientation lfWeight
    // ordered bytes:                           lfItalic lfUnderline lfStrikeOut
...
*/
void logfont_panose_swap(PU_LOGFONT_PANOSE lfp) {
    logfont_swap(&(lfp->elfLogFont)); // elfLogFont
    // UTF-16LE, already in order:              elfFullName
    // UTF-16LE, already in order:              elfStyle
    U_swap4(&(lfp->elfVersion),
            4); // elfVersion elfStyleSize elfMatch elfReserved
...
*/
void bitmapinfoheader_swap(PU_BITMAPINFOHEADER Bmi) {
    U_swap4(Bmi, 3);                   // biSize biWidth biHeight
    U_swap2(&(Bmi->biPlanes), 2);      // biPlanes biBitCount
    U_swap4(&(Bmi->biCompression), 6); // biCompression biSizeImage
...
*/
void bitmapinfo_swap(const char *Bmi) {
...
/**
    ...
    \param offBmi     offset from the start of the record to the start of the
   bitmapinfo structure
    \param cbBmi      declared space for the bitmapinfo structure in the record
    \param offBits    offset from the start of the record to the start of the
   bitmap
    \param cbBits     declared space for the bitmap in the record (amount used
...
*/
int DIB_swap(const char *record, uint32_t iUsage, uint32_t offBmi,
             uint32_t cbBmi, uint32_t offBits, uint32_t cbBits,
             const char *blimit, int torev) {
    ...
    if (!cbBmi)
        return (1); // No DIB in a record where it is optional
    if (IS_MEM_UNSAFE(record, offBmi + cbBmi, blimit))
        return (0);
    if (cbBits && IS_MEM_UNSAFE(record, offBits + cbBits, blimit))
        return (0);
    if (iUsage == U_DIB_RGB_COLORS) {
        uint32_t width, height, colortype, numCt,
            invert; // these values will be set in get_DIB_params
        // next call returns pointers and values, but allocates no memory
        if (torev) {
            dibparams = get_DIB_params(record, offBits, offBmi, &px,
                                       (const U_RGBQUAD **)&ct, &numCt, &width,
        ...
        if (dibparams == U_BI_RGB) {
            // this is the only DIB type where we can calculate how big it
            // should be when stored in the EMF file
            bs = colortype / 8;
            if (bs < 1) {
                usedbytes =
                    (width * colortype + 7) /
            ...
            if (IS_MEM_UNSAFE(record + offBits, usedbytes, blimit))
                return (0);
        }
    } else {
...
/**
    ...
    \param blimit one byte past the end of the record
*/
int extlogpen_swap(PU_EXTLOGPEN elp, const char *blimit, int torev) {
    int count = 0;
    ...
    if (IS_MEM_UNSAFE(&(elp->elpStyleEntry), count * 4, blimit))
        return (0);
...
*/
void logpalette_swap(PU_LOGPALETTE lp) {
...
*/
void rgndataheader_swap(PU_RGNDATAHEADER rdh) {
...
*/
int rgndata_swap(PU_RGNDATA rd, int cbRgnData, int torev) {
    int count = 0;
    if (torev) {
        count = rd->rdh.nCount;
    }
    ...
    if (4 * count + (int)sizeof(U_RGNDATAHEADER) > cbRgnData)
        return (0);
...
*/
void coloradjustment_swap(PU_COLORADJUSTMENT ca) {
    U_swap2(ca, 12); // caSize caFlags caIlluminantIndex caRedGamma caGreenGamma
                     // caBlueGamma caReferenceBlack caReferenceWhite caContrast
                     // caBrightness caColorfulness caRedGreenTint
}
...
*/
void pixelformatdescriptor_swap(PU_PIXELFORMATDESCRIPTOR pfd) {
    U_swap2(pfd, 2);             // nSize nVersion
    U_swap4(&(pfd->dwFlags), 1); // dwFlags
    // ordered bytes:                           iPixelType cColorBits cRedBits
...
/**
   ...
   U_EMRTEXT
    ...
    \param torev     1 for native to reversed, 0 for reversed to native
*/
int emrtext_swap(PU_EMRTEXT pemt, char *record, const char *blimit, int torev) {
    int off;
    uint32_t count = 0;
    uint32_t offDx = 0;
    uint32_t fOptions = 0;
    ...
    if (!(fOptions & U_ETO_NO_RECT)) {
        if (IS_MEM_UNSAFE(pemt, sizeof(U_RECTL), blimit))
            return (0);
    ...
    if (IS_MEM_UNSAFE(pemt, off + 4, blimit))
        return (0);
    U_swap4(((char *)pemt + off), 1); // offDx
    if (!torev) {
        offDx = *(uint32_t *)((char *)pemt + off);
    }
    if (IS_MEM_UNSAFE(record, count * 4, blimit))
        return (0);
    U_swap4((record + offDx),
            count); // Dx[], offset with respect to the Record, NOT the object
    return (1);
}

/* **********************************************************************************************
These functions contain shared code used by various U_EMR*_swap functions.
...
// numbered as core5 to be consistent with uemf.c, but must appear before the
// others as there is no prototype
...
int core1_swap(char *record, int torev) {
    ...
    if (IS_MEM_UNSAFE((pEmr->aptl), count * sizeof(U_POINTL), blimit))
        return (0);
...
int core2_swap(char *record, int torev) {
    ...
    if (IS_MEM_UNSAFE((pEmr->aPolyCounts), nPolys * 4, blimit))
        return (0);
    U_swap4(pEmr->aPolyCounts, nPolys); // aPolyCounts[]
    record += sizeof(U_EMRPOLYPOLYLINE) - 4 + sizeof(uint32_t) * nPolys;
    if (IS_MEM_UNSAFE(record, count * sizeof(U_POINTL), blimit))
        return (0);
...
// Functions taking a single U_RECT or U_RECTL, starting with U_EMRELLIPSE_swap,
// also U_EMRFILLPATH_swap,
...
int core6_swap(char *record, int torev) {
    ...
    if (IS_MEM_UNSAFE((pEmr->apts), count * sizeof(U_POINT16), blimit))
        return (0);
    point16_swap((pEmr->apts), count); // apts[]
    return (1);
}

// Records with the same form starting with U_EMRSETWINDOWEXTEX_swap, that is,
// all with two uint32_t values after the emr
...
int core8_swap(char *record, int torev) {
    ...
    if (!emrtext_swap(&(pEmr->emrtext), record, blimit, torev))
        return (0);
    return (1);
}
...
int core10_swap(char *record, int torev) {
    ...
    if (IS_MEM_UNSAFE(record, count * sizeof(U_POINT16), blimit))
        return (0);
    point16_swap((PU_POINT16)(record), count); // apts[]
    return (1);
}

// Functions with the same form starting with  U_EMRINVERTRGN_swap and
// U_EMRPAINTRGN_swap,
int core11_swap(char *record, int torev) {
    ...
    if (IS_MEM_UNSAFE(pEmr->RgnData, cbRgnData, blimit))
        return (0);
    return (rgndata_swap(pEmr->RgnData, cbRgnData, torev));
}

// common code for U_EMRCREATEMONOBRUSH_swap and
...
int core12_swap(char *record, int torev) {
    ...
    if (torev) {
        ...
        iUsage = pEmr->iUsage;
        blimit = record + pEmr->emr.nSize;
        if (!DIB_swap(record, iUsage, offBmi, cbBmi, offBits, cbBits, blimit,
                      torev))
            return (0);
    }
    if (!core5_swap(record, torev))
    ...
    // ordered bytes:                           bitmap (including 16 bit
    // 5bit/channel color mode, which is done bytewise).
    return (1);
}
...
int core13_swap(char *record, int torev) {
    ...
    if (torev) {
        ...
        blimit = record + pEmr->emr.nSize;
        if (!DIB_swap(record, iUsageSrc, offBmiSrc, cbBmiSrc, offBitsSrc,
                      cbBitsSrc, blimit, torev))
            return (0);
    }
    if (!core5_swap(record, torev))
        return (0);
    ...
    U_swap4(&(pEmr->iUsageSrc),
            5); // iUsageSrc offBmiSrc cbBmiSrc offBitsSrc cbBitsSrc
    if (!torev) {
        offBmiSrc = pEmr->offBmiSrc;
        cbBmiSrc = pEmr->cbBmiSrc;
        offBitsSrc = pEmr->offBitsSrc;
...
/* **********************************************************************************************
These are the core EMR_swap functions, each converts a particular type of
record.
All operate in place on the chunk of memory holding that record.
Some of these have offsets or counts which, if corrupt or evil would result in
access outside
  the record.  These cases return a status value of 0 if that happens, 1 on
success.  Other
...
/**
    ...
    \brief Convert a pointer to a U_EMR_whatever record which has not been
   implemented.
    \param record   pointer to a buffer holding the EMR record
    \param torev    1 for native to reversed, 0 for reversed to native
*/
int U_EMRNOTIMPLEMENTED_swap(char *record, int torev) {
    fprintf(stderr, "EMF WARNING:  could not swap data bytes on record because "
                    "that type has not been implemented!\n");
...
int U_EMRHEADER_swap(char *record, int torev) {
    ...
    U_swap4(&(pEmr->nDescription),
            3); // nDescription offDescription nPalEntries
    if (!torev) {
        nSize = pEmr->emr.nSize;
        nDesc = pEmr->nDescription;
        offDesc = pEmr->offDescription;
    ...
    if ((nDesc && (offDesc >= 100)) || (!offDesc && nSize >= 100)) {
        if (torev) {
        ...
        if (cbPix)
            pixelformatdescriptor_swap(
                (PU_PIXELFORMATDESCRIPTOR)(record + pEmr->offPixelFormat));
        if ((nDesc && (offDesc >= 108)) || (cbPix && (offPix >= 108)) ||
...
int U_EMREOF_swap(char *record, int torev) {
    ...
    if (cbPalEntries) {
        if (IS_MEM_UNSAFE(record, pEmr->offPalEntries + 2 * 2, blimit))
            return (0); // 2 16 bit values in U_LOGPALLETE
    ...
    if (IS_MEM_UNSAFE(record, off + 4, blimit))
        return (0);
...
// U_EMRUNDEF69                       69
#define U_EMRUNDEF69_swap(A, B)                                                \
    U_EMRNOTIMPLEMENTED_swap(A, B) //!< Not implemented.
...
int U_EMRCOMMENT_swap(char *record, int torev) {
    ...
    if (IS_MEM_UNSAFE(record, cbData + sizeof(U_SIZE_EMRCOMMENT), blimit))
        return (0);
...
int U_EMRMASKBLT_swap(char *record, int torev) {
    ...
    U_swap4(&(pEmr->iUsageMask),
            5); // iUsageMask offBmiMask cbBmiMask offBitsMask cbBitsMask
    if (!torev) {
        offBmiSrc = pEmr->offBmiSrc;
        cbBmiSrc = pEmr->cbBmiSrc;
        offBitsSrc = pEmr->offBitsSrc;
...
int U_EMRSETDIBITSTODEVICE_swap(char *record, int torev) {
    ...
    pointl_swap(&(pEmr->Src), 2);      // Src cSrc
    U_swap4(&(pEmr->offBmiSrc), 7); // offBmiSrc cbBmiSrc offBitsSrc cbBitsSrc
                                    // iUsageSrc iStartScan cScans
    if (!torev) {
        offBmiSrc = pEmr->offBmiSrc;
        cbBmiSrc = pEmr->cbBmiSrc;
        offBitsSrc = pEmr->offBitsSrc;
...
int U_EMRSTRETCHDIBITS_swap(char *record, int torev) {
    ...
    pointl_swap(&(pEmr->Dest), 1);     // Dest
    pointl_swap(&(pEmr->Src), 2);      // Src cSrc
    U_swap4(&(pEmr->offBmiSrc),
            6); // offBmiSrc cbBmiSrc offBitsSrc cbBitsSrc iUsageSrc dwRop
...
int U_EMREXTCREATEFONTINDIRECTW_swap(char *record, int torev) {
    ...
    } else { // logfont or logfontExDv (which starts with logfont, which can be
             // swapped, and the rest is already in byte order
...
int U_EMREXTCREATEPEN_swap(char *record, int torev) {
    ...
    if (torev) {
        ...
        cbBits = pEmr->cbBits;
        blimit = record + pEmr->emr.nSize;
        if (!DIB_swap(record, U_DIB_RGB_COLORS, offBmi, cbBmi, offBits, cbBits,
                      blimit, torev))
            return (0);
    }
    if (!core5_swap(record, torev))
...
#define U_EMRPOLYTEXTOUTA_swap(A, B)                                           \
    U_EMRNOTIMPLEMENTED_swap(A, B) //!< Not implemented.
// U_EMRPOLYTEXTOUTW         97 NOT IMPLEMENTED, denigrated after Windows NT
#define U_EMRPOLYTEXTOUTW_swap(A, B)                                           \
    U_EMRNOTIMPLEMENTED_swap(A, B) //!< Not implemented.
...
#define U_EMRGLSRECORD_swap(A, B)                                              \
    U_EMRNOTIMPLEMENTED_swap(A, B) //!< Not implemented.
// U_EMRGLSBOUNDEDRECORD    103  Not implemented
#define U_EMRGLSBOUNDEDRECORD_swap(A, B)                                       \
    U_EMRNOTIMPLEMENTED_swap(A, B) //!< Not implemented.
...
#define U_EMRDRAWESCAPE_swap(A, B)                                             \
    U_EMRNOTIMPLEMENTED_swap(A, B) //!< Not implemented.
// U_EMREXTESCAPE           106  Not implemented
#define U_EMREXTESCAPE_swap(A, B)                                              \
    U_EMRNOTIMPLEMENTED_swap(A, B) //!< Not implemented.
// U_EMRUNDEF107            107  Not implemented
#define U_EMRUNDEF107_swap(A, B)                                               \
    U_EMRNOTIMPLEMENTED_swap(A, B) //!< Not implemented.
...
int U_EMRSMALLTEXTOUT_swap(char *record, int torev) {
    ...
    pointl_swap(&(pEmr->Dest), 1); // Dest
    U_swap4(&(pEmr->cChars),
            5); // cChars fuOptions iGraphicsMode exScale eyScale
    if (!torev) {
        fuOptions = pEmr->fuOptions;
        cChars = pEmr->cChars;
        blimit = record + pEmr->emr.nSize;
    ...
    if (!(fuOptions & U_ETO_NO_RECT)) {
        if (IS_MEM_UNSAFE(record, roff + sizeof(U_RECTL), blimit))
            return (0);
        rectl_swap((PU_RECTL)(record + roff), 1); // rclBounds
    }
    if (IS_MEM_UNSAFE(record, roff + sizeof(U_RECTL) + cChars, blimit))
        return (0);
...
#define U_EMRFORCEUFIMAPPING_swap(A, B)                                        \
    U_EMRNOTIMPLEMENTED_swap(A, B) //!< Not implemented.
// U_EMRNAMEDESCAPE         110  Not implemented
#define U_EMRNAMEDESCAPE_swap(A, B)                                            \
    U_EMRNOTIMPLEMENTED_swap(A, B) //!< Not implemented.
// U_EMRCOLORCORRECTPALETTE 111  Not implemented
#define U_EMRCOLORCORRECTPALETTE_swap(A, B)                                    \
    U_EMRNOTIMPLEMENTED_swap(A, B) //!< Not implemented.
// U_EMRSETICMPROFILEA      112  Not implemented
#define U_EMRSETICMPROFILEA_swap(A, B)                                         \
    U_EMRNOTIMPLEMENTED_swap(A, B) //!< Not implemented.
// U_EMRSETICMPROFILEW      113  Not implemented
#define U_EMRSETICMPROFILEW_swap(A, B)                                         \
    U_EMRNOTIMPLEMENTED_swap(A, B) //!< Not implemented.
...
int U_EMRGRADIENTFILL_swap(char *record, int torev) {
    ...
    if (IS_MEM_UNSAFE(record, nTriVert * sizeof(U_TRIVERTEX), blimit))
        return (0);
    if (nTriVert) {
...
#define U_EMRSETLINKEDUFIS_swap(A, B)                                          \
    U_EMRNOTIMPLEMENTED_swap(A, B) //!< Not implemented.
// U_EMRSETTEXTJUSTIFICATION120  Not implemented (denigrated)
#define U_EMRSETTEXTJUSTIFICATION_swap(A, B)                                   \
    U_EMRNOTIMPLEMENTED_swap(A, B) //!< Not implemented.
// U_EMRCOLORMATCHTOTARGETW 121  Not implemented
#define U_EMRCOLORMATCHTOTARGETW_swap(A, B)                                    \
    U_EMRNOTIMPLEMENTED_swap(A, B) //!< Not implemented.
...
/**
    ...
    Normally this would be called immediately after reading the data from a
   file.
    Verifies that the declared size is consistent with this type of record.
*/
int U_emf_record_sizeok(const char *record, const char *blimit, uint32_t *nSize,
                        uint32_t *iType, int torev) {
    uint32_t rsize = 0;
    if (!nSize || !iType)
        return (0); // programming error

    /* Check that COMMON data in record can be touched without an access
       violation.  If it cannot be
        this is either a corrupt EMF or one engineered to cause a buffer
       overflow.  Pointer math
        could wrap so check both sides of the range, and fail any indications of
       such a wrap.
    */
    if (IS_MEM_UNSAFE(record, sizeof(U_EMR), blimit))
        return (0);
    ...
    *nSize = pEmr->nSize;
    ...
    if (IS_MEM_UNSAFE(record, *nSize, blimit))
        return (0);

    switch (*iType) {
    // next line, ancient EMF files used a smaller header, to be safe, test for
    // that
    case U_EMR_HEADER:
        rsize = U_SIZE_EMRHEADER_MIN;
        break;
    case U_EMR_POLYBEZIER:
        rsize = U_SIZE_EMRPOLYBEZIER;
        break;
    case U_EMR_POLYGON:
        rsize = U_SIZE_EMRPOLYGON;
        break;
    case U_EMR_POLYLINE:
        rsize = U_SIZE_EMRPOLYLINE;
        break;
    case U_EMR_POLYBEZIERTO:
        rsize = U_SIZE_EMRPOLYBEZIERTO;
        break;
    case U_EMR_POLYLINETO:
        rsize = U_SIZE_EMRPOLYLINETO;
        break;
    case U_EMR_POLYPOLYLINE:
        rsize = U_SIZE_EMRPOLYPOLYLINE;
        break;
    case U_EMR_POLYPOLYGON:
        rsize = U_SIZE_EMRPOLYPOLYGON;
        break;
    case U_EMR_SETWINDOWEXTEX:
        rsize = U_SIZE_EMRSETWINDOWEXTEX;
        break;
    case U_EMR_SETWINDOWORGEX:
        rsize = U_SIZE_EMRSETWINDOWORGEX;
        break;
    case U_EMR_SETVIEWPORTEXTEX:
        rsize = U_SIZE_EMRSETVIEWPORTEXTEX;
        break;
    case U_EMR_SETVIEWPORTORGEX:
        rsize = U_SIZE_EMRSETVIEWPORTORGEX;
        break;
    case U_EMR_SETBRUSHORGEX:
        rsize = U_SIZE_EMRSETBRUSHORGEX;
        break;
    case U_EMR_EOF:
        rsize = U_SIZE_EMREOF;
        break;
    case U_EMR_SETPIXELV:
        rsize = U_SIZE_EMRSETPIXELV;
        break;
    case U_EMR_SETMAPPERFLAGS:
        rsize = U_SIZE_EMRSETMAPPERFLAGS;
        break;
    case U_EMR_SETMAPMODE:
        rsize = U_SIZE_EMRSETMAPMODE;
        break;
    case U_EMR_SETBKMODE:
        rsize = U_SIZE_EMRSETBKMODE;
        break;
    case U_EMR_SETPOLYFILLMODE:
        rsize = U_SIZE_EMRSETPOLYFILLMODE;
        break;
    case U_EMR_SETROP2:
        rsize = U_SIZE_EMRSETROP2;
        break;
    case U_EMR_SETSTRETCHBLTMODE:
        rsize = U_SIZE_EMRSETSTRETCHBLTMODE;
        break;
    case U_EMR_SETTEXTALIGN:
        rsize = U_SIZE_EMRSETTEXTALIGN;
        break;
    case U_EMR_SETCOLORADJUSTMENT:
        rsize = U_SIZE_EMRSETCOLORADJUSTMENT;
        break;
    case U_EMR_SETTEXTCOLOR:
        rsize = U_SIZE_EMRSETTEXTCOLOR;
        break;
    case U_EMR_SETBKCOLOR:
        rsize = U_SIZE_EMRSETBKCOLOR;
        break;
    case U_EMR_OFFSETCLIPRGN:
        rsize = U_SIZE_EMROFFSETCLIPRGN;
        break;
    case U_EMR_MOVETOEX:
        rsize = U_SIZE_EMRMOVETOEX;
        break;
    case U_EMR_SETMETARGN:
        rsize = U_SIZE_EMRSETMETARGN;
        break;
    case U_EMR_EXCLUDECLIPRECT:
        rsize = U_SIZE_EMREXCLUDECLIPRECT;
        break;
    case U_EMR_INTERSECTCLIPRECT:
        rsize = U_SIZE_EMRINTERSECTCLIPRECT;
        break;
    case U_EMR_SCALEVIEWPORTEXTEX:
        rsize = U_SIZE_EMRSCALEVIEWPORTEXTEX;
        break;
    case U_EMR_SCALEWINDOWEXTEX:
        rsize = U_SIZE_EMRSCALEWINDOWEXTEX;
        break;
    case U_EMR_SAVEDC:
        rsize = U_SIZE_EMRSAVEDC;
        break;
    case U_EMR_RESTOREDC:
        rsize = U_SIZE_EMRRESTOREDC;
        break;
    case U_EMR_SETWORLDTRANSFORM:
        rsize = U_SIZE_EMRSETWORLDTRANSFORM;
        break;
    case U_EMR_MODIFYWORLDTRANSFORM:
        rsize = U_SIZE_EMRMODIFYWORLDTRANSFORM;
        break;
    case U_EMR_SELECTOBJECT:
        rsize = U_SIZE_EMRSELECTOBJECT;
        break;
    case U_EMR_CREATEPEN:
        rsize = U_SIZE_EMRCREATEPEN;
        break;
    case U_EMR_CREATEBRUSHINDIRECT:
        rsize = U_SIZE_EMRCREATEBRUSHINDIRECT;
        break;
    case U_EMR_DELETEOBJECT:
        rsize = U_SIZE_EMRDELETEOBJECT;
        break;
    case U_EMR_ANGLEARC:
        rsize = U_SIZE_EMRANGLEARC;
        break;
    case U_EMR_ELLIPSE:
        rsize = U_SIZE_EMRELLIPSE;
        break;
    case U_EMR_RECTANGLE:
        rsize = U_SIZE_EMRRECTANGLE;
        break;
    case U_EMR_ROUNDRECT:
        rsize = U_SIZE_EMRROUNDRECT;
        break;
    case U_EMR_ARC:
        rsize = U_SIZE_EMRARC;
        break;
    case U_EMR_CHORD:
        rsize = U_SIZE_EMRCHORD;
        break;
    case U_EMR_PIE:
        rsize = U_SIZE_EMRPIE;
        break;
    case U_EMR_SELECTPALETTE:
        rsize = U_SIZE_EMRSELECTPALETTE;
        break;
    case U_EMR_CREATEPALETTE:
        rsize = U_SIZE_EMRCREATEPALETTE;
        break;
    case U_EMR_SETPALETTEENTRIES:
        rsize = U_SIZE_EMRSETPALETTEENTRIES;
        break;
    case U_EMR_RESIZEPALETTE:
        rsize = U_SIZE_EMRRESIZEPALETTE;
        break;
    case U_EMR_REALIZEPALETTE:
        rsize = U_SIZE_EMRREALIZEPALETTE;
        break;
    case U_EMR_EXTFLOODFILL:
        rsize = U_SIZE_EMREXTFLOODFILL;
        break;
    case U_EMR_LINETO:
        rsize = U_SIZE_EMRLINETO;
        break;
    case U_EMR_ARCTO:
        rsize = U_SIZE_EMRARCTO;
        break;
    case U_EMR_POLYDRAW:
        rsize = U_SIZE_EMRPOLYDRAW;
        break;
    case U_EMR_SETARCDIRECTION:
        rsize = U_SIZE_EMRSETARCDIRECTION;
        break;
    case U_EMR_SETMITERLIMIT:
        rsize = U_SIZE_EMRSETMITERLIMIT;
        break;
    case U_EMR_BEGINPATH:
        rsize = U_SIZE_EMRBEGINPATH;
        break;
    case U_EMR_ENDPATH:
        rsize = U_SIZE_EMRENDPATH;
        break;
    case U_EMR_CLOSEFIGURE:
        rsize = U_SIZE_EMRCLOSEFIGURE;
        break;
    case U_EMR_FILLPATH:
        rsize = U_SIZE_EMRFILLPATH;
        break;
    case U_EMR_STROKEANDFILLPATH:
        rsize = U_SIZE_EMRSTROKEANDFILLPATH;
        break;
    case U_EMR_STROKEPATH:
        rsize = U_SIZE_EMRSTROKEPATH;
        break;
    case U_EMR_FLATTENPATH:
        rsize = U_SIZE_EMRFLATTENPATH;
        break;
    case U_EMR_WIDENPATH:
        rsize = U_SIZE_EMRWIDENPATH;
        break;
    case U_EMR_SELECTCLIPPATH:
        rsize = U_SIZE_EMRSELECTCLIPPATH;
        break;
    case U_EMR_ABORTPATH:
        rsize = U_SIZE_EMRABORTPATH;
        break;
    case U_EMR_UNDEF69:
        rsize = U_SIZE_EMRUNDEFINED;
        break;
    case U_EMR_COMMENT:
        rsize = U_SIZE_EMRCOMMENT;
        break;
    case U_EMR_FILLRGN:
        rsize = U_SIZE_EMRFILLRGN;
        break;
    case U_EMR_FRAMERGN:
        rsize = U_SIZE_EMRFRAMERGN;
        break;
    case U_EMR_INVERTRGN:
        rsize = U_SIZE_EMRINVERTRGN;
        break;
    case U_EMR_PAINTRGN:
        rsize = U_SIZE_EMRPAINTRGN;
        break;
    case U_EMR_EXTSELECTCLIPRGN:
        rsize = U_SIZE_EMREXTSELECTCLIPRGN;
        break;
    case U_EMR_BITBLT:
        rsize = U_SIZE_EMRBITBLT;
        break;
    case U_EMR_STRETCHBLT:
        rsize = U_SIZE_EMRSTRETCHBLT;
        break;
    case U_EMR_MASKBLT:
        rsize = U_SIZE_EMRMASKBLT;
        break;
    case U_EMR_PLGBLT:
        rsize = U_SIZE_EMRPLGBLT;
        break;
    case U_EMR_SETDIBITSTODEVICE:
        rsize = U_SIZE_EMRSETDIBITSTODEVICE;
        break;
    case U_EMR_STRETCHDIBITS:
        rsize = U_SIZE_EMRSTRETCHDIBITS;
        break;
    case U_EMR_EXTCREATEFONTINDIRECTW:
        rsize = U_SIZE_EMREXTCREATEFONTINDIRECTW_LOGFONT;
        break;
    case U_EMR_EXTTEXTOUTA:
        rsize = U_SIZE_EMREXTTEXTOUTA;
        break;
    case U_EMR_EXTTEXTOUTW:
        rsize = U_SIZE_EMREXTTEXTOUTW;
        break;
    case U_EMR_POLYBEZIER16:
        rsize = U_SIZE_EMRPOLYBEZIER16;
        break;
    case U_EMR_POLYGON16:
        rsize = U_SIZE_EMRPOLYGON16;
        break;
    case U_EMR_POLYLINE16:
        rsize = U_SIZE_EMRPOLYLINE16;
        break;
    case U_EMR_POLYBEZIERTO16:
        rsize = U_SIZE_EMRPOLYBEZIERTO16;
        break;
    case U_EMR_POLYLINETO16:
        rsize = U_SIZE_EMRPOLYLINETO16;
        break;
    case U_EMR_POLYPOLYLINE16:
        rsize = U_SIZE_EMRPOLYPOLYLINE16;
        break;
    case U_EMR_POLYPOLYGON16:
        rsize = U_SIZE_EMRPOLYPOLYGON16;
        break;
    case U_EMR_POLYDRAW16:
        rsize = U_SIZE_EMRPOLYDRAW16;
        break;
    case U_EMR_CREATEMONOBRUSH:
        rsize = U_SIZE_EMRCREATEMONOBRUSH;
        break;
    case U_EMR_CREATEDIBPATTERNBRUSHPT:
        rsize = U_SIZE_EMRCREATEDIBPATTERNBRUSHPT;
        break;
    case U_EMR_EXTCREATEPEN:
        rsize = U_SIZE_EMREXTCREATEPEN;
        break;
    case U_EMR_POLYTEXTOUTA:
        rsize = U_SIZE_EMRPOLYTEXTOUTA;
        break;
    case U_EMR_POLYTEXTOUTW:
        rsize = U_SIZE_EMRPOLYTEXTOUTW;
        break;
    case U_EMR_SETICMMODE:
        rsize = U_SIZE_EMRSETICMMODE;
        break;
    case U_EMR_CREATECOLORSPACE:
        rsize = U_SIZE_EMRCREATECOLORSPACE;
        break;
    case U_EMR_SETCOLORSPACE:
        rsize = U_SIZE_EMRSETCOLORSPACE;
        break;
    case U_EMR_DELETECOLORSPACE:
        rsize = U_SIZE_EMRDELETECOLORSPACE;
        break;
    case U_EMR_GLSRECORD:
        rsize = U_SIZE_EMRGLSRECORD;
        break;
    case U_EMR_GLSBOUNDEDRECORD:
        rsize = U_SIZE_EMRGLSBOUNDEDRECORD;
        break;
    case U_EMR_PIXELFORMAT:
        rsize = U_SIZE_EMRPIXELFORMAT;
        break;
    case U_EMR_DRAWESCAPE:
        rsize = U_SIZE_EMRDRAWESCAPE;
        break;
    case U_EMR_EXTESCAPE:
        rsize = U_SIZE_EMREXTESCAPE;
        break;
    case U_EMR_UNDEF107:
        rsize = U_SIZE_EMRUNDEFINED;
        break;
    case U_EMR_SMALLTEXTOUT:
        rsize = U_SIZE_EMRSMALLTEXTOUT;
        break;
    //       case U_EMR_FORCEUFIMAPPING:         rsize =
    //       U_SIZE_EMRFORCEUFIMAPPING;         break;
    case U_EMR_NAMEDESCAPE:
        rsize = U_SIZE_EMRNAMEDESCAPE;
        break;
    //       case U_EMR_COLORCORRECTPALETTE:     rsize =
    //       U_SIZE_EMRCOLORCORRECTPALETTE;     break;
    //       case U_EMR_SETICMPROFILEA:          rsize =
    //       U_SIZE_EMRSETICMPROFILEA;          break;
    //       case U_EMR_SETICMPROFILEW:          rsize =
    //       U_SIZE_EMRSETICMPROFILEW;          break;
    case U_EMR_ALPHABLEND:
        rsize = U_SIZE_EMRALPHABLEND;
        break;
    case U_EMR_SETLAYOUT:
        rsize = U_SIZE_EMRSETLAYOUT;
        break;
    case U_EMR_TRANSPARENTBLT:
        rsize = U_SIZE_EMRTRANSPARENTBLT;
        break;
    case U_EMR_UNDEF117:
        rsize = U_SIZE_EMRUNDEFINED;
        break;
    case U_EMR_GRADIENTFILL:
        ...
        break;
    //       case U_EMR_SETLINKEDUFIS:           rsize =
    //       U_SIZE_EMRSETLINKEDUFIS;           break;
    //       case U_EMR_SETTEXTJUSTIFICATION:    rsize =
    //       U_SIZE_EMRSETTEXTJUSTIFICATION;    break;
    case U_EMR_COLORMATCHTOTARGETW:
        rsize = U_SIZE_EMRCOLORMATCHTOTARGETW;
        break;
    case U_EMR_CREATECOLORSPACEW:
        rsize = U_SIZE_EMRCREATECOLORSPACEW;
        break;
    default:
        rsize = U_SIZE_EMRNOTIMPLEMENTED;
        break;
    } // end of switch
    //  record's declared size must be more than the minimum size for this type
    //  of record
    if (*nSize < rsize) {
        return (0);
    }
...
int U_emf_endian(char *contents, size_t length, int torev) {
    ...
    const char *blimit =
        contents +
        length; /* should never wrap since it describes a structure in memory */
    int rstatus = 1;
    ...
    while (OK) {
        ...
        switch (iType) {
        case U_EMR_HEADER:
            rstatus = U_EMRHEADER_swap(record, torev);
            break;
        case U_EMR_POLYBEZIER:
            rstatus = U_EMRPOLYBEZIER_swap(record, torev);
            break;
        case U_EMR_POLYGON:
            rstatus = U_EMRPOLYGON_swap(record, torev);
            break;
        case U_EMR_POLYLINE:
            rstatus = U_EMRPOLYLINE_swap(record, torev);
            break;
        case U_EMR_POLYBEZIERTO:
        ...
        case U_EMR_POLYLINETO:
            ...
            break;
        case U_EMR_POLYPOLYLINE:
            rstatus = U_EMRPOLYPOLYLINE_swap(record, torev);
            break;
        case U_EMR_POLYPOLYGON:
            rstatus = U_EMRPOLYPOLYGON_swap(record, torev);
            break;
        case U_EMR_SETWINDOWEXTEX:
            rstatus = U_EMRSETWINDOWEXTEX_swap(record, torev);
            break;
        case U_EMR_SETWINDOWORGEX:
            rstatus = U_EMRSETWINDOWORGEX_swap(record, torev);
            break;
        case U_EMR_SETVIEWPORTEXTEX:
            rstatus = U_EMRSETVIEWPORTEXTEX_swap(record, torev);
            break;
        case U_EMR_SETVIEWPORTORGEX:
            rstatus = U_EMRSETVIEWPORTORGEX_swap(record, torev);
            break;
        case U_EMR_SETBRUSHORGEX:
            rstatus = U_EMRSETBRUSHORGEX_swap(record, torev);
            break;
        case U_EMR_EOF:
            ...
            break;
        case U_EMR_SETPIXELV:
            rstatus = U_EMRSETPIXELV_swap(record, torev);
            break;
        case U_EMR_SETMAPPERFLAGS:
            rstatus = U_EMRSETMAPPERFLAGS_swap(record, torev);
            break;
        case U_EMR_SETMAPMODE:
            rstatus = U_EMRSETMAPMODE_swap(record, torev);
            break;
        case U_EMR_SETBKMODE:
            rstatus = U_EMRSETBKMODE_swap(record, torev);
            break;
        case U_EMR_SETPOLYFILLMODE:
            rstatus = U_EMRSETPOLYFILLMODE_swap(record, torev);
            break;
        case U_EMR_SETROP2:
            rstatus = U_EMRSETROP2_swap(record, torev);
            break;
        case U_EMR_SETSTRETCHBLTMODE:
            rstatus = U_EMRSETSTRETCHBLTMODE_swap(record, torev);
            break;
        case U_EMR_SETTEXTALIGN:
            rstatus = U_EMRSETTEXTALIGN_swap(record, torev);
            break;
        case U_EMR_SETCOLORADJUSTMENT:
            rstatus = U_EMRSETCOLORADJUSTMENT_swap(record, torev);
            break;
        case U_EMR_SETTEXTCOLOR:
            rstatus = U_EMRSETTEXTCOLOR_swap(record, torev);
            break;
        case U_EMR_SETBKCOLOR:
            rstatus = U_EMRSETBKCOLOR_swap(record, torev);
            break;
        case U_EMR_OFFSETCLIPRGN:
            rstatus = U_EMROFFSETCLIPRGN_swap(record, torev);
            break;
        case U_EMR_MOVETOEX:
            rstatus = U_EMRMOVETOEX_swap(record, torev);
            break;
        case U_EMR_SETMETARGN:
            rstatus = U_EMRSETMETARGN_swap(record, torev);
            break;
        case U_EMR_EXCLUDECLIPRECT:
            rstatus = U_EMREXCLUDECLIPRECT_swap(record, torev);
            break;
        case U_EMR_INTERSECTCLIPRECT:
            rstatus = U_EMRINTERSECTCLIPRECT_swap(record, torev);
            break;
        case U_EMR_SCALEVIEWPORTEXTEX:
            rstatus = U_EMRSCALEVIEWPORTEXTEX_swap(record, torev);
            break;
        case U_EMR_SCALEWINDOWEXTEX:
            rstatus = U_EMRSCALEWINDOWEXTEX_swap(record, torev);
            break;
        case U_EMR_SAVEDC:
            rstatus = U_EMRSAVEDC_swap(record, torev);
            break;
        case U_EMR_RESTOREDC:
            rstatus = U_EMRRESTOREDC_swap(record, torev);
            break;
        case U_EMR_SETWORLDTRANSFORM:
            rstatus = U_EMRSETWORLDTRANSFORM_swap(record, torev);
            break;
        case U_EMR_MODIFYWORLDTRANSFORM:
            rstatus = U_EMRMODIFYWORLDTRANSFORM_swap(record, torev);
            break;
        case U_EMR_SELECTOBJECT:
            rstatus = U_EMRSELECTOBJECT_swap(record, torev);
            break;
        case U_EMR_CREATEPEN:
            rstatus = U_EMRCREATEPEN_swap(record, torev);
            break;
        case U_EMR_CREATEBRUSHINDIRECT:
            rstatus = U_EMRCREATEBRUSHINDIRECT_swap(record, torev);
            break;
        case U_EMR_DELETEOBJECT:
            rstatus = U_EMRDELETEOBJECT_swap(record, torev);
            break;
        case U_EMR_ANGLEARC:
            rstatus = U_EMRANGLEARC_swap(record, torev);
            break;
        case U_EMR_ELLIPSE:
            rstatus = U_EMRELLIPSE_swap(record, torev);
            break;
        case U_EMR_RECTANGLE:
            rstatus = U_EMRRECTANGLE_swap(record, torev);
            break;
        case U_EMR_ROUNDRECT:
            rstatus = U_EMRROUNDRECT_swap(record, torev);
            break;
        case U_EMR_ARC:
            rstatus = U_EMRARC_swap(record, torev);
            break;
        case U_EMR_CHORD:
            rstatus = U_EMRCHORD_swap(record, torev);
            break;
        case U_EMR_PIE:
            rstatus = U_EMRPIE_swap(record, torev);
            break;
        case U_EMR_SELECTPALETTE:
            rstatus = U_EMRSELECTPALETTE_swap(record, torev);
            break;
        case U_EMR_CREATEPALETTE:
            rstatus = U_EMRCREATEPALETTE_swap(record, torev);
            break;
        case U_EMR_SETPALETTEENTRIES:
            rstatus = U_EMRSETPALETTEENTRIES_swap(record, torev);
            break;
        case U_EMR_RESIZEPALETTE:
            rstatus = U_EMRRESIZEPALETTE_swap(record, torev);
            break;
        case U_EMR_REALIZEPALETTE:
            rstatus = U_EMRREALIZEPALETTE_swap(record, torev);
            break;
        case U_EMR_EXTFLOODFILL:
            rstatus = U_EMREXTFLOODFILL_swap(record, torev);
            break;
        case U_EMR_LINETO:
            rstatus = U_EMRLINETO_swap(record, torev);
            break;
        case U_EMR_ARCTO:
            rstatus = U_EMRARCTO_swap(record, torev);
            break;
        case U_EMR_POLYDRAW:
            rstatus = U_EMRPOLYDRAW_swap(record, torev);
            break;
        case U_EMR_SETARCDIRECTION:
            rstatus = U_EMRSETARCDIRECTION_swap(record, torev);
            break;
        case U_EMR_SETMITERLIMIT:
            rstatus = U_EMRSETMITERLIMIT_swap(record, torev);
            break;
        case U_EMR_BEGINPATH:
            rstatus = U_EMRBEGINPATH_swap(record, torev);
            break;
        case U_EMR_ENDPATH:
            rstatus = U_EMRENDPATH_swap(record, torev);
            break;
        case U_EMR_CLOSEFIGURE:
            rstatus = U_EMRCLOSEFIGURE_swap(record, torev);
            break;
        case U_EMR_FILLPATH:
            rstatus = U_EMRFILLPATH_swap(record, torev);
            break;
        case U_EMR_STROKEANDFILLPATH:
            rstatus = U_EMRSTROKEANDFILLPATH_swap(record, torev);
            break;
        case U_EMR_STROKEPATH:
            rstatus = U_EMRSTROKEPATH_swap(record, torev);
            break;
        case U_EMR_FLATTENPATH:
            rstatus = U_EMRFLATTENPATH_swap(record, torev);
            break;
        case U_EMR_WIDENPATH:
            rstatus = U_EMRWIDENPATH_swap(record, torev);
            break;
        case U_EMR_SELECTCLIPPATH:
            rstatus = U_EMRSELECTCLIPPATH_swap(record, torev);
            break;
        case U_EMR_ABORTPATH:
            rstatus = U_EMRABORTPATH_swap(record, torev);
            break;
        case U_EMR_UNDEF69:
            rstatus = U_EMRUNDEF69_swap(record, torev);
            break;
        case U_EMR_COMMENT:
            rstatus = U_EMRCOMMENT_swap(record, torev);
            break;
        case U_EMR_FILLRGN:
            rstatus = U_EMRFILLRGN_swap(record, torev);
            break;
        case U_EMR_FRAMERGN:
            rstatus = U_EMRFRAMERGN_swap(record, torev);
            break;
        case U_EMR_INVERTRGN:
            rstatus = U_EMRINVERTRGN_swap(record, torev);
            break;
        case U_EMR_PAINTRGN:
            rstatus = U_EMRPAINTRGN_swap(record, torev);
            break;
        case U_EMR_EXTSELECTCLIPRGN:
            rstatus = U_EMREXTSELECTCLIPRGN_swap(record, torev);
            break;
        case U_EMR_BITBLT:
            rstatus = U_EMRBITBLT_swap(record, torev);
            break;
        case U_EMR_STRETCHBLT:
            rstatus = U_EMRSTRETCHBLT_swap(record, torev);
            break;
        case U_EMR_MASKBLT:
            rstatus = U_EMRMASKBLT_swap(record, torev);
            break;
        case U_EMR_PLGBLT:
            rstatus = U_EMRPLGBLT_swap(record, torev);
            break;
        case U_EMR_SETDIBITSTODEVICE:
            rstatus = U_EMRSETDIBITSTODEVICE_swap(record, torev);
            break;
        case U_EMR_STRETCHDIBITS:
            rstatus = U_EMRSTRETCHDIBITS_swap(record, torev);
            break;
        case U_EMR_EXTCREATEFONTINDIRECTW:
            rstatus = U_EMREXTCREATEFONTINDIRECTW_swap(record, torev);
            break;
        case U_EMR_EXTTEXTOUTA:
            rstatus = U_EMREXTTEXTOUTA_swap(record, torev);
            break;
        case U_EMR_EXTTEXTOUTW:
            rstatus = U_EMREXTTEXTOUTW_swap(record, torev);
            break;
        case U_EMR_POLYBEZIER16:
            rstatus = U_EMRPOLYBEZIER16_swap(record, torev);
            break;
        case U_EMR_POLYGON16:
            rstatus = U_EMRPOLYGON16_swap(record, torev);
            break;
        case U_EMR_POLYLINE16:
            rstatus = U_EMRPOLYLINE16_swap(record, torev);
            break;
        case U_EMR_POLYBEZIERTO16:
            rstatus = U_EMRPOLYBEZIERTO16_swap(record, torev);
            break;
        case U_EMR_POLYLINETO16:
            rstatus = U_EMRPOLYLINETO16_swap(record, torev);
            break;
        case U_EMR_POLYPOLYLINE16:
            rstatus = U_EMRPOLYPOLYLINE16_swap(record, torev);
            break;
        case U_EMR_POLYPOLYGON16:
            rstatus = U_EMRPOLYPOLYGON16_swap(record, torev);
            break;
        case U_EMR_POLYDRAW16:
            rstatus = U_EMRPOLYDRAW16_swap(record, torev);
            break;
        case U_EMR_CREATEMONOBRUSH:
            rstatus = U_EMRCREATEMONOBRUSH_swap(record, torev);
            break;
        case U_EMR_CREATEDIBPATTERNBRUSHPT:
            rstatus = U_EMRCREATEDIBPATTERNBRUSHPT_swap(record, torev);
            break;
        case U_EMR_EXTCREATEPEN:
            rstatus = U_EMREXTCREATEPEN_swap(record, torev);
            break;
        case U_EMR_POLYTEXTOUTA:
            rstatus = U_EMRPOLYTEXTOUTA_swap(record, torev);
            break;
        case U_EMR_POLYTEXTOUTW:
            rstatus = U_EMRPOLYTEXTOUTW_swap(record, torev);
            break;
        case U_EMR_SETICMMODE:
            rstatus = U_EMRSETICMMODE_swap(record, torev);
            break;
        case U_EMR_CREATECOLORSPACE:
            rstatus = U_EMRCREATECOLORSPACE_swap(record, torev);
            break;
        case U_EMR_SETCOLORSPACE:
            rstatus = U_EMRSETCOLORSPACE_swap(record, torev);
            break;
        case U_EMR_DELETECOLORSPACE:
            rstatus = U_EMRDELETECOLORSPACE_swap(record, torev);
            break;
        case U_EMR_GLSRECORD:
            rstatus = U_EMRGLSRECORD_swap(record, torev);
            break;
        case U_EMR_GLSBOUNDEDRECORD:
            rstatus = U_EMRGLSBOUNDEDRECORD_swap(record, torev);
            break;
        case U_EMR_PIXELFORMAT:
            rstatus = U_EMRPIXELFORMAT_swap(record, torev);
            break;
        case U_EMR_DRAWESCAPE:
        ...
        case U_EMR_EXTESCAPE:
            ...
            break;
        case U_EMR_UNDEF107:
            rstatus = U_EMRUNDEF107_swap(record, torev);
            break;
        case U_EMR_SMALLTEXTOUT:
            rstatus = U_EMRSMALLTEXTOUT_swap(record, torev);
            break;
        case U_EMR_FORCEUFIMAPPING:
            rstatus = U_EMRFORCEUFIMAPPING_swap(record, torev);
            break;
        case U_EMR_NAMEDESCAPE:
            rstatus = U_EMRNAMEDESCAPE_swap(record, torev);
            break;
        case U_EMR_COLORCORRECTPALETTE:
            rstatus = U_EMRCOLORCORRECTPALETTE_swap(record, torev);
            break;
        case U_EMR_SETICMPROFILEA:
            rstatus = U_EMRSETICMPROFILEA_swap(record, torev);
            break;
        case U_EMR_SETICMPROFILEW:
            rstatus = U_EMRSETICMPROFILEW_swap(record, torev);
            break;
        case U_EMR_ALPHABLEND:
            rstatus = U_EMRALPHABLEND_swap(record, torev);
            break;
        case U_EMR_SETLAYOUT:
            rstatus = U_EMRSETLAYOUT_swap(record, torev);
            break;
        case U_EMR_TRANSPARENTBLT:
            rstatus = U_EMRTRANSPARENTBLT_swap(record, torev);
            break;
        case U_EMR_UNDEF117:
            rstatus = U_EMRUNDEF117_swap(record, torev);
            break;
        case U_EMR_GRADIENTFILL:
            rstatus = U_EMRGRADIENTFILL_swap(record, torev);
            break;
        case U_EMR_SETLINKEDUFIS:
            rstatus = U_EMRSETLINKEDUFIS_swap(record, torev);
            break;
        case U_EMR_SETTEXTJUSTIFICATION:
            rstatus = U_EMRSETTEXTJUSTIFICATION_swap(record, torev);
            break;
        case U_EMR_COLORMATCHTOTARGETW:
            rstatus = U_EMRCOLORMATCHTOTARGETW_swap(record, torev);
            break;
        case U_EMR_CREATECOLORSPACEW:
            rstatus = U_EMRCREATECOLORSPACEW_swap(record, torev);
            break;
        default:
            rstatus = U_EMRNOTIMPLEMENTED_swap(record, torev);
            break;
        } // end of switch
        if (!rstatus) {
            return (rstatus);
