/**
  ...
  EMF file Record structure information has been derived from Mingw, Wine, and
  libEMF header files, and from
...
\section ov Overview
Microsoft's WMF, EMF, and EMF+ metafile types are supported.  In each case
functions are provided for reading, constructing, writing, and printing
metafile records.  The methods used to do that differ somewhat between
metafiles, and the simplest
way to get started is to have a look at the example programs provided.  The WMF,
EMF, and EMF+ structs and functions are
marked with U_WMF, U_EMF, U_PMF and U_WMR, U_EMR, and U_PMR prefixes.  (PMF
because "+" is a reserved character
in many contexts, so U_EMF+NAME would be a problem.)  Please be aware that
normally both EMF and EMF+ files have the ".emf"
file extension, and that it is very common for such files to contain both an EMF
and an EMF+ representation of the
drawing.
...
emf-inout.cpp.example Example code from Inkscape to convert graphics from EMF to
SVG.\n
emf-print.cpp.example Example code from Inkscape to print a drawing to EMF.\n
wmf-inout.cpp.example Example code from Inkscape to convert graphics from WMF to
SVG.\n
wmf-print.cpp.example Example code from Inkscape to print a drawing to WMF.

\section doxy_limits Documentation issues
There are currently some unresolved issues with Doxygen that result in some
structs
not being "defined".  This comes up when several different types of structs have
the same
layout.  When this occurs the first one listed on the "typedef struct" is
defined but all the
others will only be shown under "typedef struct" referring to the first one.
This is why
clicking on U_RECTL in a function parameter jumps to a typedef struct page,  why
U_RECTL is shown
as plain text here, but U_RECT is shown as a link here, and clicking on it jumps
directly
to its structure definition.

An additional issue is that the Enumeration names used in WMF are different from
those
used in EMF, even when the values are either identical or differ only slightly,
and no method
has been found yet to link one to the other in Doxygen.  At present the only way
to look up
these WMF enumerations is by referencing the following table:

        EMF                                        WMF WMF Manual
        EMF Binary Raster Operation Enumeration    BinaryRasterOperation
Enumeration                2.1.1.2
...
2.1.1.5
        EMF DIBColors Enumeration                  ColorUsage Enumeration  [has
1 extra value]      2.1.1.6
        EMF BI_Compression Enumeration             Compression Enumeration [has
3 extra values]     2.1.1.7
...
2.1.1.10
        EMF LCS_Intent Enumeration                 GamutMappingIntent
Enumeration                   2.1.1.11
...
2.1.1.14
        EMF Profile Enumeration                    LogicalColorSpaceV5
Enumeration                  2.1.1.15
...
2.1.1.22
        EMF PenStyle Enumeration                   PenStyle Enumeration [not
values >0xFFFF]        2.1.1.23
        -                                          PitchFont Enumeration
2.1.1.24
        EMF PolygonFillMode Enumeration            PolyFillMode Enumeration
[first 2 only]          2.1.1.25
        -                                          PostScriptCap Enumeration
2.1.1.26
        -                                          PostScriptClipping
Enumeration                   2.1.1.27
        -                                          PostFeatureSetting
Enumeration                   2.1.1.28
        -                                          PostScrioptJoin Enumeration
2.1.1.29
        EMF StretchMode Enumeration                StretchMode Enumeration
2.1.1.30
        EMF Ternary Raster Operation Enumeration   TernaryRasterOperation
Enumeration              2.1.1.31
        EMF LF_ClipPrecision Enumeration           ClipPrecision Flags 2.1.2.1
        EMF ExtTextOutOptions Enumeration          ExtTextOutOptions Flags
[subset]                 2.1.2.2
        EMF TextAlignment Enumeration              TextAlignment Enumeration
2.1.2.3
        EMF TextAlignment Enumeration              VertialTextAlignment
Enumeration                 2.1.2.4
...
#define U_RGB_GAMMA_MIN                                                        \
    (uint16_t)02500 //!< For U_COLORADJUSTMENT ca[Red|Green|Blue]Gamma fields
#define U_RGB_GAMMA_MAX                                                        \
    (uint16_t)65000 //!< For U_COLORADJUSTMENT ca[Red|Green|Blue]Gamma fields
#define U_REFERENCE_WHITE_MIN                                                  \
    (uint16_t)6000 //!< For U_COLORADJUSTMENT caReferenceWhite field
#define U_REFERENCE_WHITE_MAX                                                  \
    (uint16_t)10000 //!< For U_COLORADJUSTMENT caReferenceWhite field
#define U_REFERENCE_BLACK_MIN                                                  \
    (uint16_t)0 //!< For U_COLORADJUSTMENT caReferenceBlack field
#define U_REFERENCE_BLACK_MAX                                                  \
    (uint16_t)4000 //!< For U_COLORADJUSTMENT caReferenceBlack field
#define U_COLOR_ADJ_MIN ((int16_t)-100) //!< For U_COLORADJUSTMENT
//! ca[Contrast|Brightness|Colorfulness|RedGreenTint]
//! fields
#define U_COLOR_ADJ_MAX (int16_t)100 //!< For U_COLORADJUSTMENT
//! ca[Contrast|Brightness|Colorfulness|RedGreenTint] fields
...
#define U_ROW_ORDER_INVERT                                                     \
    1 //!< For RGBA_to_DIB, invert row order in DIB relative to pixel array
#define U_ROW_ORDER_SAME                                                       \
    0 //!< For RGBA_to_DIB, same   row order in DIB as in pixel array
#define U_CT_NO 0 //!< For RGBA_to_DIB, do not use color table
#define U_CT_BGRA                                                              \
    1 //!< For RGBA_to_DIB, use color table (16 bits or less only) BGRA colors,
      //! compatible with EMF+ ARGB
#define U_CT_ARGB                                                              \
    1 //!< For RGBA_to_DIB, use color table (16 bits or less only) BGRA colors,
      //! compatible with EMF+ ARGB
#define U_EMR_COMMENT_SPOOLFONTDEF                                             \
    0x544F4E46 //!< For U_EMRCOMMENT record that is U_EMR_COMMENT_SPOOL, comment
               //! holds font definition informtion.
/** Solaris 8 has problems with round/roundf, just use this everywhere  */
#define U_ROUND(A)                                                             \
    ((A) > 0 ? floor((A) + 0.5) : ((A) < 0 ? -floor(-(A) + 0.5) : (A)))
...
*/
#define IS_MEM_UNSAFE(A, B, C)                                                 \
    ((sizeof(B) < sizeof(int) || (int)(B) < 0)                                 \
         ? 1                                                                   \
         : ((int8_t *)(A) > (int8_t *)(C)                                      \
                ? 1                                                            \
                : ((int8_t *)(C) - (int8_t *)(A) >= (int)(B)                   \
                       ? 0                                                     \
                       : 1))) //!< Return
                              //! 1 when a
//! region of
...
//! starting
//! at
//! A of B bytes extends beyond pointer C
...
typedef uint32_t
    U_CBBITSMSK; //!< Count of Bytes in object at corresponding U_OFF*
typedef uint32_t
    U_CBBITSSRC;          //!< Count of Bytes in object at corresponding U_OFF*
typedef uint32_t U_CBBMI; //!< Count of Bytes in object at corresponding U_OFF*
typedef uint32_t
    U_CBBMIMSK; //!< Count of Bytes in object at corresponding U_OFF*
typedef uint32_t
    U_CBBMISRC;            //!< Count of Bytes in object at corresponding U_OFF*
...
typedef uint32_t
    U_CBPLENTRIES; //!< Count of Bytes in object at corresponding U_OFF*
typedef uint32_t
    U_CBPXLFMT; //!< Count of Bytes in object at corresponding U_OFF*
typedef uint32_t
    U_CBRGNDATA;          //!< Count of Bytes in object at corresponding U_OFF*
typedef uint32_t U_CBSTR; //!< Count of Bytes in an 8 or 16 bit string

typedef uint32_t U_OFFBITS; //!< Byte offset to TYPE, always measured from the
                            //! start of the RECORD (not the struct)
typedef uint32_t U_OFFBITSMSK; //!< Byte offset to TYPE, always measured from
                               //! the start of the RECORD (not the struct)
typedef uint32_t U_OFFBITSSRC; //!< Byte offset to TYPE, always measured from
                               //! the start of the RECORD (not the struct)
typedef uint32_t U_OFFBMI;    //!< Byte offset to TYPE, always measured from the
                              //! start of the RECORD (not the struct)
typedef uint32_t U_OFFBMIMSK; //!< Byte offset to TYPE, always measured from the
                              //! start of the RECORD (not the struct)
typedef uint32_t U_OFFBMISRC; //!< Byte offset to TYPE, always measured from the
                              //! start of the RECORD (not the struct)
typedef uint32_t U_OFFDATA;   //!< Byte offset to TYPE, always measured from the
                              //! start of the RECORD (not the struct)
typedef uint32_t U_OFFDESC;   //!< Byte offset to TYPE, always measured from the
                              //! start of the RECORD (not the struct)
typedef uint32_t U_OFFDX;     //!< Byte offset to TYPE, always measured from the
                              //! start of the RECORD (not the struct)
typedef uint32_t U_OFFPLENTRIES; //!< Byte offset to TYPE, always measured from
                                 //! the start of the RECORD (not the struct)
typedef uint32_t U_OFFPXLFMT; //!< Byte offset to TYPE, always measured from the
                              //! start of the RECORD (not the struct)
typedef uint32_t
    U_OFFSTR; //!< Byte offset to string of either 8 or 16 bit characters
typedef uint8_t U_DATA; //!< any binary sort of data, not otherwise classified.

//  "Types" For array components in structures, where not otherwise defined as a
//  structure
...
typedef uint32_t U_NUM_GRADOBJ;   //!< Number of U_GRADIENT4 OR U_GRADIENT3
                                  //!(determined at run time)
typedef uint32_t
    U_NUM_RGBQUAD; //!< Number of U_RGBQUAD (in bmciColors in U_BITMAPCOREINFO)

/* ************************ WMF pieces used in EMF or EMF+
 * ****************************** */

/** \defgroup U_EMF_EMRSETROP2_iMode_Qualifiers EMF Binary Raster Operation
  Enumeration
  ...
  These codes specify:
    1.  an order of operands (composed of various orders and combinations of:
  Dest, Pen)
        (There are 2, hence "Binary Raster Operation")
    2.  an order of operators to apply to the operands (composed of Not, Xor,
  Or, And)
  Only a few of the more common operations are provided here.

  The default is U_R2_COPYPEN.  If this value is changed to something else all
  subsequenty
  draw operations will use the altered logic.  For instance, if it is set to
  U_R2_BLACK and
...
/** \defgroup U_EMF_BITMAPINFOHEADER_biBitCount_Qualifiers EMF BitCount
  Enumeration
...
#define U_BCBM_COLOR16                                                         \
    16 //!< 2^16 colors. bmiColors is not used. Pixels are 5 bits B,G,R with 1
       //! unused bit
#define U_BCBM_COLOR24                                                         \
    24 //!< 2^24 colors. bmiColors is not used. Pixels are U_RGBTRIPLE.
#define U_BCBM_COLOR32                                                         \
    32 //!< 2^32 colors. bmiColors is not used. Pixels are U_RGBQUAD.  Also use
       //! for EMF+ ARGB
       /** @} */

/** \defgroup U_EMF_BITMAPINFOHEADER_biCompression_Qualifiers EMF BI_Compression
  Enumeration
...
*/
#define U_BI_UNKNOWN                                                           \
    -1             //!< not defined in EMF standard, not to be used in EMF files
...
/** \defgroup U_EMF_LOGCOLORSPACE_lcsIntent_Qualifiers EMF LCS_Intent
  Enumeration
...
/** \defgroup U_EMF_LOGCOLORSPACE_lcsCSType_Qualifiers EMF LCS_CSType
  Enumeration
...
/** \defgroup U_EMF_EMR_dwROP_Qualifiers EMF Ternary Raster Operation
  enumeration
  ...
  Or, And)
  Only a few of the more common operations are provided here.
  When the Operation does not use a Src operand the corresponding source bitmap
  may be
...
#define U_NOOP                                                                 \
    0x00aa0029 //!< Many GDI programs end with a bitblt with this ROP == "D".
               //! Seems to work like flush()
#define U_NOMIRRORBITMAP                                                       \
    0x80000000 //!< If bit set, disable horizontal reflection of bitmap.
               /** @} */

/** \defgroup U_EMF_EMRSETTEXTALIGN_iMode_Qualifiers EMF TextAlignment
  Enumeration
  ...
  Recall that EMF coordinates have UL  closest to {0,0}, LR is below and to the
  right of UL and so has LARGER
  {x,y} coordinates.  In the following "TOP" is on the horizontal line defined
  by LR, as it has larger y coordinates,
  which when viewing the EMF file, would actually be on the BOTTOM of the
  bounding rectangle.  Similarly, left and right
  are reversed.

  Microsoft documentation (WMF  manual, section 2.1.2.3) says that the text
  starts on certain edges of the bounding rectangle.
  That is apparently not true, whether the bounding rectangle is {0,0,-1,-1},
  which is effectively no bounding rectangle,
  or if a valid bounding rectangle is specified.  In all cases the text (in
  Windows XP Preview) starts, has center at, or ends
  at the center point.  Vertical offsets seem to be defined analogously, but
  with respect to the height of the font.  The bounding
  rectangle defined for the U_EMRTEXT record appears to be ignored.

  Microsoft documentation (EMF  manual,section 2.2.5) says that the same
  rectangle is used for "clipping or opaquing" by ExtTextOutA/W.
...
#define U_TA_LEFT                                                              \
    0x00 //!< Reference point is on left edge of bounding rectangle
#define U_TA_RIGHT                                                             \
    0x02 //!< Reference point is on right edge of bounding rectangle
#define U_TA_CENTER                                                            \
    0x06 //!< Reference point is on center vertical line of bounding rectangle
#define U_TA_TOP 0x00 //!< Reference point is on top edge of bounding rectangle
#define U_TA_BOTTOM                                                            \
    0x08 //!< Reference point is on bottom edge of bounding rectangle
#define U_TA_BASEBIT                                                           \
    0x10 //!< Reference point is on baseline of text if this bit is set, for
         //! 0x10 <-> 0x18
#define U_TA_BASELINE 0x18 //!< Reference point is on baseline of text
#define U_TA_RTLREADING                                                        \
    0x100 //!< Set for Right to Left languages like Hebrew and Arabic
#define U_TA_MASK                                                              \
    U_TA_BASELINE + U_TA_CENTER + U_TA_UPDATECP +                              \
        U_TA_RTLREADING //!< Mask for these bits
//  Vertical text flags
...
typedef struct {
    ...
    //!<   biHeight > 0 origin is LL corner, may be compressed, this is height
    //! after decompression.
    //!<   biHeight < 0 origin is UL corner, may not be compressed
    uint16_t biPlanes; //!< Planes (must be 1)
    uint16_t
        biBitCount; //!< BitCount Enumeration (determines number of RBG colors)
    uint32_t biCompression;  //!< BI_Compression Enumeration
    uint32_t biSizeImage;    //!< Image size in bytes or 0 = "default size
    ...
    U_NUM_RGBQUAD biClrUsed; //!< Number of bmciColors in
                             //! U_BITMAPINFO/U_BITMAPCOREINFO that are used by
    //! the bitmap
    uint32_t biClrImportant; //!< Number of bmciColors needed (0 means all).
} U_BITMAPINFOHEADER, *PU_BITMAPINFOHEADER; //!< WMF manual 2.2.2.3
...
} U_CIEXYZ, *PU_CIEXYZ; //!< WMF manual 2.2.2.6
...
} U_CIEXYZTRIPLE, *PU_CIEXYZTRIPLE; //!< WMF manual 2.2.2.7
...
} U_COLORREF, *PU_COLORREF; //!< WMF manual 2.2.2.8
...
} U_LCS_GAMMA, *PU_LCS_GAMMA; //!< WMF manual 2.2.2.11
...
} U_LCS_GAMMARGB, *PU_LCS_GAMMARGB; //!< WMF manual 2.2.2.11
...
typedef struct {
    ...
    char lcsFilename[U_MAX_PATH]; //!< Names an external color profile file,
                                  //! otherwise empty string
} U_LOGCOLORSPACEA, *PU_LOGCOLORSPACEA; //!< WMF manual 2.2.2.11
...
typedef struct {
    ...
    U_CIEXYZTRIPLE lcsEndpoints;      //!< CIE XYZ color space endpoints
    U_LCS_GAMMARGB lcsGammaRGB;       //!< Gamma For RGB
    uint16_t lcsFilename[U_MAX_PATH]; //!< Could name an external color profile
                                      //! file, otherwise empty string
} U_LOGCOLORSPACEW, *PU_LOGCOLORSPACEW; //!< WMF manual 2.2.2.12
...
} U_POINT16, *PU_POINT16; //!< WMF manual 2.2.2.16
...
typedef struct {
    ...
    uint8_t Red;          //!< Red   color (0-255)
    uint8_t Reserved;     //!< Not used
} U_RGBQUAD, *PU_RGBQUAD; //!< WMF manual 2.2.2.20
...
/** \defgroup U_EMF_FONT_STRUCT_WIDTHS EMF Font name and style widths in
  characters
  For U_LOGFONT and U_LOGFONT_PANOSE,
  @{
*/
#define U_LF_FACESIZE                                                          \
    32 //!< U_LOGFONT lfFaceName and U_LOGFONT_PANOSE elfStyle fields maximum
       //! width
#define U_LF_FULLFACESIZE                                                      \
    64 //!< U_LOGFONT_PANOSE elfFullName field maximum width
       /** @} */
...
#define U_EMR_CREATEDIBPATTERNBRUSHPT                                          \
    94                                 //!< U_EMRCREATEDIBPATTERNBRUSHPT record
...
#define U_DRAW_ALTERS                                                          \
    0x020 //!< Alters draw parameters (pen, brush, coordinates...)
#define U_DRAW_PATH                                                            \
    0x040 //!< An explicit path is being used (with a BEGIN and END)
#define U_DRAW_TEXT                                                            \
    0x080 //!< Current record forces all pending text to be drawn first.
#define U_DRAW_OBJECT 0x100 //!< Creates an Object (only used in WMF)
#define U_DRAW_NOFILL                                                          \
    0x200 //!< Object is not fillable (lines and arc, only used in WMF)
...
/** \defgroup U_EMF_COLORADJUSTMENT_caFlags_Qualifiers EMF ColorAdjustment
  Enumeration
...
/** \defgroup U_EMF_EMRCOLORMATCHTOTARGETW_dwFlags_Qualifiers EMF
  ...
  @{
*/
#define U_COLORMATCHTOTARGET_NOTEMBEDDED                                       \
...
#define U_EMR_COMMENT_BEGINGROUP                                               \
    0x00000002 //!< Comment begins group of EMF records
#define U_EMR_COMMENT_ENDGROUP 0x00000003 //!< Comment ends group of EMF records
#define U_EMR_COMMENT_MULTIFORMATS                                             \
    0x40000004 //!< Comment contains some other representation of drawing
#define U_EMR_COMMENT_UNICODE_STRING 0x00000040 //!< Reserved
#define U_EMR_COMMENT_UNICODE_END 0x00000080    //!< Reserved
                                                /** @} */

/** \defgroup U_EMF_EMRTEXT_foptions_Qualifiers EMF ExtTextOutOptions
  Enumeration
...
#define U_ETO_GLYPH_INDEX                                                      \
    0x00000010 //!< Characters are glyph indices for the font.
#define U_ETO_RTLREADING 0x00000080 //!< Right to left text.
#define U_ETO_NO_RECT 0x00000100    //!< No bounding rectangle is specified.
#define U_ETO_SMALL_CHARS                                                      \
    0x00000200 //!< 8 bit characters instead of 16 bit.  For EMRSMALLTEXTOUT
               //! ONLY, does not affect EMRTEXTOUTA or EMRTEXTOUTW
...
#define U_ETO_IGNORELANGUAGE                                                   \
    0x00001000 //!< Process Right to Left languages exactly as specified in the
               //! metafile.
#define U_ETO_PDY                                                              \
    0x00002000 //!< Both horizontal and vertical displacements are provided.
...
/** \defgroup U_EMF_EMREXTFLOODFILL_iMode_Qualifiers EMF FloodFill Enumeration
  ...
  @{
*/
#define U_FLOODFILLBORDER                                                      \
    0x00000000 //!< Color specified must be the same as the border - brush fill
               //! stops at this color
#define U_FLOODFILLSURFACE                                                     \
    0x00000001 //!< Color specified must be different from the border - brush
               //! fills only this color
               /** @} */
...
/** \defgroup U_EMF_DESIGNVECTOR_Signature_Qualifiers EMF Signature Enumeration
  ...
  @{
*/
#define U_ENHMETA_SIGNATURE                                                    \
    0x464D4520 //!< "EMF" signature also for U_EMRHEADER dSignature field.
#define U_EPS_SIGNATURE                                                        \
    0x46535045 //!< "FSPE" signature, indicates encapsulated postscript.
               /** @} */

/** \defgroup U_EMF_EMRGRADIENTFILL_ulMode_Qualifiers EMF GradientFill
  Enumeration
...
#define U_GRADIENT_FILL_TRIANGLE                                               \
    0x00000002 //!< Gradient is between 3 vertices of a triangle.
               /** @} */

/** \defgroup U_EMF_EMREXTTEXTOUT_iGraphicsMode_Qualifiers EMF GraphicsMode
  Enumeration
  ...
  @{
*/
#define U_GM_COMPATIBLE                                                        \
    1 //!< TrueType text ignores world to device transform except for Scale.
      //! Arcs ignore transform
#define U_GM_ADVANCED                                                          \
    2 //!< TrueType text and Arcs must conform to all of world to device
      //! transform.
...
/** \defgroup U_EMF_COLORADJUSTMENT_caIlluminantIndex_Qualifiers EMF Illuminant
  Enumeration
...
/** \defgroup U_EMF_EMRMODIFYWORLDTRANSFORM_iMode_Qualifiers EMF
...
#define U_PS_DASH                                                              \
    0x00000001 //!< Dashed line. This only works when NO other U_PS is set.
               //! Line width is minimum no matter what pen is set to.
#define U_PS_DOT                                                               \
    0x00000002 //!< Dotted line. This only works when NO other U_PS is set.
               //! Line width is minimum no matter what pen is set to.
#define U_PS_DASHDOT                                                           \
    0x00000003 //!< Dash-Dot line. This only works when NO other U_PS is set.
               //! Line width is minimum no matter what pen is set to.
#define U_PS_DASHDOTDOT                                                        \
    0x00000004 //!< Dash-Dot-Dot line. This only works when NO other U_PS is
               //! set.  Line width is minimum no matter what pen is set to.
#define U_PS_NULL 0x00000005 //!< Invisible line.
#define U_PS_INSIDEFRAME                                                       \
    0x00000006 //!< Draw line around drawing, then shrink drawing to fit within
               //! line taking its width into account.
#define U_PS_USERSTYLE 0x00000007 //!< User defined.
#define U_PS_ALTERNATE 0x00000008 //!< Every other pixel is drawn.
#define U_PS_STYLE_MASK                                                        \
    0x0000000f //!< Mask to select just the preceding line type fields.

#define U_PS_ENDCAP_ROUND                                                      \
    0x00000000 //!< Round end cap.  Only with U_PS_GEOMETRIC
#define U_PS_ENDCAP_SQUARE                                                     \
    0x00000100 //!< Square end cap.  Only with U_PS_GEOMETRIC
#define U_PS_ENDCAP_FLAT 0x00000200 //!< Flat end cap.  Only with U_PS_GEOMETRIC
#define U_PS_ENDCAP_MASK                                                       \
    0x00000f00 //!< Mask to select just the preceding ENDCAP fields.
...
#define U_PS_JOIN_MASK                                                         \
    0x0000f000 //!< Mask to select just the preceding JOIN fields.

#define U_PS_COSMETIC                                                          \
    0x00000000 //!< width may only be 1 pixel.  (If set higher it is still drawn
               //! as 1).
#define U_PS_GEOMETRIC                                                         \
    0x00010000 //!< width may be >1 pixel, but style may only be U_PS_SOLID or
               //! U_PS_NULL.
#define U_PS_TYPE_MASK                                                         \
    0x000f0000 //!< Mask to select just the preceding TYPE fields.
               /** @} */
...
/** \defgroup U_EMF_EMRSETPOLYFILLMODE_iMode_Qualifiers EMF PolygonFillMode
  Enumeration
...
#define U_RGN_AND                                                              \
    1 //!< Region becomes intersection of existing region and new region.
...
#define U_RGN_DIFF                                                             \
    4 //!< Region becomes part of existing region not in new region.
...
/** \defgroup U_EMF_EMRSELECTOBJECT_ihObject_Qualifiers EMF StockObject
  Enumeration
...
/** \defgroup U_EMF_EMRSETSTRETCHBLTMODE_iMode_Qualifiers EMF StretchMode
  Enumeration
...
/** \defgroup U_EMF_LOGFONT_lfOutPrecision_Qualifiers EMF LF_OutPrecision
  Enumeration
...
/** \defgroup U_EMF_LOGFONT_lfClipPrecision_Qualifiers EMF LF_ClipPrecision
  Enumeration
...
#define U_CLIP_STROKE_PRECIS                                                   \
    0x02 //!< (Source documentation is vague about what this means.)
#define U_CLIP_MASK 0x0F //!< MASK for bits in preceding.
#define U_CLIP_LH_ANGLES                                                       \
    0x10 //!< Set: font rotation by coordinate system, Clear: device fonts
         //!(only) rotate counterclockwise.
#define U_CLIP_TT_ALWAYS 0x20 //!< Reserved.
#define U_CLIP_EMBEDDED                                                        \
    0x80 //!< Font embedding is required.  (Method for doing so is not
         //! documented in EMF or WMF.)
         /** @} */
...
/** \defgroup U_EMF_LOGFONT_lfPitchAndFamily_Qualifiers EMF LF_PitchAndFamily
  Enumeration
...
/** \defgroup U_EMF_PIXELFORMATDESCRIPTOR_iLayerType_Qualifiers EMF
  PFD_iLayerType Enumeration
...
/** \defgroup U_EMF_PIXELFORMATDESCRIPTOR_iPixelType_Qualifiers EMF
...
#define U_LAYOUT_BITMAPORIENTATIONPRESERVED                                    \
    0x00000008 //!< Do not flip bitmaps if layout is right to left.
               /** @} */
...
/** \defgroup U_EMF_Common_macros EMF Common Macros
  @{
*/
//  Note, many of these were originally defined using C99 (type){val,val,val}
//  format, but that turned out to
//  have incompatibilities with C++, so use functions as the basis to avoid
//  this.

//  Set/Get for U_RGBQUAD Colors, which are stored in byte order: {B,G,R,A}.
//  These are used in EMF structures and the byte order must be the same in
//  memory or on disk.
#define U_BGR(r, g, b)                                                         \
    rgbquad_set(r, g, b, 0) //!<  Set any BGR  color with an {r,g,b} triplet
#define U_BGRA(r, g, b, a)                                                     \
    rgbquad_set(r, g, b, a) //!<  Set any BGRA color with an {r,g,b,a} quad
...
//  These are used in EMF structures and the byte order must be the same in
//  memory or on disk.
//  These MAY be used in PNG and other libraries if these enforce byte order in
//  memory, otherwise
//  U_swap4 may need to also be employed.
//

#define colorref_set                                                           \
    colorref3_set //!<  Most frequent usage is 3 colors, so set the unqualified
                  //! one to that
#define U_RGB(r, g, b)                                                         \
    colorref3_set(r, g, b) //!<  Set any RGB  color with an {r,g,b} triplet
#define U_RGBA(r, g, b, a)                                                     \
    colorref4_set(r, g, b, a) //!<  Set any RGBA color with an {r,g,b,a} quad
...
#define U_RGB2BGR(rgb)                                                         \
    U_BGR(U_RGBAGetR(rgb), U_RGBAGetG(rgb),                                    \
          U_RGBAGetB(rgb)) //!<  Set any BGR color from an RGB color
#define U_BGR2RGB(rgb)                                                         \
    U_RGB(U_BGRAGetR(rgb), U_BGRAGetG(rgb),                                    \
          U_BGRAGetB(rgb)) //!<  Set any RGB color from an BGR color
...
#define U_CMYK(c, m, y, k)                                                     \
    ((COLOREF)((((uint8_t)(k) | ((uint16_t)((uint8_t)(y)) << 8)) |             \
                (((uint32_t)(uint8_t)(m)) << 16)) |                            \
               (((uint32_t)(uint8_t)(c)) << 24)))  //!<   Color CMYK Set Macro.
#define U_GetKValue(cmyk) ((uint8_t)(cmyk))        //!<  Color CMYK Get K Macro.
#define U_GetYValue(cmyk) ((uint8_t)((cymk) >> 8)) //!<  Color CMYK Get Y Macro.
#define U_GetMValue(cmyk)                                                      \
    ((uint8_t)((cymk) >> 16)) //!<  Color CMYK Get M Macro.
#define U_GetCValue(cmyk)                                                      \
    ((uint8_t)((cymk) >> 24)) //!<  Color CMYK Get C Macro.

// Other macros
#define U_Gamma(A)                                                             \
    (A < U_RGB_GAMMA_MIN                                                       \
         ? U_RGB_GAMMA_MIN                                                     \
         : (A > U_RGB_GAMMA_MAX ? U_RGB_GAMMA_MAX                              \
                                : A)) //!<  Gamma set Macro (enforce range).
#define U_PM(A, B)                                                             \
    ((A) < -(B)                                                                \
         ? -(B)                                                                \
         : ((A) > (B)                                                          \
                ? (B)                                                          \
                : (A))) //!<  Plus/Minus Range Macro (B must be postitive!).
#define U_MNMX(A, B, C)                                                        \
    ((A) < (B)                                                                 \
         ? (B)                                                                 \
         : ((A) > (C) ? (C) : (A))) //!<  Min/Max Range Macro (B <= A <= C).
...
typedef struct {
    float x;          //!< X value
    float y;          //!< Y value
} U_PAIRF, *PU_PAIRF; //!< Any generic pair of floats. Microsoft name: (none)
...
} U_BITFIX28_4, *PU_BITFIX28_4; //!< EMF manual 2.2.1
...
/**
  ...
  Note, range constants are:
  RGB_GAMMA_[MIN|MAX],REFERENCE_[WHITE|BLACK]_[MIN|MAX],COLOR_ADJ_[MIN|MAX]
*/
typedef struct {
    ...
    uint16_t caRedGamma; //!< Red   Gamma correction (range:2500:65000, 10000 is
                         //! no correction)
    uint16_t caGreenGamma; //!< Green Gamma correction (range:2500:65000, 10000
                           //! is no correction)
    uint16_t caBlueGamma;  //!< Blue  Gamma correction (range:2500:65000, 10000
                           //! is no correction)
    uint16_t
        caReferenceBlack; //!< Values less than this are black (range:0:4000)
    uint16_t caReferenceWhite; //!< Values more than this are white
                               //!(range:6000:10000)
    int16_t caContrast;   //!< Contrast     adjustment (range:-100:100, 0 is no
                          //! correction)
    int16_t caBrightness; //!< Brightness   adjustment (range:-100:100, 0 is no
                          //! correction)
    int16_t caColorfulness; //!< Colorfulness adjustment (range:-100:100, 0 is
                            //! no correction)
    int16_t caRedGreenTint; //!< Tine         adjustment (range:-100:100, 0 is
                            //! no correction)
} U_COLORADJUSTMENT, *PU_COLORADJUSTMENT; //!< EMF manual 2.2.2
...
} U_DESIGNVECTOR, *PU_DESIGNVECTOR; //!< EMF manual 2.2.3
...
typedef struct {
    ...
    U_OFFDATA
    offData; //!< Offset in bytes to the Data from the start of the RECORD
} U_EMRFORMAT, *PU_EMRFORMAT; //!< EMF manual 2.2.4
...
/**
  ...
  Differs from implementation in Mingw and Wine in that the core struct has a
  fixed size.
  Optional and movable components must be handled with offsets.
  Following invariant core there may/must be:

  U_RECTL             rcl;                (Optional, absent when fOptions &
  U_ETO_NO_RECT) grayed/clipping/opaque rectangle

  U_OFFDX             offDx;              (required) but position isn't static.
  Offset in bytes to the character spacing array measured
                                          from the start of the RECORD, NOT from
  the start of this structure.

  The order of the next two may be reversed, they are found from their offsets.

  char                string              (required) String buffer holding
  nChars (padded to a multiple of 4 bytes in length).

  uint32_t            Dx[1]               (required) character spacing, array
  with one entry per glyph.
*/
typedef struct {
    U_POINTL ptlReference; //!< String start coordinates
    U_NUM_STR nChars;      //!< Number of characters in the string
    U_OFFSTR offString; //!< Offset in bytes to the string from the start of the
                        //! RECORD
    uint32_t fOptions;  //!< ExtTextOutOptions Enumeration
} U_EMRTEXT, *PU_EMRTEXT; //!< EMF manual 2.2.5

/**
  \brief For U_EPS_DATA Points field

  EMF manual 2.2.23, Microsoft name: Point28_4 Object, out of order, needed for
  2.2.6
*/
typedef struct {
    U_BITFIX28_4 x;           //!< X coordinate
    U_BITFIX28_4 y;           //!< Y coordinate
} U_POINT28_4, *PU_POINT28_4; //!< EMF manual 2.2.23

/**
  \brief For embedding EPS in EMF via U_EMRFORMAT offData array in
  U_EMR_COMMENT_MULTIFORMATS
...
typedef struct {
    ...
    U_POINT28_4 Points[3]; //!< Defines parallelogram, UL, UR, LL corners, LR is
                           //! derived.
    U_RECTL PostScriptData; //!< Record may include optional clipping/opaque
                            //! rectangle
} U_EPS_DATA, *PU_EPS_DATA; //!< EMF manual 2.2.6

/**
  \brief For GRADIENT_[TRIANGLE|U_RECT]

  EMF manual 2.2.26, Microsoft name: TriVertex Object, out of order, needed for
  2.2.7
*/
typedef struct {
    int32_t x;                //!< X coord
...
} U_TRIVERTEX, *PU_TRIVERTEX; //!< EMF manual 2.2.26
...
typedef struct {
    uint32_t
        UpperLeft; //!< Index of UL corner in an array of U_TRIVERTEX objects
    uint32_t
        LowerRight; //!< Index of LR corner in an array of U_TRIVERTEX objects
} U_GRADIENT4, *PU_GRADIENT4; //!< EMF manual 2.2.7
...
/**
  ...
  Gradient object notes.  The next two structures are used to define the shape
  with reference to an existing array
  of points stored in an array of TriVertex objects in the U_EMRGRADIENTFILL
  record.  The tricky part
  is that these two structures are different sizes.  In some implementations
  (MingW) the array is cast to uint32_t
  and basically the cast is then ignored.  For libUEMF we leave this out of the
  structure entirely and get to it with offsets.
*/
typedef struct {
...
} U_GRADIENT3, *PU_GRADIENT3; //!< EMF manual 2.2.8

// Microsoft name: Header object, EMF manual 2.2.9  defined below with record
// structs
// Microsoft name: HeaderExtension1 object, EMF manual 2.2.10 defined below with
// record structs
// Microsoft name: HeaderExtension1 object, EMF manual 2.2.11 defined below with
// record structs
...
typedef struct {
    ...
    int32_t lfEscapement;  //!< Angle in 0.1 degrees betweem escapement vector
                           //! and X axis
    ...
    uint16_t lfFaceName[U_LF_FACESIZE]; //!< Name of font.  If <U_LF_FACESIZE
                                        //! chars must be null terminated
} U_LOGFONT, *PU_LOGFONT;               //!< EMF manual 2.2.13
...
} U_PANOSE, *PU_PANOSE;       //!< EMF manual 2.2.21
...
typedef struct {
    ...
    uint16_t elfStyle[U_LF_FACESIZE];        //!< Font style (if <U_LF_FACESIZE
    //! characters, null terminate string)
    uint32_t elfVersion;   //!< Ignore
    uint32_t elfStyleSize; //!< Font hinting starting at this point size, if 0,
                           //! starts at Height
...
} U_LOGFONT_PANOSE, *PU_LOGFONT_PANOSE; //!< EMF manual 2.2.16
...
} U_LOGPLTNTRY, *PU_LOGPLTNTRY; //!< EMF manual 2.2.18
...
} U_LOGPALETTE, *PU_LOGPALETTE;  //!< EMF manual 2.2.17
#define U_SIZE_LOGPALETTE (2 * sizeof(uint16_t))

// Microsoft name: LogPaletteEntry Object, EMF manual 2.2.18, defined above,
// before 2.2.17
...
} U_LOGPEN, *PU_LOGPEN;   //!< EMF manual 2.2.19

// Microsoft name: LogPenEx Object (not implemented)  EMF manual 2.2.20
// Microsoft name: Panose Object, EMF manual 2.2.21, defined above, before
// 2.2.16
...
} U_PIXELFORMATDESCRIPTOR, *PU_PIXELFORMATDESCRIPTOR; //!< EMF manual 2.2.22

// Microsoft name: Point28_4 Object.  EMF manual 2.2.23, defined above, before
// 2.2.6
...
} U_RGNDATAHEADER, *PU_RGNDATAHEADER; //!< EMF manual 2.2.25
...
} U_RGNDATA, *PU_RGNDATA; //!< EMF manual 2.2.24
#define U_SIZE_RGNDATA U_SIZE_RGNDATAHEADER

// Microsoft name: RegionDataHeader Object.  EMF manual 2.2.25, defined above,
// before 2.2.24
// Microsoft name: TriVertex Object.  EMF manual 2.2.26, defined above, before
// 2.2.7
...
} U_XFORM, *PU_XFORM; //!< EMF manual 2.2.28
...
typedef struct {
    ...
    uint32_t elpWidth;              //!< Width in logical units (elpPenStyle &
                                    //! U_PS_GEOMETRIC) or 1 (pixel)
    ...
    U_STYLEENTRY elpStyleEntry[1];  //!< Array of StyleEntry (For user specified
                                    //! dot/dash patterns)
} U_EXTLOGPEN, *PU_EXTLOGPEN;       //!< EMF manual 2.2.20
#define U_SIZE_EXTLOGPEN                                                       \
    (sizeof(U_EXTLOGPEN) -                                                     \
     sizeof(U_STYLEENTRY)) // there may not be any style entries
...
/**
  ...
  Description of a Bitmap which in some cases is a Device Independent Bitmap
  (DIB)
*/
typedef struct {
    U_BITMAPINFOHEADER bmiHeader; //!< Geometry and pixel properties
    U_RGBQUAD bmiColors[1]; //!< Color table.  24 bit images do not use color
                            //! table values.
} U_BITMAPINFO, *PU_BITMAPINFO; //!< WMF Manual 2.2.2.9
#define U_SIZE_BITMAPINFO U_SIZE_BITMAPINFOHEADER

/**
  \brief U_EMRALPHABLEND Blend field

  EMF Manual 2.3.1.1, Microsoft name: BLENDFUNCTION field of EMR_ALPHABLEND
  record.
*/
typedef struct {
    uint8_t Operation; //!< Must be 0
    uint8_t Flags;     //!< Must be 0
    uint8_t Global;    //!< Alpha for whole thing if Op is U_AC_SRC_GLOBAL (AKA
                       //! U_AC_SRC_GLOBAL)
    uint8_t Op;        //!< Blend Enumeration
} U_BLEND, *PU_BLEND;  //!< EMF Manual 2.3.1.1
#
...
} U_ENHMETARECORD, *PU_ENHMETARECORD; //!< General form of an EMF record.
...
typedef struct {
    uint32_t iType; //!< Type of EMR record
    uint32_t nSize; //!< Size of entire record in bytes (multiple of 4).
} U_EMR, *PU_EMR;   //!< First two fields of all EMF records,
...
typedef struct {
    ...
    *PU_EMRGENERICPAIR; //!< Generic EMR record with two 32 bit values.
                        //! Microsoft name: (none)

// ***********************************************************************************
// The following have U_EMR_# records.  They are ordered by their record index,
// not by EMF manual position.
...
/**
  ...
  EMF manual 2.2.9, Microsoft name: Header object, HeaderExtension1 object,
  HeaderExtension2 object

  Microsoft names instead: Header, HeaderExtension1, and HeaderExtension2
  objects.  These are
  used nowhere else, so they are combined here, along with the first two fields
  which were not listed in the Header.

  Note also that three fields in this file (nBytes, nRecords, nHandles) must be
  (re)set after the entire EMF
  is constructed, since typically they are not known until then.  bOpenGL may or
  may not be knowable when this
  header is written.

  Note also that rclBounds and rclFrame are supposed to be the region bounding
  the drawn content within the
  EMF.  This is generally smaller than the size from szlDevice.  However, since
  libUEMF does not actually draw
  anything it has no way of knowing what these values are.  Instead when it
  creates a header it sets these to
...
typedef struct {
    ...
    uint32_t dSignature;   //!< FormatSignature Enumeration  (must be
                           //! U_ENHMETA_SIGNATURE)
    ...
    uint32_t nDescription; //!< Characters in the Description field, 0 if no
                           //! description
    uint32_t offDescription; //!< Offset in bytes to optional UTF-16BE string
                             //! Description field
    ...
    U_CBPXLFMT
    cbPixelFormat; //!< Size in bytes of PixelFormatDescriptor, 0 if no PFD
    U_OFFPXLFMT offPixelFormat; //!< Offset in bytes to optional
                                //! PixelFormatDescriptor from the start of the
    //! RECORD, 0 if no PFD
...
} U_EMRHEADER, *PU_EMRHEADER; //!< EMF manual 2.2.9
#define U_SIZE_EMRHEADER                                                       \
    sizeof(U_EMRHEADER) // modern EMF files, for _really_ old ones the _MIN size
                        // applies
#define U_SIZE_EMRHEADER_MIN                                                   \
    (U_SIZE_EMRHEADER - sizeof(U_CBPXLFMT) - sizeof(U_OFFPXLFMT) -             \
     sizeof(uint32_t) - sizeof(U_SIZEL))
...
#define U_SIZE_EMRPOLYBEZIER                                                   \
    (sizeof(U_EMR) + sizeof(U_RECTL) + sizeof(U_NUM_POINTL))
...
#define U_SIZE_EMRPOLYPOLYLINE                                                 \
    (sizeof(U_EMR) + sizeof(U_RECTL) + sizeof(U_NUM_POLYCOUNTS) +              \
     sizeof(U_NUM_POINTL))
...
This is a very odd structure because the nSizeLast follows an optional variable
size field.  Consequently
even though nSizeLast has a name it cannot actually be accessed by it!
Following the core structure
appear these fields:\n

 U_LOGPLTNTRY        PalEntries[1];      Record may include optional array of
PalEntries

     uint32_t            nSizeLast;      Mandatory, but position isn't fixed.
Must have same value as emr.nSize in header record
...
typedef struct {
    ...
    U_OFFPLENTRIES
    offPalEntries;      //!< Offset in bytes to array of palette entries
} U_EMREOF, *PU_EMREOF; //!< EMF manual 3.2.4.1
...
} U_EMRSETPIXELV, *PU_EMRSETPIXELV; //!< EMF manual 2.3.5.36
...
} U_EMRSETMAPPERFLAGS, *PU_EMRSETMAPPERFLAGS; //!< EMF manual 2.3.11.20
...
typedef struct {
    ...
    *PU_EMRSETPOLYFILLMODE, //!< EMF manual 2.3.11.22 PolygonFillMode
                            //! Enumeration
    *PU_EMRSETROP2,         //!< EMF manual 2.3.11.23 Binary Raster Operation
                            //! Enumeration
...
} U_EMRSETCOLORADJUSTMENT, *PU_EMRSETCOLORADJUSTMENT; //!< EMF manual 2.3.11.13
...
} U_EMROFFSETCLIPRGN, *PU_EMROFFSETCLIPRGN; //!< EMF manual 2.3.2.4
...
} U_EMRRESTOREDC, *PU_EMRRESTOREDC; //!< EMF manual 2.3.11.6
...
} U_EMRSETWORLDTRANSFORM, *PU_EMRSETWORLDTRANSFORM; //!< EMF manual 2.3.12.2
...
typedef struct {
    U_EMR emr;      //!< U_EMR
    uint32_t ihPen; //!< Index to place object in EMF object table (this entry
                    //! must not yet exist)
    U_LOGPEN lopn;  //!< Pen properties
} U_EMRCREATEPEN, *PU_EMRCREATEPEN; //!< EMF manual 2.3.7.7
...
typedef struct {
    U_EMR emr;        //!< U_EMR
    uint32_t ihBrush; //!< Index to place object in EMF object table (this entry
                      //! must not yet exist)
    U_LOGBRUSH lb;    //!< Brush properties
} U_EMRCREATEBRUSHINDIRECT, *PU_EMRCREATEBRUSHINDIRECT; //!< EMF manual 2.3.7.1
...
typedef struct {
    ...
    U_FLOAT eStartAngle; //!< Starting angle in degrees (counter clockwise from
                         //! x axis)
    U_FLOAT eSweepAngle; //!< Sweep angle in degrees
} U_EMRANGLEARC, *PU_EMRANGLEARC; //!< EMF manual 2.3.5.1
...
} U_EMRROUNDRECT, *PU_EMRROUNDRECT; //!< EMF manual 2.3.5.35
...
} U_EMRSELECTPALETTE, *PU_EMRSELECTPALETTE; //!< EMF manual 2.3.8.6
...
typedef struct {
    U_EMR emr;      //!< U_EMR
    uint32_t ihPal; //!< Index to place object in EMF object table (this entry
                    //! must not yet exist)
    U_LOGPALETTE lgpl; //!< Palette properties (variable size)
} U_EMRCREATEPALETTE, *PU_EMRCREATEPALETTE; //!< EMF manual 2.3.7.6
#define U_SIZE_EMRCREATEPALETTE                                                \
    (sizeof(U_EMR) + sizeof(uint32_t) + U_SIZE_LOGPALETTE)
...
} U_EMRSETPALETTEENTRIES, *PU_EMRSETPALETTEENTRIES; //!< EMF manual 2.3.8.8
#define U_SIZE_EMRSETPALETTEENTRIES                                            \
    (sizeof(U_EMR) + 2 * sizeof(uint32_t) + sizeof(U_NUM_LOGPLTNTRY))
...
} U_EMRRESIZEPALETTE, *PU_EMRRESIZEPALETTE; //!< EMF manual 2.3.8.4
...
} U_EMREXTFLOODFILL, *PU_EMREXTFLOODFILL; //!< EMF manual 2.3.5.6
...
} U_EMRPOLYDRAW, *PU_EMRPOLYDRAW; //!< EMF manual 2.3.5.20
#define U_SIZE_EMRPOLYDRAW                                                     \
    (sizeof(U_EMR) + sizeof(U_RECTL) + sizeof(U_NUM_POINTL))
...
} U_EMRSETARCDIRECTION, *PU_EMRSETARCDIRECTION; //!< EMF manual 2.3.11.9
#define U_SIZE_EMRSETARCDIRECTION (sizeof(U_EMRSETARCDIRECTION))

/* Index  58 */
/** EMF manual 2.3.11.21

IMPORTANT!!!!  The Microsoft structure uses a float for the miterlimit but the
EMF file record
...
typedef struct {
    U_EMR emr; //!< U_EMR
    uint32_t
        eMiterLimit; //!< Miter limit (max value of mitered length / line width)
} U_EMRSETMITERLIMIT, *PU_EMRSETMITERLIMIT; //!< EMF manual 2.3.11.21
...
typedef struct {
    ...
    uint8_t Data[1]; //!< Comment (any binary data, interpretation is program
                     //! specific)
} U_EMRCOMMENT, *PU_EMRCOMMENT; //!< EMF manual 2.3.3.1, AKA GDICOMMENT
...
typedef struct {
    ...
    uint32_t esrIdent; //!< EMFSpoolRecordIdentifier, may be
                       //! U_EMR_COMMENT_SPOOLFONTDEF
    uint8_t Data[1];   //!< EMF Spool records
} U_EMRCOMMENT_SPOOL,
    *PU_EMRCOMMENT_SPOOL; //!< EMF manual 2.3.3.3, EMF Spool comment
#define U_SIZE_EMRCOMMENT_SPOOL                                                \
...
typedef struct {
    ...
    uint32_t
        pcIdent;     //!< Public Comment Identifier, from EMRComment Enumeration
    uint8_t Data[1]; //!< Public comment data
} U_EMRCOMMENT_PUBLIC,
    *PU_EMRCOMMENT_PUBLIC; //!< EMF manual 2.3.3.4, EMF Public comment
#define U_SIZE_EMRCOMMENT_PUBLIC                                               \
    (sizeof(U_EMR) + sizeof(U_CBDATA) + 2 * sizeof(uint32_t))
...
} U_EMRFILLRGN, *PU_EMRFILLRGN; //!< EMF manual 2.3.5.10
#define U_SIZE_EMRFILLRGN                                                      \
    (sizeof(U_EMR) + sizeof(U_RECTL) + sizeof(U_CBRGNDATA) + sizeof(uint32_t))
...
typedef struct {
    ...
    U_RGNDATA RgnData[1];  //!< Variable size U_RGNDATA structure
} U_EMRFRAMERGN, *PU_EMRFRAMERGN; //!< EMF manual 2.3.5.11
#define U_SIZE_EMRFRAMERGN                                                     \
    (sizeof(U_EMR) + sizeof(U_RECTL) + sizeof(U_CBRGNDATA) +                   \
     sizeof(uint32_t) + sizeof(U_SIZEL))
...
typedef struct {
    ...
    U_RGNDATA RgnData[1];  //!< Variable size U_RGNDATA structure
} U_EMREXTSELECTCLIPRGN, *PU_EMREXTSELECTCLIPRGN; //!< EMF manual 2.3.2.2
#define U_SIZE_EMREXTSELECTCLIPRGN                                             \
    (sizeof(U_EMR) + sizeof(U_CBRGNDATA) + sizeof(uint32_t))
...
typedef struct {
    ...
    U_OFFBMISRC
    offBmiSrc; //!< Offset in bytes to U_BITMAPINFO (within bitmapbuffer)
    U_CBBMISRC cbBmiSrc; //!< Size   in bytes of U_BITMAPINFO
    U_OFFBITSSRC
    offBitsSrc;         //!< Offset in bytes to the bitmap (within bitmapbuffer)
    U_CBBITS cbBitsSrc; //!< Size in bytes of bitmap
                        //!< Record may include optional bitmapbuffer
} U_EMRBITBLT, *PU_EMRBITBLT; //!< EMF manual 2.3.1.2
...
typedef struct {
    ...
    U_POINTL cSrc;      //!< Src W & H in logical units
                        //!< Record may include optional bitmapbuffer
} U_EMRSTRETCHBLT, *PU_EMRSTRETCHBLT; //!< EMF manual 2.3.1.6
...
typedef struct {
    ...
    uint32_t iUsageSrc;      //!< DIBcolors Enumeration
    U_OFFBMISRC
    offBmiSrc; //!< Offset in bytes to U_BITMAPINFO (within srcbitmapbuffer)
    U_CBBMISRC cbBmiSrc;      //!< Size   in bytes of U_BITMAPINFO
    U_OFFBITSSRC offBitsSrc;  //!< Offset in bytes to the src bitmap (within
                              //! srcbitmapbuffer)
    ...
    U_OFFBMIMSK offBmiMask;   //!< Offset in bytes to U_BITMAPINFO (within
                              //! maskbitmapbuffer)
    U_CBBMIMSK cbBmiMask;     //!< Size   in bytes of U_BITMAPINFO
    U_OFFBITSMSK offBitsMask; //!< Offset in bytes to the mask bitmap (within
                              //! maskbitmapbuffer)
    U_CBBITSMSK cbBitsMask;   //!< Size in bytes of bitmap
    //!< Record may include optional Source and mask bitmapbuffers
} U_EMRMASKBLT, *PU_EMRMASKBLT; //!< EMF manual 2.3.1.3
...
typedef struct {
    U_EMR emr;           //!< U_EMR
    U_RECTL rclBounds;   //!< Bounding rectangle in device units
    U_POINTL aptlDst[3]; //!< Defines parallelogram, UL, UR, LL corners, LR is
                         //! derived.
    U_POINTL Src;        //!< Source UL corner in logical units
    U_POINTL cSrc;       //!< Src W & H in logical units
    U_XFORM xformSrc;    //!< Transform to apply to source
    ...
    U_CBBITSMSK cbBitsMask;   //!< Size in bytes of bitmap
    //!< Record may include optional Source and mask bitmapbuffers
} U_EMRPLGBLT, *PU_EMRPLGBLT; //!< EMF manual 2.3.1.4
...
} U_EMRSETDIBITSTODEVICE, *PU_EMRSETDIBITSTODEVICE; //!< EMF manual 2.3.1.5
...
typedef struct {
    ...
    U_POINTL cDest;          //!< Destination W & H in logical units
                             //!< Record may includes optional bitmapbuffer
} U_EMRSTRETCHDIBITS, *PU_EMRSTRETCHDIBITS; //!< EMF manual 2.3.1.7
...
typedef struct {
    ...
    U_LOGFONT_PANOSE elfw; //!< Font parameters, either U_LOGFONT or
    //! U_LOGFONT_PANOSE, the latter is bigger so use that
    //! type here
} U_EMREXTCREATEFONTINDIRECTW,
    *PU_EMREXTCREATEFONTINDIRECTW; //!< EMF manual 2.3.7.8
#define U_SIZE_EMREXTCREATEFONTINDIRECTW_LOGFONT_PANOSE                        \
    (sizeof(U_EMR) + 4 + sizeof(U_LOGFONT_PANOSE))
#define U_SIZE_EMREXTCREATEFONTINDIRECTW_LOGFONT                               \
    (sizeof(U_EMR) + 4 + sizeof(U_LOGFONT))
...
Variable and optional fields may follow core structure in record:\n
 ...
 U_RECTL             rcl;                absent when fOptions & U_ETO_NO_RECT)
grayed/clipping/opaque rectangle

 U_OFFDX             offDx;              (required) Offset in bytes to the
character spacing array from the start of the RECORD

 uint32_t             Dx                 (optional) character spacing array
(Required, but position is not static.)
*/
typedef struct {
    U_EMR emr;              //!< U_EMR
    U_RECTL rclBounds;      //!< Bounding rectangle in device units
    uint32_t iGraphicsMode; //!< GraphicsMode Enumeration
    U_FLOAT exScale;   //!< scale to 0.01 mm units ( only if iGraphicsMode &
                       //! U_GM_COMPATIBLE)
    U_FLOAT eyScale;   //!< scale to 0.01 mm units ( only if iGraphicsMode &
                       //! U_GM_COMPATIBLE)
...
#define U_SIZE_EMRPOLYBEZIER16                                                 \
    (sizeof(U_EMR) + sizeof(U_RECTL) + sizeof(U_NUM_POINT16))
...
#define U_SIZE_EMRPOLYPOLYLINE16                                               \
    (sizeof(U_EMR) + sizeof(U_RECTL) + sizeof(U_NUM_POLYCOUNTS) +              \
     sizeof(U_NUM_POINT16))
...
} U_EMRPOLYDRAW16, *PU_EMRPOLYDRAW16; //!< EMF manual 2.3.5.21
#define U_SIZE_EMRPOLYDRAW16                                                   \
    (sizeof(U_EMR) + sizeof(U_RECTL) + sizeof(U_NUM_POINT16))
...
typedef struct {
    ...
    U_OFFBMI
    offBmi;        //!< Offset in bytes to U_BITMAPINFO (within DIBbitmapbuffer)
    U_CBBMI cbBmi; //!< Size   in bytes of U_BITMAPINFO
    U_OFFBITS offBits; //!< Offset in bytes to the DIB bitmap data (within
                       //! DIBbitmapbuffer
    U_CBBITS cbBits;   //!< Size in bytes of DIB bitmap
                       //!< Record may include optional DIB bitmapbuffer
} U_EMRCREATEMONOBRUSH, *PU_EMRCREATEMONOBRUSH; //!< EMF manual 2.3.7.5
...
typedef struct {
    ...
    uint32_t iUsage;  //!< DIBcolors Enumeration
    U_OFFBMI
    offBmi; //!< Offset in bytes to U_BITMAPINFO (within DIB bitmapbuffer)
    U_CBBMI cbBmi;     //!< Size   in bytes of U_BITMAPINFO
    U_OFFBITS offBits; //!< Offset in bytes to the DIB bitmap data (within DIB
                       //! bitmapbuffer
    U_CBBITS cbBits;   //!< Size in bytes of DIB bitmap
                       //!< Record may include optional DIB bitmapbuffer
...
} U_EMREXTCREATEPEN, *PU_EMREXTCREATEPEN; //!< EMF manual 2.3.7.9
#define U_SIZE_EMREXTCREATEPEN                                                 \
    (sizeof(U_EMREXTCREATEPEN) - sizeof(U_EXTLOGPEN) + U_SIZE_EXTLOGPEN)
...
#define U_SIZE_EMRPOLYTEXTOUTA                                                 \
    (sizeof(U_EMR) + sizeof(U_RECTL) + sizeof(uint32_t) +                      \
     2 * sizeof(U_FLOAT) + sizeof(U_NUM_EMRTEXT))
...
typedef struct {
    U_EMR emr;     //!< U_EMR
    uint32_t ihCS; //!< Index to place object in EMF object table (this entry
                   //! must not yet exist)
    U_LOGCOLORSPACEA lcs;                         //!< ColorSpace parameters
} U_EMRCREATECOLORSPACE, *PU_EMRCREATECOLORSPACE; //!< EMF manual 2.3.7.2
...
} U_EMRGLSRECORD, *PU_EMRGLSRECORD; //!< EMF manual 2.3.9.2
...
typedef struct {
    ...
    U_CBDATA cbData;   //!< Size of OpenGL data in bytes
    U_DATA Data[1];    //!< OpenGL data
} U_EMRGLSBOUNDEDRECORD, *PU_EMRGLSBOUNDEDRECORD; //!<  EMF manual 2.3.9.1
#define U_SIZE_EMRGLSBOUNDEDRECORD                                             \
    (sizeof(U_EMR) + sizeof(U_RECTL) + sizeof(U_CBDATA))
...
} U_EMRPIXELFORMAT, *PU_EMRPIXELFORMAT; //!< EMF manual 2.3.11.5
...
} U_EMRDRAWESCAPE, *PU_EMRDRAWESCAPE; //!< EMF manual 2.3.6.1
...
typedef struct {
    ...
    U_CBDATA cjIn;  //!< Number of bytes to send to printer driver
    U_DATA Data[1]; //!< Data to send
} U_EMREXTESCAPE, *PU_EMREXTESCAPE; //!< EMF manual 2.3.6.2
...
typedef struct {
    ...
    //!<    U_RECTL             rclBounds;           Record may include optional
    //! Bounding rectangle (absent when: fuOPtions & ETO_NO_U_RECT)
    //!<    uint32_t            TextString;          text to output (fuOptions &
    //! ETO_SMALL_CHARS ? 8 bit : 16 bit)
} U_EMRSMALLTEXTOUT, *PU_EMRSMALLTEXTOUT; //!< EMF manual 2.3.5.37
...
typedef struct {
    U_EMR emr;          //!< U_EMR
    U_CBDATA cbDriver;  //!< Number of bytes in driver name (note, BYTES, not
                        //! CHARACTERS)
    U_CBDATA cbData;    //!< Number of bytes in data
    uint16_t Driver[1]; //!< Driver name in uint16_t characters, null terminated
} U_EMRNAMEDESCAPE, *PU_EMRNAMEDESCAPE; //!< EMF manual 2.3.6.3
...
typedef struct {
    ...
    U_POINTL cSrc;      //!< Source W & H in logical units
                        //!< Record may include optional DIB bitmap
} U_EMRALPHABLEND, *PU_EMRALPHABLEND; //!< EMF manual 2.3.1.1
...
typedef struct {
    ...
    U_POINTL cSrc;      //!< Source W & H in logical units
                        //!< Record may includes optional bitmapbuffer
} U_EMRTRANSPARENTBLT, *PU_EMRTRANSPARENTBLT; //!< EMF manual 2.3.1.8
...
/** EMF manual 2.3.5.12
    ...
    uint32_t            GradObj[1];          Array of gradient objects (each has
   2 or 3 indices into TriVert array)
*/
typedef struct {
    U_EMR emr;                //!< U_EMR
    U_RECTL rclBounds;        //!< Bounding rectangle in device units
...
} U_EMRGRADIENTFILL, *PU_EMRGRADIENTFILL; //!< EMF manual 2.3.5.12
...
typedef struct {
    ...
    uint8_t Data[1];   //!< Data of size cbName+cbData: Name in UTF16 then color
                       //! profile data
} U_EMRCOLORMATCHTOTARGETW, *PU_EMRCOLORMATCHTOTARGETW; //!< EMF manual 2.3.11.1
#define U_SIZE_EMRCOLORMATCHTOTARGETW                                          \
    (sizeof(U_EMR) + 2 * sizeof(uint32_t) + sizeof(U_CBNAME) + sizeof(U_CBDATA))
...
typedef struct {
    U_EMR emr;     //!< U_EMR
    uint32_t ihCS; //!< Index of the logical color space object in the EMF
                   //! object table
...
} U_EMRCREATECOLORSPACEW, *PU_EMRCREATECOLORSPACEW; //!< EMF manual 2.3.7.3
#define U_SIZE_EMRCREATECOLORSPACEW                                            \
    (sizeof(U_EMR) + 2 * sizeof(uint32_t) + sizeof(U_LOGCOLORSPACEW) +         \
     sizeof(U_CBDATA))
...
/**
  Storage for keeping track of properties of the growing EMF file as records are
  added.
*/
typedef struct {
    FILE *fp;            //!< Open file
...
/**
  ...
  program finally plays back the EMF, but
  we do need to keep track of the numbers so that they are not accidentally
  reused.  This structure is used for that,
  and all *_set functions that touch a handle reference it.

  Stock objects are not used in this limited model, so libUEMF cannot detect if
  a handle is still in use.  Nor can it
  tell when a handle has been deselected (by selecting another handle for the
  same type of graphic object, and thus
...
} U_MAT2X2, *PU_MAT2X2; //!< 2 x 2 matrix, used by xform_alt_set() function.
...
int emr_arc_points(PU_ENHMETARECORD record, int *f1, int f2, PU_PAIRF center,
                   PU_PAIRF start, PU_PAIRF end, PU_PAIRF size);
int emr_arc_points_common(PU_RECTL rclBox, PU_POINTL ArcStart, PU_POINTL ArcEnd,
                          int *f1, int f2, PU_PAIRF center, PU_PAIRF start,
                          PU_PAIRF end, PU_PAIRF size);
...
int RGBA_to_DIB(char **px, uint32_t *cbPx, PU_RGBQUAD *ct, int *numCt,
                const char *rgba_px, int w, int h, int stride,
                uint32_t colortype, int use_ct, int invert);
int get_DIB_params(const char *record, uint32_t offBitsSrc, uint32_t offBmiSrc,
                   ...
                   uint32_t *invert);
int DIB_to_RGBA(const char *px, const U_RGBQUAD *ct, int numCt, char **rgba_px,
                int w, int h, uint32_t colortype, int use_ct, int invert);
char *RGBA_to_RGBA(char *rgba_px, int w, int h, int sl, int st, int *ew,
                   int *eh);

int device_size(const int xmm, const int ymm, const float dpmm, U_SIZEL *szlDev,
                U_SIZEL *szlMm);
int drawing_size(const int xmm, const int yum, const float dpmm,
                 U_RECTL *rclBounds, U_RECTL *rclFrame);

int emf_start(const char *name, const uint32_t initsize,
              const uint32_t chunksize, EMFTRACK **et);
...
U_PANOSE panose_set(uint8_t bFamilyType, uint8_t bSerifStyle, uint8_t bWeight,
                    uint8_t bProportion, uint8_t bContrast,
                    uint8_t bStrokeVariation, uint8_t bArmStyle,
                    uint8_t bLetterform, uint8_t bMidline, uint8_t bXHeight);
U_COLORREF colorref3_set(uint8_t red, uint8_t green, uint8_t blue);
U_COLORREF colorref4_set(uint8_t red, uint8_t green, uint8_t blue,
                         uint8_t reserved);
U_RGBQUAD rgbquad_set(uint8_t red, uint8_t green, uint8_t blue,
                      uint8_t reserved);
U_LOGBRUSH logbrush_set(uint32_t lbStyle, U_COLORREF lbColor, int32_t lbHatch);
U_XFORM xform_set(U_FLOAT eM11, U_FLOAT eM12, U_FLOAT eM21, U_FLOAT eM22,
                  U_FLOAT eDx, U_FLOAT eDy);
U_XFORM xform_alt_set(U_FLOAT scale, U_FLOAT ratio, U_FLOAT rot,
                      U_FLOAT axisrot, U_FLOAT eDx, U_FLOAT eDy);
U_LOGPEN logpen_set(uint32_t lopnStyle, U_POINT lopnWidth,
                    U_COLORREF lopnColor);
PU_EXTLOGPEN extlogpen_set(uint32_t elpPenStyle, uint32_t elpWidth,
                           uint32_t elpBrushStyle, U_COLORREF elpColor,
                           int32_t elpHatch, U_NUM_STYLEENTRY elpNumEntries,
                           U_STYLEENTRY *elpStyleEntry);
U_LOGFONT_PANOSE logfont_panose_set(U_LOGFONT elfLogFont, uint16_t *elfFullName,
                                    uint16_t *elfStyle, uint32_t elfStyleSize,
                                    U_PANOSE elfPanose);
U_LOGFONT logfont_set(int32_t lfHeight, int32_t lfWidth, int32_t lfEscapement,
                      int32_t lfOrientation, int32_t lfWeight, uint8_t lfItalic,
                      uint8_t lfUnderline, uint8_t lfStrikeOut,
                      uint8_t lfCharSet, uint8_t lfOutPrecision,
                      uint8_t lfClipPrecision, uint8_t lfQuality,
                      uint8_t lfPitchAndFamily, uint16_t *lfFaceName);
char *emrtext_set(U_POINTL ptlReference, U_NUM_STR NumString, uint32_t cbChar,
                  void *String, uint32_t fOptions, U_RECTL rcl, uint32_t *Dx);
U_LOGPLTNTRY logpltntry_set(uint8_t peReserved, uint8_t peRed, uint8_t peGreen,
                            uint8_t peBlue);
...
U_BITMAPINFOHEADER bitmapinfoheader_set(
    int32_t biWidth, int32_t biHeight, uint16_t biPlanes, uint16_t biBitCount,
    uint32_t biCompression, uint32_t biSizeImage, int32_t biXPelsPerMeter,
...
U_COLORADJUSTMENT
coloradjustment_set(uint16_t Size, uint16_t Flags, uint16_t IlluminantIndex,
                    uint16_t RedGamma, uint16_t GreenGamma, uint16_t BlueGamma,
                    uint16_t ReferenceBlack, uint16_t ReferenceWhite,
                    int16_t Contrast, int16_t Brightness, int16_t Colorfulness,
                    int16_t RedGreenTint);
U_PIXELFORMATDESCRIPTOR pixelformatdescriptor_set(
    uint32_t dwFlags, uint8_t iPixelType, uint8_t cColorBits, uint8_t cRedBits,
    uint8_t cRedShift, uint8_t cGreenBits, uint8_t cGreenShift,
    uint8_t cBlueBits, uint8_t cBlueShift, uint8_t cAlphaBits,
    uint8_t cAlphaShift, uint8_t cAccumBits, uint8_t cAccumRedBits,
    uint8_t cAccumGreenBits, uint8_t cAccumBlueBits, uint8_t cAccumAlphaBits,
    uint8_t cDepthBits, uint8_t cStencilBits, uint8_t cAuxBuffers,
    uint8_t iLayerType, uint8_t bReserved, uint32_t dwLayerMask,
...
char *extcreatepen_set(uint32_t *ihPen, EMFHANDLES *eht, PU_BITMAPINFO Bmi,
                       const uint32_t cbPx, char *Px, PU_EXTLOGPEN elp);
char *createbrushindirect_set(uint32_t *ihBrush, EMFHANDLES *eht,
                              U_LOGBRUSH lb);
char *createdibpatternbrushpt_set(uint32_t *ihBrush, EMFHANDLES *eht,
                                  uint32_t iUsage, PU_BITMAPINFO Bmi,
                                  const uint32_t cbPx, const char *Px);
char *createmonobrush_set(uint32_t *ihBrush, EMFHANDLES *eht, uint32_t iUsage,
                          PU_BITMAPINFO Bmi, const uint32_t cbPx,
                          const char *Px);
char *extcreatefontindirectw_set(uint32_t *ihFont, EMFHANDLES *eht,
                                 const char *elf, const char *elfw);
char *createpalette_set(uint32_t *ihPal, EMFHANDLES *eht, U_LOGPALETTE lgpl);
char *setpaletteentries_set(uint32_t *ihPal, EMFHANDLES *eht, uint32_t iStart,
                            U_NUM_LOGPLTNTRY cEntries,
                            PU_LOGPLTNTRY aPalEntries);
char *fillrgn_set(uint32_t *ihBrush, EMFHANDLES *eht, U_RECTL rclBounds,
                  PU_RGNDATA RgnData);
char *framergn_set(uint32_t *ihBrush, EMFHANDLES *eht, U_RECTL rclBounds,
                   U_SIZEL szlStroke, PU_RGNDATA RgnData);
char *createcolorspace_set(uint32_t *ihCS, EMFHANDLES *eht,
                           U_LOGCOLORSPACEA lcs);
char *createcolorspacew_set(uint32_t *ihCS, EMFHANDLES *eht,
                            U_LOGCOLORSPACEW lcs, uint32_t dwFlags,
                            U_CBDATA cbData, uint8_t *Data);

char *U_EMRHEADER_set(const U_RECTL rclBounds, const U_RECTL rclFrame,
                      U_PIXELFORMATDESCRIPTOR *const pfmtDesc, U_CBSTR nDesc,
                      uint16_t *const Description, const U_SIZEL szlDevice,
                      const U_SIZEL szlMillimeters, const uint32_t bOpenGL);
char *U_EMRPOLYBEZIER_set(const U_RECTL rclBounds, const uint32_t count,
                          const U_POINTL *points);
char *U_EMRPOLYGON_set(const U_RECTL rclBounds, const uint32_t count,
                       const U_POINTL *points);
char *U_EMRPOLYLINE_set(const U_RECTL rclBounds, const uint32_t count,
                        const U_POINTL *points);
char *U_EMRPOLYBEZIERTO_set(const U_RECTL rclBounds, const uint32_t count,
                            const U_POINTL *points);
char *U_EMRPOLYLINETO_set(const U_RECTL rclBounds, const uint32_t count,
                          const U_POINTL *points);

char *U_EMRPOLYPOLYLINE_set(const U_RECTL rclBounds, const uint32_t nPolys,
                            const uint32_t *aPolyCounts, const uint32_t cptl,
                            const U_POINTL *points);
char *U_EMRPOLYPOLYGON_set(const U_RECTL rclBounds, const uint32_t nPolys,
                           const uint32_t *aPolyCounts, const uint32_t cptl,
                           const U_POINTL *points);
...
char *U_EMREOF_set(const U_CBPLENTRIES cbPalEntries,
                   const PU_LOGPLTNTRY PalEntries, EMFTRACK *et);
...
char *U_EMRSCALEVIEWPORTEXTEX_set(const int32_t xNum, const int32_t xDenom,
                                  const int32_t yNum, const int32_t yDenom);
char *U_EMRSCALEWINDOWEXTEX_set(const int32_t xNum, const int32_t xDenom,
                                const int32_t yNum, const int32_t yDenom);
...
char *U_EMRSELECTOBJECT_set(
    const uint32_t ihObject); // better to call selectobject_set()
...
char *U_EMRDELETEOBJECT_set(
    const uint32_t ihObject); // better to call deleteobject_set()
char *U_EMRANGLEARC_set(const U_POINTL ptlCenter, const uint32_t nRadius,
                        const U_FLOAT eStartAngle, const U_FLOAT eSweepAngle);
...
char *U_EMRARC_set(const U_RECTL rclBox, const U_POINTL ptlStart,
                   const U_POINTL ptlEnd);
char *U_EMRCHORD_set(const U_RECTL rclBox, const U_POINTL ptlStart,
                     const U_POINTL ptlEnd);
char *U_EMRPIE_set(const U_RECTL rclBox, const U_POINTL ptlStart,
                   const U_POINTL ptlEnd);
char *U_EMRSELECTPALETTE_set(const uint32_t ihPal);
char *U_EMRCREATEPALETTE_set(const uint32_t ihPal, const U_LOGPALETTE lgpl);
char *U_EMRSETPALETTEENTRIES_set(const uint32_t ihPal, const uint32_t iStart,
...
char *U_EMREXTFLOODFILL_set(const U_POINTL ptlStart, const U_COLORREF crColor,
                            const uint32_t iMode);
char *U_EMRLINETO_set(const U_POINTL ptl);
char *U_EMRARCTO_set(const U_RECTL rclBox, const U_POINTL ptlStart,
                     const U_POINTL ptlEnd);
char *U_EMRPOLYDRAW_set(const U_RECTL rclBounds, const U_NUM_POINTL cptl,
                        const U_POINTL *aptl, const uint8_t *abTypes);
...
char *U_EMRFILLRGN_set(const U_RECTL rclBounds, const uint32_t ihBrush,
                       const PU_RGNDATA RgnData);
char *U_EMRFRAMERGN_set(const U_RECTL rclBounds, const uint32_t ihBrush,
                        const U_SIZEL szlStroke, const PU_RGNDATA RgnData);
...
char *U_EMRBITBLT_set(const U_RECTL rclBounds, const U_POINTL Dest,
                      const U_POINTL cDest, const U_POINTL Src,
                      const U_XFORM xformSrc, const U_COLORREF crBkColorSrc,
...
char *U_EMRSTRETCHBLT_set(U_RECTL rclBounds, U_POINTL Dest, U_POINTL cDest,
                          const U_POINTL Src, const U_POINTL cSrc,
                          const U_XFORM xformSrc, const U_COLORREF crBkColorSrc,
                          const uint32_t iUsageSrc, const uint32_t dwRop,
                          const PU_BITMAPINFO Bmi, const uint32_t cbPx,
                          char *Px);
char *U_EMRMASKBLT_set(U_RECTL rclBounds, U_POINTL Dest, U_POINTL cDest,
                       const U_POINTL Src, const U_XFORM xformSrc,
                       const U_COLORREF crBkColorSrc, const uint32_t iUsageSrc,
                       const U_POINTL Mask, const uint32_t iUsageMask,
                       const uint32_t dwRop, const PU_BITMAPINFO Bmi,
                       const uint32_t cbPx, char *Px,
                       const PU_BITMAPINFO BmiMsk, const uint32_t cbMsk,
                       char *Msk);
char *U_EMRPLGBLT_set(const U_RECTL rclBounds, const PU_POINTL aptlDst,
                      const U_POINTL Src, const U_POINTL cSrc,
                      const U_XFORM xformSrc, const U_COLORREF crBkColorSrc,
                      const uint32_t iUsageSrc, const U_POINTL Mask,
                      const uint32_t iUsageMask, const PU_BITMAPINFO Bmi,
                      const uint32_t cbPx, char *Px, const PU_BITMAPINFO BmiMsk,
                      const uint32_t cbMsk, char *Msk);
char *U_EMRSETDIBITSTODEVICE_set(const U_RECTL rclBounds, const U_POINTL Dest,
                                 ...
                                 const uint32_t cScans, const PU_BITMAPINFO Bmi,
                                 const uint32_t cbPx, char *Px);
char *U_EMRSTRETCHDIBITS_set(const U_RECTL rclBounds, const U_POINTL Dest,
                             const U_POINTL cDest, const U_POINTL Src,
                             const U_POINTL cSrc, const uint32_t iUsageSrc,
                             const uint32_t dwRop, const PU_BITMAPINFO Bmi,
                             const uint32_t cbPx, char *Px);
char *U_EMREXTCREATEFONTINDIRECTW_set(uint32_t ihFont, const char *elf,
                                      const char *elfw);
char *U_EMREXTTEXTOUTA_set(U_RECTL rclBounds, uint32_t iGraphicsMode,
                           U_FLOAT exScale, U_FLOAT eyScale,
                           PU_EMRTEXT emrtext);
char *U_EMREXTTEXTOUTW_set(U_RECTL rclBounds, uint32_t iGraphicsMode,
                           U_FLOAT exScale, U_FLOAT eyScale,
                           PU_EMRTEXT emrtext);
char *U_EMRPOLYBEZIER16_set(const U_RECTL rclBounds, const uint32_t cpts,
                            const U_POINT16 *points);
char *U_EMRPOLYGON16_set(const U_RECTL rclBounds, const uint32_t cpts,
                         const U_POINT16 *points);
char *U_EMRPOLYLINE16_set(const U_RECTL rclBounds, const uint32_t cpts,
                          const U_POINT16 *points);
char *U_EMRPOLYBEZIERTO16_set(const U_RECTL rclBounds, const uint32_t cpts,
                              const U_POINT16 *points);
char *U_EMRPOLYLINETO16_set(const U_RECTL rclBounds, const uint32_t cpts,
                            const U_POINT16 *points);
char *U_EMRPOLYPOLYLINE16_set(const U_RECTL rclBounds, const uint32_t nPolys,
                              const uint32_t *aPolyCounts, const uint32_t cpts,
                              const U_POINT16 *points);
char *U_EMRPOLYPOLYGON16_set(const U_RECTL rclBounds, const uint32_t nPolys,
                             const uint32_t *aPolyCounts, const uint32_t cpts,
                             const U_POINT16 *points);
char *U_EMRPOLYDRAW16_set(const U_RECTL rclBounds, const U_NUM_POINT16 cpts,
...
char *U_EMRCREATEDIBPATTERNBRUSHPT_set(const uint32_t ihBrush,
                                       ...
                                       const uint32_t cbPx, const char *Px);
char *U_EMREXTCREATEPEN_set(const uint32_t ihPen, const PU_BITMAPINFO Bmi,
                            const uint32_t cbPx, char *Px,
...
char *U_EMRSMALLTEXTOUT_set(const U_POINTL Dest, const U_NUM_STR cChars,
                            const uint32_t fuOptions,
                            const uint32_t iGraphicsMode, const U_FLOAT exScale,
                            const U_FLOAT eyScale, const U_RECTL rclBounds,
...
char *U_EMRALPHABLEND_set(const U_RECTL rclBounds, const U_POINTL Dest,
                          const U_POINTL cDest, const U_POINTL Src,
                          const U_POINTL cSrc, const U_XFORM xformSrc,
                          const U_COLORREF crBkColorSrc,
                          const uint32_t iUsageSrc, const U_BLEND Blend,
                          const PU_BITMAPINFO Bmi, const uint32_t cbPx,
                          char *Px);
char *U_EMRSETLAYOUT_set(const uint32_t iMode);
char *U_EMRTRANSPARENTBLT_set(const U_RECTL rclBounds, const U_POINTL Dest,
                              const U_POINTL cDest, const U_POINTL Src,
                              const U_POINTL cSrc, const U_XFORM xformSrc,
                              const U_COLORREF crBkColorSrc,
                              const uint32_t iUsageSrc, const uint32_t TColor,
                              const PU_BITMAPINFO Bmi, const uint32_t cbPx,
                              char *Px);
// U_EMRUNDEF117_set                 117  Not implemented
char *U_EMRGRADIENTFILL_set(const U_RECTL rclBounds,
                            const U_NUM_TRIVERTEX nTriVert,
                            const U_NUM_GRADOBJ nGradObj, const uint32_t ulMode,
...
char *U_EMRCREATECOLORSPACEW_set(const uint32_t ihCS,
                                 ...
                                 const uint32_t dwFlags, const U_CBDATA cbData,
                                 const uint8_t *Data);
