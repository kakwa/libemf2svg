/**
  ...
  These functions check the record types and report if there are any issues
    that could cause a memory access problem.  All counts and offsets are
  examined
...
*/
int extlogpen_safe(PU_EXTLOGPEN elp, const char *blimit) {
...
/**
   ...
   U_EMRTEXT
    \param blimit    one byte past the end of the record.
*/
int emrtext_safe(PU_EMRTEXT pemt, const char *record, const char *blimit) {
    int off;
    ...
    if (IS_MEM_UNSAFE(pemt, off + 4, blimit))
        return (0);
    if (IS_MEM_UNSAFE(record, offDx + count * 4, blimit))
        return (0);
    return (1);
}
...
*/
int rgndata_safe(PU_RGNDATA rd, int cbRgnData) {
...
/**
    \brief Check that the bitmap in the specified DIB is compatible with the
   record size
...
*/
int DIB_safe(const char *record, uint32_t iUsage, uint32_t offBmi,
    ...
    if (IS_MEM_UNSAFE(record, offBmi + cbBmi, blimit))
        return (0);
    if (!bitmapinfo_safe(record + offBmi, blimit))
        return (0); // checks the number of colors
...
/* **********************************************************************************************
These functions contain shared code used by various U_EMR*_safe functions.
...
int core1_safe(const char *record) {
    if (!core5_safe(record, U_SIZE_EMRPOLYLINETO))
        return (0);
...
int core2_safe(const char *record) {
    if (!core5_safe(record, U_SIZE_EMRPOLYPOLYLINE))
        return (0);
...
int core3_safe(const char *record) {
    if (!core5_safe(record, U_SIZE_EMRSETMAPMODE))
        return (0);
    return (1);
}

// Functions taking a single U_RECT or U_RECTL, starting with U_EMRELLIPSE_safe,
// also U_EMRFILLPATH_safe,
int core4_safe(const char *record) {
    if (!core5_safe(record, U_SIZE_EMRELLIPSE))
        return (0);
    return (1);
}
...
int core6_safe(const char *record) {
    if (!core5_safe(record, U_SIZE_EMRPOLYBEZIER16))
        return (0);
...
// Records with the same form starting with U_EMRSETWINDOWEXTEX_safe, that is,
// all with two uint32_t values after the emr
int core7_safe(const char *record) {
    if (!core5_safe(record, U_SIZE_EMRSETWINDOWEXTEX))
        return (0);
    return (1);
}
...
int core8_safe(const char *record) {
    if (!core5_safe(record, U_SIZE_EMREXTTEXTOUTA))
        return (0);
    PU_EMREXTTEXTOUTA pEmr = (PU_EMREXTTEXTOUTA)(record);
    const char *blimit = record + pEmr->emr.nSize;
    if (!emrtext_safe(&(pEmr->emrtext), record, blimit))
        return (0);
    return (1);
}
...
int core9_safe(const char *record) {
    if (!core5_safe(record, U_SIZE_EMRARC))
        return (0);
    return (1);
}
...
int core10_safe(const char *record) {
    if (!core5_safe(record, U_SIZE_EMRPOLYPOLYLINE16))
        return (0);
...
// Functions with the same form starting with  U_EMRINVERTRGN_safe and
// U_EMRPAINTRGN_safe,
int core11_safe(const char *record) {
    if (!core5_safe(record, U_SIZE_EMRINVERTRGN))
        return (0);
...
// common code for U_EMRCREATEMONOBRUSH_safe and
// U_EMRCREATEDIBPATTERNBRUSHPT_safe,
int core12_safe(const char *record) {
    if (!core5_safe(record, U_SIZE_EMRCREATEMONOBRUSH))
        return (0);
...
int core13_safe(const char *record) {
    if (!core5_safe(record, U_SIZE_EMRALPHABLEND))
        return (0);
    ...
    return (DIB_safe(record, iUsageSrc, offBmiSrc, cbBmiSrc, offBitsSrc,
                     cbBitsSrc, blimit));
}

/* **********************************************************************************************
These are the core EMR_safe functions, each converts a particular type of
record.
...
int U_EMRNOTIMPLEMENTED_safe(const char *record) {
    fprintf(stderr, "EMF WARNING:  could not safety check record because that "
...
int U_EMREOF_safe(const char *record) {
    if (!core5_safe(record, U_SIZE_EMREOF))
        return (0);
...
int U_EMRPOLYDRAW_safe(const char *record) {
    if (!core5_safe(record, U_SIZE_EMRPOLYDRAW))
        return (0);
...
int U_EMRCOMMENT_safe(const char *record) {
    if (!core5_safe(record, U_SIZE_EMRCOMMENT))
        return (0);
...
int U_EMRFILLRGN_safe(const char *record) {
    if (!core5_safe(record, U_SIZE_EMRFILLRGN))
        return (0);
...
int U_EMRFRAMERGN_safe(const char *record) {
    if (!core5_safe(record, U_SIZE_EMRFRAMERGN))
        return (0);
...
int U_EMREXTSELECTCLIPRGN_safe(const char *record) {
    if (!core5_safe(record, U_SIZE_EMREXTSELECTCLIPRGN))
        return (0);
    ...
    if (pEmr->iMode == U_RGN_COPY && !cbRgnData)
        return (1);
    const char *blimit = record + pEmr->emr.nSize;
    if (IS_MEM_UNSAFE(pEmr->RgnData, cbRgnData, blimit))
        return (0);
...
int U_EMRBITBLT_safe(const char *record) {
    if (!core5_safe(record, U_SIZE_EMRBITBLT))
        return (0);
...
int U_EMRSTRETCHBLT_safe(const char *record) {
    if (!core5_safe(record, U_SIZE_EMRSTRETCHBLT))
        return (0);
    PU_EMRBITBLT pEmr = (PU_EMRBITBLT)(record);
    const char *blimit = record + pEmr->emr.nSize;
    U_OFFBMISRC offBmiSrc = pEmr->offBmiSrc;
...
int U_EMRMASKBLT_safe(const char *record) {
    if (!core5_safe(record, U_SIZE_EMRMASKBLT))
        return (0);
    ...
    if (!DIB_safe(record, iUsageSrc, offBmiSrc, cbBmiSrc, offBitsSrc, cbBitsSrc,
                  blimit))
        return (0);
...
int U_EMRPLGBLT_safe(const char *record) {
    if (!core5_safe(record, U_SIZE_EMRPLGBLT))
        return (0);
...
int U_EMRSETDIBITSTODEVICE_safe(const char *record) {
    if (!core5_safe(record, U_SIZE_EMRSETDIBITSTODEVICE))
        return (0);
...
int U_EMRSTRETCHDIBITS_safe(const char *record) {
    if (!core5_safe(record, U_SIZE_EMRSTRETCHDIBITS))
        return (0);
...
int U_EMREXTCREATEFONTINDIRECTW_safe(const char *record) {
    /* Panose or logfont, LogFontExDv is not supported.  Test smallest to
     * largest */
    if (core5_safe(record, U_SIZE_EMREXTCREATEFONTINDIRECTW_LOGFONT))
        return (1);
    return (
...
int U_EMRPOLYDRAW16_safe(const char *record) {
    if (!core5_safe(record, U_SIZE_EMRPOLYDRAW16))
        return (0);
...
int U_EMREXTCREATEPEN_safe(const char *record) {
    if (!core5_safe(record, U_SIZE_EMREXTCREATEPEN))
        return (0);
    ...
    U_OFFBITS offBits = pEmr->offBits;
    U_CBBITS cbBits = pEmr->cbBits;
    if (!DIB_safe(record, U_DIB_RGB_COLORS, offBmi, cbBmi, offBits, cbBits,
                  blimit))
        return (0);
...
#define U_EMRPOLYTEXTOUTA_safe(A)                                              \
    U_EMRNOTIMPLEMENTED_safe(A) //!< Not implemented.
// U_EMRPOLYTEXTOUTW         97 NOT IMPLEMENTED, denigrated after Windows NT
#define U_EMRPOLYTEXTOUTW_safe(A)                                              \
    U_EMRNOTIMPLEMENTED_safe(A) //!< Not implemented.
...
#define U_EMRGLSBOUNDEDRECORD_safe(A)                                          \
    U_EMRNOTIMPLEMENTED_safe(A) //!< Not implemented.
...
int U_EMRSMALLTEXTOUT_safe(const char *record) {
    if (!core5_safe(record, U_SIZE_EMRSMALLTEXTOUT))
        return (0);
...
#define U_EMRFORCEUFIMAPPING_safe(A)                                           \
    U_EMRNOTIMPLEMENTED_safe(A) //!< Not implemented.
// U_EMRNAMEDESCAPE         110  Not implemented
#define U_EMRNAMEDESCAPE_safe(A)                                               \
    U_EMRNOTIMPLEMENTED_safe(A) //!< Not implemented.
// U_EMRCOLORCORRECTPALETTE 111  Not implemented
#define U_EMRCOLORCORRECTPALETTE_safe(A)                                       \
    U_EMRNOTIMPLEMENTED_safe(A) //!< Not implemented.
// U_EMRSETICMPROFILEA      112  Not implemented
#define U_EMRSETICMPROFILEA_safe(A)                                            \
    U_EMRNOTIMPLEMENTED_safe(A) //!< Not implemented.
// U_EMRSETICMPROFILEW      113  Not implemented
#define U_EMRSETICMPROFILEW_safe(A)                                            \
    U_EMRNOTIMPLEMENTED_safe(A) //!< Not implemented.
...
int U_EMRGRADIENTFILL_safe(const char *record) {
    if (!core5_safe(record, U_SIZE_EMRGRADIENTFILL))
        return (0);
...
#define U_EMRSETLINKEDUFIS_safe(A)                                             \
    U_EMRNOTIMPLEMENTED_safe(A) //!< Not implemented.
// U_EMRSETTEXTJUSTIFICATION120  Not implemented (denigrated)
#define U_EMRSETTEXTJUSTIFICATION_safe(A)                                      \
    U_EMRNOTIMPLEMENTED_safe(A) //!< Not implemented.
// U_EMRCOLORMATCHTOTARGETW 121  Not implemented
#define U_EMRCOLORMATCHTOTARGETW_safe(A)                                       \
    U_EMRNOTIMPLEMENTED_safe(A) //!< Not implemented.
...
/**
    ...
    It is NOT safe to call this routine without first calling
   U_emf_record_sizeok)()!
    If the file has been converted from one endian to another calling this
   routine is
...
int U_emf_record_safe(const char *record) {
    ...
    case U_EMR_HEADER:
        rstatus = U_EMRHEADER_safe(record);
        break;
    case U_EMR_POLYBEZIER:
        rstatus = U_EMRPOLYBEZIER_safe(record);
        break;
    case U_EMR_POLYGON:
        rstatus = U_EMRPOLYGON_safe(record);
        break;
    case U_EMR_POLYLINE:
        rstatus = U_EMRPOLYLINE_safe(record);
        break;
    case U_EMR_POLYBEZIERTO:
        rstatus = U_EMRPOLYBEZIERTO_safe(record);
        break;
    case U_EMR_POLYLINETO:
        rstatus = U_EMRPOLYLINETO_safe(record);
        break;
    case U_EMR_POLYPOLYLINE:
        rstatus = U_EMRPOLYPOLYLINE_safe(record);
        break;
    case U_EMR_POLYPOLYGON:
        rstatus = U_EMRPOLYPOLYGON_safe(record);
        break;
    case U_EMR_SETWINDOWEXTEX:
        rstatus = U_EMRSETWINDOWEXTEX_safe(record);
        break;
    case U_EMR_SETWINDOWORGEX:
        rstatus = U_EMRSETWINDOWORGEX_safe(record);
        break;
    case U_EMR_SETVIEWPORTEXTEX:
        rstatus = U_EMRSETVIEWPORTEXTEX_safe(record);
        break;
    case U_EMR_SETVIEWPORTORGEX:
        rstatus = U_EMRSETVIEWPORTORGEX_safe(record);
        break;
    case U_EMR_SETBRUSHORGEX:
        rstatus = U_EMRSETBRUSHORGEX_safe(record);
        break;
    case U_EMR_EOF:
        rstatus = U_EMREOF_safe(record);
        break;
    case U_EMR_SETPIXELV:
        rstatus = U_EMRSETPIXELV_safe(record);
        break;
    case U_EMR_SETMAPPERFLAGS:
        rstatus = U_EMRSETMAPPERFLAGS_safe(record);
        break;
    case U_EMR_SETMAPMODE:
        rstatus = U_EMRSETMAPMODE_safe(record);
        break;
    case U_EMR_SETBKMODE:
        rstatus = U_EMRSETBKMODE_safe(record);
        break;
    case U_EMR_SETPOLYFILLMODE:
        rstatus = U_EMRSETPOLYFILLMODE_safe(record);
        break;
    case U_EMR_SETROP2:
        rstatus = U_EMRSETROP2_safe(record);
        break;
    case U_EMR_SETSTRETCHBLTMODE:
        rstatus = U_EMRSETSTRETCHBLTMODE_safe(record);
        break;
    case U_EMR_SETTEXTALIGN:
        rstatus = U_EMRSETTEXTALIGN_safe(record);
        break;
    case U_EMR_SETCOLORADJUSTMENT:
        rstatus = U_EMRSETCOLORADJUSTMENT_safe(record);
        break;
    case U_EMR_SETTEXTCOLOR:
        rstatus = U_EMRSETTEXTCOLOR_safe(record);
        break;
    case U_EMR_SETBKCOLOR:
        rstatus = U_EMRSETBKCOLOR_safe(record);
        break;
    case U_EMR_OFFSETCLIPRGN:
        rstatus = U_EMROFFSETCLIPRGN_safe(record);
        break;
    case U_EMR_MOVETOEX:
        rstatus = U_EMRMOVETOEX_safe(record);
        break;
    case U_EMR_SETMETARGN:
        rstatus = U_EMRSETMETARGN_safe(record);
        break;
    case U_EMR_EXCLUDECLIPRECT:
        rstatus = U_EMREXCLUDECLIPRECT_safe(record);
        break;
    case U_EMR_INTERSECTCLIPRECT:
        rstatus = U_EMRINTERSECTCLIPRECT_safe(record);
        break;
    case U_EMR_SCALEVIEWPORTEXTEX:
        rstatus = U_EMRSCALEVIEWPORTEXTEX_safe(record);
        break;
    case U_EMR_SCALEWINDOWEXTEX:
        rstatus = U_EMRSCALEWINDOWEXTEX_safe(record);
        break;
    case U_EMR_SAVEDC:
        rstatus = U_EMRSAVEDC_safe(record);
        break;
    case U_EMR_RESTOREDC:
        rstatus = U_EMRRESTOREDC_safe(record);
        break;
    case U_EMR_SETWORLDTRANSFORM:
        rstatus = U_EMRSETWORLDTRANSFORM_safe(record);
        break;
    case U_EMR_MODIFYWORLDTRANSFORM:
        rstatus = U_EMRMODIFYWORLDTRANSFORM_safe(record);
        break;
    case U_EMR_SELECTOBJECT:
        rstatus = U_EMRSELECTOBJECT_safe(record);
        break;
    case U_EMR_CREATEPEN:
        rstatus = U_EMRCREATEPEN_safe(record);
        break;
    case U_EMR_CREATEBRUSHINDIRECT:
        rstatus = U_EMRCREATEBRUSHINDIRECT_safe(record);
        break;
    case U_EMR_DELETEOBJECT:
        rstatus = U_EMRDELETEOBJECT_safe(record);
        break;
    case U_EMR_ANGLEARC:
        rstatus = U_EMRANGLEARC_safe(record);
        break;
    case U_EMR_ELLIPSE:
        rstatus = U_EMRELLIPSE_safe(record);
        break;
    case U_EMR_RECTANGLE:
        rstatus = U_EMRRECTANGLE_safe(record);
        break;
    case U_EMR_ROUNDRECT:
        rstatus = U_EMRROUNDRECT_safe(record);
        break;
    case U_EMR_ARC:
        rstatus = U_EMRARC_safe(record);
        break;
    case U_EMR_CHORD:
        rstatus = U_EMRCHORD_safe(record);
        break;
    case U_EMR_PIE:
        rstatus = U_EMRPIE_safe(record);
        break;
    case U_EMR_SELECTPALETTE:
        rstatus = U_EMRSELECTPALETTE_safe(record);
        break;
    case U_EMR_CREATEPALETTE:
        rstatus = U_EMRCREATEPALETTE_safe(record);
        break;
    case U_EMR_SETPALETTEENTRIES:
        rstatus = U_EMRSETPALETTEENTRIES_safe(record);
        break;
    case U_EMR_RESIZEPALETTE:
        rstatus = U_EMRRESIZEPALETTE_safe(record);
        break;
    case U_EMR_REALIZEPALETTE:
        rstatus = U_EMRREALIZEPALETTE_safe(record);
        break;
    case U_EMR_EXTFLOODFILL:
        rstatus = U_EMREXTFLOODFILL_safe(record);
        break;
    case U_EMR_LINETO:
        rstatus = U_EMRLINETO_safe(record);
        break;
    case U_EMR_ARCTO:
        rstatus = U_EMRARCTO_safe(record);
        break;
    case U_EMR_POLYDRAW:
        rstatus = U_EMRPOLYDRAW_safe(record);
        break;
    case U_EMR_SETARCDIRECTION:
        rstatus = U_EMRSETARCDIRECTION_safe(record);
        break;
    case U_EMR_SETMITERLIMIT:
        rstatus = U_EMRSETMITERLIMIT_safe(record);
        break;
    case U_EMR_BEGINPATH:
        rstatus = U_EMRBEGINPATH_safe(record);
        break;
    case U_EMR_ENDPATH:
        rstatus = U_EMRENDPATH_safe(record);
        break;
    case U_EMR_CLOSEFIGURE:
        rstatus = U_EMRCLOSEFIGURE_safe(record);
        break;
    case U_EMR_FILLPATH:
        rstatus = U_EMRFILLPATH_safe(record);
        break;
    case U_EMR_STROKEANDFILLPATH:
        rstatus = U_EMRSTROKEANDFILLPATH_safe(record);
        break;
    case U_EMR_STROKEPATH:
        rstatus = U_EMRSTROKEPATH_safe(record);
        break;
    case U_EMR_FLATTENPATH:
        rstatus = U_EMRFLATTENPATH_safe(record);
        break;
    case U_EMR_WIDENPATH:
        rstatus = U_EMRWIDENPATH_safe(record);
        break;
    case U_EMR_SELECTCLIPPATH:
        rstatus = U_EMRSELECTCLIPPATH_safe(record);
        break;
    case U_EMR_ABORTPATH:
        rstatus = U_EMRABORTPATH_safe(record);
        break;
    case U_EMR_UNDEF69:
        rstatus = U_EMRUNDEF69_safe(record);
        break;
    case U_EMR_COMMENT:
        rstatus = U_EMRCOMMENT_safe(record);
        break;
    case U_EMR_FILLRGN:
        rstatus = U_EMRFILLRGN_safe(record);
        break;
    case U_EMR_FRAMERGN:
        rstatus = U_EMRFRAMERGN_safe(record);
        break;
    case U_EMR_INVERTRGN:
        rstatus = U_EMRINVERTRGN_safe(record);
        break;
    case U_EMR_PAINTRGN:
        rstatus = U_EMRPAINTRGN_safe(record);
        break;
    case U_EMR_EXTSELECTCLIPRGN:
        rstatus = U_EMREXTSELECTCLIPRGN_safe(record);
        break;
    case U_EMR_BITBLT:
        rstatus = U_EMRBITBLT_safe(record);
        break;
    case U_EMR_STRETCHBLT:
        rstatus = U_EMRSTRETCHBLT_safe(record);
        break;
    case U_EMR_MASKBLT:
        rstatus = U_EMRMASKBLT_safe(record);
        break;
    case U_EMR_PLGBLT:
        rstatus = U_EMRPLGBLT_safe(record);
        break;
    case U_EMR_SETDIBITSTODEVICE:
        rstatus = U_EMRSETDIBITSTODEVICE_safe(record);
        break;
    case U_EMR_STRETCHDIBITS:
        rstatus = U_EMRSTRETCHDIBITS_safe(record);
        break;
    case U_EMR_EXTCREATEFONTINDIRECTW:
        rstatus = U_EMREXTCREATEFONTINDIRECTW_safe(record);
        break;
    case U_EMR_EXTTEXTOUTA:
        rstatus = U_EMREXTTEXTOUTA_safe(record);
        break;
    case U_EMR_EXTTEXTOUTW:
        rstatus = U_EMREXTTEXTOUTW_safe(record);
        break;
    case U_EMR_POLYBEZIER16:
        rstatus = U_EMRPOLYBEZIER16_safe(record);
        break;
    case U_EMR_POLYGON16:
        rstatus = U_EMRPOLYGON16_safe(record);
        break;
    case U_EMR_POLYLINE16:
        rstatus = U_EMRPOLYLINE16_safe(record);
        break;
    case U_EMR_POLYBEZIERTO16:
        rstatus = U_EMRPOLYBEZIERTO16_safe(record);
        break;
    case U_EMR_POLYLINETO16:
        rstatus = U_EMRPOLYLINETO16_safe(record);
        break;
    case U_EMR_POLYPOLYLINE16:
        rstatus = U_EMRPOLYPOLYLINE16_safe(record);
        break;
    case U_EMR_POLYPOLYGON16:
        rstatus = U_EMRPOLYPOLYGON16_safe(record);
        break;
    case U_EMR_POLYDRAW16:
        rstatus = U_EMRPOLYDRAW16_safe(record);
        break;
    case U_EMR_CREATEMONOBRUSH:
        rstatus = U_EMRCREATEMONOBRUSH_safe(record);
        break;
    case U_EMR_CREATEDIBPATTERNBRUSHPT:
        rstatus = U_EMRCREATEDIBPATTERNBRUSHPT_safe(record);
        break;
    case U_EMR_EXTCREATEPEN:
        rstatus = U_EMREXTCREATEPEN_safe(record);
        break;
    case U_EMR_POLYTEXTOUTA:
        rstatus = U_EMRPOLYTEXTOUTA_safe(record);
        break;
    case U_EMR_POLYTEXTOUTW:
        rstatus = U_EMRPOLYTEXTOUTW_safe(record);
        break;
    case U_EMR_SETICMMODE:
        rstatus = U_EMRSETICMMODE_safe(record);
        break;
    case U_EMR_CREATECOLORSPACE:
        rstatus = U_EMRCREATECOLORSPACE_safe(record);
        break;
    case U_EMR_SETCOLORSPACE:
        rstatus = U_EMRSETCOLORSPACE_safe(record);
        break;
    case U_EMR_DELETECOLORSPACE:
        rstatus = U_EMRDELETECOLORSPACE_safe(record);
        break;
    case U_EMR_GLSRECORD:
        rstatus = U_EMRGLSRECORD_safe(record);
        break;
    case U_EMR_GLSBOUNDEDRECORD:
        rstatus = U_EMRGLSBOUNDEDRECORD_safe(record);
        break;
    case U_EMR_PIXELFORMAT:
        rstatus = U_EMRPIXELFORMAT_safe(record);
        break;
    case U_EMR_DRAWESCAPE:
        rstatus = U_EMRDRAWESCAPE_safe(record);
        break;
    case U_EMR_EXTESCAPE:
        rstatus = U_EMREXTESCAPE_safe(record);
        break;
    case U_EMR_UNDEF107:
        rstatus = U_EMRUNDEF107_safe(record);
        break;
    case U_EMR_SMALLTEXTOUT:
        rstatus = U_EMRSMALLTEXTOUT_safe(record);
        break;
    case U_EMR_FORCEUFIMAPPING:
        rstatus = U_EMRFORCEUFIMAPPING_safe(record);
        break;
    case U_EMR_NAMEDESCAPE:
        rstatus = U_EMRNAMEDESCAPE_safe(record);
        break;
    case U_EMR_COLORCORRECTPALETTE:
        rstatus = U_EMRCOLORCORRECTPALETTE_safe(record);
        break;
    case U_EMR_SETICMPROFILEA:
        rstatus = U_EMRSETICMPROFILEA_safe(record);
        break;
    case U_EMR_SETICMPROFILEW:
        rstatus = U_EMRSETICMPROFILEW_safe(record);
        break;
    case U_EMR_ALPHABLEND:
        rstatus = U_EMRALPHABLEND_safe(record);
        break;
    case U_EMR_SETLAYOUT:
        rstatus = U_EMRSETLAYOUT_safe(record);
        break;
    case U_EMR_TRANSPARENTBLT:
        rstatus = U_EMRTRANSPARENTBLT_safe(record);
        break;
    case U_EMR_UNDEF117:
        rstatus = U_EMRUNDEF117_safe(record);
        break;
    case U_EMR_GRADIENTFILL:
        rstatus = U_EMRGRADIENTFILL_safe(record);
        break;
    case U_EMR_SETLINKEDUFIS:
        rstatus = U_EMRSETLINKEDUFIS_safe(record);
        break;
    case U_EMR_SETTEXTJUSTIFICATION:
        rstatus = U_EMRSETTEXTJUSTIFICATION_safe(record);
        break;
    case U_EMR_COLORMATCHTOTARGETW:
        rstatus = U_EMRCOLORMATCHTOTARGETW_safe(record);
        break;
    case U_EMR_CREATECOLORSPACEW:
        rstatus = U_EMRCREATECOLORSPACEW_safe(record);
        break;
    default:
        rstatus = U_EMRNOTIMPLEMENTED_safe(record);
        break;
    } // end of switch
    return (rstatus);
}
