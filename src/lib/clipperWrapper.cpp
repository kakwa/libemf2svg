...
tupleSegments i_bezierToLine(clipSegmentList *seg, uint32_t depth) {
    ...
    clipSegmentList *newSeg1 =
        (clipSegmentList *)malloc(sizeof(clipSegmentList));
    clipSegmentList *newSeg2 =
        (clipSegmentList *)malloc(sizeof(clipSegmentList));
    ...
    newSeg1->seg.c2.x =
        seg->seg.st.x / 4 + seg->seg.c1.x * 2 / 4 + seg->seg.c2.x * 1 / 4;
    newSeg1->seg.en.x = seg->seg.st.x / 8 + seg->seg.c1.x * 3 / 8 +
                        seg->seg.c2.x * 3 / 8 + seg->seg.en.x / 8;

    newSeg1->seg.st.y = seg->seg.st.y;
    newSeg1->seg.c1.y = seg->seg.st.y / 2 + seg->seg.c1.y * 1 / 2;
    newSeg1->seg.c2.y =
        seg->seg.st.y / 4 + seg->seg.c1.y * 2 / 4 + seg->seg.c2.y * 1 / 4;
    newSeg1->seg.en.y = seg->seg.st.y / 8 + seg->seg.c1.y * 3 / 8 +
                        seg->seg.c2.y * 3 / 8 + seg->seg.en.y / 8;

    newSeg2->seg.st.x = seg->seg.st.x / 8 + seg->seg.c1.x * 3 / 8 +
                        seg->seg.c2.x * 3 / 8 + seg->seg.en.x / 8;
    newSeg2->seg.c1.x =
        seg->seg.c1.x * 1 / 4 + seg->seg.c2.x * 2 / 4 + seg->seg.en.x / 4;
    newSeg2->seg.c2.x = seg->seg.c2.x * 1 / 2 + seg->seg.en.x / 2;
    newSeg2->seg.en.x = seg->seg.en.x;

    newSeg2->seg.st.y = seg->seg.st.y / 8 + seg->seg.c1.y * 3 / 8 +
                        seg->seg.c2.y * 3 / 8 + seg->seg.en.y / 8;
    newSeg2->seg.c1.y =
        seg->seg.c1.y * 1 / 4 + seg->seg.c2.y * 2 / 4 + seg->seg.en.y / 4;
