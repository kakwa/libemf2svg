/**
  ...
  [U_EMR]_set all take data and return a pointer to memory holding the
  constructed record.
  The size of that record is also returned in recsize.
  It is also in the second int32 in the record, but may have been byte swapped
  and so not usable.
...
//! \cond
/* one prototype from uemf_endian.  Put it here because end user should never
need to see it, so
...
/**
    \brief Look up the name of the EMR record by type.  Returns U_EMR_INVALID if
   out of range.
...
char *U_emr_names(unsigned int idx) {
    if (idx < U_EMR_MIN || idx > U_EMR_MAX) {
        idx = 0;
    }
    static char *U_EMR_NAMES[U_EMR_MAX + 1] = {"U_EMR_INVALID",
                                               "U_EMR_HEADER",
                                               "U_EMR_POLYBEZIER",
                                               "U_EMR_POLYGON",
                                               "U_EMR_POLYLINE",
...
#define SET_CB_FROM_PXBMI(                                                     \
    ...
    if (A) {                                                                   \
        ...
        E = sizeof(U_BITMAPINFOHEADER) +                                       \
            4 * get_real_color_count((const char *)&(                          \
                    B->bmiHeader)); /*  bmiheader + colortable*/               \
    } else {                                                                   \
        C = 0;                                                                 \
        D = 0;                                                                 \
        E = 0;                                                                 \
...
#define APPEND_PXBMISRC(                                                       \
    A, B, C, D, E, F,                                                          \
    G) /* A=record, B=U_EMR,C=cbBmi, D=Bmi, E=Px, F=cbImage, G=cbImage4 */     \
    if (C) {                                                                   \
        memcpy(A + off, D, C);                                                 \
        ((B *)A)->offBmiSrc = off;                                             \
        ((B *)A)->cbBmiSrc = C;                                                \
...
#define APPEND_MSKBMISRC(A, B, C, D, E, F,                                     \
                         G) /* A=record, B=U_EMR*,C=cbMskBmi, D=MskBmi,        \
                               E=Msk, F=cbMskImage, G=cbMskImage4 */           \
    if (C) {                                                                   \
        memcpy(A + off, D, C);                                                 \
        ((B *)A)->offBmiMask = off;                                            \
        ((B *)A)->cbBmiMask = C;                                               \
...
/* **********************************************************************************************
These functions are used for development and debugging and should be be
includied in production code.
***********************************************************************************************
*/

/**
    \brief Debugging utility, used with valgrind to find uninitialized values.
...
int memprobe(const void *buf, size_t size) {
    ...
    for (; size; size--, ptr++) {
        sum += *ptr;
    } // read all bytes, trigger valgrind warning if any uninitialized
    return (sum);
}
...
/**
    \brief Make up an approximate dx array to pass to emrtext_set(), based on
...
/**
   ...
   are U_DRAW_NOTEMPTY, etc..
    ...
    \return bitmap of EMR record properties, or U_EMR_INVALID on error or
   release of all memory
    \param type EMR record type.  If U_EMR_INVALID release memory. (There is no
   U_EMR_INVALID EMR record type)

*/
uint32_t emr_properties(uint32_t type) {
    static uint32_t *table = NULL;
    uint32_t result = U_EMR_INVALID; // initialized to indicate an error (on a
    ...
    if (type == U_EMR_INVALID) {
        ...
        if (!table) {
            ...
            //                                                               0x02
            //                                                               0x01
            //                 Path properties (U_DRAW_*)
            //                 TEXT      ALTERS    ONLYTO    VISIBLE
            //                                                                    PATH      FORCE     CLOSED    NOTEMPTY
            table[0] = 0x00;   //   Does not map to any EMR record
            table[1] = 0x80;   //   U_EMRHEADER                    1    0    0
                               //   0    0    0    0    0
            table[2] = 0x83;   //   U_EMRPOLYBEZIER                1    0    0
                               //   0    0    0    1    1
            table[3] = 0x87;   //   U_EMRPOLYGON                   1    0    0
                               //   0    0    1    1    1
            table[4] = 0x83;   //   U_EMRPOLYLINE                  1    0    0
                               //   0    0    0    1    1
            table[5] = 0x8B;   //   U_EMRPOLYBEZIERTO              1    0    0
                               //   0    1    0    1    1
            table[6] = 0x8B;   //   U_EMRPOLYLINETO                1    0    0
                               //   0    1    0    1    1
            table[7] = 0x83;   //   U_EMRPOLYPOLYLINE              1    0    0
                               //   0    0    0    1    1
            table[8] = 0x87;   //   U_EMRPOLYPOLYGON               1    0    0
                               //   0    0    1    1    1
            table[9] = 0xA0;   //   U_EMRSETWINDOWEXTEX            1    0    1
                               //   0    0    0    0    0
            table[10] = 0xA0;  //   U_EMRSETWINDOWORGEX            1    0    1
                               //   0    0    0    0    0
            table[11] = 0xA0;  //   U_EMRSETVIEWPORTEXTEX          1    0    1
                               //   0    0    0    0    0
            table[12] = 0xA0;  //   U_EMRSETVIEWPORTORGEX          1    0    1
                               //   0    0    0    0    0
            table[13] = 0xA0;  //   U_EMRSETBRUSHORGEX             1    0    1
                               //   0    0    0    0    0
            table[14] = 0x82;  //   U_EMREOF                       1    0    1
                               //   0    0    0    0    0  Force out any pending
                               //   draw
            table[15] = 0x82;  //   U_EMRSETPIXELV                 1    0    0
                               //   0    0    0    1    0
            table[16] = 0xA0;  //   U_EMRSETMAPPERFLAGS            1    0    1
                               //   0    0    0    0    0
            table[17] = 0xA0;  //   U_EMRSETMAPMODE                1    0    1
                               //   0    0    0    0    0
            table[18] = 0x20;  //   U_EMRSETBKMODE                 0    0    1
                               //   0    0    0    0    0
            table[19] = 0xA0;  //   U_EMRSETPOLYFILLMODE           1    0    1
                               //   0    0    0    0    0
            table[20] = 0xA0;  //   U_EMRSETROP2                   1    0    1
                               //   0    0    0    0    0
            table[21] = 0xA0;  //   U_EMRSETSTRETCHBLTMODE         1    0    1
                               //   0    0    0    0    0
            table[22] = 0x20;  //   U_EMRSETTEXTALIGN              0    0    1
                               //   0    0    0    0    0
            table[23] = 0xA0;  //   U_EMRSETCOLORADJUSTMENT        1    0    1
                               //   0    0    0    0    0
            table[24] = 0x20;  //   U_EMRSETTEXTCOLOR              0    0    1
                               //   0    0    0    0    0
            table[25] = 0x20;  //   U_EMRSETBKCOLOR                0    0    1
                               //   0    0    0    0    0
            table[26] = 0xA0;  //   U_EMROFFSETCLIPRGN             1    0    1
                               //   0    0    0    0    0
            table[27] = 0x89;  //   U_EMRMOVETOEX                  1    0    0
                               //   0    1    0    0    1
            table[28] = 0xA0;  //   U_EMRSETMETARGN                1    0    1
                               //   0    0    0    0    0
            table[29] = 0xA0;  //   U_EMREXCLUDECLIPRECT           1    0    1
                               //   0    0    0    0    0
            table[30] = 0xA0;  //   U_EMRINTERSECTCLIPRECT         1    0    1
                               //   0    0    0    0    0
            table[31] = 0xA0;  //   U_EMRSCALEVIEWPORTEXTEX        1    0    1
                               //   0    0    0    0    0
            table[32] = 0xA0;  //   U_EMRSCALEWINDOWEXTEX          1    0    1
                               //   0    0    0    0    0
            table[33] = 0xA0;  //   U_EMRSAVEDC                    1    0    1
                               //   0    0    0    0    0
            table[34] = 0xA0;  //   U_EMRRESTOREDC                 1    0    1
                               //   0    0    0    0    0
            table[35] = 0xA0;  //   U_EMRSETWORLDTRANSFORM         1    0    1
                               //   0    0    0    0    0
            table[36] = 0xA0;  //   U_EMRMODIFYWORLDTRANSFORM      1    0    1
                               //   0    0    0    0    0
            table[37] = 0x20;  //   U_EMRSELECTOBJECT              0    0    1
                               //   0    0    0    0    0
            table[38] = 0x20;  //   U_EMRCREATEPEN                 0    0    1
                               //   0    0    0    0    0
            table[39] = 0x20;  //   U_EMRCREATEBRUSHINDIRECT       0    0    1
                               //   0    0    0    0    0
            table[40] = 0x20;  //   U_EMRDELETEOBJECT              0    0    1
                               //   0    0    0    0    0
            table[41] = 0x83;  //   U_EMRANGLEARC                  1    0    0
                               //   0    0    0    1    1
            table[42] = 0x87;  //   U_EMRELLIPSE                   1    0    0
                               //   0    0    1    1    1
            table[43] = 0x87;  //   U_EMRRECTANGLE                 1    0    0
                               //   0    0    1    1    1
            table[44] = 0x87;  //   U_EMRROUNDRECT                 1    0    0
                               //   0    0    1    1    1
            table[45] = 0x83;  //   U_EMRARC                       1    0    0
                               //   0    0    0    1    1
            table[46] = 0x87;  //   U_EMRCHORD                     1    0    0
                               //   0    0    1    1    1
            table[47] = 0x87;  //   U_EMRPIE                       1    0    0
                               //   0    0    1    1    1
            table[48] = 0xA0;  //   U_EMRSELECTPALETTE             1    0    1
                               //   0    0    0    0    0
            table[49] = 0xA0;  //   U_EMRCREATEPALETTE             1    0    1
                               //   0    0    0    0    0
            table[50] = 0xA0;  //   U_EMRSETPALETTEENTRIES         1    0    1
                               //   0    0    0    0    0
            table[51] = 0xA0;  //   U_EMRRESIZEPALETTE             1    0    1
                               //   0    0    0    0    0
            table[52] = 0xA0;  //   U_EMRREALIZEPALETTE            1    0    1
                               //   0    0    0    0    0
            table[53] = 0x82;  //   U_EMREXTFLOODFILL              1    0    0
                               //   0    0    0    1    0
            table[54] = 0x8B;  //   U_EMRLINETO                    1    0    0
                               //   0    1    0    1    1
            table[55] = 0x8B;  //   U_EMRARCTO                     1    0    0
                               //   0    1    0    1    1
            table[56] = 0x83;  //   U_EMRPOLYDRAW                  1    0    0
                               //   0    0    0    1    1
            table[57] = 0xA0;  //   U_EMRSETARCDIRECTION           1    0    1
                               //   0    0    0    0    0
            table[58] = 0xA0;  //   U_EMRSETMITERLIMIT             1    0    1
                               //   0    0    0    0    0
            table[59] = 0xE0;  //   U_EMRBEGINPATH                 1    1    1
                               //   0    0    0    0    0
            table[60] = 0x80;  //   U_EMRENDPATH                   1    0    0
                               //   0    0    0    0    0
            table[61] = 0x84;  //   U_EMRCLOSEFIGURE               1    0    0
                               //   0    0    1    0    0
            table[62] = 0x94;  //   U_EMRFILLPATH                  1    0    0
                               //   1    0    1    0    0
            table[63] = 0x94;  //   U_EMRSTROKEANDFILLPATH         1    0    0
                               //   1    0    1    0    0
            table[64] = 0x90;  //   U_EMRSTROKEPATH                1    0    0
                               //   1    0    0    0    0
            table[65] = 0xA0;  //   U_EMRFLATTENPATH               1    0    1
                               //   0    0    0    0    0
            table[66] = 0xA0;  //   U_EMRWIDENPATH                 1    0    1
                               //   0    0    0    0    0
            table[67] = 0x80;  //   U_EMRSELECTCLIPPATH            1    0    0
                               //   0    0    0    0    0  consumes the path,
                               //   draws nothing
            table[68] = 0xA0;  //   U_EMRABORTPATH                 1    0    1
                               //   0    0    0    0    0
            table[69] = 0xA0;  //   U_EMRUNDEF69                   1    0    1
                               //   0    0    0    0    0
            table[70] = 0x00;  //   U_EMRCOMMENT                   0    0    0
                               //   0    0    0    0    0
            table[71] = 0x82;  //   U_EMRFILLRGN                   1    0    0
                               //   0    0    0    1    0
            table[72] = 0x82;  //   U_EMRFRAMERGN                  1    0    0
                               //   0    0    0    1    0
            table[73] = 0x82;  //   U_EMRINVERTRGN                 1    0    0
                               //   0    0    0    1    0
            table[74] = 0x82;  //   U_EMRPAINTRGN                  1    0    0
                               //   0    0    0    1    0
            table[75] = 0xA0;  //   U_EMREXTSELECTCLIPRGN          1    0    1
                               //   0    0    0    0    0
            table[76] = 0x82;  //   U_EMRBITBLT                    1    0    0
                               //   0    0    0    1    0
            table[77] = 0x82;  //   U_EMRSTRETCHBLT                1    0    0
                               //   0    0    0    1    0
            table[78] = 0x82;  //   U_EMRMASKBLT                   1    0    0
                               //   0    0    0    1    0
            table[79] = 0x82;  //   U_EMRPLGBLT                    1    0    0
                               //   0    0    0    1    0
            table[80] = 0xA0;  //   U_EMRSETDIBITSTODEVICE         1    0    1
                               //   0    0    0    0    0
            table[81] = 0xA0;  //   U_EMRSTRETCHDIBITS             1    0    1
                               //   0    0    0    0    0
            table[82] = 0x20;  //   U_EMREXTCREATEFONTINDIRECTW    0    0    1
                               //   0    0    0    0    0
            table[83] = 0x02;  //   U_EMREXTTEXTOUTA               0    0    0
                               //   0    0    0    1    0
            table[84] = 0x02;  //   U_EMREXTTEXTOUTW               0    0    0
                               //   0    0    0    1    0
            table[85] = 0x83;  //   U_EMRPOLYBEZIER16              1    0    0
                               //   0    0    0    1    1
            table[86] = 0x83;  //   U_EMRPOLYGON16                 1    0    0
                               //   0    0    0    1    1
            table[87] = 0x83;  //   U_EMRPOLYLINE16                1    0    0
                               //   0    0    0    1    1
            table[88] = 0x8B;  //   U_EMRPOLYBEZIERTO16            1    0    0
                               //   0    1    0    1    1
            table[89] = 0x8B;  //   U_EMRPOLYLINETO16              1    0    0
                               //   0    1    0    1    1
            table[90] = 0x83;  //   U_EMRPOLYPOLYLINE16            1    0    0
                               //   0    0    0    1    1
            table[91] = 0x87;  //   U_EMRPOLYPOLYGON16             1    0    0
                               //   0    0    1    1    1
            table[92] = 0x83;  //   U_EMRPOLYDRAW16                1    0    0
                               //   0    0    0    1    1
            table[93] = 0x80;  //   U_EMRCREATEMONOBRUSH           1    0    0
                               //   0    0    0    0    0  Not selected yet, so
                               //   no change in drawing conditions
            table[94] = 0x80;  //   U_EMRCREATEDIBPATTERNBRUSHPT   1    0    0
                               //   0    0    0    0    0  "
            table[95] = 0x00;  //   U_EMREXTCREATEPEN              0    0    0
                               //   0    0    0    0    0  "
            table[96] = 0x02;  //   U_EMRPOLYTEXTOUTA              0    0    0
                               //   0    0    0    1    0
            table[97] = 0x02;  //   U_EMRPOLYTEXTOUTW              0    0    0
                               //   0    0    0    1    0
            table[98] = 0xA0;  //   U_EMRSETICMMODE                1    0    1
                               //   0    0    0    0    0
            table[99] = 0xA0;  //   U_EMRCREATECOLORSPACE          1    0    1
                               //   0    0    0    0    0
            table[100] = 0xA0; //   U_EMRSETCOLORSPACE             1    0    1
                               //   0    0    0    0    0
            table[101] = 0xA0; //   U_EMRDELETECOLORSPACE          1    0    1
                               //   0    0    0    0    0
            table[102] = 0xA0; //   U_EMRGLSRECORD                 1    0    1
                               //   0    0    0    0    0
            table[103] = 0xA0; //   U_EMRGLSBOUNDEDRECORD          1    0    1
                               //   0    0    0    0    0
            table[104] = 0xA0; //   U_EMRPIXELFORMAT               1    0    1
                               //   0    0    0    0    0
            table[105] = 0xA0; //   U_EMRDRAWESCAPE                1    0    1
                               //   0    0    0    0    0
            table[106] = 0xA0; //   U_EMREXTESCAPE                 1    0    1
                               //   0    0    0    0    0
            table[107] = 0xA0; //   U_EMRUNDEF107                  1    0    1
                               //   0    0    0    0    0
            table[108] = 0x02; //   U_EMRSMALLTEXTOUT              0    0    0
                               //   0    0    0    1    0
            table[109] = 0xA0; //   U_EMRFORCEUFIMAPPING           1    0    1
                               //   0    0    0    0    0
            table[110] = 0xA0; //   U_EMRNAMEDESCAPE               1    0    1
                               //   0    0    0    0    0
            table[111] = 0xA0; //   U_EMRCOLORCORRECTPALETTE       1    0    1
                               //   0    0    0    0    0
            table[112] = 0xA0; //   U_EMRSETICMPROFILEA            1    0    1
                               //   0    0    0    0    0
            table[113] = 0xA0; //   U_EMRSETICMPROFILEW            1    0    1
                               //   0    0    0    0    0
            table[114] = 0x82; //   U_EMRALPHABLEND                1    0    0
                               //   0    0    0    1    0
            table[115] = 0xA0; //   U_EMRSETLAYOUT                 1    0    1
                               //   0    0    0    0    0
            table[116] = 0x82; //   U_EMRTRANSPARENTBLT            1    0    0
                               //   0    0    0    1    0
            table[117] = 0xA0; //   U_EMRUNDEF117                  1    0    1
                               //   0    0    0    0    0
            table[118] = 0x82; //   U_EMRGRADIENTFILL              1    0    1
                               //   0    0    0    1    0
            table[119] = 0xA0; //   U_EMRSETLINKEDUFIS             1    0    1
                               //   0    0    0    0    0
            table[120] = 0x20; //   U_EMRSETTEXTJUSTIFICATION      0    0    1
                               //   0    0    0    0    0
            table[121] = 0xA0; //   U_EMRCOLORMATCHTOTARGETW       1    0    1
                               //   0    0    0    0    0
            table[122] = 0xA0; //   U_EMRCREATECOLORSPACEW         1    0    1
                               //   0    0    0    0    0
        }
        result = table[type];
    }
...
/**
    \brief Derive from bounding rect, start and end radials, for arc, chord, or
   pie, the center, start, and end points, and the bounding rectangle.
...
int emr_arc_points_common(PU_RECTL rclBox, PU_POINTL ArcStart, PU_POINTL ArcEnd,
    ...
    if (!scale)
        return (1);        // bogus record, has start at center
    ...
    if (!scale)
        return (2);      // bogus record, has end at center
    vec_eend.x /= scale; // now a unit vector
    vec_eend.y /= scale;

    // Find the intersection of the vectors with the ellipse.  With no loss of
    // generality
    // we can translate the ellipse to the origin, then we just need to find tu
    // (t a factor, u the unit vector)
    // that also satisfies (x/Rx)^2 + (y/Ry)^2 = 1.  x is t*(ux), y is t*(uy),
    // where ux,uy are the x,y components
    ...
    // lastly figure out if the swept angle is >180 degrees or not, based on the
...
/**
    \brief Derive from an EMF arc, chord, or pie the center, start, and end
   points, and the bounding rectangle.
...
int emr_arc_points(PU_ENHMETARECORD record, int *f1, int f2, PU_PAIRF center,
    ...
    return emr_arc_points_common(&(pEmr->rclBox), &(pEmr->ptlStart),
                                 &(pEmr->ptlEnd), f1, f2, center, start, end,
                                 size);
}

/**
    \brief Convert a U_RGBA 32 bit pixmap to one of many different types of DIB
   pixmaps.

    Conversions to formats using color tables assume that the color table can
   hold every color
    in the input image.  If that assumption is false then the conversion will
   fail.  Conversion
    from 8 bit color to N bit colors (N<8) do so by shifting the appropriate
   number of bits.
...
int RGBA_to_DIB(char **px, uint32_t *cbPx, PU_RGBQUAD *ct, int *numCt,
    ...
    *cbPx = 0;
    // sanity checking
    if (!w || !h || !stride || !colortype || !rgba_px)
        return (1);
    if (use_ct && colortype >= U_BCBM_COLOR16)
        return (2); // color tables not used above 16 bit pixels
    if (!use_ct && colortype < U_BCBM_COLOR16)
        return (3); // color tables mandatory for < 16 bit

    bs = colortype / 8;
    if (bs < 1) {
        usedbytes = (w * colortype + 7) /
                    8; // width of line in fully and partially occupied bytes
    } else {
        usedbytes = w * bs;
    }
    pad = UP4(usedbytes) - usedbytes; // DIB rows must be aligned on 4 byte
                                      // boundaries, they are padded at the end
                                      // to accomplish this.;
    ...
    for (i = istart; i != iend; i += iinc) {
        ...
        for (j = 0; j < w; j++) {
            ...
            if (use_ct) {
                color = U_BGRA(
                    r, g, b,
                    a); // color has order in memory: b,g,r,a, same as EMF+ ARGB
                index = -1;
                for (lct = *ct, k = 0; k < found; k++,
                    lct++) { // Is this color in the table (VERY inefficient if
                             // there are a lot of colors!!!)
                    if (*(uint32_t *)lct != *(uint32_t *)&color)
                        continue;
                    index = k;
                    break;
                }
                if (index == -1) { // add a color
                    found++;
                    if (found > *numCt) { // More colors found than are
                                          // supported by the color table
                        free(*ct);
                        free(*px);
                        *numCt = 0;
                        *cbPx = 0;
                ...
                switch (colortype) {
                case U_BCBM_MONOCHROME: // 2 colors.    bmiColors array has two
                                        // entries
                    tmp8 = tmp8 >> 1; // This seems wrong, as it fills from the
                                      // top of each byte.  But it works.
                    tmp8 |= index << 7;
                    if (!((j + 1) % 8)) {
                        *pxptr++ = tmp8;
                        tmp8 = 0;
                    ...
                    break;
                case U_BCBM_COLOR4: // 2^4 colors.  bmiColors array has 16
                                    // entries
                    tmp8 = tmp8 << 4;
                    tmp8 |= index;
                    if (!((j + 1) % 2)) {
                    ...
                    break;
                case U_BCBM_COLOR8: // 2^8 colors.  bmiColors array has 256
                                    // entries
                    tmp8 = index;
                    *pxptr++ = tmp8;
                    break;
                case U_BCBM_COLOR16:  // 2^16 colors. (Several different color
                                      // methods))
                case U_BCBM_COLOR24:  // 2^24 colors. bmiColors is not used.
                                      // Pixels are U_RGBTRIPLE.
                case U_BCBM_COLOR32:  // 2^32 colors. bmiColors is not used.
                                      // Pixels are U_RGBQUAD.
                case U_BCBM_EXPLICIT: // Derinved from JPG or PNG compressed
                                      // image or ?
                default:
                    return (7); // This should not be possible, but might happen
                ...
                case U_BCBM_COLOR16: // 2^16 colors. (Several different color
                    ...
                    g /= 8;
                    r /= 8;
                    // Do it in this way so that the bytes are always stored
                    // Little Endian
                    tmp8 = b;
                    tmp8 |= g << 5; // least significant 3 bits of green
                    *pxptr++ = tmp8;
                    tmp8 = g >> 3; // most  significant 2 bits of green (there
                                   // are only 5 bits of data)
                    tmp8 |= r << 2;
                    *pxptr++ = tmp8;
                    break;
                case U_BCBM_COLOR24: // 2^24 colors. bmiColors is not used.
        ...
        if (pad) {
            memset(pxptr, 0, pad); // not strictly necessary, but set all bytes
                                   // so that we can find important unset ones
                                   // with valgrind
            pxptr += pad;
        }
    }
...
/**
    \brief Get the actual number of colors in the color table from the
   BitMapInfoHeader.
    ...
    Note, this may be called by WMF code, so it is not safe to assume the data
   is aligned.
*/
int get_real_color_count(const char *Bmih) {
    ...
    memcpy(&utmp4, cBmih + offsetof(U_BITMAPINFOHEADER, biClrUsed), 4);
    Colors = utmp4;
    memcpy(&utmp2, cBmih + offsetof(U_BITMAPINFOHEADER, biBitCount), 2);
    BitCount = utmp2;
    memcpy(&tmp4, cBmih + offsetof(U_BITMAPINFOHEADER, biWidth), 4);
    Width = tmp4;
    memcpy(&tmp4, cBmih + offsetof(U_BITMAPINFOHEADER, biHeight), 4);
    Height = tmp4;
    return (get_real_color_icount(Colors, BitCount, Width, Height));
}

/**
    \brief Get the actual number of colors in the color table from the ClrUsed,
   BitCount, Width, and Height.
    \return Number of entries in the color table.
...
int get_real_color_icount(int Colors, int BitCount, int Width, int Height) {
    ...
    if (Colors == 0) {
        if (BitCount == U_BCBM_MONOCHROME) {
            Colors = 2;
        } else if (BitCount == U_BCBM_COLOR4) {
            Colors = 16;
        } else if (BitCount == U_BCBM_COLOR8) {
            Colors = 256;
        }
        if (Colors > area) {
            Colors = area;
...
/**
    \brief Get the DIB parameters from the BMI of the record for use by
   DBI_to_RGBA()
...
/**
    \brief Convert one of many different types of DIB pixmaps to an RGBA 32 bit
   pixmap.
    ...
    \param ct         DIB color table
    \param numCt      DIB color table number of entries
    \param rgba_px    U_RGBA pixel array (32 bits), created by this routine,
   caller must free.
...
int DIB_to_RGBA(const char *px, const U_RGBQUAD *ct, int numCt, char **rgba_px,
    ...
    if (!use_ct && colortype < U_BCBM_COLOR16)
        return (3); // color tables mandatory for < 16 bit
    if (use_ct && !numCt)
        return (4); // color table not adequately described
    ...
    *rgba_px = (char *)malloc(cbRgba_px);
    ...
    pxptr = px;
    tmp8 = 0; // silences a compiler warning, tmp8 always sets when j=0, so
    ...
    for (i = istart; i != iend; i += iinc) {
        ...
        for (j = 0; j < w; j++) {
            if (use_ct) {
                ...
                case U_BCBM_MONOCHROME: // 2 colors.    bmiColors array has two
                    ...
                    index = 0x80 & tmp8; // This seems wrong, as lowest position
                                         // is top bit, but it works.
                    index = index >> 7;
                    tmp8 = tmp8 << 1;
                    break;
                case U_BCBM_COLOR4: // 2^4 colors.  bmiColors array has 16
                ...
                case U_BCBM_COLOR16: // 2^16 colors. (Several different color
                                     // methods))
                    // Do it in this way because the bytes are always stored
                    // Little Endian
                    tmp8 = *pxptr++;
                    ...
                    g |= (0x3 & tmp8) << 3; // most  significant 2 bits of green
                                            // (there are only 5 bits of data)
                    g = g << 3; //  restore intensity (have lost 3 bits of
                                //  accuracy)
                    a = 0;
                    break;
                case U_BCBM_COLOR24: // 2^24 colors. bmiColors is not used.
        ...
        for (j = 0; j < pad; j++) {
            pxptr++;
        } // DIB rows are all 4 byte aligned
    }
    return (0);
}
...
/**
    ...
    Frees the incoming bitmap array IF a subset is extracted, otherwise it is
   left alone.
    If the entire array is extracted it just returns the incoming pointer.
    If the subset requested is partially outside of the bitmap the region is
   clipped to the
      bitmap boundaries and extracted.  This seems to be a (very) grey area in
   EMF files, and
      even different Microsoft applications do not always do the same thing.
   For instance,
      XP Preview gives some different images for EMR_BITBLT records than does
   the "import image"
      (but not unpacked) view in PowerPoint. Since all of these states are
   probably best viewed
      as undefined or errors we can only try to do something reasonable and not
   blow up when
      encountering one.
   ...
   caller must free.
    \param w          Width of pixel array in the record
    \param h          Height of pixel array in the record
    \param sl         start left position in the pixel array in the record to
   start extracting
    \param st         start top  position in the pixel array in the record to
   start extracting
    \param eew        Width of pixel array to extract
    \param eeh        Height of pixel array to extract
*/
char *RGBA_to_RGBA(char *rgba_px, int w, int h, int sl, int st, int *eew,
                   int *eeh) {
    int i;
    char *sub;
    char *sptr;
    ...
    // sanity checking
    if (w <= 0 || h <= 0 || ew <= 0 || eh <= 0 || !rgba_px)
        return (NULL);

    if (sl > w || st > h)
        return (
            NULL); // This is hopeless, the start point is outside of the array.
    if (sl < 0) {
        if (sl + ew <= 0)
            return (NULL); // This is hopeless, the start point is outside of
                           // the array.
        ew += sl;
        sl = 0;
    }
...
/* **********************************************************************************************
These functions are for setting up, appending to, and then tearing down an EMF
structure, including
...
char *emr_dup(const char *emr) {
    ...
    dup = malloc(irecsize);
    if (dup) {
        memcpy(dup, emr, irecsize);
    }
    return (dup);
}

/**
    \brief Start constructing an emf in memory. Supply the file name and initial
   size.
...
int emf_finish(EMFTRACK *et, EMFHANDLES *eht) {
    ...
    if (!et->fp)
        return (1); // This could happen if something stomps on memory,
                    // otherwise should be caught in emf_start
    ...
    if (1 != fwrite(et->buf, et->used, 1, et->fp))
        return (2);
    (void)fclose(et->fp);
    et->fp = NULL;
    return (0);
...
/**
    \brief Release memory for an emf structure in memory. Call this after
   emf_finish().
    \return 0 on success, >=1 on failure
    \param et EMF in memory
*/
...
FILE *emf_fopen(const char *filename, const int mode) {
    ...
    uint16_t *md16;
    if (mode == U_READ) {
        md16 = U_Utf8ToUtf16le("rb", 0, NULL);
    } else {
        md16 = U_Utf8ToUtf16le("wb", 0, NULL);
    }
...
/**
    \brief Append an EMF record to an emf in memory. This may reallocate buf
   memory.
...
int emf_append(U_ENHMETARECORD *rec, EMFTRACK *et, int freerec) {
    ...
    et->used += rec->nSize;
    et->records++;
    if (rec->iType == U_EMR_EOF) {
        et->PalEntries = ((U_EMREOF *)rec)->cbPalEntries;
    }
    if (freerec) {
        free(rec);
    }
...
int emf_htable_insert(uint32_t *ih, EMFHANDLES *eht) {
    ...
    if (eht->sptr >= eht->allocated - 1) { // need to reallocate
        ...
        if (!eht->table)
            return (5);
        memset(&eht->table[eht->allocated], 0,
               eht->chunk *
        ...
        if (!eht->stack)
            return (6);
        for (i = eht->allocated; i < newsize; i++) {
            eht->stack[i] = i;
        } // init all NEW slots in the stack
    ...
    *ih = eht->stack[eht->sptr]; // handle that is inserted
    if (eht->table[*ih])
        return (7);
    eht->table[*ih] =
        *ih; // handle goes into preexisting (but zero) slot in table
    eht->stack[eht->sptr] = 0;
    if (*ih > eht->top) {
        eht->top = *ih;
    }
    if (eht->sptr > eht->peak) {
        eht->peak = eht->sptr;
    }
...
/**
    \brief Set up fields for an EMR_HEADER from the physical device's width and
   height in mm and dots per millimeter.
    Typically this is something like 216,279,47.244 (Letter paper, 1200 DPI =
   47.244 DPmm)
    \return 0 for success, >=1 for failure.
...
/**
   ...
   and dots per millimeter.
    Technically rclBounds is supposed to be the extent of the drawing within the
   EMF, but libUEMF has no way
    of knowing this since it never actually draws anything.  Instead this is set
   to the full drawing size.
...
*/
int drawing_size(const int xmm, const int ymm, const float dpmm,
    ...
    rclBounds->top = 0;
    rclBounds->right =
        U_ROUND((float)xmm * dpmm) -
        1; // because coordinate system is 0,0 in upper left, N,M in lower right
...
/**
    \brief Set rect and rectl objects from Upper Left and Lower Right corner
   points.
...
/**
    ...
    \param rot       Rotation angle in degrees, positive is counter clockwise
   from the x axis.
    \param axisrot   Angle in degrees defining the major axis before rotation,
   positive is counter clockwise from the x axis.
    \param eDx       Translation element
    \param eDy       Translation element
...
U_XFORM xform_alt_set(U_FLOAT scale, U_FLOAT ratio, U_FLOAT rot,
    ...
    if (ratio != 1.0) { // set scale/ellipticity matrix
        mat2.M11 = scale * (cos(axisrot) * cos(axisrot) +
                            ratio * sin(axisrot) * sin(axisrot));
        mat2.M12 = mat2.M21 =
            scale * (sin(axisrot) * cos(axisrot) * (1.0 - ratio));
        mat2.M22 = scale * (sin(axisrot) * sin(axisrot) +
                            ratio * cos(axisrot) * cos(axisrot));
    } else { // when the ratio is 1.0 then the major axis angle is ignored and
             // only scale matters
        mat2.M11 = scale;
        mat2.M12 = 0.0;
        mat2.M21 = 0.0;
...
/**
    ...
    \param lcsFilename   Could name an external color profile file, otherwise
   empty string
*/
...
/**
    ...
    \param lfEscapement     Angle in 0.1 degrees betweem escapement vector and X
   axis
    ...
    \param lfFaceName       Name of font. truncates at U_LF_FACESIZE, smaller
...
U_LOGFONT logfont_set(int32_t lfHeight, int32_t lfWidth, int32_t lfEscapement,
    ...
    wchar16strncpypad(lf.lfFaceName, lfFaceName,
                      U_LF_FACESIZE); // pad this one as the intial structure
                                      // was not set to zero
...
/**
    ...
    \param elfFullName   Font full name, truncates at U_LF_FULLFACESIZE, smaller
   must be null terminated
    \param elfStyle      Font style, truncates at U_LF_FULLFACESIZE, smaller
   must be null terminated
    \param elfStyleSize  Font hinting starting at this point size, if 0, starts
   at Height
...
U_LOGFONT_PANOSE logfont_panose_set(U_LOGFONT elfLogFont, uint16_t *elfFullName,
    ...
    memset(&lfp, 0, sizeof(U_LOGFONT_PANOSE)); // all fields zero unless needed.
                                               // Many should be ignored or must
                                               // be 0.
...
/**
    ...
    \param BmiColors   Color table (must be NULL for some  values of
...
PU_BITMAPINFO bitmapinfo_set(U_BITMAPINFOHEADER BmiHeader,
    ...
    if (record) {
        ...
        if (cbColors) {
            ...
            off += cbColors;
            if (cbColors4 - cbColors) {
                memset(record + off, 0, cbColors4 - cbColors);
            }
        }
    }
...
/**
    ...
    \param elpWidth      Width in logical units (elpPenStyle & U_PS_GEOMETRIC)
   or 1 (pixel)
    ...
    \param elpStyleEntry Array of StyleEntry (For user specified dot/dash
   patterns)
*/
PU_EXTLOGPEN extlogpen_set(uint32_t elpPenStyle, uint32_t elpWidth,
    ...
    if (elpNumEntries) {
        ...
        irecsize = sizeof(U_EXTLOGPEN) + szSyleArray -
                   sizeof(U_STYLEENTRY); // first one is in the record
    } else {
        szSyleArray = 0;
    ...
    if (record) {
        ...
        if (elpNumEntries) {
            memcpy(((PU_EXTLOGPEN)record)->elpStyleEntry, elpStyleEntry,
                   szSyleArray);
        } else {
            memset(((PU_EXTLOGPEN)record)->elpStyleEntry, 0,
                   sizeof(U_STYLEENTRY));
        } // not used, but this stops valgrind warnings
    }
...
PU_LOGPALETTE logpalette_set(U_NUM_LOGPLTNTRY palNumEntries,
    ...
    if (palNumEntries == 0 || !palPalEntry)
        return (NULL);
...
PU_RGNDATA rgndata_set(U_RGNDATAHEADER rdh, PU_RECTL Buffer) {
    ...
    if (!Buffer || !rdh.nCount || !rdh.nRgnSize)
        return (NULL);
    szRgnArray = rdh.nRgnSize; // size of the U_RECTL array
    irecsize = sizeof(U_RGNDATA) + szRgnArray -
               sizeof(U_RECTL); // core + array - overlap
    record = malloc(irecsize);
    if (record) {
...
/**
    ...
    \param RedGamma        Red   Gamma correction (range:2500:65000, 10000 is no
   correction)
    \param GreenGamma      Green Gamma correction (range:2500:65000, 10000 is no
   correction)
    \param BlueGamma       Blue  Gamma correction (range:2500:65000, 10000 is no
   correction)
    ...
    \param Contrast        Contrast     adjustment (range:-100:100, 0 is no
   correction)
    \param Brightness      Brightness   adjustment (range:-100:100, 0 is no
   correction)
    \param Colorfulness    Colorfulness adjustment (range:-100:100, 0 is no
   correction)
    \param RedGreenTint    Tine         adjustment (range:-100:100, 0 is no
   correction)
*/
U_COLORADJUSTMENT
coloradjustment_set(uint16_t Size, uint16_t Flags, uint16_t IlluminantIndex,
    ...
    // Next one is different to eliminate compiler warning -  U_R_B_MIN is 0 and
    // unsigned
    ca.caReferenceBlack = U_MAX(ReferenceBlack, U_REFERENCE_BLACK_MAX);
    ca.caReferenceWhite =
        U_MNMX(ReferenceWhite, U_REFERENCE_WHITE_MIN, U_REFERENCE_WHITE_MAX);
...
/**
    \brief Allocate and create a U_EMRTEXT structure followed by its variable
   pieces via a char* pointer.
    Dx cannot be NULL, if the calling program has no appropriate values call
   dx_set() first.
    \return char* pointer to U_EMRTEXT structure followed by its variable
   pieces, or NULL on error
    \param ptlReference String start coordinates
    \param NumString    Number of characters in string, does NOT include a
   terminator
    \param cbChar       Number of bytes per character
    \param String       String to write
    \param fOptions     ExtTextOutOptions Enumeration
    \param rcl          (Optional, when fOptions & 7) grayed/clipping/opaque
   rectangle
...
char *emrtext_set(U_POINTL ptlReference, U_NUM_STR NumString, uint32_t cbChar,
    ...
    if (fOptions & U_ETO_PDY)
        cbDxArray +=
            cbDxArray; // of the Dx buffer, here do both X and Y coordinates
    irecsize = sizeof(U_EMRTEXT) + sizeof(uint32_t) + cbString4 +
               cbDxArray; // core structure  + offDx + string buf + dx buf
    if (!(fOptions & U_ETO_NO_RECT)) {
        irecsize += sizeof(U_RECTL);
    } // plus variable U_RECTL, when it is present
    record = malloc(irecsize);
    if (record) {
        ...
        off = sizeof(U_EMRTEXT); // location where variable pieces will start to
                                 // be written
        if (!(fOptions &
              U_ETO_NO_RECT)) { // variable field, may or may not be present
            memcpy(record + off, &rcl, sizeof(U_RECTL));
            off += sizeof(U_RECTL);
        }
        loffDx = (uint32_t *)(record + off); // offDx will go here, but we do
                                             // not know with what value yet
        off += sizeof(uint32_t);
        memcpy(record + off, String,
               cbString); // copy the string data to its buffer
        ((PU_EMRTEXT)record)->offString =
            off; // now save offset in the structure
        off += cbString;
        if (cbString < cbString4) {
            memset(record + off, 0, cbString4 - cbString); // keeps valgrind
                                                           // happy (initialize
                                                           // padding after
                                                           // string)
...
/**
    ...
    A U_EMRCOMMENT contains application specific data, and that may include
   contain null characters. This function may be used when the
...
/**
    \brief Allocate and construct a U_EMRDELETEOBJECT structure and also delete
   the requested object from the table.
    ...
    \param ihObject  Pointer to handle to delete.  This value is set to 0 if the
   function succeeds.
    \param eht       EMF handle table
   ...
   handled here because the only  values which
    may be returned are a valid U_EMRDELETEOBJECT record or a NULL, and other
...
char *deleteobject_set(uint32_t *ihObject, EMFHANDLES *eht) {
    ...
    if (emf_htable_delete(ihObject, eht))
        return (NULL); // invalid handle or other problem, cannot be deleted
    return (U_EMRDELETEOBJECT_set(saveObject));
}

/**
    \brief Allocate and construct a U_EMRSELECTOBJECT structure, checks that the
...
char *selectobject_set(uint32_t ihObject, EMFHANDLES *eht) {
    if (!(U_STOCK_OBJECT &
          ihObject)) { // not a stock object, those go straight through
        if (ihObject > eht->top)
            return (NULL); // handle this high is not in the table
        if (!eht->table[ihObject])
            return (NULL); // handle is not in the table, so not active, so
                           // cannot be selected
    }
...
/**
    \brief Allocate and construct a U_EMREXTCREATEPEN structure, create a handle
   and return it.
...
char *extcreatepen_set(uint32_t *ihPen, EMFHANDLES *eht, PU_BITMAPINFO Bmi,
    ...
    if (emf_htable_insert(ihPen, eht))
        return (NULL);
    return (U_EMREXTCREATEPEN_set(*ihPen, Bmi, cbPx, Px, elp));
}

/**
    \brief Allocate and construct a U_EMRCREATEPEN structure, create a handle
   and returns it
...
/**
    \brief Allocate and construct a U_EMRCREATEBRUSHINDIRECT structure, create a
   handle and returns it
    Use this function instead of calling U_EMRCREATEBRUSHINDIRECT_set()
   directly.
...
char *createbrushindirect_set(uint32_t *ihBrush, EMFHANDLES *eht,
                              U_LOGBRUSH lb) {
    if (emf_htable_insert(ihBrush, eht))
        return (NULL);
    return (U_EMRCREATEBRUSHINDIRECT_set(*ihBrush, lb));
}

/**
    \brief Allocate and construct a U_EMRCREATEDIBPATTERNBRUSHPT_set structure,
   create a handle and returns it
    Use this function instead of calling U_EMRCREATEDIBPATTERNBRUSHPT_set()
   directly.
    \return pointer to U_EMRCREATEDIBPATTERNBRUSHPT_set structure, or NULL on
   error.
    \param ihBrush handle to be used by new object
    \param eht     EMF handle table
...
char *createdibpatternbrushpt_set(uint32_t *ihBrush, EMFHANDLES *eht,
                                  const uint32_t iUsage, PU_BITMAPINFO Bmi,
...
/**
    \brief Allocate and construct a U_EMRCREATEMONOBRUSH_set structure, create a
   handle and returns it
   ...
   be some padding at the end of each row)
    \param Px      (Optional) bitmapbuffer (pixel array section )
*/
char *createmonobrush_set(uint32_t *ihBrush, EMFHANDLES *eht,
                          const uint32_t iUsage, PU_BITMAPINFO Bmi,
                          const uint32_t cbPx, const char *Px
...
/**
    \brief Allocate and construct a U_EMRCREATECOLORSPACE structure, create a
   handle and returns it
...
char *createcolorspace_set(uint32_t *ihCS, EMFHANDLES *eht,
    ...
    if (emf_htable_insert(ihCS, eht))
        return (NULL);
    return (U_EMRCREATECOLORSPACE_set(*ihCS, lcs));
}

/**
    \brief Allocate and construct a U_EMRCREATECOLORSPACEW structure, create a
   handle and returns it
...
/**
    \brief Allocate and construct a U_EMREXTCREATEFONTINDIRECTW structure,
   create a handle and returns it
    Use this function instead of calling U_EMREXTCREATEFONTINDIRECTW_set()
   directly.
...
char *extcreatefontindirectw_set(uint32_t *ihFont, EMFHANDLES *eht,
    ...
    if (emf_htable_insert(ihFont, eht))
        return (NULL);
    return (U_EMREXTCREATEFONTINDIRECTW_set(*ihFont, elf, elfw));
}

/**
    \brief Allocate and construct a U_EMRCREATEPALETTE structure, create a
   handle and returns it
...
char *createpalette_set(uint32_t *ihPal, EMFHANDLES *eht, U_LOGPALETTE lgpl) {
    if (emf_htable_insert(ihPal, eht))
        return (NULL);
    return (U_EMRCREATEPALETTE_set(*ihPal, lgpl));
}

/**
    \brief Allocate and construct a U_EMRSETPALETTEENTRIES structure, create a
   handle and returns it
...
*/
char *setpaletteentries_set(uint32_t *ihPal, EMFHANDLES *eht,
...
/**
    \brief Allocate and construct a U_EMRFILLRGN structure, create a handle and
   returns it
...
*/
char *fillrgn_set(uint32_t *ihBrush, EMFHANDLES *eht, const U_RECTL rclBounds,
...
/**
    \brief Allocate and construct a U_EMRFRAMERGN structure, create a handle and
   returns it
    ...
    \param RgnData   Pointer to a U_RGNDATA structure
*/
char *framergn_set(uint32_t *ihBrush, EMFHANDLES *eht, const U_RECTL rclBounds,
...
/**
    \brief Allocate and construct an array of U_POINT objects which has been
   subjected to a U_XFORM
    ...
    May also be used to modify U_RECT by doubling the count and casting the
   pointer.
*/
...
/**
    \brief Allocate and construct an array of U_POINT16 objects  which has been
   subjected to a U_XFORM
...
/**
    \brief Allocate and construct an array of U_TRIVERTEX objects  which has
   been subjected to a U_XFORM
    ...
    Transformed Trivertex points {x0,y0} appear at {x0*xscale + x, y0*yscale +
   y}
*/
...
/**
    \brief Allocate and construct an array of U_POINT objects from a set of
   U_POINT16 objects
    \returns pointer to an array of U_POINT structures.
    \param points  pointer to the source U_POINT16 structures
    \param count   number of members in points
...
//! @cond
/* **********************************************************************************************
These functions contain shared code used by various U_EMR*_set functions.  These
...
are hidden from Doxygen.
   ...
   For all _set functions the caller must eventually call free() on the returned
pointer.

    CORE1(uint32_t iType, U_RECTL rclBounds, const uint32_t cptl, const U_POINTL
*points){
    CORE2(uint32_t iType, U_RECTL rclBounds, const uint32_t nPolys, const
uint32_t *aPolyCounts,const uint32_t cptl, const U_POINTL *points){
    CORE3(uint32_t iType, uint32_t iMode){       (generic 1 uint)
    CORE4(uint32_t iType, U_RECTL rclBox){
    CORE5(uint32_t iType){                       (generic noargs)
    CORE6(uint32_t iType, U_RECTL rclBounds, const uint32_t cpts, const
U_POINT16 *points){  (16bit form of CORE1)
    CORE7(uint32_t iType, U_PAIR pair){
    CORE8(uint32_t iType, U_RECTL rclBounds, uint32_t iGraphicsMode, U_FLOAT
exScale, U_FLOAT eyScale, PU_EMRTEXT emrtext){
    CORE9(uint32_t iType, U_RECTL rclBox, U_POINTL ptlStart, U_POINTL ptlEnd){
    CORE10(uint32_t iType, U_RECTL rclBounds, const uint32_t nPolys, const
uint32_t *aPolyCounts,const uint32_t cpts, const U_POINT16 *points){ (16bit form
of CORE2)
    CORE11(uint32_t iType, PU_RGNDATA RgnData){
    CORE12(uint32_t iType, uint32_t ihBrush, uint32_t iUsage, PU_BITMAPINFO
Bmi){
    CORE13(uint32_t iType, U_RECTL rclBounds, U_POINTL Dest, U_POINTL cDest,
              U_POINTL Src, U_POINTL cSrc, U_XFORM xformSrc, U_COLORREF
crBkColorSrc, uint32_t iUsageSrc,
              uint32_t Data, PU_BITMAPINFO Bmi);
***********************************************************************************************
*/

// Functions with the same form starting with U_EMRPOLYBEZIER_set
char *U_EMR_CORE1_set(uint32_t iType, U_RECTL rclBounds, const uint32_t cptl,
    ...
    irecsize = sizeof(U_EMRPOLYBEZIER) + cbPoints -
               sizeof(U_POINTL); // First instance is in struct
    record = malloc(irecsize);
    if (record) {
...
char *U_EMR_CORE2_set(uint32_t iType, U_RECTL rclBounds, const uint32_t nPolys,
    ...
    irecsize = sizeof(U_EMRPOLYPOLYLINE) + cbPoints + cbPolys -
               sizeof(uint32_t); // First instance of each is in struct
    record = malloc(irecsize);
    if (record) {
        ((PU_EMR)record)->iType = iType;
        ((PU_EMR)record)->nSize = irecsize;
...
// Functions taking a single U_RECT or U_RECTL, starting with U_EMRELLIPSE_set,
// also U_EMRFILLPATH,
...
char *U_EMR_CORE6_set(uint32_t iType, U_RECTL rclBounds, const uint32_t cpts,
    ...
    off = sizeof(U_EMR) + sizeof(U_RECTL) +
          sizeof(U_NUM_POINT16); // offset to the start of the variable region
...
// Functions that take a single struct argument which contains two uint32_t,
// starting with U_EMRSETWINDOWEXTEX_set
...
char *U_EMR_CORE8_set(uint32_t iType,
                      U_RECTL rclBounds, // Bounding rectangle in device units
                      uint32_t iGraphicsMode, // Graphics mode Enumeration
                      U_FLOAT exScale,   // scale to 0.01 mm units ( only if
                                         // iGraphicsMode & GM_COMPATIBLE)
                      U_FLOAT eyScale,   // scale to 0.01 mm units ( only if
                                         // iGraphicsMode & GM_COMPATIBLE)
    ...
    if (iType == U_EMR_EXTTEXTOUTA) {
        csize = 1;
    } // how many bytes per character
    else if (iType == U_EMR_EXTTEXTOUTW) {
        csize = 2;
    } else {
        return (NULL);
    }
    ...
    cbString4 = UP4(cbString); // size of the string buffer
    cbEmrtext = sizeof(
        U_EMRTEXT); // size of the constant part of the U_EMRTEXT structure
    if (!(emrtext->fOptions & U_ETO_NO_RECT)) {
        cbEmrtext += sizeof(U_RECTL);
    } // plus the variable U_RECTL, when it is present
    cbDx = emrtext->nChars * sizeof(int32_t); // size of Dx buffer
    if (emrtext->fOptions & U_ETO_PDY)
        cbDx += cbDx; // size of Dx buffer when both x and y offsets are used
    cbEmrtextAll =
        cbEmrtext + sizeof(uint32_t) + cbString4 +
        cbDx; // structure (+- rect) + offDx + string buf + dx buf + offDx

    /* adjust offset fields in emrtext to match the EMRTEXTOUT* field, currently
       they match EMRTEXT.
       This works because the variable pieces have all been moved outside of the
       U_EMRTEXT and U_EMRTEXTOUTA strutures.
    */
    ((PU_EMRTEXT)emrtext)->offString +=
        sizeof(U_EMREXTTEXTOUTA) - sizeof(U_EMRTEXT); // adjust offString
    loffDx = (uint32_t *)((char *)emrtext + cbEmrtext);
    *loffDx += sizeof(U_EMREXTTEXTOUTA) - sizeof(U_EMRTEXT);

    // final record size is: U_EMREXTTEXTOUTA (includes constant part of
    // U_EMRTEXT) + U_RECTL (if present) + offDx + dx buffer + string buffer
    irecsize = sizeof(U_EMREXTTEXTOUTA) + cbEmrtextAll -
               sizeof(U_EMRTEXT); // do not count core emrtext strcture twice
    record = malloc(irecsize);
    if (record) {
        ((PU_EMR)record)->iType = iType;
        ((PU_EMR)record)->nSize = irecsize;
        ...
        // copy the adjusted U_EMRTEXT into the emrtext part of the full
        // record..
...
char *U_EMR_CORE9_set(uint32_t iType, U_RECTL rclBox, U_POINTL ptlStart,
                      U_POINTL ptlEnd) {
    char *record;
...
char *U_EMR_CORE10_set(uint32_t iType, U_RECTL rclBounds, const uint32_t nPolys,
    ...
    cbPolys = sizeof(uint32_t) * nPolys;
    cbPoints = sizeof(U_POINT16) * cpts;
    irecsize = sizeof(U_EMRPOLYPOLYLINE16) + cbPoints + cbPolys -
               sizeof(uint32_t); // First instance of each is in struct
    record = malloc(irecsize);
    if (record) {
        ((PU_EMR)record)->iType = iType;
        ...
        memcpy(((PU_EMRPOLYPOLYLINE16)record)->aPolyCounts, aPolyCounts,
               cbPolys);
...
char *U_EMR_CORE11_set(uint32_t iType, PU_RGNDATA RgnData) {
    ...
    rds4 = UP4(rds);
    irecsize = sizeof(U_EMRINVERTRGN) - sizeof(U_RECTL) +
               cbRgns4; // core + array - overlap of one rectL
    record = malloc(irecsize);
    if (record) {
        ((PU_EMR)record)->iType = iType;
        ((PU_EMR)record)->nSize = irecsize;
        ((PU_EMRINVERTRGN)record)->rclBounds =
            ((PU_RGNDATAHEADER)RgnData)->rclBounds;
        ((PU_EMRINVERTRGN)record)->cbRgnData = rds;
        off = sizeof(U_EMRINVERTRGN) - sizeof(U_RGNDATA);
        memcpy(record + off, RgnData, rds);
        off += rds;
        if (rds < rds4) {
            memset(record + off, 0, rds4 - rds);
        } // clear any unused bytes
    }
    return (record);
}

// common code for U_EMRCREATEMONOBRUSH_set and
// U_EMRCREATEDIBPATTERNBRUSHPT_set,
char *U_EMR_CORE12_set(
    ...
    const uint32_t cbPx, // Size in bytes of pixel array (row stride * height,
                         // there may be some padding at the end of each row)
...
char *U_EMR_CORE13_set(
    ...
    uint32_t Data, // The meaning and type of this field varies, but it is
                   // always 4 bytes
    ...
    if (record) {
        ...
        APPEND_PXBMISRC(record, U_EMRALPHABLEND, cbBmi, Bmi, Px, cbImage,
                        cbImage4);
...
/**
    ...
    \param nDesc           number of characters in Description, will include
   first three '\0'
...
char *U_EMRHEADER_set(const U_RECTL rclBounds, const U_RECTL rclFrame,
    ...
    if (pfmtDesc) {
        cbPFD = sizeof(U_PIXELFORMATDESCRIPTOR);
    } else {
        cbPFD = 0;
    }
    if (Description) {
        cbDesc = 2 * nDesc;
    } // also copy the terminator.  Size is in bytes
    else {
        cbDesc = 0;
    }
    cbDesc4 = UP4(cbDesc);
    ...
    if (record) {
        ...
        if (cbDesc4) {
            ...
            if (cbDesc < cbDesc4)
                memset(record + off, 0,
                       cbDesc4 - cbDesc); // clear any unused bytes
            off += cbDesc4 - cbDesc;
        }
        if (cbPFD) {
...
*/
char *U_EMRPOLYBEZIER_set(const U_RECTL rclBounds, const uint32_t cptl,
                          const U_POINTL *points) {
...
/**
    ...
    \param points      array of points
*/
char *U_EMRPOLYGON_set(const U_RECTL rclBounds, const uint32_t cptl,
                       const U_POINTL *points) {
...
/**
    ...
    \param points      array of points
*/
char *U_EMRPOLYLINE_set(const U_RECTL rclBounds, const uint32_t cptl,
                        const U_POINTL *points) {
...
/**
    ...
    \param points      array of points
*/
char *U_EMRPOLYBEZIERTO_set(const U_RECTL rclBounds, const uint32_t cptl,
                            const U_POINTL *points) {
...
/**
    ...
    \param points      array of points
*/
char *U_EMRPOLYLINETO_set(const U_RECTL rclBounds, const uint32_t cptl,
                          const U_POINTL *points) {
...
char *U_EMRPOLYPOLYLINE_set(const U_RECTL rclBounds, const uint32_t nPolys,
                            const uint32_t *aPolyCounts, const uint32_t cptl,
                            const U_POINTL *points) {
    return (U_EMR_CORE2_set(U_EMR_POLYPOLYLINE, rclBounds, nPolys, aPolyCounts,
                            cptl, points));
}
...
char *U_EMRPOLYPOLYGON_set(const U_RECTL rclBounds, const uint32_t nPolys,
                           const uint32_t *aPolyCounts, const uint32_t cptl,
                           const U_POINTL *points) {
    return (U_EMR_CORE2_set(U_EMR_POLYPOLYGON, rclBounds, nPolys, aPolyCounts,
                            cptl, points));
}
...
char *U_EMRSETWINDOWORGEX_set(const U_POINTL ptlOrigin) {
    return (
        U_EMR_CORE7_set(U_EMR_SETWINDOWORGEX,
                        ptlOrigin)); // U_PAIR and U_POINTL are the same thing
}
...
char *U_EMRSETVIEWPORTORGEX_set(const U_POINTL ptlOrigin) {
    return (
        U_EMR_CORE7_set(U_EMR_SETVIEWPORTORGEX,
                        ptlOrigin)); // U_PAIR and U_POINTL are the same thing
}
...
char *U_EMREOF_set(const U_CBPLENTRIES cbPalEntries,
    ...
    if (cbPalEntries && !PalEntries)
        return (NULL);
    if (!et)
        return (NULL);
    cbPals = cbPalEntries * sizeof(U_LOGPLTNTRY);
    irecsize =
        sizeof(U_EMREOF) + cbPals +
        sizeof(
            uint32_t); // invariant core, variable palette, palette byte count
    record = malloc(irecsize);
    if (record) {
        ...
        ptr = record + off;
        *(uint32_t *)ptr = irecsize + et->used; // EMREOF nSizeLast field, not
                                                // at a fixed position, cannot
                                                // be accessed by field name
    }
...
char *U_EMRSCALEVIEWPORTEXTEX_set(const int32_t xNum, const int32_t xDenom,
    ...
    return (U_EMR_CORE4_set(U_EMR_SCALEVIEWPORTEXTEX,
                            (U_RECTL){xNum, xDenom, yNum, yDenom}));
}
...
char *U_EMRSCALEWINDOWEXTEX_set(const int32_t xNum, const int32_t xDenom,
                                const int32_t yNum, const int32_t yDenom) {
    return (U_EMR_CORE4_set(U_EMR_SCALEWINDOWEXTEX,
                            (U_RECTL){xNum, xDenom, yNum, yDenom}));
}
...
char *U_EMRSELECTOBJECT_set(const uint32_t ihObject) {
    ...
    if (record) {
        ...
        ((PU_EMR)record)->nSize = irecsize;
        ((PU_EMRSELECTOBJECT)record)->ihObject =
            ihObject; // Index of object to SELECT
    }
    return (record);
}
...
/**
    ...
    \param ihBrush Index to place object in EMF object table (this entry must
   not yet exist)
...
char *U_EMRCREATEBRUSHINDIRECT_set(const uint32_t ihBrush,
    ...
    if (record) {
        ...
        ((PU_EMR)record)->nSize = irecsize;
        ((PU_EMRCREATEBRUSHINDIRECT)record)->ihBrush =
            ihBrush; // Index to place object in EMF object table (this entry
                     // must not yet exist)
...
char *U_EMRDELETEOBJECT_set(const uint32_t ihObject) {
    ...
    if (record) {
        ...
        ((PU_EMR)record)->nSize = irecsize;
        ((PU_EMRDELETEOBJECT)record)->ihObject =
            ihObject; // Index of object to DELETE
    }
    return (record);
}
...
/**
    ...
    \param ihPal Index to place object in EMF object table (this entry must not
   yet exist)
...
char *U_EMRSETPALETTEENTRIES_set(const uint32_t ihPal, const uint32_t iStart,
    ...
    if (record) {
        ...
        memcpy(((PU_EMRSETPALETTEENTRIES)record)->aPalEntries, aPalEntries,
               cbPals);
    }
    return (record);
}
...
/**
    ...
    Note that the draw begins with a line from the current point to ptlStart,
   which is
...
char *U_EMRPOLYDRAW_set(const U_RECTL rclBounds, const U_NUM_POINTL cptl,
    ...
    irecsize =
        sizeof(U_EMRPOLYDRAW) + cbPoints + cbAbTypes4 - sizeof(U_POINTL) - 1;
    record = malloc(irecsize);
    if (record) {
        ...
        off = sizeof(U_EMR) + sizeof(U_RECTL) +
              sizeof(uint32_t); // offset to first variable part
        memcpy(record + off, aptl, cbPoints);
        off += cbPoints;
        memcpy(record + off, abTypes, cbAbTypes);
        off += cbAbTypes;
        if (cbAbTypes4 > cbAbTypes) {
            memset(record + off, 0, cbAbTypes4 - cbAbTypes);
        } // keeps valgrind happy (initialize padding after byte array)
    }
    return (record);
}
...
/**
    ...
    There appears to be no way to fill an open path while stroking it, as any
   one
    of U_EMRFILLPATH, U_EMRSTROKEPATH, or U_EMRSTROKEANDFILEPATH will "use up"
   the path,
*/
...
// U_EMRCOMMENT_set                   70  Comment (any binary data,
// interpretation is program specific)
/**
...
char *U_EMRCOMMENT_set(const U_CBDATA cbData, const char *Data) {
    ...
    if (record) {
        ...
        if (cbData4 > cbData)
            memset(record + irecsize - cbData4 + cbData, 0,
                   cbData4 - cbData); // clear any unused bytes
    }
    return (record);
}
...
char *U_EMRFILLRGN_set(const U_RECTL rclBounds, const uint32_t ihBrush,
    ...
    rds = sizeof(U_RGNDATAHEADER) + cbRgns;
    rds4 = UP4(rds);
    irecsize = sizeof(U_EMRFILLRGN) - sizeof(U_RECTL) +
               cbRgns4; // core + array - overlap of one rectL
    record = malloc(irecsize);
    if (record) {
...
char *U_EMRFRAMERGN_set(const U_RECTL rclBounds, const uint32_t ihBrush,
    ...
    rds = sizeof(U_RGNDATAHEADER) + cbRgns;
    rds4 = UP4(rds);
    irecsize = sizeof(U_EMRFRAMERGN) - sizeof(U_RECTL) +
               cbRgns4; // core + array - overlap of one rectL
    record = malloc(irecsize);
    if (record) {
...
char *U_EMREXTSELECTCLIPRGN_set(const uint32_t iMode,
    ...
    rds = sizeof(U_RGNDATAHEADER) + cbRgns;
    rds4 = UP4(rds);
    irecsize = sizeof(U_EMREXTSELECTCLIPRGN) - sizeof(U_RECTL) +
               cbRgns4; // core + array - overlap of one rectL
    record = malloc(irecsize);
    if (record) {
...
/**
   ...
   may be some padding at the end of each row)
    \param Px           (Optional) bitmapbuffer (pixel array section )
*/
char *U_EMRSTRETCHBLT_set(const U_RECTL rclBounds, const U_POINTL Dest,
                          const U_POINTL cDest, const U_POINTL Src,
    ...
    if (record) {
        ...
        APPEND_PXBMISRC(record, U_EMRSTRETCHBLT, cbBmi, Bmi, Px, cbImage,
                        cbImage4);
...
/**
   ...
   may be some padding at the end of each row)
    ...
    \param cbMsk        Size in bytes of mask array (row stride * height, there
   may be some padding at the end of each row)
    \param Msk          (Optional) bitmapbuffer (mask section )
*/
char *U_EMRMASKBLT_set(const U_RECTL rclBounds, const U_POINTL Dest,
                       const U_POINTL cDest, const U_POINTL Src,
                       const U_XFORM xformSrc, const U_COLORREF crBkColorSrc,
                       const uint32_t iUsageSrc, const U_POINTL Mask,
                       const uint32_t iUsageMask, const uint32_t dwRop,
                       const PU_BITMAPINFO Bmi, const uint32_t cbPx, char *Px,
                       const PU_BITMAPINFO MskBmi, const uint32_t cbMsk,
                       char *Msk) {
    char *record;
    int irecsize;
    ...
    if (record) {
        ...
        APPEND_PXBMISRC(record, U_EMRMASKBLT, cbBmi, Bmi, Px, cbImage,
                        cbImage4);
        APPEND_MSKBMISRC(record, U_EMRMASKBLT, cbMskBmi, MskBmi, Msk,
                         cbMskImage, cbMskImage4);
    }
    return (record);
}
...
/**
    ...
    \param rclBounds    Bounding rectangle in device units
    \param aptlDst      Defines parallelogram, UL, UR, LL corners, LR is derived
   (3 points)
    \param Src          Source UL corner in logical units
...
char *U_EMRPLGBLT_set(const U_RECTL rclBounds, const PU_POINTL aptlDst,
                      ...
                      const uint32_t cbPx, char *Px, const PU_BITMAPINFO MskBmi,
    ...
    if (record) {
        ...
        APPEND_MSKBMISRC(record, U_EMRPLGBLT, cbMskBmi, MskBmi, Msk, cbMskImage,
                         cbMskImage4);
    }
    return (record);
}
...
char *U_EMRSETDIBITSTODEVICE_set(const U_RECTL rclBounds, const U_POINTL Dest,
    ...
    if (record) {
        ...
        APPEND_PXBMISRC(record, U_EMRSETDIBITSTODEVICE, cbBmi, Bmi, Px, cbImage,
                        cbImage4);
    }
    return (record);
}
...
char *U_EMRSTRETCHDIBITS_set(const U_RECTL rclBounds, const U_POINTL Dest,
    ...
    if (record) {
        ...
        APPEND_PXBMISRC(record, U_EMRSTRETCHDIBITS, cbBmi, Bmi, Px, cbImage,
                        cbImage4);
    }
    return (record);
}
...
*/
char *U_EMREXTCREATEFONTINDIRECTW_set(const uint32_t ihFont, const char *elf,
                                      const char *elfw) {
    char *record;
    const char *cptr;
    int irecsize;
    ...
    if ((elf && elfw) || (!elf && !elfw))
        return (NULL); // ONE only must be passed
    if (elf) {
        cbLf = sizeof(U_LOGFONT);
        cptr = elf;
    } else {
        cbLf = sizeof(U_LOGFONT_PANOSE);
        cptr = elfw;
    }
    ...
    if (record) {
        ...
        memcpy(record + off, cptr,
               cbLf); // No need to add padding for either structure
    }
    return (record);
}
...
/**
    ...
    \param exScale       scale to 0.01 mm units ( only if iGraphicsMode &
   GM_COMPATIBLE)
    \param eyScale       scale to 0.01 mm units ( only if iGraphicsMode &
   GM_COMPATIBLE)
...
char *U_EMREXTTEXTOUTA_set(const U_RECTL rclBounds,
                           ...
                           const U_FLOAT eyScale, const PU_EMRTEXT emrtext) {
    return (U_EMR_CORE8_set(U_EMR_EXTTEXTOUTA, rclBounds, iGraphicsMode,
                            exScale, eyScale, emrtext));
}
...
char *U_EMREXTTEXTOUTW_set(const U_RECTL rclBounds,
                           const uint32_t iGraphicsMode, const U_FLOAT exScale,
                           const U_FLOAT eyScale, const PU_EMRTEXT emrtext) {
    return (U_EMR_CORE8_set(U_EMR_EXTTEXTOUTW, rclBounds, iGraphicsMode,
                            exScale, eyScale, emrtext));
}
...
char *U_EMRPOLYPOLYLINE16_set(const U_RECTL rclBounds, const uint32_t nPolys,
                              const uint32_t *aPolyCounts, const uint32_t cpts,
                              const U_POINT16 *points) {
    return (U_EMR_CORE10_set(U_EMR_POLYPOLYLINE16, rclBounds, nPolys,
                             aPolyCounts, cpts, points));
}
...
char *U_EMRPOLYPOLYGON16_set(const U_RECTL rclBounds, const uint32_t nPolys,
                             const uint32_t *aPolyCounts, const uint32_t cpts,
                             const U_POINT16 *points) {
    return (U_EMR_CORE10_set(U_EMR_POLYPOLYGON16, rclBounds, nPolys,
                             aPolyCounts, cpts, points));
}
...
char *U_EMRCREATEMONOBRUSH_set(const uint32_t ihBrush, const uint32_t iUsage,
                               const PU_BITMAPINFO Bmi, const uint32_t cbPx,
                               const char *Px) {
    return (U_EMR_CORE12_set(U_EMR_CREATEMONOBRUSH, ihBrush, iUsage, Bmi, cbPx,
                             Px));
}
...
char *U_EMRCREATEDIBPATTERNBRUSHPT_set(const uint32_t ihBrush,
                                       ...
                                       const uint32_t cbPx, const char *Px) {
    return (U_EMR_CORE12_set(U_EMR_CREATEDIBPATTERNBRUSHPT, ihBrush, iUsage,
                             Bmi, cbPx, Px));
}
...
/**
    ...
    \param ihPen ihPen Index to place object in EMF object table (this entry
   must not yet exist)
    \param Bmi   Bmi   bitmapbuffer
    \param cbPx  cbPx  Size in bytes of pixel array (row stride * height, there
   may be some padding at the end of each row)
...
char *U_EMREXTCREATEPEN_set(const uint32_t ihPen, const PU_BITMAPINFO Bmi,
    ...
    cbStyleArray =
        elp->elpNumEntries *
        sizeof(U_STYLEENTRY); // space actually used by penstyle entries
    // EXTLOGPEN is already included in EMREXTCREATEPEN, including the possibly
    // unused first penstyle entry
    if (cbStyleArray) {
        cbElp = sizeof(U_EXTLOGPEN) + cbStyleArray -
                sizeof(U_STYLEENTRY); // space actually used by elp
        irecsize = sizeof(U_EMREXTCREATEPEN) + cbBmi + cbImage4 + cbStyleArray -
                   sizeof(U_STYLEENTRY);
    } else {
        cbElp =
            sizeof(U_EXTLOGPEN); //    first U_STYLEENTRY is present but unused
    ...
    if (record) {
        ...
        if (cbStyleArray) {
            off =
                sizeof(U_EMREXTCREATEPEN) + cbStyleArray - sizeof(U_STYLEENTRY);
        } else {
            off = sizeof(U_EMREXTCREATEPEN);
        }
        // Cannot use APPEND_PXBMISRC here because there is no "Src" in the
        // field names
        if (cbBmi) {
            memcpy(record + off, Bmi, cbBmi);
...
// U_EMRPOLYTEXTOUTA_set              96 NOT IMPLEMENTED, denigrated after
// Windows NT
// U_EMRPOLYTEXTOUTW_set              97 NOT IMPLEMENTED, denigrated after
// Windows NT
...
/**
    ...
    \param ihCS Index to place object in EMF object table (this entry must not
   yet exist)
    \param lcs  ColorSpace parameters
*/
...
/**
    ...
    \param rclBounds     OPTIONAL Bounding rectangle (absent when: fuOPtions &
   ETO_NO_U_RECT)
    \param TextString    text to output (fuOptions & ETO_SMALL_CHARS ? 8 bit :
   16 bit)
*/
char *U_EMRSMALLTEXTOUT_set(const U_POINTL Dest, const U_NUM_STR cChars,
    ...
    if (fuOptions & U_ETO_SMALL_CHARS) {
        csize = 1;
    } // how many bytes per character
    else {
        csize = 2;
    ...
    if (fuOptions & U_ETO_NO_RECT) {
        cbRectl = 0;
    } // size of the optional U_RECTL field
    else {
        cbRectl = sizeof(U_RECTL);
    }
    ...
    if (record) {
        ...
        off = sizeof(
            U_EMRSMALLTEXTOUT); // offset to the start of the variable fields
        if (cbRectl) {
...
char *U_EMRALPHABLEND_set(const U_RECTL rclBounds, const U_POINTL Dest,
                          ...
                          const PU_BITMAPINFO Bmi, const uint32_t cbPx,
                          char *Px) {
    return (U_EMR_CORE13_set(U_EMR_ALPHABLEND, rclBounds, Dest, cDest, Src,
                             cSrc, xformSrc, crBkColorSrc, iUsageSrc,
                             *((uint32_t *)&Blend), Bmi, cbPx, Px));
}
...
char *U_EMRTRANSPARENTBLT_set(const U_RECTL rclBounds, const U_POINTL Dest,
                              ...
                              const PU_BITMAPINFO Bmi, const uint32_t cbPx,
                              char *Px) {
    return (U_EMR_CORE13_set(U_EMR_TRANSPARENTBLT, rclBounds, Dest, cDest, Src,
                             cSrc, xformSrc, crBkColorSrc, iUsageSrc, TColor,
                             Bmi, cbPx, Px));
}
...
/**
    ...
    \param GradObj      Array of gradient objects (each has 2 [rect] or 3
[triangle] indices into TriVert array)

There is an MS documentation or library problem for this record, as the size of
the GradObj must always be set
as if it was an array of U_GRADIENT3 objects for both rect and triangle.  For
horizontal and vertical gradients
this means that there will be unused bytes at the end of the record.  This is
...
char *U_EMRGRADIENTFILL_set(const U_RECTL rclBounds,
    ...
    unsigned int cbGradObjAlloc; /* larger than cbGradObj, because of problem
                                    described above */
    int irecsize;

    cbTriVert = sizeof(U_TRIVERTEX) *
                nTriVert; // all of the cb's will be a multiple of 4 bytes
    if (ulMode == U_GRADIENT_FILL_TRIANGLE) {
        cbGradObj = sizeof(U_GRADIENT3) * nGradObj;
    } else if (ulMode == U_GRADIENT_FILL_RECT_H ||
               ulMode == U_GRADIENT_FILL_RECT_V) {
        cbGradObj = sizeof(U_GRADIENT4) * nGradObj;
    } else {
        return (NULL);
    }
...
/**
    ...
    \param ihCS     Index to place object in EMF object table (this entry must
   not yet exist)
    \param lcs      ColorSpace parameters
    \param dwFlags  If low bit set Data is present
    \param cbData   Number of bytes of theData field.
...
char *U_EMRCREATECOLORSPACEW_set(const uint32_t ihCS,
    ...
    if (record) {
        ...
        off = sizeof(U_EMR) + sizeof(uint32_t) + sizeof(U_LOGCOLORSPACEW) +
              sizeof(uint32_t) + sizeof(U_CBDATA); // offset to Data field
