...
/* iconv() has a funny cast on some older systems, on most recent ones
   ...
   on another funky system this code may need to be modified, or define
   ICONV_CAST
...
void wchartshow(const wchar_t *src) {
    ...
    if (!src) {
        ...
        if (!src)
            return;
        while (*src) {
            val = *src; // because *src is wchar_t is not strictly an integer
                        // type, can cause warnings on next line
            printf("%d %d %x\n", (int)srclen, val, val);
            srclen++;
            src++;
        }
...
/* **********************************************************************************************
These functions are used for character type conversions, Image conversions, and
other
utility operations
***********************************************************************************************
*/
...
/**
    \brief Find the number of (storage) characters in a 16 bit character string,
...
/**
    \brief Find the number of (storage) characters in a 32 bit character
   string, not including terminator.
    \param src string to examine
*/
...
/**
    \brief Fill the output string with N characters, if the input string is
   shorter than N, pad with nulls.
    \param dst destination (already allocated)
    \param src source
    \param nchars number of characters to copy
...
void wchar16strncpypad(uint16_t *dst, const uint16_t *src, size_t nchars) {
    if (src) {
        for (; *src && nchars; nchars--, dst++, src++) {
            *dst = *src;
        }
        for (; nchars; nchars--, dst++) {
            *dst = 0;
        } // Pad the remainder
    }
}

/*  For the following converstion functions, remember that iconv() modifies ALL
   of its parameters,
...
uint16_t *U_Utf32leToUtf16le(const uint32_t *src, size_t max, size_t *len) {
    ...
    if (max) {
        ...
        srclen = 4 + 4 * wchar32len(src);
    } // include terminator, length in BYTES
...
/**
    ...
    U_EMR_EXTTEXTOUTA records are "8 bit ASCII".  In theory that is ASCII in an
   8
    bit character, but numerous applications store Latin1 in them, and some
    _may_ store UTF-8 in them.  Since very vew Latin1 strings are valid UTF-8
   strings,
...
uint32_t *U_Latin1ToUtf32le(const char *src, size_t max, size_t *len) {
    ...
    dstlen = sizeof(uint32_t) *
             (1 + srclen); // This should always work but might waste some space
    dst2 = dst = calloc(dstlen, 1);
    if (dst) {
...
char *U_Utf16leToLatin1(const uint16_t *src, size_t max, size_t *len) {
    ...
    dstlen =
        1 +
        srclen; // this will always work as latin1 is always 1 byte/character
    dst2 = dst = (char *)calloc(dstlen, 1);
    if (dst) {
...
char *U_Utf8ToLatin1(const char *src, size_t max, size_t *len) {
    ...
    dst2 = dst = calloc(dstlen, 1);
    if (dst) {
        iconv_t conv = iconv_open("LATIN1//TRANSLIT",
                                  "UTF-8"); // translate what can be, fill in
                                            // with something close for the rest
        if (conv == (iconv_t)-1) {
            free(dst2);
            dst2 = NULL;
        } else {
...
/**
    ...
    WMF uses latin1, others UTF-8, all Latin1 should be able to convert to
   utf-8.
...
char *U_Latin1ToUtf8(const char *src, size_t max, size_t *len) {
    ...
    if (max) {
        ...
        srclen = strlen(src) + 1;
    }                          // include terminator, will waste some space
    dstlen = (1 + 2 * srclen); // This should always work because all latin1
                               // convert to 1 or 2 byte UTF8, it might waste
                               // some space
    dst2 = dst = calloc(dstlen, 1);
    if (dst) {
        iconv_t conv =
            iconv_open("UTF-8", "LATIN1"); // everything should translate
        if (conv == (iconv_t)-1) {
            free(dst2);
            dst2 = NULL;
        } else {
...
/**
    ...
    embedded nulls, which makes it difficult to manipulate.  Use some other
   character and then swap it.
